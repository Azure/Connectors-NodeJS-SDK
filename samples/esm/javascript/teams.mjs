...
/**
 * Teams Connector SDK Sample — ESM JavaScript
 *
 * Demonstrates how to use the Teams connector with ESM imports in plain JavaScript.
 *
 * Prerequisites:
...
async function main() {
    console.log("Teams Connector SDK — ESM JavaScript Sample");
            ...
            const result = await client.postMessageToConversationAsync(
                {
                    ...
                    messageBody: `Hello from the <strong>ESM JavaScript</strong> Teams SDK sample! (${new Date().toISOString()})`,
                },
                "user",
                "channel",
            );
