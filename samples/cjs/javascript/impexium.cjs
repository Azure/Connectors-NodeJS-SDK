...
/** Impexium Connector SDK Sample - CJS JavaScript. */
const { ConnectorError, ManagedIdentityTokenProvider } = require("@azure/connectors");
const { ImpexiumClient } = require("@azure/connectors/generated/ImpexiumExtensions");
...
async function main() {
    try {
        const checkouts = await new ImpexiumClient(CONNECTION_URL, new ManagedIdentityTokenProvider()).getAbandonedCheckouts("1");
