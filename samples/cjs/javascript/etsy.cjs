...
/** Etsy Connector SDK Sample - CJS JavaScript. */
const { ConnectorError, ManagedIdentityTokenProvider } = require("@azure/connectors");
const { EtsyClient } = require("@azure/connectors/generated/EtsyExtensions");
const CONNECTION_URL = process.env.ETSY_CONNECTION_URL ?? "";
if (!CONNECTION_URL) throw new Error("ETSY_CONNECTION_URL is required.");
async function main() {
    try {
        const response = await new EtsyClient(CONNECTION_URL, new ManagedIdentityTokenProvider()).ping();
