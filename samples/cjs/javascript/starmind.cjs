...
/** Starmind Connector SDK Sample - CJS JavaScript. */
const { ConnectorError, ManagedIdentityTokenProvider } = require("@azure/connectors");
const { StarmindClient } = require("@azure/connectors/generated/StarmindExtensions");
...
async function main() {
    try {
        const questions = await new StarmindClient(CONNECTION_URL, new ManagedIdentityTokenProvider()).findQuestions(process.env.STARMIND_QUERY);
