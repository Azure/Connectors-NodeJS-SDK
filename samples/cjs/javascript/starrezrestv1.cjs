...
/** StarRez REST V1 Connector SDK Sample - CJS JavaScript. */
const { ConnectorError, ManagedIdentityTokenProvider } = require("@azure/connectors");
const { Starrezrestv1Client } = require("@azure/connectors/generated/Starrezrestv1Extensions");
...
async function main() {
    try {
        const entries = await new Starrezrestv1Client(CONNECTION_URL, new ManagedIdentityTokenProvider()).selectEntry({ _pageSize: 10, _pageIndex: 0 });
