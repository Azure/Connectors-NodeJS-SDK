...
/** Rev.ai Connector SDK Sample - CJS JavaScript. */
const { ConnectorError, ManagedIdentityTokenProvider } = require("@azure/connectors");
const { RevaiClient } = require("@azure/connectors/generated/RevaiExtensions");
const CONNECTION_URL = process.env.REVAI_CONNECTION_URL ?? "";
if (!CONNECTION_URL) throw new Error("REVAI_CONNECTION_URL is required.");
async function main() {
    try {
        const transcriptions = await new RevaiClient(CONNECTION_URL, new ManagedIdentityTokenProvider()).getTranscriptions("10");
