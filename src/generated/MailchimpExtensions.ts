// MailchimpExtensions.ts - Auto-generated DirectClient SDK for TypeScript
// Do not edit this file directly.

import type { TokenCredential } from "@azure/core-auth";
import { ConnectorClientBase } from "../azureConnectors/clientBase.ts";
import type { ConnectorPagedAsyncIterableIterator } from "../azureConnectors/clientBase.ts";
import type { ConnectorClientOptions, ConnectorOperationOptions } from "../azureConnectors/options.ts";

// #region Types

/**
 * Response for List campaigns
 */
export interface GetCampaignsResponse {
    /** An array of objects, each representing a campaign */
    campaigns?: Array<CampaignResponseModel>;
}

/**
 * Definition: Recipient
 */
export interface Recipient {
    /** The unique list id */
    list_id: string;
    segment_opts?: SegmentOpts;
}

/**
 * Definition: Variate_Settings
 */
export interface VariateSettings {
    /** The combination that performs the best. This may be determined automatically by click rate, open rate, or total revenue—or you may choose manually based on the reporting data you find the most valuable. For Multivariate Campaigns testing send_time, winner_critera is ignored. For Multivariate Campaigns with 'manual' as the winner_citeria, the winner must be chosen in the MailChimp web application */
    winner_criteria?: string;
    /** The number of minutes to wait before choosing the winning campaign. The value of wait_time must be greater than 0 and in whole hours, specified in minutes */
    wait_time?: number;
    /** The percentage of recipients to send the test combinations to, must be a value between 10 and 100 */
    test_size?: number;
    /** The possible subject lines to test. If no subject lines are provided, settings.subject_line will be used */
    subject_lines?: Array<string>;
    /** The possible send times to test. The times provided should be in the format YYYY-MM-DD HH:MM:SS. If send_times are provided to test, the test_size will be set to 100% and winner_criteria will be ignored */
    send_times?: Array<string>;
    /** The possible from names. The number of from_names provided must match the number of reply_to_addresses. If no from_names are provided, settings.from_name will be used */
    from_names?: Array<string>;
    /** The possible from names. The number of from_names provided must match the number of reply_to_addresses. If no from_names are provided, settings.from_name will be used */
    reply_to_addresses?: Array<string>;
}

/**
 * Definition: Tracking
 */
export interface Tracking {
    /** Whether to track opens. Defaults to true. Cannot be set to false for variate campaigns */
    opens?: boolean;
    /** Whether to track clicks in the HTML version of the campaign. Defaults to true. Cannot be set to false for variate campaigns */
    html_clicks?: boolean;
    /** Whether to track clicks in the plain-text version of the campaign. Defaults to true. Cannot be set to false for variate campaigns */
    text_clicks?: boolean;
    /** Whether to enable Goal tracking */
    goal_tracking?: boolean;
    /** Whether to enable eCommerce360 tracking */
    ecomm360?: boolean;
    /** The custom slug for Google Analytics tracking (max of 50 bytes) */
    google_analytics?: string;
    /** The custom slug for ClickTale tracking (max of 50 bytes) */
    clicktale?: string;
    salesforce?: Salesforce;
    highrise?: Highrise;
    capsule?: Capsule;
}

/**
 * Definition: RSS_Opts
 */
export interface RSSOpts {
    /** The URL for the RSS feed */
    feed_url?: string;
    /** The frequency of the RSS Campaign */
    frequency?: string;
    /** Whether to add CSS to images in the RSS feed to constrain their width in campaigns */
    constrain_rss_img?: string;
    schedule?: Schedule;
}

/**
 * Definition: Social_Card
 */
export interface SocialCard {
    /** The url for the header image for the card */
    image_url?: string;
    /** A short summary of the campaign to display */
    description?: string;
    /** The title for the card. Typically the subject line of the campaign */
    title?: string;
}

/**
 * Definition: Segment_Opts
 */
export interface SegmentOpts {
    /** The id for an existing saved segment */
    saved_segment_id?: number;
    /** Segment match type: Possible Values: any, all */
    match?: string;
}

/**
 * Definition: Salesforce
 */
export interface Salesforce {
    /** Create a campaign in a connected Salesforce account */
    campaign?: boolean;
    /** Update contact notes for a campaign based on subscriber email addresses */
    notes?: boolean;
}

/**
 * Definition: Highrise
 */
export interface Highrise {
    /** Create a campaign in a connected Highrise account */
    campaign?: boolean;
    /** Update contact notes for a campaign based on subscriber email addresses */
    notes?: boolean;
}

/**
 * Definition: Capsule
 */
export interface Capsule {
    /** Update contact notes for a campaign based on subscriber email addresses */
    notes?: boolean;
}

/**
 * Definition: Schedule
 */
export interface Schedule {
    /** The hour to send the campaign in local time. Acceptable hours are 0-23. For example, ‘4’ would be 4am in your account’s default time zone */
    hour?: number;
    daily_send?: DailySend;
    /** The day of the week to send a weekly RSS Campaign */
    weekly_send_day?: string;
    /** The day of the month to send a monthly RSS Campaign. Acceptable days are 1-32, where ‘0’ is always the last day of a month. Months with fewer than the selected number of days will not have an RSS campaign sent out that day. For example, RSS Campaigns set to send on the 30th will not go out in February */
    monthly_send_date?: number;
}

/**
 * Definition: Daily_Send
 */
export interface DailySend {
    /** Sends the daily RSS Campaign on Sundays */
    sunday?: boolean;
    /** Sends the daily RSS Campaign on Mondays */
    monday?: boolean;
    /** Sends the daily RSS Campaign on Tuesdays */
    tuesday?: boolean;
    /** Sends the daily RSS Campaign on Wednesdays */
    wednesday?: boolean;
    /** Sends the daily RSS Campaign on Thursdays */
    thursday?: boolean;
    /** Sends the daily RSS Campaign on Fridays */
    friday?: boolean;
    /** Sends the daily RSS Campaign on Saturdays */
    saturday?: boolean;
}

/**
 * Definition: CampaignResponseModel
 */
export interface CampaignResponseModel {
    /** A string that uniquely identifies this campaign */
    id?: string;
    /** There are four types of campaigns you can create in MailChimp. A/B Split campaigns have been deprecated and variate campaigns should be used instead */
    type?: string;
    /** The date and time the campaign was created */
    create_time?: string;
    /** The link to the campaign’s archive version */
    archive_url?: string;
    /** The current status of the campaign. */
    status?: string;
    /** The total number of emails sent for this campaign */
    emails_sent?: number;
    /** The time and date a campaign was sent */
    send_time?: string;
    /** How the campaign’s content is put together (‘template’, ‘drag_and_drop’, ‘html’, ‘url’) */
    content_type?: string;
    /** List settings for the campaign */
    recipient?: Array<Recipient>;
    settings?: Settings;
    variate_settings?: VariateSettings;
    tracking?: Tracking;
    rss_opts?: RSSOpts;
    ab_split_opts?: ABSplitOpts;
    social_card?: SocialCard;
    report_summary?: ReportSummary;
    delivery_status?: DeliveryStatus;
    /** A list of link types and descriptions for the API schema documents */
    _links?: Array<Link>;
}

/**
 * Definition: Settings
 */
export interface Settings {
    /** The subject line for the campaign */
    subject_line: string;
    /** The title of the campaign */
    title?: string;
    /** The 'from' name on the campaign (not an email address) */
    from_name: string;
    /** The reply-to email address for the campaign */
    reply_to: string;
    /** Use MailChimp Conversation feature to manage out-of-office replies */
    use_conversation?: boolean;
    /** The campaign's custom 'To' name. Typically the first name merge field */
    to_name?: string;
    /** If the campaign is listed in a folder, the id for that folder */
    folder_id?: string;
    /** Whether MailChimp authenticated the campaign. Defaults to true */
    authenticate?: boolean;
    /** Automatically append MailChimp's default footer to the campaign */
    auto_footer?: boolean;
    /** Automatically inline the CSS included with the campaign content */
    inline_css?: boolean;
    /** Automatically tweet a link to the campaign archive page when the campaign is sent */
    auto_tweet?: boolean;
    /** An array of Facebook page ids to auto-post to */
    auto_fb_post?: Array<number>;
    /** Allows Facebook comments on the campaign (also force-enables the Campaign Archive toolbar). Defaults to true */
    fb_comments?: boolean;
}

/**
 * Definition: AB_Split_Opts
 */
export interface ABSplitOpts {
    /** The type of AB split to run */
    split_test?: string;
    /** How we should evaluate a winner. Based on 'opens', 'clicks', or 'manual' */
    pick_winner?: string;
    /** How unit of time for measuring the winner ('hours' or 'days'). This cannot be changed after a campaign is sent */
    wait_units?: string;
    /** The amount of time to wait before picking a winner. This cannot be changed after a campaign is sent */
    wait_time?: number;
    /** The size of the split groups. Campaigns split based on 'schedule' are forced to have a 50/50 split. Valid split integers are between 1-50 */
    split_size?: number;
    /** For campaigns split on 'From Name', the name for Group A */
    from_name_a?: string;
    /** For campaigns split on 'From Name', the name for Group B */
    from_name_b?: string;
    /** For campaigns split on ‘From Name’, the reply-to address for Group A */
    reply_email_a?: string;
    /** For campaigns split on ‘From Name’, the reply-to address for Group B */
    reply_email_b?: string;
    /** For campaigns split on ‘Subject Line’, the subject line for Group A */
    subject_a?: string;
    /** For campaigns split on ‘Subject Line’, the subject line for Group B */
    subject_b?: string;
    /** The send time for Group A */
    send_time_a?: string;
    /** The send time for Group B */
    send_time_b?: string;
    /** The send time for the winning version */
    send_time_winner?: string;
}

/**
 * Definition: Report_Summary
 */
export interface ReportSummary {
    /** The total number of opens for a campaign */
    opens?: number;
    /** The number of unique opens */
    unique_opens?: number;
    /** The number of unique opens divided by the total number of successful deliveries */
    open_rate?: number;
    /** The total number of clicks for an campaign */
    clicks?: number;
    /** The number of unique clicks */
    subscriber_clicks?: number;
    /** The number of unique clicks divided by the total number of successful deliveries */
    click_rate?: number;
}

/**
 * Definition: Delivery_Status
 */
export interface DeliveryStatus {
    /** Whether Campaign Delivery Status is enabled for this account and campaign */
    enabled?: boolean;
    /** Whether a campaign send can be canceled */
    can_cancel?: boolean;
    /** The current state of a campaign delivery */
    status?: string;
    /** The total number of emails confirmed sent for this campaign so far */
    emails_sent?: number;
    /** The total number of emails canceled for this campaign */
    emails_canceled?: number;
}

/**
 * Definition: Link
 */
export interface Link {
    /** As with an HTML ‘rel’ attribute, this describes the type of link */
    rel?: string;
    /** This property contains a fully-qualified URL that can be called to retrieve the linked resource or perform the linked action */
    href?: string;
    /** The HTTP method that should be used when accessing the URL defined in ‘href’. (GET, POST, PUT, PATCH, DELETE, OPTIONS, HEAD) */
    method?: string;
    /** For GETs, this is a URL representing the schema that the response should conform to */
    targetSchema?: string;
    /** For HTTP methods that can receive bodies (POST and PUT), this is a URL representing the schema that the body should conform to */
    schema?: string;
}

/**
 * Definition: NewCampaignRequest
 */
export interface NewCampaignRequest {
    /** There are four types of campaigns you can create in MailChimp. A/B Split campaigns have been deprecated and variate campaigns should be used instead. Possible Values: regular, plaintext, absplit, rss, variate */
    type: string;
    recipients: Recipient;
    settings: Settings;
    variate_settings?: VariateSettings;
    tracking?: Tracking;
    rss_opts?: RSSOpts;
    social_card?: SocialCard;
}

/**
 * Definition: NewListRequest
 */
export interface NewListRequest {
    /** The name of the list */
    name: string;
    contact: Contact;
    /** Text to remind people how they signed up to this list */
    permission_reminder: string;
    /** Whether campaigns for this list use the Archive Bar in archives by default */
    use_archive_bar?: boolean;
    campaign_defaults: CampaignDefaults;
    /** The email address to send subscribe notifications to */
    notify_on_subscribe?: string;
    /** The email address to send unsubscribe notifications to */
    notify_on_unsubscribe?: string;
    /** When set to true, subscribers can choose whether they want to receive HTML or plain-text emails. When set to false, subscribers will receive HTML emails, with a plain-text alternative backup */
    email_type_option: boolean;
    /** Whether this list is public or private */
    visibility?: string;
}

/**
 * Definition: Contact
 */
export interface Contact {
    /** The company name for the list */
    company: string;
    /** The street address for the list contact */
    address1: string;
    /** The street address for the list contact */
    address2?: string;
    /** The city for the list contact */
    city: string;
    /** The state for the list contact */
    state: string;
    /** The postal or zip code for the list contact */
    zip: string;
    /** A two-character ISO3166 country code. Defaults to US if invalid */
    country: string;
    /** The phone number for the list contact */
    phone: string;
}

/**
 * Definition: Campaign_Defaults
 */
export interface CampaignDefaults {
    /** The default from name for campaigns sent to this list */
    from_name: string;
    /** The default from email for campaigns sent to this list */
    from_email: string;
    /** The default subject line for campaigns sent to this list */
    subject: string;
    /** The default language for this lists’s forms */
    language: string;
}

/**
 * Definition: CreateNewListResponseModel
 */
export interface CreateNewListResponseModel {
    /** A string that uniquely identifies this list */
    id: string;
    /** The name of the list */
    name: string;
    contact: Contact;
    /** The permission reminder for the list */
    permission_reminder: string;
    /** Whether campaigns for this list use the Archive Bar in archives by default */
    use_archive_bar?: boolean;
    campaign_defaults: CampaignDefaults;
    /** The email address to send subscribe notifications to */
    notify_on_subscribe?: string;
    /** The email address to send unsubscribe notifications to */
    notify_on_unsubscribe?: string;
    /** The date and time that this list was created */
    date_created?: string;
    /** An auto-generated activity score for the list (0-5) */
    list_rating?: number;
    /** Whether the list supports multiple formats for emails.When set to true, subscribers can choose whether they want to receive HTML or plain-text emails.When set to false, subscribers will receive HTML emails, with a plain-text alternative backup */
    email_type_option: boolean;
    /** Our EepURL shortened version of this list’s subscribe form */
    subscribe_url_short?: string;
    /** The full version of this list’s subscribe form (host will vary) */
    subscribe_url_long?: string;
    /** The list’s Email Beamer address */
    beamer_address?: string;
    /** Whether this list is public or private */
    visibility?: string;
    /** Any list-specific modules installed for this list */
    modules?: Array<string>;
    stats?: Stats;
    /** A list of link types and descriptions for the API schema documents */
    _links?: Array<Link>;
}

/**
 * Definition: Stats
 */
export interface Stats {
    /** The number of active members in the list */
    member_count?: number;
    /** The number of members who have unsubscribed from the list */
    unsubscribe_count?: number;
    /** The number of members cleaned from the list */
    cleaned_count?: number;
    /** The number of active members in the list since the last campaign was sent */
    member_count_since_send?: number;
    /** The number of members who have unsubscribed since the last campaign was sent */
    unsubscribe_count_since_send?: number;
    /** The number of members cleaned from the list since the last campaign was sent */
    cleaned_count_since_send?: number;
    /** The number of campaigns in any status that use this list */
    campaign_count?: number;
    /** The date and time the last campaign was sent to this list */
    campaign_last_sent?: string;
    /** The number of merge vars for this list (not EMAIL, which is required) */
    merge_field_count?: number;
    /** The average number of subscriptions per month for the list(not returned if we haven’t calculated it yet) */
    avg_sub_rate?: number;
    /** The average number of unsubscriptions per month for the list(not returned if we haven’t calculated it yet) */
    avg_unsub_rate?: number;
    /** The target number of subscriptions per month for the list to keep it growing(not returned if we haven’t calculated it yet) */
    target_sub_rate?: number;
    /** The average open rate(a percentage represented as a number between 0 and 100) per campaign for the list(not returned if we haven’t calculated it yet) */
    open_rate?: number;
    /** The average click rate(a percentage represented as a number between 0 and 100) per campaign for the list(not returned if we haven’t calculated it yet) */
    click_rate?: number;
    /** The date and time of the last time someone subscribed to this list */
    last_sub_date?: string;
    /** The date and time of the last time someone unsubscribed from this list */
    last_unsub_date?: string;
}

/**
 * Definition: GetListsResponseModel
 */
export interface GetListsResponseModel {
    /** An array of objects, each representing a list */
    lists?: Array<CreateNewListResponseModel>;
    /** The total number of items matching the query regardless of pagination */
    total_items?: number;
}

/**
 * Definition: NewMembersInListRequest
 */
export interface NewMembersInListRequest {
    /** An array of objects, each representing an email address and the subscription status for a specific list. Up to 500 members may be added or updated with each API call. */
    members: Array<NewMemberInListRequest>;
    /** Whether this batch operation will change existing members' subscription status. */
    update_existing?: boolean;
}

/**
 * Definition: NewMemberInListRequest
 */
export interface NewMemberInListRequest {
    /** Type of email this member asked to get (‘html’ or ‘text’) */
    email_type?: string;
    /** Subscriber’s current status. Possible Values: subscribed, unsubscribed, cleaned, pending */
    status: string;
    merge_fields?: FirstAndLastName;
    /** If set/detected, the subscriber’s language */
    language?: string;
    /** VIP status for subscriber */
    vip?: boolean;
    location?: Location;
    /** Email address for a subscriber */
    email_address: string;
}

/**
 * Definition: FirstAndLastName
 */
export interface FirstAndLastName {
    /** First name of the subscriber */
    FNAME?: string;
    /** Last name of the subscriber */
    LNAME?: string;
}

/**
 * Definition: Location
 */
export interface Location {
    /** The location latitude */
    latitude?: number;
    /** The location longitude */
    longitude?: number;
}

/**
 * Definition: MemberResponseModel
 */
export interface MemberResponseModel {
    /** The MD5 hash of the lowercase version of the list member’s email address */
    id?: string;
    /** Email address for a subscriber */
    email_address?: string;
    /** An identifier for the address across all of MailChimp */
    unique_email_id?: string;
    /** Type of email this member asked to get (‘html’ or ‘text’). */
    email_type?: string;
    /** Subscriber’s current status. Possible Values: subscribed, unsubscribed, cleaned, pending */
    status?: string;
    merge_fields?: FirstAndLastName;
    stats?: Stats;
    /** IP address the subscriber signed up from */
    ip_signup?: string;
    /** Date and time the subscriber signed up for the list */
    timestamp_signup?: string;
    /** The IP address the subscriber used to confirm their opt-in status */
    ip_opt?: string;
    /** Date and time the subscribe confirmed their opt-in status */
    timestamp_opt?: string;
    /** Star rating for this member, between 1 and 5 */
    member_rating?: number;
    /** Date and time the member’s info was last changed */
    last_changed?: string;
    /** If set/detected, the subscriber’s language */
    language?: string;
    /** VIP status for subscriber */
    vip?: boolean;
    /** The list member’s email client */
    email_client?: string;
    location?: Location;
    last_note?: LastNote;
    /** The list id */
    list_id?: string;
    /** A list of link types and descriptions for the API schema documents */
    _links?: Array<Link>;
}

/**
 * Definition: Last_Note
 */
export interface LastNote {
    /** The note id */
    note_id?: number;
    /** The date the note was created */
    created_at?: string;
    /** The author of the note */
    created_by?: string;
    /** The content of the note */
    note?: string;
}

/**
 * Definition: GetAddMembersBatchResponseModel
 */
export interface GetAddMembersBatchResponseModel {
    /** Total number of members that were added to the list */
    total_created?: number;
    /** The total number of an existing list members whose subscription status was updated */
    total_updated?: number;
    /** The number email addresses that could not be added to the list or updated */
    error_count?: number;
}

/**
 * Definition: GetAllMembersResponseModel
 */
export interface GetAllMembersResponseModel {
    /** An array of objects, each representing a specific list member */
    members?: Array<MemberResponseModel>;
    /** The list id */
    list_id?: string;
    /** The total number of items matching the query regardless of pagination */
    total_items?: number;
}

/**
 * Definition: Object
 */
export interface ObjectEntity {
    [key: string]: unknown;
}

/**
 * Definition: UpdateMemberInListRequest
 */
export interface UpdateMemberInListRequest {
    /** Type of email this member asked to get (‘html’ or ‘text’) */
    email_type?: string;
    /** Subscriber’s current status. Possible Values: subscribed, unsubscribed, cleaned, pending */
    status: string;
    merge_fields?: FirstAndLastName;
    /** If set/detected, the subscriber’s language */
    language?: string;
    /** VIP status for subscriber */
    vip?: boolean;
    location?: Location;
}

/**
 * Definition: GetMembersResponseModel
 */
export interface GetMembersResponseModel {
    /** An array of objects, each representing a specific list member */
    members?: Array<AddUserResponseModel>;
    /** The list id */
    list_id?: string;
    /** The total number of items matching the query regardless of pagination */
    total_items?: number;
}

/**
 * Definition: AddUserResponseModel
 */
export interface AddUserResponseModel {
    /** The MD5 hash of the lowercase version of the list member’s email address */
    id: string;
    /** Email address for a subscriber */
    email_address: string;
    /** An identifier for the address across all of MailChimp */
    unique_email_id?: string;
    /** Type of email this member asked to get (‘html’ or ‘text’). */
    email_type?: string;
    /** Subscriber’s current status. Possible Values: subscribed, unsubscribed, cleaned, pending */
    status?: string;
    merge_fields: FirstAndLastName;
    stats?: Stats;
    /** IP address the subscriber signed up from */
    ip_signup?: string;
    /** Date and time the subscriber signed up for the list */
    timestamp_signup?: string;
    /** The IP address the subscriber used to confirm their opt-in status */
    ip_opt?: string;
    /** Date and time the subscribe confirmed their opt-in status */
    timestamp_opt?: string;
    /** Star rating for this member, between 1 and 5 */
    member_rating?: number;
    /** Date and time the member’s info was last changed */
    last_changed?: string;
    /** If set/detected, the subscriber’s language */
    language?: string;
    /** VIP status for subscriber */
    vip?: boolean;
    /** The list member’s email client */
    email_client?: string;
    location?: Location;
    last_note?: LastNote;
    /** The list id */
    list_id?: string;
    /** A list of link types and descriptions for the API schema documents */
    _links?: Array<Link>;
}

/**
 * Options for the getLists operation.
 */
export interface GetListsOptions extends ConnectorOperationOptions {
    /** The number of records to return. Default value is 10. Maximum value is 1000 */
    count?: number;
    /** The number of records from a collection to skip. Default value is 0. */
    offset?: number;
}

/**
 * Options for the addMembers operation.
 */
export interface AddMembersOptions extends ConnectorOperationOptions {
    /** Allows member data to be accepted without merge field values */
    skipMergeValidation?: boolean;
    /** Ignores duplicates sent in the request */
    skipDuplicateCheck?: boolean;
}

/**
 * Options for the getListMembers operation.
 */
export interface GetListMembersOptions extends ConnectorOperationOptions {
    /** The number of records to return. Default value is 10. Maximum value is 1000 */
    count?: number;
    /** The number of records from a collection to skip. Default value is 0. */
    offset?: number;
}
// #endregion Types

export const MailchimpTriggerOperations = {
    OnCreateList: "OnCreateList",
    OnMemberSubscribed: "OnMemberSubscribed",
} as const;

export type MailchimpTriggerOperation = typeof MailchimpTriggerOperations[keyof typeof MailchimpTriggerOperations];

export const MailchimpTriggerParameters = {
    OnMemberSubscribed: {
        listId: {
            name: "list_id",
            type: "string",
            required: true,
            description: "The unique id for the list",
            summary: "List ID",
            dynamicValuesOperationId: "GetLists",
        },
    },
} as const;

// #region Client

/**
 * Typed client for the mailchimp connector.
 */
export class MailchimpClient extends ConnectorClientBase {

    /**
     * Creates a new MailchimpClient.
     * @param connectionRuntimeUrl The connection runtime URL from Azure Portal.
     * @param credential The credential used for authentication.
     * @param options Optional connector client options.
     */
    constructor(connectionRuntimeUrl: string, credential: TokenCredential, options?: ConnectorClientOptions) {
        super(connectionRuntimeUrl, credential, options);
    }

    public get connectorName(): string {
        return "mailchimp";
    }

    /**
     * List campaigns
     * @remarks This operation retrieves a list of campaigns in an account
     */
    public getCampaigns(options: ConnectorOperationOptions = {}): ConnectorPagedAsyncIterableIterator<CampaignResponseModel> {
        const requestPath = `/campaigns`;
        return this.createPageable<GetCampaignsResponse, CampaignResponseModel>(
            requestPath,
            async (requestUrl) => {
                const httpResponse = await this.sendWithTracingAsync<GetCampaignsResponse>("Mailchimp.getCampaigns", "GetCampaigns", "GET", requestUrl, undefined, options);

                return httpResponse.value as GetCampaignsResponse;
            },
            "campaigns",
            undefined,
        );
    }

    /**
     * Send Campaign
     * @remarks Send an existing campaign
     */
    public async sendcampaign(campaignId: string, options: ConnectorOperationOptions = {}): Promise<void> {
        const requestPath = `/campaigns/${encodeURIComponent(String(campaignId))}/actions/send`;
        const requestUrl = this.resolveUrl(requestPath);
        await this.sendWithTracingAsync<void>("Mailchimp.sendcampaign", "sendcampaign", "POST", requestUrl, undefined, options);
    }

    /**
     * Get all the lists
     * @remarks Find all the lists for the current user
     */
    public async getLists(options: GetListsOptions = {}): Promise<GetListsResponseModel> {
        const queryParams: string[] = [];
        if (options.count !== undefined) {
            queryParams.push(`count=${encodeURIComponent(String(options.count))}`);
        }
        if (options.offset !== undefined) {
            queryParams.push(`offset=${encodeURIComponent(String(options.offset))}`);
        }
        const requestPath = `/lists` + (queryParams.length > 0 ? "?" + queryParams.join("&") : "");
        const requestUrl = this.resolveUrl(requestPath);
        const httpResponse = await this.sendWithTracingAsync<GetListsResponseModel>("Mailchimp.getLists", "GetLists", "GET", requestUrl, undefined, options);

        return httpResponse.value as GetListsResponseModel;
    }

    /**
     * New List
     * @remarks Create a new list in your MailChimp account
     */
    public async newlist(input: NewListRequest, options: ConnectorOperationOptions = {}): Promise<CreateNewListResponseModel> {
        const requestPath = `/lists`;
        const requestUrl = this.resolveUrl(requestPath);
        const httpResponse = await this.sendWithTracingAsync<CreateNewListResponseModel>("Mailchimp.newlist", "newlist", "POST", requestUrl, input, options);

        return httpResponse.value as CreateNewListResponseModel;
    }

    /**
     * Subscribe or unsubscribe list members
     * @remarks Batch subscribe or unsubscribe list members.
     */
    public async addMembers(input: NewMembersInListRequest, listId: string, options: AddMembersOptions = {}): Promise<GetAddMembersBatchResponseModel> {
        const queryParams: string[] = [];
        if (options.skipMergeValidation !== undefined) {
            queryParams.push(`skip_merge_validation=${encodeURIComponent(String(options.skipMergeValidation))}`);
        }
        if (options.skipDuplicateCheck !== undefined) {
            queryParams.push(`skip_duplicate_check=${encodeURIComponent(String(options.skipDuplicateCheck))}`);
        }
        const requestPath = `/lists/${encodeURIComponent(String(listId))}` + (queryParams.length > 0 ? "?" + queryParams.join("&") : "");
        const requestUrl = this.resolveUrl(requestPath);
        const httpResponse = await this.sendWithTracingAsync<GetAddMembersBatchResponseModel>("Mailchimp.addMembers", "AddMembers", "POST", requestUrl, input, options);

        return httpResponse.value as GetAddMembersBatchResponseModel;
    }

    /**
     * Show list members
     * @remarks Show all the members of a list
     */
    public async getListMembers(listId: string, options: GetListMembersOptions = {}): Promise<GetAllMembersResponseModel> {
        const queryParams: string[] = [];
        if (options.count !== undefined) {
            queryParams.push(`count=${encodeURIComponent(String(options.count))}`);
        }
        if (options.offset !== undefined) {
            queryParams.push(`offset=${encodeURIComponent(String(options.offset))}`);
        }
        const requestPath = `/lists/${encodeURIComponent(String(listId))}/members` + (queryParams.length > 0 ? "?" + queryParams.join("&") : "");
        const requestUrl = this.resolveUrl(requestPath);
        const httpResponse = await this.sendWithTracingAsync<GetAllMembersResponseModel>("Mailchimp.getListMembers", "GetListMembers", "GET", requestUrl, undefined, options);

        return httpResponse.value as GetAllMembersResponseModel;
    }

    /**
     * Add member to list
     * @remarks Add or update a list member
     */
    public async addmember(input: NewMemberInListRequest, listId: string, options: ConnectorOperationOptions = {}): Promise<MemberResponseModel> {
        const requestPath = `/lists/${encodeURIComponent(String(listId))}/members`;
        const requestUrl = this.resolveUrl(requestPath);
        const httpResponse = await this.sendWithTracingAsync<MemberResponseModel>("Mailchimp.addmember", "addmember", "POST", requestUrl, input, options);

        return httpResponse.value as MemberResponseModel;
    }

    /**
     * New Campaign
     * @remarks Create a new campaign based on a Campaign Type, Recipients list and Campaign Settings (subject line, title, from_name and reply_to).
     */
    public async newcampaign(input: NewCampaignRequest, options: ConnectorOperationOptions = {}): Promise<CampaignResponseModel> {
        const requestPath = `/v2/campaigns`;
        const requestUrl = this.resolveUrl(requestPath);
        const httpResponse = await this.sendWithTracingAsync<CampaignResponseModel>("Mailchimp.newcampaign", "newcampaignV2", "POST", requestUrl, input, options);

        return httpResponse.value as CampaignResponseModel;
    }

    /**
     * Remove Member from list
     * @remarks Delete a member from a list.
     */
    public async removemember(listId: string, memberEmail: string, options: ConnectorOperationOptions = {}): Promise<void> {
        const requestPath = `/lists/replacemailwithhash/${encodeURIComponent(String(listId))}/members`;
        const requestHeaders: Record<string, string> = {};
        if (memberEmail !== undefined) {
            requestHeaders["member_email"] = String(memberEmail);
        }
        const requestUrl = this.resolveUrl(requestPath);
        await this.sendWithTracingAsync<void>("Mailchimp.removemember", "removemember_v2", "DELETE", requestUrl, undefined, options, requestHeaders);
    }

    /**
     * Update member information
     * @remarks Update information for a specific list member.
     */
    public async updatemember(input: UpdateMemberInListRequest, listId: string, memberEmail: string, options: ConnectorOperationOptions = {}): Promise<MemberResponseModel> {
        const requestPath = `/lists/replacemailwithhash/${encodeURIComponent(String(listId))}/members`;
        const requestHeaders: Record<string, string> = {};
        if (memberEmail !== undefined) {
            requestHeaders["member_email"] = String(memberEmail);
        }
        const requestUrl = this.resolveUrl(requestPath);
        const httpResponse = await this.sendWithTracingAsync<MemberResponseModel>("Mailchimp.updatemember", "updatemember_v2", "PATCH", requestUrl, input, options, requestHeaders);

        return httpResponse.value as MemberResponseModel;
    }

}

// #endregion Client
