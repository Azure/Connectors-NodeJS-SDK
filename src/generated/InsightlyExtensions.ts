// InsightlyExtensions.ts - Auto-generated DirectClient SDK for TypeScript
// Do not edit this file directly.

import { ConnectorClientBase } from "../azureConnectors/clientBase.ts";
import { ConnectorException } from "../azureConnectors/connectorException.ts";
import { ConnectorClientOptions } from "../azureConnectors/options.ts";
import { TokenProvider } from "../azureConnectors/authentication.ts";

// #region Types

/**
 * Definition: ListTasksResponse
 */
export interface ListTasksResponse {
    /** A collection of tasks */
    tasks?: Array<TaskObject>;
}

/**
 * Definition: Task
 */
export interface TaskObject {
    /** The unique identifier of the task. */
    TASK_ID?: number;
    /** The title of the task. */
    TITLE?: string;
    /** yyyy-MM-dd HH:mm:ss */
    DUE_DATE?: string;
    /** The unique identifier of the category. */
    CATEGORY_ID?: number;
    /** yyyy-MM-dd HH:mm:ss */
    COMPLETED_DATE_UTC?: string;
    /** A boolean value determining whether or not the task is completed. */
    COMPLETED?: boolean;
    /** The details of the task. */
    DETAILS?: string;
    /** The status of the task. */
    STATUS?: string;
    /** The priority of the task. */
    PRIORITY?: number;
    /** yyyy-MM-dd HH:mm:ss */
    START_DATE?: string;
    /** The unique identifier of the project. */
    PROJECT_ID?: number;
    /** The unique identifier of the opportunity. */
    OPPORTUNITY_ID?: number;
    /** The unique identifier of the milestone. */
    MILESTONE_ID?: number;
    /** The unique identifier of the pipeline. */
    PIPELINE_ID?: number;
    /** The unique identifier of the stage. */
    STAGE_ID?: number;
    /** The task completed value in percent. */
    PERCENT_COMPLETE?: number;
    /** The unique identifier of the task owner. */
    OWNER_USER_ID?: number;
    /** The unique identifier of the user assigned to the task. */
    RESPONSIBLE_USER_ID?: number;
    /** A boolean value determining whether or not the task is publicly visible. */
    PUBLICLY_VISIBLE?: boolean;
    /** yyyy-MM-dd HH:mm:ss */
    DATE_CREATED_UTC?: string;
    /** yyyy-MM-dd HH:mm:ss */
    DATE_UPDATED_UTC?: string;
    /** The unique identifier of the assigned by user. */
    ASSIGNED_BY_USER_ID?: number;
    /** The unique identifier of the parent task. */
    PARENT_TASK_ID?: number;
    /** A boolean value determining whether or not the owner is publicly visible. */
    OWNER_VISIBLE?: boolean;
    /** The unique identifier of the assigned team. */
    ASSIGNED_TEAM_ID?: number;
    /** yyyy-MM-dd HH:mm:ss */
    ASSIGNED_DATE_UTC?: string;
    /** yyyy-MM-dd HH:mm:ss */
    REMINDER_DATE_UTC?: string;
    /** A boolean value determining whether or not a reminder is set. */
    REMINDER_SENT?: boolean;
    /** Recurrence */
    RECURRENCE?: string;
    /** Can edit. */
    CAN_EDIT?: boolean;
    /** Can delete */
    CAN_DELETE?: boolean;
}

/**
 * Definition: TaskRequest
 */
export interface TaskRequest {
    /** The title of the task. */
    TITLE: string;
    /** yyyy-MM-dd HH:mm:ss */
    DUE_DATE?: string;
    /** A boolean value determining whether or not the task is completed. */
    COMPLETED: boolean;
    /** The details of the task. */
    DETAILS?: string;
    /** The status of the task. */
    STATUS: string;
    /** The priority of the task. */
    PRIORITY?: number;
    /** A boolean value determining whether or not the task is publicly visible. */
    PUBLICLY_VISIBLE: boolean;
}

/**
 * Definition: ListProjectsResponse
 */
export interface ListProjectsResponse {
    /** A collection of projects */
    projects?: Array<Project>;
}

/**
 * Definition: Project
 */
export interface Project {
    /** The unique identifier of the project. */
    PROJECT_ID?: number;
    /** The name of the project. */
    PROJECT_NAME?: string;
    /** The status of the project. */
    STATUS?: string;
    /** The details of the project. */
    PROJECT_DETAILS?: string;
    /** The image URL of the project. */
    IMAGE_URL?: string;
    /** yyyy-MM-dd HH:mm:ss */
    DATE_CREATED_UTC?: string;
    /** Project Visibility. */
    VISIBLE_TO?: string;
    /** yyyy-MM-dd HH:mm:ss */
    DATE_UPDATED_UTC?: string;
    /** yyyy-MM-dd HH:mm:ss */
    STARTED_DATE?: string;
    /** yyyy-MM-dd HH:mm:ss */
    COMPLETED_DATE?: string;
    /** The unique identifier of the project owner. */
    OWNER_USER_ID?: number;
    /** The unique identifier of the assigned user. */
    RESPONSIBLE_USER_ID?: number;
    /** The unique identifier of the opportunity. */
    OPPORTUNITY_ID?: number;
    /** The unique identifier of the pipeline. */
    PIPELINE_ID?: number;
    /** The unique identifier of the stage. */
    STAGE_ID?: number;
    /** The unique identifier of the category. */
    CATEGORY_ID?: number;
    /** The unique identifier of the team. */
    VISIBLE_TEAM_ID?: number;
    /** The unique identifiers of the users. */
    VISIBLE_USER_IDS?: number;
    /** Can edit */
    CAN_EDIT?: boolean;
    /** Can delete */
    CAN_DELETE?: boolean;
}

/**
 * Definition: Tag
 */
export interface Tag {
    /** The name of the tag. */
    TAG_NAME?: string;
}

/**
 * Definition: ProjectRequest
 */
export interface ProjectRequest {
    /** The name of the project. */
    PROJECT_NAME: string;
    /** The details of the project. */
    PROJECT_DETAILS?: string;
    /** The status of the project. */
    STATUS: string;
    /** The image URL of the project. */
    IMAGE_URL?: string;
    /** Project Visibility. */
    VISIBLE_TO?: string;
    /** Visible Team Id (Required when team visibility state is selected) */
    VISIBLE_TEAM_ID?: number;
    /** Visible User IDs (Required when individuals visibility state is selected) */
    VISIBLE_USER_IDS?: number;
    /** The unique identifier of the opportunity. */
    OPPORTUNITY_ID?: number;
    /** The unique identifier of the pipeline. */
    PIPELINE_ID?: number;
    /** The unique identifier of the stage. */
    STAGE_ID?: number;
    /** Tags */
    TAGS?: Array<Tag>;
}

/**
 * Definition: ListLeadsResponse
 */
export interface ListLeadsResponse {
    /** A collection of leads */
    leads?: Array<Lead>;
}

/**
 * Definition: Lead
 */
export interface Lead {
    /** The unique identifier of the lead. */
    LEAD_ID?: number;
    /** The salutation of the lead. */
    SALUTATION?: string;
    /** The title of the lead. */
    TITLE?: string;
    /** First name of the lead. */
    FIRST_NAME?: string;
    /** Last name of the lead. */
    LAST_NAME?: string;
    /** A brief description of the lead. */
    LEAD_DESCRIPTION?: string;
    /** yyyy-MM-dd HH:mm:ss */
    DATE_CREATED_UTC?: string;
    /** yyyy-MM-dd HH:mm:ss */
    DATE_UPDATED_UTC?: string;
    /** yyyy-MM-dd HH:mm:ss */
    CONVERTED_DATE_UTC?: string;
    /** The status Id of the lead. */
    LEAD_STATUS_ID?: number;
    /** Lead visibility. */
    VISIBLE_TO?: string;
    /** The name of the organization. */
    ORGANIZATION_NAME?: string;
    /** Lead phone number. */
    PHONE_NUMBER?: string;
    /** Lead mobile phone number. */
    MOBILE_PHONE_NUMBER?: string;
    /** Lead email address. */
    EMAIL_ADDRESS?: string;
    /** A boolean value determining whether or not the lead is converted. */
    CONVERTED?: boolean;
    /** Lead website URL */
    WEBSITE_URL?: string;
    /** The unique identifier of the lead owner. */
    OWNER_USER_ID?: number;
    /** The unique identifier of the responsible user. */
    RESPONSIBLE_USER_ID?: number;
    /** The employee count of the lead. */
    EMPLOYEE_COUNT?: number;
    /** The rating of the lead. */
    LEAD_RATING?: number;
    /** The industry of the lead. */
    INDUSTRY?: string;
    /** Address street. */
    ADDRESS_STREET?: string;
    /** Address city. */
    ADDRESS_CITY?: string;
    /** Address state. */
    ADDRESS_STATE?: string;
    /** Address postcode. */
    ADDRESS_POSTCODE?: string;
    /** Address country. */
    ADDRESS_COUNTRY?: string;
    /** Lead fax number. */
    FAX_NUMBER?: string;
    /** The unique identifier of the converted contact. */
    CONVERTED_CONTACT_ID?: number;
    /** The unique identifier of the converted organization. */
    CONVERTED_ORGANIZATION_ID?: number;
    /** The unique identifier of the opportunity. */
    CONVERTED_OPPORTUNITY_ID?: number;
    /** The unique identifier of the lead source. */
    LEAD_SOURCE_ID?: number;
    /** The unique identifier of the team. */
    VISIBLE_TEAM_ID?: number;
    /** The unique identifiers of the users. */
    VISIBLE_USER_IDS?: number;
    /** Lead Image URL */
    IMAGE_URL?: string;
    /** Can edit */
    CAN_EDIT?: boolean;
    /** Can delete */
    CAN_DELETE?: boolean;
}

/**
 * Definition: LeadRequest
 */
export interface LeadRequest {
    /** The title of the lead. */
    TITLE?: string;
    /** First name of the lead. */
    FIRST_NAME?: string;
    /** Last name of the lead. */
    LAST_NAME: string;
    /** A brief description of the lead. */
    LEAD_DESCRIPTION?: string;
    /** yyyy-MM-dd HH:mm:ss */
    CONVERTED_DATE_UTC?: string;
    /** Lead Visibility */
    VISIBLE_TO?: string;
    /** Visible Team Id (Required when team visibility state is selected) */
    VISIBLE_TEAM_ID?: number;
    /** Visible User IDs (Required when individuals visibility state is selected) */
    VISIBLE_USER_IDS?: number;
    /** The name of the organization. */
    ORGANIZATION_NAME?: string;
    /** Lead phone number. */
    PHONE_NUMBER?: string;
    /** Lead mobile phone number. */
    MOBILE_PHONE_NUMBER?: string;
    /** Lead email address. */
    EMAIL_ADDRESS?: string;
    /** A boolean value determining whether or not the lead is converted. */
    CONVERTED?: boolean;
    /** Lead website URL */
    WEBSITE_URL?: string;
    /** The unique identifier of the lead owner. */
    OWNER_USER_ID?: number;
    /** The unique identifier of the responsible user. */
    RESPONSIBLE_USER_ID?: number;
    /** The employee count of the lead. */
    EMPLOYEE_COUNT?: number;
    /** The rating of the lead. */
    LEAD_RATING?: number;
    /** The industry of the lead. */
    INDUSTRY?: string;
    /** Tag */
    TAGS?: Array<Tag>;
}

/**
 * Definition: ListContactsResponse
 */
export interface ListContactsResponse {
    /** A collection of contacts */
    contacts?: Array<Contact>;
}

/**
 * Definition: Contact
 */
export interface Contact {
    ADDRESS_WORK?: Record<string, unknown>;
    ADDRESS_HOME?: Record<string, unknown>;
    ADDRESS_POSTAL?: Record<string, unknown>;
    ADDRESS_PRIMARY?: Record<string, unknown>;
    ADDRESS_OTHER?: Record<string, unknown>;
    /** The work email of the contact. */
    EMAIL_WORK?: string;
    /** The home email of the contact. */
    EMAIL_HOME?: string;
    /** The personal email of the contact. */
    EMAIL_PERSONAL?: string;
    /** The other email of the contact. */
    EMAIL_OTHER?: string;
    /** The work phone number of the contact. */
    PHONE_WORK?: string;
    /** The home phone number of the contact. */
    PHONE_HOME?: string;
    /** The mobile phone number of the contact. */
    PHONE_MOBILE?: string;
    /** The other phone number of the contact. */
    PHONE_OTHER?: string;
    /** The unique identifier of the contact. */
    CONTACT_ID?: number;
    /** The salutation of the contact. */
    SALUTATION?: string;
    /** First name of the contact. */
    FIRST_NAME?: string;
    /** Last name of the contact. */
    LAST_NAME?: string;
    /** Contact image URL. */
    IMAGE_URL?: string;
    /** yyyy-MM-dd HH:mm:ss */
    DATE_CREATED_UTC?: string;
    /** Contact visibility. */
    VISIBLE_TO?: string;
    /** Background */
    BACKGROUND?: string;
    /** Default linked organization. */
    DEFAULT_LINKED_ORGANISATION?: number;
    /** The unique identifier of the owner. */
    OWNER_USER_ID?: number;
    /** yyyy-MM-dd HH:mm:ss */
    DATE_UPDATED_UTC?: string;
    /** The unique identifier of the team. */
    VISIBLE_TEAM_ID?: number;
    /** The unique identifiers of the users. */
    VISIBLE_USER_IDS?: number;
    /** Can edit */
    CAN_EDIT?: boolean;
    /** Can delete */
    CAN_DELETE?: boolean;
    /** Social LinkedIn. */
    SOCIAL_LINKEDIN?: string;
    /** Social Facebook. */
    SOCIAL_FACEBOOK?: string;
    /** Social Twitter. */
    SOCIAL_TWITTER?: string;
}

/**
 * Definition: Address
 */
export interface Address {
    /** The unique identifier of the address. */
    ADDRESS_ID?: number;
    /** Address type. */
    ADDRESS_TYPE?: string;
    /** Address street. */
    STREET?: string;
    /** Address city. */
    CITY?: string;
    /** Address state. */
    STATE?: string;
    /** Address postcode. */
    POSTCODE?: string;
    /** Address country. */
    COUNTRY?: string;
}

/**
 * Definition: ContactInfo
 */
export interface ContactInfo {
    /** The unique identifier of the contact info. */
    CONTACT_INFO_ID?: number;
    /** Contact info type. */
    TYPE?: string;
    /** Contact info sub-type. */
    SUBTYPE?: string;
    /** Contact info label. */
    LABEL?: string;
    /** Contact info detail. */
    DETAIL?: string;
}

/**
 * Definition: ContactRequest
 */
export interface ContactRequest {
    /** The salutation of the contact. */
    SALUTATION?: string;
    /** First name of the contact. */
    FIRST_NAME: string;
    /** Last name of the lead. */
    LAST_NAME: string;
    /** Background */
    BACKGROUND?: string;
    /** Contact Visibility */
    VISIBLE_TO?: string;
    /** Visible Team Id (Required when team visibility state is selected) */
    VISIBLE_TEAM_ID?: number;
    /** Visible User IDs (Required when individuals visibility state is selected) */
    VISIBLE_USER_IDS?: number;
    /** Contact Information */
    CONTACTINFOS?: Array<ContactInfo>;
    /** Tag */
    TAGS?: Array<Tag>;
}

/**
 * Definition: ListUsersResponse
 */
export interface ListUsersResponse {
    /** A collection of users */
    users?: Array<User>;
}

/**
 * Definition: User
 */
export interface User {
    /** The unique identifier of the user. */
    USER_ID?: number;
    /** The unique identifier of the contact. */
    CONTACT_ID?: number;
    /** First name of the user. */
    FIRST_NAME?: string;
    /** Last name of the user. */
    LAST_NAME?: string;
    /** User email address. */
    EMAIL_ADDRESS?: string;
    /** The unique identifier of the timezone. */
    TIMEZONE_ID?: string;
    /** The unique identifier of the email dropbox. */
    EMAIL_DROPBOX_IDENTIFIER?: string;
    /** Email dropbox address */
    EMAIL_DROPBOX_ADDRESS?: string;
    /** A boolean value determining whether or not the user is an administrator. */
    ADMINISTRATOR?: boolean;
    /** A boolean value determining whether or not the user is the account owner. */
    ACCOUNT_OWNER?: boolean;
    /** A boolean value determining whether or not the user is active. */
    ACTIVE?: boolean;
    /** yyyy-MM-dd HH:mm:ss */
    DATE_CREATED_UTC?: string;
    /** yyyy-MM-dd HH:mm:ss */
    DATE_UPDATED_UTC?: string;
    /** User currency. */
    USER_CURRENCY?: string;
    /** Contact display. */
    CONTACT_DISPLAY?: string;
    /** Contact order. */
    CONTACT_ORDER?: string;
    /** Task week start. */
    TASK_WEEK_START?: number;
    /** The unique identifier of the instance. */
    INSTANCE_ID?: number;
}

/**
 * Definition: Object
 */
export interface ObjectEntity {
    [key: string]: unknown;
}

/**
 * Definition: ListEventsResponse
 */
export interface ListEventsResponse {
    /** A collection of events */
    events?: Array<Event>;
}

/**
 * Definition: Event
 */
export interface Event {
    /** The unique identifier of the event. */
    EVENT_ID?: number;
    /** The title of the event. */
    TITLE?: string;
    /** yyyy-MM-dd HH:mm:ss */
    START_DATE_UTC?: string;
    /** yyyy-MM-dd HH:mm:ss */
    END_DATE_UTC?: string;
    /** yyyy-MM-dd HH:mm:ss */
    DATE_CREATED_UTC?: string;
    /** yyyy-MM-dd HH:mm:ss */
    DATE_UPDATED_UTC?: string;
    /** The unique identifier of the owner. */
    OWNER_USER_ID?: number;
    /** A boolean value determining whether or not the reminder was sent. */
    REMINDER_SENT?: boolean;
    /** A boolean value determining whether or not the event is all day. */
    ALL_DAY?: boolean;
    /** A boolean value determining whether or not the event is publicly visible. */
    PUBLICLY_VISIBLE?: boolean;
    /** Can edit */
    CAN_EDIT?: boolean;
    /** Can delete */
    CAN_DELETE?: boolean;
}

/**
 * Definition: OrganizationRequest
 */
export interface OrganizationRequest {
    /** The name of the organization. */
    ORGANISATION_NAME: string;
    /** The background of the organization. */
    ORGANISATION_BACKGROUND?: string;
    /** Organization visibility. */
    VISIBLE_TO?: string;
    /** Visible Team Id (Required when team visibility state is selected) */
    VISIBLE_TEAM_ID?: number;
    /** Visible User IDs (Required when individuals visibility state is selected) */
    VISIBLE_USER_IDS?: number;
    /** Organization Addresses */
    ADDRESSES?: Array<Address>;
    /** Contact Information */
    CONTACTINFOS?: Array<ContactInfo>;
    /** Tags */
    TAGS?: Array<Tag>;
}

/**
 * Definition: Organization
 */
export interface Organization {
    /** The unique identifier of the organization. */
    ORGANISATION_ID?: number;
    /** The background of the organization. */
    ORGANISATION_BACKGROUND?: string;
    /** The name of the organization. */
    ORGANISATION_NAME?: string;
    /** yyyy-MM-dd HH:mm:ss */
    DATE_CREATED_UTC?: string;
    /** yyyy-MM-dd HH:mm:ss */
    DATE_UPDATED_UTC?: string;
    /** The unique identifier of the owner. */
    OWNER_USER_ID?: number;
    /** Organization visibility. */
    VISIBLE_TO?: string;
    /** Organization Addresses */
    ADDRESSES?: Array<Address>;
    /** Contact Information */
    CONTACTINFOS?: Array<ContactInfo>;
    /** The background of the organization. */
    BACKGROUND?: string;
    /** Organization image URL. */
    IMAGE_URL?: string;
    /** The unique identifier of the team. */
    VISIBLE_TEAM_ID?: number;
    /** The unique identifiers of the users. */
    VISIBLE_USER_IDS?: number;
}
// #endregion Types

export const InsightlyTriggerOperations = {
    OnContactCreated: "OnContactCreated",
    OnContactUpdated: "OnContactUpdated",
    OnEventCreated: "OnEventCreated",
    OnEventUpdated: "OnEventUpdated",
    OnLeadCreated: "OnLeadCreated",
    OnLeadUpdated: "OnLeadUpdated",
    OnProjectCreated: "OnProjectCreated",
    OnProjectUpdated: "OnProjectUpdated",
    OnTaskAssignedToMe: "OnTaskAssignedToMe",
    OnTaskCreated: "OnTaskCreated",
    OnTaskUpdated: "OnTaskUpdated",
} as const;

export type InsightlyTriggerOperation = typeof InsightlyTriggerOperations[keyof typeof InsightlyTriggerOperations];

// #region Client

/**
 * Typed client for the insightly connector.
 */
export class InsightlyClient extends ConnectorClientBase {

    /**
     * Creates a new InsightlyClient.
     * @param connectionRuntimeUrl The connection runtime URL from Azure Portal.
     * @param tokenProvider The token provider for authentication.
     * @param options Optional connector client options.
     */
    constructor(connectionRuntimeUrl: string, tokenProvider: TokenProvider, options?: ConnectorClientOptions) {
        super(connectionRuntimeUrl, tokenProvider, options);
    }

    public get connectorName(): string {
        return "insightly";
    }

    /**
     * List tasks
     * @remarks List tasks
     */
    public async listTasksAsync(abortSignal?: AbortSignal): Promise<ListTasksResponse> {
        const requestPath = `/Tasks`;
        const url = this.resolveUrl(requestPath);
        const httpResponse = await this.httpClient.sendAsync<ListTasksResponse>("GET", url, undefined, undefined, abortSignal);

        if (!httpResponse.isSuccessStatusCode) {
            throw new ConnectorException(this.connectorName, `GET ${requestPath}`, httpResponse.statusCode, httpResponse.text);
        }

        return httpResponse.value as ListTasksResponse;
    }

    /**
     * Update task
     * @remarks Update task
     */
    public async updateTaskAsync(input: TaskRequest, id?: string, abortSignal?: AbortSignal): Promise<TaskObject> {
        const queryParams: string[] = [];
        if (id !== undefined) {
            queryParams.push(`id=${encodeURIComponent(String(id))}`);
        }
        const requestPath = `/Tasks` + (queryParams.length > 0 ? "?" + queryParams.join("&") : "");
        const url = this.resolveUrl(requestPath);
        const httpResponse = await this.httpClient.sendAsync<TaskObject>("PUT", url, undefined, input, abortSignal);

        if (!httpResponse.isSuccessStatusCode) {
            throw new ConnectorException(this.connectorName, `PUT ${requestPath}`, httpResponse.statusCode, httpResponse.text);
        }

        return httpResponse.value as TaskObject;
    }

    /**
     * Add task
     * @remarks Add task
     */
    public async addTaskAsync(input: TaskRequest, abortSignal?: AbortSignal): Promise<TaskObject> {
        const requestPath = `/Tasks`;
        const url = this.resolveUrl(requestPath);
        const httpResponse = await this.httpClient.sendAsync<TaskObject>("POST", url, undefined, input, abortSignal);

        if (!httpResponse.isSuccessStatusCode) {
            throw new ConnectorException(this.connectorName, `POST ${requestPath}`, httpResponse.statusCode, httpResponse.text);
        }

        return httpResponse.value as TaskObject;
    }

    /**
     * List projects
     * @remarks List projects
     */
    public async listProjectsAsync(abortSignal?: AbortSignal): Promise<ListProjectsResponse> {
        const requestPath = `/Projects`;
        const url = this.resolveUrl(requestPath);
        const httpResponse = await this.httpClient.sendAsync<ListProjectsResponse>("GET", url, undefined, undefined, abortSignal);

        if (!httpResponse.isSuccessStatusCode) {
            throw new ConnectorException(this.connectorName, `GET ${requestPath}`, httpResponse.statusCode, httpResponse.text);
        }

        return httpResponse.value as ListProjectsResponse;
    }

    /**
     * Update project
     * @remarks Update project
     */
    public async updateProjectAsync(input: ProjectRequest, id?: string, abortSignal?: AbortSignal): Promise<Project> {
        const queryParams: string[] = [];
        if (id !== undefined) {
            queryParams.push(`id=${encodeURIComponent(String(id))}`);
        }
        const requestPath = `/Projects` + (queryParams.length > 0 ? "?" + queryParams.join("&") : "");
        const url = this.resolveUrl(requestPath);
        const httpResponse = await this.httpClient.sendAsync<Project>("PUT", url, undefined, input, abortSignal);

        if (!httpResponse.isSuccessStatusCode) {
            throw new ConnectorException(this.connectorName, `PUT ${requestPath}`, httpResponse.statusCode, httpResponse.text);
        }

        return httpResponse.value as Project;
    }

    /**
     * Add project
     * @remarks Add project
     */
    public async addProjectAsync(input: ProjectRequest, abortSignal?: AbortSignal): Promise<Project> {
        const requestPath = `/Projects`;
        const url = this.resolveUrl(requestPath);
        const httpResponse = await this.httpClient.sendAsync<Project>("POST", url, undefined, input, abortSignal);

        if (!httpResponse.isSuccessStatusCode) {
            throw new ConnectorException(this.connectorName, `POST ${requestPath}`, httpResponse.statusCode, httpResponse.text);
        }

        return httpResponse.value as Project;
    }

    /**
     * List leads
     * @remarks List leads
     */
    public async listLeadsAsync(abortSignal?: AbortSignal): Promise<ListLeadsResponse> {
        const requestPath = `/Leads`;
        const url = this.resolveUrl(requestPath);
        const httpResponse = await this.httpClient.sendAsync<ListLeadsResponse>("GET", url, undefined, undefined, abortSignal);

        if (!httpResponse.isSuccessStatusCode) {
            throw new ConnectorException(this.connectorName, `GET ${requestPath}`, httpResponse.statusCode, httpResponse.text);
        }

        return httpResponse.value as ListLeadsResponse;
    }

    /**
     * Update lead
     * @remarks Update lead
     */
    public async updateLeadAsync(input: LeadRequest, id?: string, abortSignal?: AbortSignal): Promise<Lead> {
        const queryParams: string[] = [];
        if (id !== undefined) {
            queryParams.push(`id=${encodeURIComponent(String(id))}`);
        }
        const requestPath = `/Leads` + (queryParams.length > 0 ? "?" + queryParams.join("&") : "");
        const url = this.resolveUrl(requestPath);
        const httpResponse = await this.httpClient.sendAsync<Lead>("PUT", url, undefined, input, abortSignal);

        if (!httpResponse.isSuccessStatusCode) {
            throw new ConnectorException(this.connectorName, `PUT ${requestPath}`, httpResponse.statusCode, httpResponse.text);
        }

        return httpResponse.value as Lead;
    }

    /**
     * Add lead
     * @remarks Add lead
     */
    public async addLeadAsync(input: LeadRequest, abortSignal?: AbortSignal): Promise<Lead> {
        const requestPath = `/Leads`;
        const url = this.resolveUrl(requestPath);
        const httpResponse = await this.httpClient.sendAsync<Lead>("POST", url, undefined, input, abortSignal);

        if (!httpResponse.isSuccessStatusCode) {
            throw new ConnectorException(this.connectorName, `POST ${requestPath}`, httpResponse.statusCode, httpResponse.text);
        }

        return httpResponse.value as Lead;
    }

    /**
     * List contacts
     * @remarks List contacts
     */
    public async listContactsAsync(abortSignal?: AbortSignal): Promise<ListContactsResponse> {
        const requestPath = `/Contacts`;
        const url = this.resolveUrl(requestPath);
        const httpResponse = await this.httpClient.sendAsync<ListContactsResponse>("GET", url, undefined, undefined, abortSignal);

        if (!httpResponse.isSuccessStatusCode) {
            throw new ConnectorException(this.connectorName, `GET ${requestPath}`, httpResponse.statusCode, httpResponse.text);
        }

        return httpResponse.value as ListContactsResponse;
    }

    /**
     * Update contact
     * @remarks Update contact
     */
    public async updateContactAsync(input: ContactRequest, id?: string, abortSignal?: AbortSignal): Promise<Contact> {
        const queryParams: string[] = [];
        if (id !== undefined) {
            queryParams.push(`id=${encodeURIComponent(String(id))}`);
        }
        const requestPath = `/Contacts` + (queryParams.length > 0 ? "?" + queryParams.join("&") : "");
        const url = this.resolveUrl(requestPath);
        const httpResponse = await this.httpClient.sendAsync<Contact>("PUT", url, undefined, input, abortSignal);

        if (!httpResponse.isSuccessStatusCode) {
            throw new ConnectorException(this.connectorName, `PUT ${requestPath}`, httpResponse.statusCode, httpResponse.text);
        }

        return httpResponse.value as Contact;
    }

    /**
     * Add contact
     * @remarks Add contact
     */
    public async addContactAsync(input: ContactRequest, abortSignal?: AbortSignal): Promise<Contact> {
        const requestPath = `/Contacts`;
        const url = this.resolveUrl(requestPath);
        const httpResponse = await this.httpClient.sendAsync<Contact>("POST", url, undefined, input, abortSignal);

        if (!httpResponse.isSuccessStatusCode) {
            throw new ConnectorException(this.connectorName, `POST ${requestPath}`, httpResponse.statusCode, httpResponse.text);
        }

        return httpResponse.value as Contact;
    }

    /**
     * List users
     * @remarks List users
     */
    public async listUsersAsync(abortSignal?: AbortSignal): Promise<ListUsersResponse> {
        const requestPath = `/Users`;
        const url = this.resolveUrl(requestPath);
        const httpResponse = await this.httpClient.sendAsync<ListUsersResponse>("GET", url, undefined, undefined, abortSignal);

        if (!httpResponse.isSuccessStatusCode) {
            throw new ConnectorException(this.connectorName, `GET ${requestPath}`, httpResponse.statusCode, httpResponse.text);
        }

        return httpResponse.value as ListUsersResponse;
    }

    /**
     * Delete task
     * @remarks Delete task
     */
    public async deleteTaskAsync(taskId: string, abortSignal?: AbortSignal): Promise<ObjectEntity> {
        const requestPath = `/Tasks/${taskId}`;
        const url = this.resolveUrl(requestPath);
        const httpResponse = await this.httpClient.sendAsync<ObjectEntity>("DELETE", url, undefined, undefined, abortSignal);

        if (!httpResponse.isSuccessStatusCode) {
            throw new ConnectorException(this.connectorName, `DELETE ${requestPath}`, httpResponse.statusCode, httpResponse.text);
        }

        return httpResponse.value as ObjectEntity;
    }

    /**
     * Follow a task
     * @remarks Follow a task
     */
    public async followTaskAsync(taskId: string, abortSignal?: AbortSignal): Promise<ObjectEntity> {
        const requestPath = `/Tasks/${taskId}/Follow`;
        const url = this.resolveUrl(requestPath);
        const httpResponse = await this.httpClient.sendAsync<ObjectEntity>("POST", url, undefined, undefined, abortSignal);

        if (!httpResponse.isSuccessStatusCode) {
            throw new ConnectorException(this.connectorName, `POST ${requestPath}`, httpResponse.statusCode, httpResponse.text);
        }

        return httpResponse.value as ObjectEntity;
    }

    /**
     * Delete project
     * @remarks Delete project
     */
    public async deleteProjectAsync(projectId: string, abortSignal?: AbortSignal): Promise<ObjectEntity> {
        const requestPath = `/Projects/${projectId}`;
        const url = this.resolveUrl(requestPath);
        const httpResponse = await this.httpClient.sendAsync<ObjectEntity>("DELETE", url, undefined, undefined, abortSignal);

        if (!httpResponse.isSuccessStatusCode) {
            throw new ConnectorException(this.connectorName, `DELETE ${requestPath}`, httpResponse.statusCode, httpResponse.text);
        }

        return httpResponse.value as ObjectEntity;
    }

    /**
     * Delete lead
     * @remarks Delete lead
     */
    public async deleteLeadAsync(leadId: string, abortSignal?: AbortSignal): Promise<ObjectEntity> {
        const requestPath = `/Leads/${leadId}`;
        const url = this.resolveUrl(requestPath);
        const httpResponse = await this.httpClient.sendAsync<ObjectEntity>("DELETE", url, undefined, undefined, abortSignal);

        if (!httpResponse.isSuccessStatusCode) {
            throw new ConnectorException(this.connectorName, `DELETE ${requestPath}`, httpResponse.statusCode, httpResponse.text);
        }

        return httpResponse.value as ObjectEntity;
    }

    /**
     * Delete contact
     * @remarks Delete contact
     */
    public async deleteContactAsync(contactId: string, abortSignal?: AbortSignal): Promise<ObjectEntity> {
        const requestPath = `/Contacts/${contactId}`;
        const url = this.resolveUrl(requestPath);
        const httpResponse = await this.httpClient.sendAsync<ObjectEntity>("DELETE", url, undefined, undefined, abortSignal);

        if (!httpResponse.isSuccessStatusCode) {
            throw new ConnectorException(this.connectorName, `DELETE ${requestPath}`, httpResponse.statusCode, httpResponse.text);
        }

        return httpResponse.value as ObjectEntity;
    }

    /**
     * Add organization
     * @remarks Add organization
     */
    public async addOrganizationAsync(input: OrganizationRequest, abortSignal?: AbortSignal): Promise<Organization> {
        const requestPath = `/Organisations`;
        const url = this.resolveUrl(requestPath);
        const httpResponse = await this.httpClient.sendAsync<Organization>("POST", url, undefined, input, abortSignal);

        if (!httpResponse.isSuccessStatusCode) {
            throw new ConnectorException(this.connectorName, `POST ${requestPath}`, httpResponse.statusCode, httpResponse.text);
        }

        return httpResponse.value as Organization;
    }

}

// #endregion Client
