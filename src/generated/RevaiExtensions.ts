// RevaiExtensions.ts - Auto-generated DirectClient SDK for TypeScript
// Do not edit this file directly.

import type { TokenCredential } from "@azure/core-auth";
import { ConnectorClientBase } from "../azureConnectors/clientBase.ts";
import type { ConnectorClientOptions, ConnectorOperationOptions } from "../azureConnectors/options.ts";

// #region Types

/**
 * Response for Get transcription job
 */
export interface TranscriptionGetResponse {
    /** The identifier. */
    id?: string;
    /** The status. */
    status?: string;
    /** The language. */
    language?: string;
    /** When created on. */
    created_on?: string;
    /** The transcriber. */
    transcriber?: string;
}

/**
 * Submit transcription job
 */
export interface TranscriptionInput {
    source_config?: Record<string, unknown>;
    /** The metadata. */
    metadata?: string;
    notification_config: Record<string, unknown>;
    /** The delete after seconds. */
    delete_after_seconds?: number;
    /** The transcriber. */
    transcriber?: string;
    /** Whether verbatim. */
    verbatim?: boolean;
    /** Whether to rush. */
    rush?: boolean;
    /** Whether test mode. */
    test_mode?: boolean;
    segments_to_transcribe?: Array<Record<string, unknown>>;
    speakers_names?: Array<Record<string, unknown>>;
    /** Whether to skip diarization. */
    skip_diarization?: boolean;
    /** Whether to skip postprocessing. */
    skip_postprocessing?: boolean;
    /** Whether to skip punctuations. */
    skip_punctuation?: boolean;
    /** Whether to remove disfluencies. */
    remove_disfluencies?: boolean;
    /** Whether to remove atmospherics. */
    remove_atmospherics?: boolean;
    /** Whether to filter profanity. */
    filter_profanity?: boolean;
    /** The speaker channels count. */
    speaker_channels_count?: number;
    /** The speakers count. */
    speakers_count?: number;
    /** The custom vocabulary identifier. */
    custom_vocabulary_id?: string;
    custom_vocabularies?: Array<Record<string, unknown>>;
    /** The language. */
    language?: string;
}

/**
 * Response for Submit transcription job
 */
export interface TranscriptionPostResponse {
    /** The identifier. */
    id?: string;
    /** The status. */
    status?: string;
    /** The language. */
    language?: string;
    /** When created on. */
    created_on?: string;
    /** The transcriber. */
    transcriber?: string;
}

/**
 * Response for Get transcript
 */
export interface TranscriptGetResponse {
    monologues?: Array<Record<string, unknown>>;
}

/**
 * Response for Get account
 */
export interface AccountGetResponse {
    /** The email address. */
    email?: string;
    /** The free balance. */
    free_balance?: number;
    /** The purchased balance. */
    purchased_balance?: number;
    /** The total balance. */
    total_balance?: number;
    /** The invoiced balance. */
    invoiced_balance?: number;
    /** The balance seconds. */
    balance_seconds?: number;
    /** Whether HIPAA is enabled. */
    hipaa_enabled?: boolean;
}

/**
 * Submit custom vocabulary
 */
export interface VocabularyInput {
    /** The metadata. */
    metadata?: string;
    notification_config?: Record<string, unknown>;
    custom_vocabularies?: Array<Record<string, unknown>>;
}

/**
 * Response for Submit custom vocabulary
 */
export interface VocabularyPostResponse {
    /** The identifier. */
    id?: string;
    /** The status. */
    status?: string;
    /** When created on. */
    created_on?: string;
    /** The callback URL address. */
    callback_url?: string;
}

/**
 * Response for Get vocabulary
 */
export interface VocabularyGetResponse {
    /** The identifier. */
    id?: string;
    /** The status. */
    status?: string;
    /** When created on. */
    created_on?: string;
    /** The callback URL address. */
    callback_url?: string;
}

/**
 * Submit topic extraction job
 */
export interface ExtractionInput {
    /** The language. */
    language?: string;
    /** The metadata. */
    metadata?: string;
    notification_config?: Record<string, unknown>;
    /** The delete after seconds. */
    delete_after_seconds?: number;
    json?: Record<string, unknown>;
}

/**
 * Response for Submit topic extraction job
 */
export interface ExtractionPostResponse {
    /** The identifier. */
    id?: string;
    /** The status. */
    status?: string;
    /** The language. */
    language?: string;
    /** When created on. */
    created_on?: string;
    /** The type. */
    type?: string;
}

/**
 * Response for Get topic extraction job
 */
export interface ExtractionGetResponse {
    /** The identifier. */
    id?: string;
    /** The status. */
    status?: string;
    /** The language. */
    language?: string;
    /** When created on. */
    created_on?: string;
    /** The type. */
    type?: string;
}

/**
 * Response for Get topic extraction result
 */
export interface ExtractionResultGetResponse {
    topics?: Array<Record<string, unknown>>;
}

/**
 * Submit sentiment analysis job
 */
export interface AnalysisInput {
    /** The language. */
    language?: string;
    /** The metadata. */
    metadata?: string;
    notification_config?: Record<string, unknown>;
    /** The delete after seconds. */
    delete_after_seconds?: number;
    json?: Record<string, unknown>;
}

/**
 * Response for Submit sentiment analysis job
 */
export interface AnalysisPostResponse {
    /** The identifier. */
    id?: string;
    /** The status. */
    status?: string;
    /** The language. */
    language?: string;
    /** When created on. */
    created_on?: string;
    /** The callback URL address. */
    callback_url?: string;
    /** The type. */
    type?: string;
}

/**
 * Response for Get sentiment analysis job
 */
export interface AnalysisGetResponse {
    /** The identifier. */
    id?: string;
    /** The status. */
    status?: string;
    /** The language. */
    language?: string;
    /** When created on. */
    created_on?: string;
    /** The callback URL address. */
    callback_url?: string;
    /** The type. */
    type?: string;
}

/**
 * Response for Get sentiment analysis result
 */
export interface AnalysisResultGetResponse {
    messages?: Array<Record<string, unknown>>;
}

/**
 * Submit language identification job
 */
export interface IdentificationInput {
    /** The metadata. */
    metadata?: string;
    notification_config?: Record<string, unknown>;
    /** The delete after seconds. */
    delete_after_seconds?: number;
    source_config?: Record<string, unknown>;
}

/**
 * Response for Submit language identification job
 */
export interface IdentificationPostResponse {
    /** The identifier. */
    id?: string;
    /** The status. */
    status?: string;
    /** When created on. */
    created_on?: string;
    /** The type. */
    type?: string;
}

/**
 * Response for Get language identification job
 */
export interface IdentificationGetResponse {
    /** The identifier. */
    id?: string;
    /** The status. */
    status?: string;
    /** When created on. */
    created_on?: string;
    /** The type. */
    type?: string;
}

/**
 * Response for Get language identification result
 */
export interface IdentificationResultGetResponse {
    /** The top language. */
    top_language?: string;
    language_confidences?: Array<Record<string, unknown>>;
}

/**
 * Submit forced alignment job
 */
export interface AlignmentInput {
    /** The metadata. */
    metadata?: string;
    notification_config?: Record<string, unknown>;
    /** The delete after seconds. */
    delete_after_seconds?: number;
    source_config?: Record<string, unknown>;
    /** The transcript text. */
    transcript_text?: string;
    /** The language. */
    language?: string;
}

/**
 * Response for Submit forced alignment job
 */
export interface AlignmentPostResponse {
    /** The identifier. */
    id?: string;
    /** The status. */
    status?: string;
    /** When created on. */
    created_on?: string;
    /** The type. */
    type?: string;
}

/**
 * Response for Get forced alignment job
 */
export interface AlignmentGetResponse {
    /** The identifier. */
    id?: string;
    /** The status. */
    status?: string;
    /** When created on. */
    created_on?: string;
    /** The type. */
    type?: string;
}

/**
 * Response for Get forced alignment transcript
 */
export interface AlignmentTranscriptGetResponse {
    monologues?: Array<Record<string, unknown>>;
}

/**
 * Options for the getTranscriptions operation.
 */
export interface GetTranscriptionsOptions extends ConnectorOperationOptions {
    /** The limit. */
    limit?: number;
    /** The starting after. */
    startingAfter?: string;
}

/**
 * Options for the getCaptions operation.
 */
export interface GetCaptionsOptions extends ConnectorOperationOptions {
    /** The response format. */
    accept?: string;
}

/**
 * Options for the getVocabularies operation.
 */
export interface GetVocabulariesOptions extends ConnectorOperationOptions {
    /** The limit. */
    limit?: number;
}

/**
 * Options for the getExtractions operation.
 */
export interface GetExtractionsOptions extends ConnectorOperationOptions {
    /** The limit. */
    limit?: number;
    /** The starting after. */
    startingAfter?: string;
}

/**
 * Options for the getExtractionResult operation.
 */
export interface GetExtractionResultOptions extends ConnectorOperationOptions {
    /** The threshold. */
    threshold?: number;
}

/**
 * Options for the getAnalysises operation.
 */
export interface GetAnalysisesOptions extends ConnectorOperationOptions {
    /** The limit. */
    limit?: number;
    /** The starting after. */
    startingAfter?: string;
}

/**
 * Options for the getAnalysisResult operation.
 */
export interface GetAnalysisResultOptions extends ConnectorOperationOptions {
    /** The filter for. */
    filterFor?: string;
}

/**
 * Options for the getIdentifications operation.
 */
export interface GetIdentificationsOptions extends ConnectorOperationOptions {
    /** The limit. */
    limit?: number;
    /** The starting after. */
    startingAfter?: string;
}

/**
 * Options for the getAlignments operation.
 */
export interface GetAlignmentsOptions extends ConnectorOperationOptions {
    /** The limit. */
    limit?: number;
    /** The starting after. */
    startingAfter?: string;
}
// #endregion Types

// #region Client

/**
 * Typed client for the revai connector.
 */
export class RevaiClient extends ConnectorClientBase {

    /**
     * Creates a new RevaiClient.
     * @param connectionRuntimeUrl The connection runtime URL from Azure Portal.
     * @param credential The credential used for authentication.
     * @param options Optional connector client options.
     */
    constructor(connectionRuntimeUrl: string, credential: TokenCredential, options?: ConnectorClientOptions) {
        super(connectionRuntimeUrl, credential, options);
    }

    public get connectorName(): string {
        return "revai";
    }

    /**
     * Get transcription job
     * @remarks Returns information about a transcription job.
     */
    public async getTranscription(id: string, options: ConnectorOperationOptions = {}): Promise<TranscriptionGetResponse> {
        const requestPath = `/speechtotext/v1/jobs/${encodeURIComponent(String(id))}`;
        const requestUrl = this.resolveUrl(requestPath);
        const httpResponse = await this.sendWithTracingAsync<TranscriptionGetResponse>("Revai.getTranscription", "TranscriptionGet", "GET", requestUrl, undefined, options);

        return httpResponse.value as TranscriptionGetResponse;
    }

    /**
     * Delete transcription job
     * @remarks Deletes a transcription job. All data related to the job, such as input media and transcript, will be permanently deleted. A job can only be deleted once it's completed (either with success or failure).
     */
    public async deleteTranscription(id: string, options: ConnectorOperationOptions = {}): Promise<string> {
        const requestPath = `/speechtotext/v1/jobs/${encodeURIComponent(String(id))}`;
        const requestUrl = this.resolveUrl(requestPath);
        const httpResponse = await this.sendWithTracingAsync<string>("Revai.deleteTranscription", "TranscriptionDelete", "DELETE", requestUrl, undefined, options);

        return httpResponse.value as string;
    }

    /**
     * Get transcription job list
     * @remarks Retrieve a list of transcription jobs submitted within the last 30 days.
     */
    public async getTranscriptions(options: GetTranscriptionsOptions = {}): Promise<Array<Record<string, unknown>>> {
        const queryParams: string[] = [];
        if (options.limit !== undefined) {
            queryParams.push(`limit=${encodeURIComponent(String(options.limit))}`);
        }
        if (options.startingAfter !== undefined) {
            queryParams.push(`starting_after=${encodeURIComponent(String(options.startingAfter))}`);
        }
        const requestPath = `/speechtotext/v1/jobs` + (queryParams.length > 0 ? "?" + queryParams.join("&") : "");
        const requestUrl = this.resolveUrl(requestPath);
        const httpResponse = await this.sendWithTracingAsync<Array<Record<string, unknown>>>("Revai.getTranscriptions", "TranscriptionsGet", "GET", requestUrl, undefined, options);

        return httpResponse.value as Array<Record<string, unknown>>;
    }

    /**
     * Submit transcription job
     * @remarks Starts an asynchronous job to transcribe speech-to-text for a media file.
     */
    public async transcription(input: TranscriptionInput, options: ConnectorOperationOptions = {}): Promise<TranscriptionPostResponse> {
        const requestPath = `/speechtotext/v1/jobs`;
        const requestUrl = this.resolveUrl(requestPath);
        const httpResponse = await this.sendWithTracingAsync<TranscriptionPostResponse>("Revai.transcription", "TranscriptionPost", "POST", requestUrl, input, options);

        return httpResponse.value as TranscriptionPostResponse;
    }

    /**
     * Get transcript
     * @remarks Returns the transcript for a completed transcription job.
     */
    public async getTranscript(id: string, options: ConnectorOperationOptions = {}): Promise<TranscriptGetResponse> {
        const requestPath = `/speechtotext/v1/jobs/${encodeURIComponent(String(id))}/transcript`;
        const requestUrl = this.resolveUrl(requestPath);
        const httpResponse = await this.sendWithTracingAsync<TranscriptGetResponse>("Revai.getTranscript", "TranscriptGet", "GET", requestUrl, undefined, options);

        return httpResponse.value as TranscriptGetResponse;
    }

    /**
     * Get captions
     * @remarks Returns the caption output for a transcription job. We currently support the SubRip (SRT) format.
     */
    public async getCaptions(id: string, options: GetCaptionsOptions = {}): Promise<string> {
        const requestPath = `/speechtotext/v1/jobs/${encodeURIComponent(String(id))}/captions`;
        const requestHeaders: Record<string, string> = {};
        if (options.accept !== undefined) {
            requestHeaders["Accept"] = String(options.accept);
        }
        const requestUrl = this.resolveUrl(requestPath);
        const httpResponse = await this.sendWithTracingAsync<string>("Revai.getCaptions", "CaptionsGet", "GET", requestUrl, undefined, options, requestHeaders);

        return httpResponse.value as string;
    }

    /**
     * Get account
     * @remarks Get the developer's account information.
     */
    public async getAccount(options: ConnectorOperationOptions = {}): Promise<AccountGetResponse> {
        const requestPath = `/speechtotext/v1/account`;
        const requestUrl = this.resolveUrl(requestPath);
        const httpResponse = await this.sendWithTracingAsync<AccountGetResponse>("Revai.getAccount", "AccountGet", "GET", requestUrl, undefined, options);

        return httpResponse.value as AccountGetResponse;
    }

    /**
     * Get list of vocabularies
     * @remarks Retrieves a list of most recent custom vocabularies' processing information.
     */
    public async getVocabularies(options: GetVocabulariesOptions = {}): Promise<Array<Record<string, unknown>>> {
        const queryParams: string[] = [];
        if (options.limit !== undefined) {
            queryParams.push(`limit=${encodeURIComponent(String(options.limit))}`);
        }
        const requestPath = `/speechtotext/v1/vocabularies` + (queryParams.length > 0 ? "?" + queryParams.join("&") : "");
        const requestUrl = this.resolveUrl(requestPath);
        const httpResponse = await this.sendWithTracingAsync<Array<Record<string, unknown>>>("Revai.getVocabularies", "VocabulariesGet", "GET", requestUrl, undefined, options);

        return httpResponse.value as Array<Record<string, unknown>>;
    }

    /**
     * Submit custom vocabulary
     * @remarks Submits a Custom Vocabulary for asynchronous processing.
     */
    public async vocabulary(input: VocabularyInput, options: ConnectorOperationOptions = {}): Promise<VocabularyPostResponse> {
        const requestPath = `/speechtotext/v1/vocabularies`;
        const requestUrl = this.resolveUrl(requestPath);
        const httpResponse = await this.sendWithTracingAsync<VocabularyPostResponse>("Revai.vocabulary", "VocabularyPost", "POST", requestUrl, input, options);

        return httpResponse.value as VocabularyPostResponse;
    }

    /**
     * Get vocabulary
     * @remarks Retrieve a custom vocabulary processing information.
     */
    public async getVocabulary(id: string, options: ConnectorOperationOptions = {}): Promise<VocabularyGetResponse> {
        const requestPath = `/speechtotext/v1/vocabularies/${encodeURIComponent(String(id))}`;
        const requestUrl = this.resolveUrl(requestPath);
        const httpResponse = await this.sendWithTracingAsync<VocabularyGetResponse>("Revai.getVocabulary", "VocabularyGet", "GET", requestUrl, undefined, options);

        return httpResponse.value as VocabularyGetResponse;
    }

    /**
     * Delete vocabulary
     * @remarks Deletes a custom vocabulary. All data related to the custom vocabulary, such as webhooks and metadata, will be permanently deleted.
     */
    public async deleteVocabulary(id: string, options: ConnectorOperationOptions = {}): Promise<string> {
        const requestPath = `/speechtotext/v1/vocabularies/${encodeURIComponent(String(id))}`;
        const requestUrl = this.resolveUrl(requestPath);
        const httpResponse = await this.sendWithTracingAsync<string>("Revai.deleteVocabulary", "VocabularyDelete", "DELETE", requestUrl, undefined, options);

        return httpResponse.value as string;
    }

    /**
     * Get topic extraction jobs
     * @remarks Retrieves a list of topic extraction jobs submitted within the last 30 days in reverse chronological order.
     */
    public async getExtractions(options: GetExtractionsOptions = {}): Promise<Array<Record<string, unknown>>> {
        const queryParams: string[] = [];
        if (options.limit !== undefined) {
            queryParams.push(`limit=${encodeURIComponent(String(options.limit))}`);
        }
        if (options.startingAfter !== undefined) {
            queryParams.push(`starting_after=${encodeURIComponent(String(options.startingAfter))}`);
        }
        const requestPath = `/topic_extraction/v1/jobs` + (queryParams.length > 0 ? "?" + queryParams.join("&") : "");
        const requestUrl = this.resolveUrl(requestPath);
        const httpResponse = await this.sendWithTracingAsync<Array<Record<string, unknown>>>("Revai.getExtractions", "ExtractionsGet", "GET", requestUrl, undefined, options);

        return httpResponse.value as Array<Record<string, unknown>>;
    }

    /**
     * Submit topic extraction job
     * @remarks Starts an asynchronous job to extract topics from a transcript.
     */
    public async extraction(input: ExtractionInput, options: ConnectorOperationOptions = {}): Promise<ExtractionPostResponse> {
        const requestPath = `/topic_extraction/v1/jobs`;
        const requestUrl = this.resolveUrl(requestPath);
        const httpResponse = await this.sendWithTracingAsync<ExtractionPostResponse>("Revai.extraction", "ExtractionPost", "POST", requestUrl, input, options);

        return httpResponse.value as ExtractionPostResponse;
    }

    /**
     * Get topic extraction job
     * @remarks Returns information about a topic extraction job.
     */
    public async getExtraction(id: string, options: ConnectorOperationOptions = {}): Promise<ExtractionGetResponse> {
        const requestPath = `/topic_extraction/v1/jobs/${encodeURIComponent(String(id))}`;
        const requestUrl = this.resolveUrl(requestPath);
        const httpResponse = await this.sendWithTracingAsync<ExtractionGetResponse>("Revai.getExtraction", "ExtractionGet", "GET", requestUrl, undefined, options);

        return httpResponse.value as ExtractionGetResponse;
    }

    /**
     * Delete topic extraction jo
     * @remarks Deletes a topic extraction job. All data related to the job will be permanently deleted.
     */
    public async deleteExtraction(id: string, options: ConnectorOperationOptions = {}): Promise<string> {
        const requestPath = `/topic_extraction/v1/jobs/${encodeURIComponent(String(id))}`;
        const requestUrl = this.resolveUrl(requestPath);
        const httpResponse = await this.sendWithTracingAsync<string>("Revai.deleteExtraction", "ExtractionDelete", "DELETE", requestUrl, undefined, options);

        return httpResponse.value as string;
    }

    /**
     * Get topic extraction result
     * @remarks Returns the results for a completed topic extraction job.
     */
    public async getExtractionResult(id: string, options: GetExtractionResultOptions = {}): Promise<ExtractionResultGetResponse> {
        const queryParams: string[] = [];
        if (options.threshold !== undefined) {
            queryParams.push(`threshold=${encodeURIComponent(String(options.threshold))}`);
        }
        const requestPath = `/topic_extraction/v1/jobs/${encodeURIComponent(String(id))}/result` + (queryParams.length > 0 ? "?" + queryParams.join("&") : "");
        const requestUrl = this.resolveUrl(requestPath);
        const httpResponse = await this.sendWithTracingAsync<ExtractionResultGetResponse>("Revai.getExtractionResult", "ExtractionResultGet", "GET", requestUrl, undefined, options);

        return httpResponse.value as ExtractionResultGetResponse;
    }

    /**
     * Get sentiment analysis jobs
     * @remarks Gets a list of sentiment analysis jobs submitted within the last 30 days in reverse chronological order.
     */
    public async getAnalysises(options: GetAnalysisesOptions = {}): Promise<void> {
        const queryParams: string[] = [];
        if (options.limit !== undefined) {
            queryParams.push(`limit=${encodeURIComponent(String(options.limit))}`);
        }
        if (options.startingAfter !== undefined) {
            queryParams.push(`starting_after=${encodeURIComponent(String(options.startingAfter))}`);
        }
        const requestPath = `/sentiment_analysis/v1/jobs` + (queryParams.length > 0 ? "?" + queryParams.join("&") : "");
        const requestUrl = this.resolveUrl(requestPath);
        await this.sendWithTracingAsync<void>("Revai.getAnalysises", "AnalysisesGet", "GET", requestUrl, undefined, options);
    }

    /**
     * Submit sentiment analysis job
     * @remarks Starts an asynchronous job to analyze sentiments in a transcript.
     */
    public async analysis(input: AnalysisInput, options: ConnectorOperationOptions = {}): Promise<AnalysisPostResponse> {
        const requestPath = `/sentiment_analysis/v1/jobs`;
        const requestUrl = this.resolveUrl(requestPath);
        const httpResponse = await this.sendWithTracingAsync<AnalysisPostResponse>("Revai.analysis", "AnalysisPost", "POST", requestUrl, input, options);

        return httpResponse.value as AnalysisPostResponse;
    }

    /**
     * Get sentiment analysis job
     * @remarks Returns information about a sentiment analysis job.
     */
    public async getAnalysis(id: string, options: ConnectorOperationOptions = {}): Promise<AnalysisGetResponse> {
        const requestPath = `/sentiment_analysis/v1/jobs/${encodeURIComponent(String(id))}`;
        const requestUrl = this.resolveUrl(requestPath);
        const httpResponse = await this.sendWithTracingAsync<AnalysisGetResponse>("Revai.getAnalysis", "AnalysisGet", "GET", requestUrl, undefined, options);

        return httpResponse.value as AnalysisGetResponse;
    }

    /**
     * Delete sentiment analysis job
     * @remarks Deletes a sentiment analysis job. All data related to the job will be permanently deleted.
     */
    public async deleteAnalysis(id: string, options: ConnectorOperationOptions = {}): Promise<string> {
        const requestPath = `/sentiment_analysis/v1/jobs/${encodeURIComponent(String(id))}`;
        const requestUrl = this.resolveUrl(requestPath);
        const httpResponse = await this.sendWithTracingAsync<string>("Revai.deleteAnalysis", "AnalysisDelete", "DELETE", requestUrl, undefined, options);

        return httpResponse.value as string;
    }

    /**
     * Get sentiment analysis result
     * @remarks Returns the results for a completed sentiment analysis job.
     */
    public async getAnalysisResult(id: string, options: GetAnalysisResultOptions = {}): Promise<AnalysisResultGetResponse> {
        const queryParams: string[] = [];
        if (options.filterFor !== undefined) {
            queryParams.push(`filter_for=${encodeURIComponent(String(options.filterFor))}`);
        }
        const requestPath = `/sentiment_analysis/v1/jobs/${encodeURIComponent(String(id))}/result` + (queryParams.length > 0 ? "?" + queryParams.join("&") : "");
        const requestUrl = this.resolveUrl(requestPath);
        const httpResponse = await this.sendWithTracingAsync<AnalysisResultGetResponse>("Revai.getAnalysisResult", "AnalysisResultGet", "GET", requestUrl, undefined, options);

        return httpResponse.value as AnalysisResultGetResponse;
    }

    /**
     * Get language identification jobs
     * @remarks Retrieves a list of language identification jobs submitted within the last 30 days in reverse chronological order.
     */
    public async getIdentifications(options: GetIdentificationsOptions = {}): Promise<Array<Record<string, unknown>>> {
        const queryParams: string[] = [];
        if (options.limit !== undefined) {
            queryParams.push(`limit=${encodeURIComponent(String(options.limit))}`);
        }
        if (options.startingAfter !== undefined) {
            queryParams.push(`starting_after=${encodeURIComponent(String(options.startingAfter))}`);
        }
        const requestPath = `/languageid/v1/jobs` + (queryParams.length > 0 ? "?" + queryParams.join("&") : "");
        const requestUrl = this.resolveUrl(requestPath);
        const httpResponse = await this.sendWithTracingAsync<Array<Record<string, unknown>>>("Revai.getIdentifications", "IdentificationsGet", "GET", requestUrl, undefined, options);

        return httpResponse.value as Array<Record<string, unknown>>;
    }

    /**
     * Submit language identification job
     * @remarks Starts an asynchronous job to identify the most probable language in the audio.
     */
    public async identification(input: IdentificationInput, options: ConnectorOperationOptions = {}): Promise<IdentificationPostResponse> {
        const requestPath = `/languageid/v1/jobs`;
        const requestUrl = this.resolveUrl(requestPath);
        const httpResponse = await this.sendWithTracingAsync<IdentificationPostResponse>("Revai.identification", "IdentificationPost", "POST", requestUrl, input, options);

        return httpResponse.value as IdentificationPostResponse;
    }

    /**
     * Get language identification job
     * @remarks Returns information about a language identification job.
     */
    public async getIdentification(id: string, options: ConnectorOperationOptions = {}): Promise<IdentificationGetResponse> {
        const requestPath = `/languageid/v1/jobs/${encodeURIComponent(String(id))}`;
        const requestUrl = this.resolveUrl(requestPath);
        const httpResponse = await this.sendWithTracingAsync<IdentificationGetResponse>("Revai.getIdentification", "IdentificationGet", "GET", requestUrl, undefined, options);

        return httpResponse.value as IdentificationGetResponse;
    }

    /**
     * Delete language identification job
     * @remarks Deletes a language identification job. All data related to the job will be permanently deleted.
     */
    public async deleteIdentification(id: string, options: ConnectorOperationOptions = {}): Promise<string> {
        const requestPath = `/languageid/v1/jobs/${encodeURIComponent(String(id))}`;
        const requestUrl = this.resolveUrl(requestPath);
        const httpResponse = await this.sendWithTracingAsync<string>("Revai.deleteIdentification", "IdentificationDelete", "DELETE", requestUrl, undefined, options);

        return httpResponse.value as string;
    }

    /**
     * Get language identification result
     * @remarks Returns the results for a completed language identification job.
     */
    public async getIdentificationResult(id: string, options: ConnectorOperationOptions = {}): Promise<IdentificationResultGetResponse> {
        const requestPath = `/languageid/v1/jobs/${encodeURIComponent(String(id))}/result`;
        const requestUrl = this.resolveUrl(requestPath);
        const httpResponse = await this.sendWithTracingAsync<IdentificationResultGetResponse>("Revai.getIdentificationResult", "IdentificationResultGet", "GET", requestUrl, undefined, options);

        return httpResponse.value as IdentificationResultGetResponse;
    }

    /**
     * Get forced alignment jobs
     * @remarks Retrieve a list of alignment jobs submitted within the last 30 days in reverse chronological order.
     */
    public async getAlignments(options: GetAlignmentsOptions = {}): Promise<Array<Record<string, unknown>>> {
        const queryParams: string[] = [];
        if (options.limit !== undefined) {
            queryParams.push(`limit=${encodeURIComponent(String(options.limit))}`);
        }
        if (options.startingAfter !== undefined) {
            queryParams.push(`starting_after=${encodeURIComponent(String(options.startingAfter))}`);
        }
        const requestPath = `/alignment/v1/jobs` + (queryParams.length > 0 ? "?" + queryParams.join("&") : "");
        const requestUrl = this.resolveUrl(requestPath);
        const httpResponse = await this.sendWithTracingAsync<Array<Record<string, unknown>>>("Revai.getAlignments", "AlignmentsGet", "GET", requestUrl, undefined, options);

        return httpResponse.value as Array<Record<string, unknown>>;
    }

    /**
     * Submit forced alignment job
     * @remarks Starts an asynchronous job to obtain precise word timings for an audio file and text transcript.
     */
    public async alignment(input: AlignmentInput, options: ConnectorOperationOptions = {}): Promise<AlignmentPostResponse> {
        const requestPath = `/alignment/v1/jobs`;
        const requestUrl = this.resolveUrl(requestPath);
        const httpResponse = await this.sendWithTracingAsync<AlignmentPostResponse>("Revai.alignment", "AlignmentPost", "POST", requestUrl, input, options);

        return httpResponse.value as AlignmentPostResponse;
    }

    /**
     * Get forced alignment job
     * @remarks Returns information about a forced alignment job.
     */
    public async getAlignment(id: string, options: ConnectorOperationOptions = {}): Promise<AlignmentGetResponse> {
        const requestPath = `/alignment/v1/jobs/${encodeURIComponent(String(id))}`;
        const requestUrl = this.resolveUrl(requestPath);
        const httpResponse = await this.sendWithTracingAsync<AlignmentGetResponse>("Revai.getAlignment", "AlignmentGet", "GET", requestUrl, undefined, options);

        return httpResponse.value as AlignmentGetResponse;
    }

    /**
     * Delete forced alignment job
     * @remarks Deletes a forced alignment job. All data related to the job will be permanently deleted.
     */
    public async deleteAlignment(id: string, options: ConnectorOperationOptions = {}): Promise<string> {
        const requestPath = `/alignment/v1/jobs/${encodeURIComponent(String(id))}`;
        const requestUrl = this.resolveUrl(requestPath);
        const httpResponse = await this.sendWithTracingAsync<string>("Revai.deleteAlignment", "AlignmentDelete", "DELETE", requestUrl, undefined, options);

        return httpResponse.value as string;
    }

    /**
     * Get forced alignment transcript
     * @remarks Returns the results for a completed forced alignment job.
     */
    public async getAlignmentTranscript(id: string, options: ConnectorOperationOptions = {}): Promise<AlignmentTranscriptGetResponse> {
        const requestPath = `/alignment/v1/jobs/${encodeURIComponent(String(id))}/transcript`;
        const requestUrl = this.resolveUrl(requestPath);
        const httpResponse = await this.sendWithTracingAsync<AlignmentTranscriptGetResponse>("Revai.getAlignmentTranscript", "AlignmentTranscriptGet", "GET", requestUrl, undefined, options);

        return httpResponse.value as AlignmentTranscriptGetResponse;
    }

}

// #endregion Client
