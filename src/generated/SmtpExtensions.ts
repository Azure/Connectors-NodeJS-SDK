// SmtpExtensions.ts - Auto-generated DirectClient SDK for TypeScript
// Do not edit this file directly.

import type { TokenCredential } from "@azure/core-auth";
import { ConnectorClientBase } from "../azureConnectors/clientBase.ts";
import type { ConnectorClientOptions, ConnectorOperationOptions } from "../azureConnectors/options.ts";

// #region Types

/**
 * Definition: Attachment
 */
export interface Attachment {
    /** Content data */
    ContentData: string;
    /** Content type */
    ContentType: string;
    /** File name */
    FileName?: string;
    /** Content id */
    ContentId?: string;
}

/**
 * Definition: Email
 */
export interface Email {
    /** Email address of sender like sender@domain.com */
    From?: string;
    /** Specify email addresses separated by semicolons like recipient1@domain.com;recipient2@domain.com */
    To?: string;
    /** Specify email addresses separated by semicolons like recipient1@domain.com;recipient2@domain.com */
    CC?: string;
    /** Email subject */
    Subject?: string;
    /** Email body */
    Body?: string;
    /** Specify email addresses separated by semicolons like recipient1@domain.com;recipient2@domain.com. */
    Bcc?: string;
    /** Importance of the email (High, Normal, or Low) */
    Importance?: string;
    /** Specify email address for Read receipt */
    ReadReceipt?: string;
    /** Specify email address for Delivery receipt */
    DeliveryReceipt?: string;
    /** Attachments to be sent along with the email */
    Attachments?: Array<Attachment>;
}
// #endregion Types

// #region Client

/**
 * Typed client for the smtp connector.
 */
export class SmtpClient extends ConnectorClientBase {

    /**
     * Creates a new SmtpClient.
     * @param connectionRuntimeUrl The connection runtime URL from Azure Portal.
     * @param credential The credential used for authentication.
     * @param options Optional connector client options.
     */
    constructor(connectionRuntimeUrl: string, credential: TokenCredential, options?: ConnectorClientOptions) {
        super(connectionRuntimeUrl, credential, options);
    }

    public get connectorName(): string {
        return "smtp";
    }

    /**
     * Send Email
     * @remarks This operation sends an email to one or more recipients.
     */
    public async sendEmail(input: Email, options: ConnectorOperationOptions = {}): Promise<void> {
        const requestPath = `/SendEmailV3`;
        const requestUrl = this.resolveUrl(requestPath);
        await this.sendWithTracingAsync<void>("Smtp.sendEmail", "SendEmailV3", "POST", requestUrl, input, options);
    }

}

// #endregion Client
