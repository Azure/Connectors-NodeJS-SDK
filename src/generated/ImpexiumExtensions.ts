// ImpexiumExtensions.ts - Auto-generated DirectClient SDK for TypeScript
// Do not edit this file directly.

import type { TokenCredential } from "@azure/core-auth";
import { ConnectorClientBase } from "../azureConnectors/clientBase.ts";
import type { ConnectorPagedAsyncIterableIterator } from "../azureConnectors/clientBase.ts";
import type { ConnectorClientOptions, ConnectorOperationOptions } from "../azureConnectors/options.ts";

// #region Types

/**
 * Response for Get Abandoned Checkouts
 */
export interface GetAbandonedCheckoutsResponse {
    /** Page Number */
    pageNumber?: number;
    /** dataList */
    dataList?: Array<AbandonedCheckoutData>;
}

/**
 * Response for List All Exhibitors
 */
export interface ListAllExhibitorsResponse {
    /** Page Number */
    pageNumber?: number;
    /** dataList */
    dataList?: Array<ExhibitorData>;
}

/**
 * Response for List of Exams
 */
export interface ListOfExamsResponse {
    /** Page Number */
    pageNumber?: number;
    /** dataList */
    dataList?: Array<ExamData>;
}

/**
 * Response for List Registrants
 */
export interface ListRegistrantsResponse {
    /** Page Number. */
    pageNumber?: number;
    /** dataList */
    dataList?: Array<RegistrantData>;
}

/**
 * Response for Get Course Attendees
 */
export interface GetCourseAttendeesResponse {
    /** Page Number. */
    pageNumber?: number;
    /** dataList */
    dataList?: Array<CourseAttendeeData>;
}

/**
 * Add Exam Scores
 */
export type AddExamScoresInput = Array<ExamScoreData>;

/**
 * Response for Find Members by Name
 */
export interface FindMembersByNameResponse {
    /** Page Number. */
    pageNumber?: number;
    /** dataList */
    dataList?: Array<ContactData>;
}

/**
 * Response for Get Purchases for an Individual
 */
export interface GetPurchasesForAnIndividualResponse {
    /** Page Number. */
    pageNumber?: number;
    /** dataList */
    dataList?: Array<PurchasedItemData>;
}

/**
 * Add or Update a List of Custom Fields Per Organization
 */
export type AddOrUpdateAListOfCustomFieldsPerOrganizationInput = Array<CustomFieldValueData>;

/**
 * Response for List All Event Cancellations by Event
 */
export interface ListAllEventCancellationsByEventResponse {
    /** Page Number. */
    pageNumber?: number;
    /** dataList */
    dataList?: Array<RegistrantCancellationData>;
}

/**
 * Response for Get All Open Orders for an Individual
 */
export interface GetAllOpenOrdersForAnIndividualResponse {
    /** Page Number */
    pageNumber?: number;
    /** dataList */
    dataList?: Array<PayableOrderData>;
}

/**
 * Response for List Completed User Tasks by User ID or Email
 */
export interface ListCompletedUserTasksByUserIdOrEmailResponse {
    /** Page Number. */
    pageNumber?: number;
    /** dataList */
    dataList?: Array<UserTaskData>;
}

/**
 * Response for List Pending User Tasks by User ID or Email
 */
export interface ListPendingUserTasksByUserIdOrEmailResponse {
    /** Page Number. */
    pageNumber?: number;
    /** dataList */
    dataList?: Array<UserTaskData>;
}

/**
 * Response for List All Countries
 */
export interface ListAllCountriesResponse {
    /** Page Number */
    pageNumber?: number;
    /** dataList */
    dataList?: Array<CountryData>;
}

/**
 * Response for Get All States by Country
 */
export interface GetAllStatesByCountryResponse {
    /** Page Number */
    pageNumber?: number;
    /** dataList */
    dataList?: Array<StateProvinceData>;
}

/**
 * Response for List All Exhibits
 */
export interface ListAllExhibitsResponse {
    /** Page Number */
    pageNumber?: number;
    /** dataList */
    dataList?: Array<ExhibitData>;
}

/**
 * Add Categories for an Organization
 */
export type AddCategoriesForAnOrganizationInput = Array<SaveCategoryBasicData>;

/**
 * Response for List of Customer Relationships
 */
export interface ListOfCustomerRelationshipsResponse {
    /** Page Number. */
    pageNumber?: number;
    /** dataList */
    dataList?: Array<RelationshipTypeData>;
}

/**
 * Response for List All Open Customer Request
 */
export interface ListAllOpenCustomerRequestResponse {
    /** Page Number. */
    pageNumber?: number;
    /** dataList */
    dataList?: Array<RequestData>;
}

/**
 * Add Categories for an Individual
 */
export type AddCategoriesForAnIndividualInput = Array<SaveCategoryBasicData>;

/**
 * Response for List of All Organization Members
 */
export interface ListOfAllOrganizationMembersResponse {
    /** Page Number. */
    pageNumber?: number;
    /** dataList */
    dataList?: Array<OrganizationData>;
}

/**
 * Response for List of All Individual Members
 */
export interface ListOfAllIndividualMembersResponse {
    /** Page Number. */
    pageNumber?: number;
    /** dataList */
    dataList?: Array<IndividualData>;
}

/**
 * Response for Get List of Active Certifications for an Organization
 */
export interface GetListOfActiveCertificationsForAnOrganizationResponse {
    /** Page Number. */
    pageNumber?: number;
    /** dataList */
    dataList?: Array<CertificationData>;
}

/**
 * Response for Get List of Active Certifications for an Individual
 */
export interface GetListOfActiveCertificationsForAnIndividualResponse {
    /** Page Number. */
    pageNumber?: number;
    /** dataList */
    dataList?: Array<CertificationData>;
}

/**
 * Register an Individual for a Free Session
 */
export type RegisterAnIndividualForAFreeSessionInput = Array<SessionRegistrationData>;

/**
 * Response for Get a List of Licenses
 */
export interface GetAListOfLicensesResponse {
    /** Page Number. */
    pageNumber?: number;
    /** dataList */
    dataList?: Array<LicenseData>;
}

/**
 * Response for List All Awards
 */
export interface ListAllAwardsResponse {
    /** Page Number. */
    pageNumber?: number;
    /** dataList */
    dataList?: Array<AwardData>;
}

/**
 * Response for Find Members (Individuals) by Name
 */
export interface FindMembersOrIndividualsByNameResponse {
    /** Page Number. */
    pageNumber?: number;
    /** dataList */
    dataList?: Array<IndividualData>;
}

/**
 * Response for Get a List of All Services of an Organization
 */
export interface GetAListOfAllServicesOfAnOrganizationResponse {
    /** Page Number. */
    pageNumber?: number;
    /** dataList */
    dataList?: Array<ServiceData>;
}

/**
 * Delete an Individual Web Link
 */
export interface DeleteAnIndividualWebLinkInput {
    /** Type. */
    type?: string;
    /** Url. */
    url?: string;
}

/**
 * Add Web Link for Individual
 */
export interface AddWebLinkForIndividualInput {
    /** Type. */
    type?: string;
    /** Url. */
    url?: string;
}

/**
 * Response for Get Nominees by Committee
 */
export interface GetNomineesByCommitteeResponse {
    /** Page Number. */
    pageNumber?: number;
    /** Data List */
    dataList?: Array<CommitteeNomineeData>;
}

/**
 * Response for Get Organization's Active Subscriptions
 */
export interface GetOrganizationsActiveSubscriptionsResponse {
    /** Page Number. */
    pageNumber?: number;
    /** dataList */
    dataList?: Array<SubscriptionData>;
}

/**
 * Delete an Organization Web Link
 */
export interface DeleteAnOrganizationWebLinkInput {
    /** Type. */
    type: string;
    /** Url. */
    url: string;
}

/**
 * Add Web Link for Organization
 */
export interface AddWebLinkForOrganizationInput {
    /** Type. */
    type: string;
    /** Url. */
    url: string;
}

/**
 * Response for Get Organization's Relationships
 */
export interface GetOrganizationsRelationshipsResponse {
    /** Page Number. */
    pageNumber?: number;
    /** DataList */
    dataList?: Array<RelationshipData>;
}

/**
 * Add or Update a List of Custom Fields Per Individual
 */
export type AddOrUpdateAListOfCustomFieldsPerIndividualInput = Array<Record<string, unknown>>;

/**
 * Response for Get Upcoming Events
 */
export interface GetUpcomingEventsResponse {
    /** Page Number. */
    pageNumber?: number;
    /** DataList */
    dataList?: Array<EventData>;
}

/**
 * Response for Get Committee Information for an Individual
 */
export interface GetCommitteeInformationForAnIndividualResponse {
    /** Page Number */
    pageNumber?: number;
    /** Data List. */
    dataList?: Array<CommitteeMemberData>;
}

/**
 * Response for Get All Events
 */
export interface GetAllEventsResponse {
    /** Page Number. */
    pageNumber?: number;
    /** DataList */
    dataList?: Array<EventData>;
}

/**
 * Response for Get All Event Registrations Information for an Individual
 */
export interface GetAllEventRegistrationsInformationForAnIndividualResponse {
    /** Page Number. */
    pageNumber?: number;
    /** dataList */
    dataList?: Array<RegistrationData>;
}

/**
 * Response for Get Individual's Relationships
 */
export interface GetIndividualsRelationshipsResponse {
    /** Page Number. */
    pageNumber?: number;
    /** Data List. */
    dataList?: Array<RelationshipData>;
}

/**
 * Update an Individual Email
 */
export interface UpdateAnIndividualEmailInput {
    /** Primary. */
    primary?: boolean;
    /** Address. */
    address?: string;
    /** Type. */
    type?: string;
    /** Show In Directory */
    showInDirectory?: boolean;
}

/**
 * Save Relationship for Organization
 */
export interface SaveRelationshipForOrganizationInput {
    /** Relationship Name. */
    relationshipName: string;
    /** Related To Customer Record Number. */
    relatedToCustomerRecordNumber: string;
    /** Reciprocal Relationship Name. */
    reciprocalRelationshipName: string;
    /** Is Primary. */
    isPrimary?: boolean;
    /** Start Date. */
    startDate?: string;
    /** End Date. */
    endDate?: string;
    /** Is Reciprocal Primary. */
    isReciprocalPrimary?: boolean;
    /** Title. */
    title?: string;
}

/**
 * Add Individual
 */
export interface AddIndividualInput {
    /** First Name. */
    firstName: string;
    /** Last Name. */
    lastName: string;
    /** Email Address. */
    email?: string;
    /** Record Number. */
    recordNumber?: string;
}

/**
 * Add Email to Individual
 */
export interface AddEmailToIndividualInput {
    /** Primary */
    primary?: boolean;
    /** Address. */
    address?: string;
    /** Type. */
    type?: string;
    /** Show In Directory */
    showInDirectory?: boolean;
}

/**
 * Response for Find Members (Individuals) by First Name and Last Name
 */
export interface FindMembersOrIndividualsByFirstAndLastNameResponse {
    /** Page Number. */
    pageNumber?: number;
    /** dataList */
    dataList?: Array<IndividualData>;
}

/**
 * Add Activity
 */
export interface AddActivityInput {
    /** Category. */
    category: string;
    /** Text. */
    text: string;
    /** URL. */
    url?: string;
    /** Activity Date. */
    activityDate?: string;
}

/**
 * Response for Find Individual by Id or Email or Record Number
 */
export interface FindIndividualIdOrEmailResponse {
    /** Page Number. */
    pageNumber?: number;
    /** Data List. */
    dataList?: Array<IndividualData>;
}

/**
 * Add Relationship to Individual
 */
export interface AddRelationshipToIndividualInput {
    /** Relationship Name. */
    relationshipName: string;
    /** Related To Customer Record Number. */
    relatedToCustomerRecordNumber: string;
    /** Reciprocal Relationship Name. */
    reciprocalRelationshipName: string;
    /** Is Primary. */
    isPrimary?: boolean;
    /** Is Reciprocal Primary. */
    isReciprocalPrimary?: boolean;
    /** Start Date. */
    startDate?: string;
    /** End Date. */
    endDate?: string;
}

/**
 * Response for Lookup Individuals
 */
export interface IndividualsLookupByNameResponse {
    /** Page Number. */
    pageNumber?: number;
    /** Data List. */
    dataList?: Array<IndividualLookupBasicData>;
}

/**
 * Response for Add Individual to Committee
 */
export interface AddToCommitteeResponse {
    /** Member Id. */
    id?: string;
    /** First Name. */
    firstName?: string;
    /** Last Name. */
    lastName?: string;
    /** Code. */
    code?: string;
    /** Record Number. */
    recordNumber?: string;
    /** Position. */
    position?: string;
    /** Position Code. */
    positionCode?: string;
    /** Start Date. */
    startDate?: string;
    /** End Date. */
    endDate?: string;
    primaryOrganization?: Record<string, unknown>;
    organizationRepresenting?: Record<string, unknown>;
    nominatedByCustomer?: Record<string, unknown>;
    addresses?: Array<Record<string, unknown>>;
    phones?: Array<Record<string, unknown>>;
    emails?: Array<Record<string, unknown>>;
    /** Rank. */
    rank?: number;
}

/**
 * Add Organization
 */
export interface AddOrganizationInput {
    /** Id. */
    id?: string;
    /** Name. */
    name: string;
    /** Show In Directory. */
    showInDirectory: boolean;
    /** Branch Name. */
    branchName?: string;
    /** Website. */
    website?: string;
}

/**
 * Update Organization
 */
export interface UpdateOrganizationInput {
    /** Name. */
    name?: string;
    /** Description. */
    description?: string;
    /** Show In Directory. */
    showInDirectory?: boolean;
    /** Branch Name. */
    branchName?: string;
    /** Website. */
    website?: string;
}

/**
 * Response for Lookup Organizations
 */
export interface OrganizationLookupByNameResponse {
    /** Page Number. */
    pageNumber?: number;
    /** Data List. */
    dataList?: Array<OrganizationLookupBasicData>;
}

/**
 * Response for Get All Committees
 */
export interface GetAllCommitteesResponse {
    /** Page Number. */
    pageNumber?: number;
    /** dataList */
    dataList?: Array<CommitteeData>;
}

/**
 * Response for Get Positions by Committee
 */
export interface GetPositionsByCommitteeResponse {
    /** Page Number. */
    pageNumber?: number;
    /** dataList */
    dataList?: Array<CommitteePositionData>;
}

/**
 * Response for Get Sub-Committees
 */
export interface GetSubCommitteesResponse {
    /** pageNumber */
    pageNumber?: number;
    /** dataList */
    dataList?: Array<CommitteeData>;
}

/**
 * Response for Get Individual's Active Subscriptions
 */
export interface GetIndividualsActiveSubscriptionsResponse {
    /** Page Number. */
    pageNumber?: number;
    /** dataList */
    dataList?: Array<SubscriptionData>;
}

/**
 * Response for List all Individuals
 */
export interface ListAllIndividualsResponse {
    /** Page Number. */
    pageNumber?: number;
    dataList?: Array<IndividualData>;
}

/**
 * Response for Find Customer by Phone Number
 */
export interface FindCustomerPhoneResponse {
    /** Page Number. */
    pageNumber?: number;
    dataList?: Array<Record<string, unknown>>;
}

/**
 * Mark Registrant as Attended
 */
export type MarkRegistrantAttendedInput = Array<Record<string, unknown>>;

/**
 * Response for List Award Individual Recipients
 */
export interface AwardsGetIndividualAwardRecipientsResponse {
    /** Page Number. */
    pageNumber?: number;
    /** Data List. */
    dataList?: Array<AwardRecipientIndividualData>;
}

/**
 * Response for List Award Organization Recipients
 */
export interface AwardsGetOrganizationAwardRecipientsResponse {
    /** Page Number. */
    pageNumber?: number;
    /** Data List. */
    dataList?: Array<AwardRecipientOrganizationData>;
}

/**
 * When an individual is created
 */
export interface WhenIndividualCreatedInput {
    /** Type Name. */
    typeName: string;
    /** Secret. */
    secret: string;
    /** Callback URL. */
    callBackUrl: string;
    /** Description. */
    description: string;
}

/**
 * When an individual is deleted
 */
export interface WhenIndividualDeletedInput {
    /** Type Name. */
    typeName: string;
    /** Secret. */
    secret: string;
    /** Callback URL. */
    callBackUrl: string;
    /** Description. */
    description: string;
}

/**
 * When an individual requests to be forgotten
 */
export interface WhenIndividualRequestForgottenInput {
    /** Type Name. */
    typeName: string;
    /** Secret. */
    secret: string;
    /** Callback URL. */
    callBackUrl: string;
    /** Description. */
    description: string;
}

/**
 * When a product is purchased
 */
export interface WhenProductPurchasedInput {
    /** Id. */
    id?: string;
    /** Type Name. */
    typeName: string;
    /** Filters. */
    filters: Array<string>;
    /** Secret. */
    secret: string;
    /** Callback URL. */
    callBackUrl: string;
    /** Description. */
    description: string;
}

/**
 * When a committee member is updated
 */
export interface WhenCommitteeMemberUpdatedInput {
    /** Type Name. */
    typeName: string;
    /** Secret. */
    secret: string;
    /** Callback URL. */
    callBackUrl: string;
    /** Description. */
    description: string;
}

/**
 * When a purchase is canceled
 */
export interface WhenPurchaseCancelledInput {
    /** Id. */
    id?: string;
    /** Type Name. */
    typeName: string;
    /** Filters. */
    filters: Array<string>;
    /** Secret. */
    secret: string;
    /** Callback URL. */
    callBackUrl: string;
    /** Description. */
    description: string;
}

/**
 * When a customer request is updated
 */
export interface WhenRequestUpdatedInput {
    /** Type Name. */
    typeName: string;
    /** Secret. */
    secret: string;
    /** Callback URL. */
    callBackUrl: string;
    /** Description. */
    description: string;
}

/**
 * When a customer email is updated
 */
export interface WhenEmailUpdatedInput {
    /** Type Name. */
    typeName: string;
    /** Secret. */
    secret: string;
    /** Callback URL. */
    callBackUrl: string;
    /** Description. */
    description: string;
}

/**
 * When a customer custom field value is updated
 */
export interface WhenCustomerCustomFieldValueUpdatedInput {
    /** Type Name. */
    typeName: string;
    /** Filters. */
    filters: Array<string>;
    /** Secret. */
    secret: string;
    /** Callback URL. */
    callBackUrl: string;
    /** Description. */
    description: string;
}

/**
 * When a customer is merged
 */
export interface WhenCustomerIsMergedInput {
    /** Type Name. */
    typeName: string;
    /** Secret. */
    secret: string;
    /** Callback URL. */
    callBackUrl: string;
    /** Description. */
    description: string;
}

/**
 * When a customer relationship is updated
 */
export interface WhenCustomerRelationshipUpdatedInput {
    /** Type Name. */
    typeName: string;
    /** Secret. */
    secret: string;
    /** Callback URL. */
    callBackUrl: string;
    /** Description. */
    description: string;
}

/**
 * When a customer phone is updated
 */
export interface WhenCustomerPhoneUpdatedInput {
    /** Type Name. */
    typeName: string;
    /** Secret. */
    secret: string;
    /** Callback URL. */
    callBackUrl: string;
    /** Description. */
    description: string;
}

/**
 * When a customer address is updated
 */
export interface WhenCustomerAddressUpdatedInput {
    /** Type Name. */
    typeName: string;
    /** Secret. */
    secret: string;
    /** Callback URL. */
    callBackUrl: string;
    /** Description. */
    description: string;
}

/**
 * When an event registration is substituted
 */
export interface WhenEventRegistrationSubstitutedInput {
    /** Type Name. */
    typeName: string;
    /** Filters. */
    filters: Array<string>;
    /** Secret. */
    secret: string;
    /** Callback URL. */
    callBackUrl: string;
    /** Description. */
    description: string;
}

/**
 * When a purchase is paid
 */
export interface WhenPurchasePaidInput {
    /** Type Name. */
    typeName: string;
    /** Filters. */
    filters: Array<string>;
    /** Secret. */
    secret: string;
    /** Callback URL. */
    callBackUrl: string;
    /** Description. */
    description: string;
}

/**
 * When a membership is terminated
 */
export interface WhenMembershipTerminatedInput {
    /** Id. */
    id?: string;
    /** Type Name. */
    typeName: string;
    /** Secret. */
    secret: string;
    /** Callback URL. */
    callBackUrl: string;
    /** Description. */
    description: string;
}

/**
 * Definition: AbandonedCheckoutData
 */
export interface AbandonedCheckoutData {
    /** Created On */
    createdOn?: string;
    /** Created By Email Address */
    createdByEmailAddress?: string;
    /** Currency */
    currency?: string;
    customer?: Record<string, unknown>;
    /** Line Items */
    lineItems?: Array<Record<string, unknown>>;
}

/**
 * Definition: ExhibitorData
 */
export interface ExhibitorData {
    /** Organization */
    organization?: Record<string, unknown>;
    /** Booths */
    booths?: Array<Record<string, unknown>>;
}

/**
 * Definition: ExamData
 */
export interface ExamData {
    /** Exam Start Date */
    examStartDate?: string;
    /** Exam End Date */
    examEndDate?: string;
    /** Exam Number */
    examNumber?: string;
    /** Education Credits */
    educationCredits?: Array<Record<string, unknown>>;
    /** categories */
    categories?: Array<Record<string, unknown>>;
    /** Active Prices */
    activePrices?: Array<Record<string, unknown>>;
    /** Related Products */
    relatedProducts?: Array<Record<string, unknown>>;
    /** Tags. */
    tags?: Array<string>;
    /** ID. */
    id?: string;
    /** Code. */
    code?: string;
    /** Name. */
    name?: string;
    /** Short Description. */
    shortDescription?: string;
    /** Long Description. */
    longDescription?: string;
    /** Type. */
    type?: string;
    /** Url. */
    url?: string;
    /** Image Url. */
    imageUrl?: Array<string>;
    /** Is Public. */
    isPublic?: boolean;
    /** Available From. */
    availableFrom?: string;
    /** Available Until. */
    availableUntil?: string;
}

/**
 * Definition: RegistrantData
 */
export interface RegistrantData {
    /** Title. */
    title?: string;
    /** Badge Name */
    badgeName?: string;
    /** Badge Organization. */
    badgeOrganization?: string;
    /** Badge City. */
    badgeCity?: string;
    /** Badge State. */
    badgeState?: string;
    /** Attended Date. */
    attendedDate?: string;
    sessions?: Array<Record<string, unknown>>;
    /** Itemized Custom Fields */
    itemizedCustomFields?: Array<Record<string, unknown>>;
    /** Guest Of Record Number. */
    guestOfRecordNumber?: string;
    /** Registrant Type Code. */
    registrantTypeCode?: string;
    /** Registrant Type Name. */
    registrantTypeName?: string;
    /** Registration Number. */
    registrationNumber?: string;
    boughtTogetherWith?: Array<Record<string, unknown>>;
    /** First Name. */
    firstName?: string;
    /** Last Name. */
    lastName?: string;
    /** Middle Name. */
    middleName?: string;
    /** Preferred First Name. */
    preferredFirstName?: string;
    /** Second Last Name. */
    secondLastName?: string;
    /** Prefix. */
    prefix?: string;
    /** Suffix. */
    suffix?: string;
    /** Gender. */
    gender?: string;
    primaryOrganization?: Record<string, unknown>;
    /** ID. */
    id?: string;
    /** Customer Type */
    customerType?: string;
    /** Record Number. */
    recordNumber?: string;
    /** Show In Directory. */
    showInDirectory?: boolean;
    addresses?: Array<Record<string, unknown>>;
    phones?: Array<Record<string, unknown>>;
    emails?: Array<Record<string, unknown>>;
}

/**
 * Definition: CourseAttendeeData
 */
export interface CourseAttendeeData {
    /** Title. */
    title?: string;
    /** Registered Date. */
    registeredDate?: string;
    /** Attended Date. */
    attendedDate?: string;
    /** Itemized Custom Fields */
    itemizedCustomFields?: Array<Record<string, unknown>>;
    /** First Name. */
    firstName?: string;
    /** Last Name. */
    lastName?: string;
    /** Middle Name. */
    middleName?: string;
    /** Preferred First Name. */
    preferredFirstName?: string;
    /** Second Last Name. */
    secondLastName?: string;
    /** Prefix. */
    prefix?: string;
    /** Suffix */
    suffix?: string;
    /** Gender. */
    gender?: string;
    primaryOrganization?: Record<string, unknown>;
    /** ID. */
    id?: string;
    /** Customer Type. */
    customerType?: string;
    /** Record Number. */
    recordNumber?: string;
    /** Show In Directory. */
    showInDirectory?: boolean;
    /** Addresses. */
    addresses?: Array<Record<string, unknown>>;
    /** Emails. */
    emails?: Array<Record<string, unknown>>;
    /** Phones. */
    phones?: Array<Record<string, unknown>>;
}

/**
 * Definition: ExamScoreResultData
 */
export interface ExamScoreResultData {
    /** Individual Record Number. */
    individualRecordNumber?: string;
    /** Status. */
    status?: string;
    /** Message. */
    message?: string;
}

/**
 * Definition: ExamScoreData
 */
export interface ExamScoreData {
    /** Individual Record Number. */
    individualRecordNumber?: string;
    /** Score. */
    score?: string;
}

/**
 * Definition: ContactData
 */
export interface ContactData {
    /** ID. */
    id?: string;
    /** Customer Type. */
    customerType?: string;
    /** Record Number. */
    recordNumber?: string;
    /** Title. */
    title?: string;
    /** Name. */
    name?: string;
    addresses?: Array<Record<string, unknown>>;
    /** Image Uri. */
    imageUri?: string;
    /** Twitter. */
    twitter?: string;
    /** LinkedIn. */
    linkedIn?: string;
    /** emails */
    emails?: Array<Record<string, unknown>>;
    phones?: Array<Record<string, unknown>>;
    /** Website. */
    webSite?: string;
    /** Memberships. */
    memberships?: Array<Record<string, unknown>>;
    /** Category. */
    category?: string;
    /** Categories */
    categories?: Array<Record<string, unknown>>;
    /** Custom Fields */
    customFields?: Array<Record<string, unknown>>;
    /** Show In Directory. */
    showInDirectory?: boolean;
    /** Links */
    links?: Array<Record<string, unknown>>;
    /** Old ID. */
    oldId?: string;
}

/**
 * Definition: PurchasedItemData
 */
export interface PurchasedItemData {
    /** Order Number. */
    orderNumber?: string;
    /** Product Code. */
    productCode?: string;
    /** Product Name. */
    productName?: string;
    /** Product Type. */
    productType?: string;
    /** Purchase Date. */
    purchaseDate?: string;
    /** Additional Info. */
    additionalInfo?: string;
    /** Quantity. */
    quantity?: number;
    /** Itemized Custom Fields */
    itemizedCustomFields?: Array<Record<string, unknown>>;
    /** Product Categories */
    productCategories?: Array<Record<string, unknown>>;
}

/**
 * Definition: CustomFieldValueData
 */
export interface CustomFieldValueData {
    /** Name. */
    name?: string;
    /** Value. */
    value?: string;
}

/**
 * Definition: CommitteeNomineeSaveData
 */
export interface CommitteeNomineeSaveData {
    /** Nominee Record Number. */
    nomineeRecordNumber?: string;
    /** Code. */
    code?: string;
    /** Position Code. */
    positionCode?: string;
    /** Status. */
    status?: string;
    /** Representing Organization Record Number. */
    representingOrganizationRecordNumber?: string;
    /** Representing Organization Relationship Name. */
    representingOrganizationRelationshipName?: string;
    /** Nominated By Customer Record Number. */
    nominatedByCustomerRecordNumber?: string;
    /** Committee Term. */
    committeeTerm?: number;
    /** Rank. */
    rank?: number;
}

/**
 * Definition: RegistrantCancellationData
 */
export interface RegistrantCancellationData {
    /** Title. */
    title?: string;
    /** Registration Number. */
    registrationNumber?: string;
    /** Event Cancellation Date. */
    eventCancellationDate?: string;
    /** Guest Of Record Number. */
    guestOfRecordNumber?: string;
    /** Cancelled Sessions. */
    cancelledSessions?: Array<Record<string, unknown>>;
    /** First Name. */
    firstName?: string;
    /** Last Name. */
    lastName?: string;
    /** Middle Name. */
    middleName?: string;
    /** Preferred First Name. */
    preferredFirstName?: string;
    /** Second Last Name. */
    secondLastName?: string;
    /** Prefix. */
    prefix?: string;
    /** Suffix. */
    suffix?: string;
    /** Gender. */
    gender?: string;
    /** primaryOrganization */
    primaryOrganization?: Record<string, unknown>;
    /** ID. */
    id?: string;
    /** Customer Type. */
    customerType?: string;
    /** Record Number. */
    recordNumber?: string;
    /** Show In Directory. */
    showInDirectory?: boolean;
    /** Addresses. */
    addresses?: Array<Record<string, unknown>>;
    /** Emails */
    emails?: Array<Record<string, unknown>>;
    /** Phones. */
    phones?: Array<Record<string, unknown>>;
}

/**
 * Definition: PayableOrderData
 */
export interface PayableOrderData {
    /** Number. */
    number?: string;
    /** Date. */
    date?: string;
    /** Balance. */
    balance?: number;
    /** lineItems */
    lineItems?: Array<Record<string, unknown>>;
    /** Type. */
    type?: string;
    /** ID. */
    id?: string;
    /** Customer Id. */
    customerId?: string;
    /** Total Amount. */
    totalAmount?: number;
    /** Title. */
    title?: string;
}

/**
 * Definition: BaseNoteData
 */
export interface BaseNoteData {
    /** Content. */
    Content?: string;
    /** Category. */
    Category?: string;
    /** Is Internal. */
    isInternal?: boolean;
}

/**
 * Definition: TaskSaveData
 */
export interface TaskSaveData {
    /** Title. */
    title?: string;
    /** Category. */
    category?: string;
    /** Description. */
    description?: string;
    /** Start Date. */
    startDate?: string;
    /** Due Date. */
    dueDate?: string;
}

/**
 * Definition: CountryData
 */
export interface CountryData {
    /** ID. */
    id?: string;
    /** Name. */
    name?: string;
    /** Two Letter Iso Code. */
    twoLetterIsoCode?: string;
    /** Three Letter Iso Code. */
    threeLetterIsoCode?: string;
    /** Code. */
    code?: number;
}

/**
 * Definition: StateProvinceData
 */
export interface StateProvinceData {
    /** ID. */
    id?: string;
    /** Name. */
    name?: string;
    /** Abbreviation. */
    abbreviation?: string;
    /** Iso Code. */
    isoCode?: string;
}

/**
 * Definition: ExhibitData
 */
export interface ExhibitData {
    /** Code. */
    code?: string;
    /** Name. */
    name?: string;
    /** Start Date. */
    startDate?: string;
    /** Start Time. */
    startTime?: string;
    /** End Date. */
    endDate?: string;
    /** End Time. */
    endTime?: string;
    /** Categories */
    categories?: Array<Record<string, unknown>>;
    /** Short Description. */
    shortDescription?: string;
    /** Long Description. */
    longDescription?: string;
    featuredListing?: Record<string, unknown>;
}

/**
 * Definition: RequestUpdateData
 */
export interface RequestUpdateData {
    /** Request Number. */
    requestNumber?: string;
    /** Closed Date. */
    closedDate?: string;
}

/**
 * Definition: RequestSaveData
 */
export interface RequestSaveData {
    /** Requested By Customer Record Number. */
    requestedByCustomerRecordNumber?: string;
    /** Request Type. */
    requestType?: string;
    /** Description. */
    description?: string;
    /** Source. */
    source?: string;
}

/**
 * Definition: RelationshipTypeData
 */
export interface RelationshipTypeData {
    /** Name. */
    name?: string;
    /** Reciprocal Relationship Name. */
    reciprocalRelationshipName?: string;
    /** Relationship Type. */
    relationshipType?: string;
    /** Allow Primary. */
    allowPrimary?: boolean;
    /** Can Purchase For Organization. */
    canPurchaseForOrganization?: boolean;
    /** Can Manage PAC. */
    canManagePAC?: boolean;
    /** Can Manage Organization. */
    canManageOrganization?: boolean;
}

/**
 * Definition: RequestData
 */
export interface RequestData {
    /** Request Number. */
    requestNumber?: string;
    /** Request Date. */
    requestDate?: string;
    /** Requested By Customer Record Number. */
    requestedByCustomerRecordNumber?: string;
    /** Request Type. */
    requestType?: string;
    /** Description. */
    description?: string;
    /** Source. */
    source?: string;
    /** Closed Date. */
    closedDate?: string;
}

/**
 * Definition: UserTaskData
 */
export interface UserTaskData {
    /** Task Number. */
    taskNumber?: string;
    /** Title. */
    title?: string;
    /** Category. */
    category?: string;
    /** Description. */
    description?: string;
    /** Assigned By. */
    assignedBy?: string;
    /** Start Date. */
    startDate?: string;
    /** Due Date. */
    dueDate?: string;
    /** Progress. */
    progress?: string;
    /** Completed Date. */
    completedDate?: string;
}

/**
 * Definition: NotificationData
 */
export interface NotificationData {
    /** Title. */
    title?: string;
    /** Message. */
    message?: string;
    /** Url. */
    url?: string;
    /** Date. */
    date?: string;
    /** Is Read. */
    isRead?: boolean;
}

/**
 * Definition: SaveCategoryBasicData
 */
export interface SaveCategoryBasicData {
    /** Code. */
    code?: string;
    /** Is Primary. */
    isPrimary?: boolean;
}

/**
 * Definition: TaskData
 */
export interface TaskData {
    /** Task Number. */
    taskNumber?: string;
    /** Title. */
    title?: string;
    /** Category. */
    category?: string;
    /** Description. */
    description?: string;
    /** Start Date. */
    startDate?: string;
    /** Due Date. */
    dueDate?: string;
}

/**
 * Definition: CertificationData
 */
export interface CertificationData {
    /** Code. */
    code?: string;
    /** Name. */
    name?: string;
    /** Expire Date. */
    expireDate?: string;
}

/**
 * Definition: ActivityData
 */
export interface ActivityData {
    /** Category. */
    category?: string;
    /** Text. */
    text?: string;
    /** Url. */
    url?: string;
    /** Activity Date. */
    activityDate?: string;
}

/**
 * Definition: SessionRegistrationData
 */
export interface SessionRegistrationData {
    /** Session Code. */
    sessionCode?: string;
}

/**
 * Definition: AwardData
 */
export interface AwardData {
    /** ID. */
    id?: string;
    /** Name. */
    name?: string;
    /** Description. */
    description?: string;
    /** Year. */
    year?: number;
    /** Customer Type. */
    customerType?: string;
    /** Allow Public Nomination. */
    allowPublicNomination?: boolean;
    /** Nomination Start Date. */
    nominationStartDate?: string;
    /** Nomination End Date. */
    nominationEndDate?: string;
}

/**
 * Definition: LicenseData
 */
export interface LicenseData {
    /** ID. */
    id?: string;
    /** License Type. */
    licenseType?: string;
    /** Number. */
    number?: string;
    /** State. */
    state?: string;
    /** Status. */
    status?: string;
    /** Start Date. */
    startDate?: string;
    /** Experation Date. */
    expirationDate?: string;
}

/**
 * Definition: ServiceData
 */
export interface ServiceData {
    /** Code. */
    code?: string;
    /** Name. */
    name?: string;
}

/**
 * Definition: CommitteeNomineeData
 */
export interface CommitteeNomineeData {
    /** ID. */
    id?: string;
    /** First Name. */
    firstName?: string;
    /** Last Name. */
    lastName?: string;
    /** Code. */
    code?: string;
    /** Position. */
    position?: string;
    /** Position Code. */
    positionCode?: string;
    /** Start Date. */
    startDate?: string;
    /** End Date. */
    endDate?: string;
    primaryOrganization?: Record<string, unknown>;
    organizationRepresenting?: Record<string, unknown>;
    nominatedByCustomer?: Record<string, unknown>;
    addresses?: Array<Record<string, unknown>>;
    phones?: Array<Record<string, unknown>>;
    emails?: Array<Record<string, unknown>>;
    /** Rank. */
    rank?: number;
}

/**
 * Definition: CustomFieldResultData
 */
export interface CustomFieldResultData {
    /** Status. */
    status?: string;
    /** Message. */
    message?: string;
    /** Custom Field Name. */
    customFieldName?: string;
}

/**
 * Definition: RelationshipData
 */
export interface RelationshipData {
    /** Relationship Name. */
    relationshipName?: string;
    /** Related To Customer. */
    relatedToCustomer?: Record<string, unknown>;
    /** Reciprocal Relationship Name. */
    reciprocalRelationshipName?: string;
    /** Is Primary. */
    isPrimary?: boolean;
    /** Start Date. */
    startDate?: string;
    /** End Date. */
    endDate?: string;
}

/**
 * Definition: RegistrationData
 */
export interface RegistrationData {
    /** Individual ID. */
    individualId?: string;
    /** Event. */
    event?: Record<string, unknown>;
    /** Sessions. */
    sessions?: Array<Record<string, unknown>>;
    /** Badge Name. */
    badgeName?: string;
    /** Badge Organization. */
    badgeOrganization?: string;
    /** Badge City. */
    badgeCity?: string;
    /** Badge State. */
    badgeState?: string;
    boughtTogetherWith?: Array<Record<string, unknown>>;
    /** Registrant Type Code. */
    registrantTypeCode?: string;
    /** Registrant Type Name. */
    registrantTypeName?: string;
    /** Registration Number. */
    registrationNumber?: string;
}

/**
 * Definition: CommitteeMemberData
 */
export interface CommitteeMemberData {
    committee?: Record<string, unknown>;
    /** Position. */
    position?: string;
    /** Position Code. */
    positionCode?: string;
    /** Code. */
    code?: string;
    /** Start Date. */
    startDate?: string;
    /** End Date. */
    endDate?: string;
}

/**
 * Definition: CustomFieldData
 */
export interface CustomFieldData {
    /** Name. */
    name?: string;
    /** Caption. */
    caption?: string;
    /** Custom Field Value. For multi-selects, use "|" as delimiter. */
    value?: string;
}

/**
 * Definition: MembershipData
 */
export interface MembershipData {
    /** Membership Type. */
    membershipType?: string;
    /** Membership Type ID. */
    membershipTypeId?: string;
    /** Code. */
    code?: string;
    /** Expire Date. */
    expireDate?: string;
    /** Grace Expire Date. */
    graceExpireDate?: string;
    /** Inherited Membership Benefits. */
    inheritedMembershipBenefits?: boolean;
    /** Renewal URL. */
    renewalUrl?: string;
    /** Join Date. */
    joinDate?: string;
    /** Terminate Date. */
    terminateDate?: string;
    /** Effective Date. */
    effectiveDate?: string;
    /** Membership Unique ID. */
    membershipUniqueId?: string;
    /** Terms List */
    termsList?: Array<Record<string, unknown>>;
}

/**
 * Definition: EventData
 */
export interface EventData {
    /** Event ID. */
    eventId?: string;
    /** Event Code. */
    eventCode?: string;
    /** Event Name. */
    eventName?: string;
    /** Event Start Date. */
    eventStartDate?: string;
    /** Event Start Time. */
    eventStartTime?: string;
    /** Event End Date. */
    eventEndDate?: string;
    /** Event End Time. */
    eventEndTime?: string;
    /** Event Time Zone. */
    eventTimezone?: string;
    /** Event Description. */
    eventDescription?: string;
    /** Short Description. */
    shortDescription?: string;
    /** Public Event. */
    publicEvent?: boolean;
    /** Image Url. */
    imageUrl?: Array<string>;
    /** External Url. */
    externalUrl?: string;
    /** External Code. */
    externalCode?: string;
    /** Event Locations. */
    eventLocations?: Array<Record<string, unknown>>;
    /** Event Locations - Categories. */
    categories?: Array<Record<string, unknown>>;
    /** Event Locations - Education Credits. */
    educationCredits?: Array<Record<string, unknown>>;
    /** Event Locations - Uploads Data */
    uploadsData?: Array<Record<string, unknown>>;
    /** Event Locations - Related Products. */
    relatedProducts?: Array<Record<string, unknown>>;
    /** Event Locations - Tags */
    tags?: Array<string>;
}

/**
 * Definition: PhoneSaveData
 */
export interface PhoneSaveData {
    /** Country Name. */
    countryName: string;
    /** Primary. */
    primary?: boolean;
    /** Show in Directory. */
    showInDirectory?: boolean;
    /** Number. */
    number: string;
    /** Extension. */
    extension?: string;
    /** Type Name. */
    typeName: string;
}

/**
 * Definition: PhoneDataSet
 */
export interface PhoneDataSet {
    /** ID. */
    id?: string;
    /** Is Primary. */
    primary?: boolean;
    /** Type. */
    type?: number;
    country?: Record<string, unknown>;
    /** Show In Directory. */
    showInDirectory?: boolean;
    /** Number. */
    number?: string;
    /** Extension. */
    extension?: string;
    /** Type Name. */
    typeName?: string;
}

/**
 * Definition: EmailData
 */
export interface EmailData {
    /** Is Primary Email. */
    primary?: boolean;
    /** Address. */
    address?: string;
    /** Type. */
    type?: string;
    /** Show In Directory. */
    showInDirectory?: boolean;
}

/**
 * Definition: AddressSaveData
 */
export interface AddressSaveData {
    /** ID. */
    id?: string;
    /** Primary. */
    primary?: boolean;
    /** Line 1. */
    line1?: string;
    /** Line 2. */
    line2?: string;
    /** Line 3. */
    line3?: string;
    /** City. */
    city?: string;
    /** State. */
    state?: string;
    /** Zipcode. */
    zipcode?: string;
    /** Country. */
    country?: string;
    /** Type. */
    type?: string;
    /** Show In Directory. */
    showInDirectory?: boolean;
    /** Is Preferred Shipping. */
    isPreferredShipping?: boolean;
    /** Is Preferred Billing. */
    isPreferredBilling?: boolean;
    /** Is Bad Address. */
    isBadAddress?: boolean;
}

/**
 * Definition: IndividualData
 */
export interface IndividualData {
    /** First Name. */
    firstName?: string;
    /** Last Name. */
    lastName?: string;
    /** Middle Name. */
    middleName?: string;
    /** Preferred First Name. */
    preferredFirstName?: string;
    /** Second Last Name. */
    secondLastName?: string;
    /** Prefix. */
    prefix?: string;
    /** Suffix. */
    suffix?: string;
    /** Gender. */
    gender?: string;
    primaryOrganization?: Record<string, unknown>;
    /** Committees. */
    committees?: Array<Record<string, unknown>>;
    /** Security Roles. */
    securityRoles?: Array<Record<string, unknown>>;
    user?: Record<string, unknown>;
    /** Designations. */
    designationData?: Array<Record<string, unknown>>;
    /** Relationships. */
    relationships?: Array<Record<string, unknown>>;
    /** Job Roles. */
    jobRoles?: Array<Record<string, unknown>>;
    /** Tags. */
    tags?: Array<string>;
    /** Individual Id. */
    id?: string;
    /** Customer Type. */
    customerType?: string;
    /** Record Number. */
    recordNumber?: string;
    /** Title. */
    title?: string;
    /** Name. */
    name?: string;
    /** Addresses. */
    addresses?: Array<Record<string, unknown>>;
    /** Email. */
    email?: string;
    /** Image URI. */
    imageUri?: string;
    /** Twitter. */
    twitter?: string;
    /** LinkedIn. */
    linkedIn?: string;
    /** Emails. */
    emails?: Array<Record<string, unknown>>;
    /** Phones. */
    phones?: Array<Record<string, unknown>>;
    /** Website. */
    webSite?: string;
    /** Memberships. */
    memberships?: Array<Record<string, unknown>>;
    /** Category. */
    category?: string;
    /** Categories. */
    categories?: Array<Record<string, unknown>>;
    /** Custom Fields. */
    customFields?: Array<Record<string, unknown>>;
    /** Show In Directory. */
    showInDirectory?: boolean;
    /** Links. */
    links?: Array<Record<string, unknown>>;
    /** Old Id. */
    oldId?: string;
    /** Is Deceased. */
    isDeceased?: boolean;
}

/**
 * Definition: IndividualPayload
 */
export interface IndividualPayload {
    /** Webhook Id. */
    WebhookId?: string;
    /** Properties. */
    Properties?: Record<string, unknown>;
    /** Notifications. */
    Notifications?: Array<Record<string, unknown>>;
}

/**
 * Definition: OrganizationData
 */
export interface OrganizationData {
    /** Contact Ids. */
    contactIds?: Array<string>;
    /** Parent Company Id. */
    parentCompanyId?: string;
    /** Description. */
    description?: string;
    /** Branch Name. */
    branchName?: string;
    /** Tags. */
    tags?: Array<string>;
    /** Organization Id. */
    id?: string;
    /** Customer Type. */
    customerType?: string;
    /** Record Number. */
    recordNumber?: string;
    /** Title. */
    title?: string;
    /** Organization Name. */
    name?: string;
    /** Addresses. */
    addresses?: Array<Record<string, unknown>>;
    /** Email. */
    email?: string;
    /** Image URI. */
    imageUri?: string;
    /** Twitter. */
    twitter?: string;
    /** LinkedIn. */
    linkedIn?: string;
    /** Emails. */
    emails?: Array<Record<string, unknown>>;
    /** Phones. */
    phones?: Array<Record<string, unknown>>;
    /** Website. */
    webSite?: string;
    /** Memberships. */
    memberships?: Array<Record<string, unknown>>;
    /** Category. */
    category?: string;
    /** Categories. */
    categories?: Array<Record<string, unknown>>;
    /** Custom Fields. */
    customFields?: Array<Record<string, unknown>>;
    /** Show In Directory. */
    showInDirectory?: boolean;
    /** Links. */
    links?: Array<Record<string, unknown>>;
    /** Old Id. */
    oldId?: string;
}

/**
 * Definition: CommitteeMemberPayload
 */
export interface CommitteeMemberPayload {
    /** Webhook Id. */
    WebhookId?: string;
    /** Properties. */
    Properties?: Record<string, unknown>;
    /** Notifications. */
    Notifications?: Array<Record<string, unknown>>;
}

/**
 * Definition: PurchaseCancelledPayload
 */
export interface PurchaseCancelledPayload {
    /** Webhook Id. */
    WebhookId?: string;
    /** Properties. */
    Properties?: Record<string, unknown>;
    /** Notifications. */
    Notifications?: Array<Record<string, unknown>>;
}

/**
 * Definition: CustomerRequestPayload
 */
export interface CustomerRequestPayload {
    /** Webhook Id,. */
    WebhookId?: string;
    /** Properties. */
    Properties?: Record<string, unknown>;
    /** Notifications. */
    Notifications?: Array<Record<string, unknown>>;
}

/**
 * Definition: EmailPayload
 */
export interface EmailPayload {
    /** Webhook Id. */
    WebhookId?: string;
    /** Properties. */
    Properties?: Record<string, unknown>;
    /** Notifications. */
    Notifications?: Array<Record<string, unknown>>;
}

/**
 * Definition: CommitteeMemberCreateData
 */
export interface CommitteeMemberCreateData {
    /** Record Number. */
    recordNumber: string;
    /** Member Code. */
    code?: string;
    /** Position Code. */
    positionCode: string;
    /** Start Date. */
    startDate: string;
    /** End Date. */
    endDate?: string;
    /** Representing Organization - Record Number. */
    representingOrganizationRecordNumber?: string;
    /** Representing Organization - Relationship Name. */
    representingOrganizationRelationshipName?: string;
    /** Nominated By - Record Number */
    nominatedByCustomerRecordNumber?: string;
    /** Committee Term. */
    committeeTerm?: number;
    /** Rank. */
    rank?: number;
}

/**
 * Definition: SubscriptionData
 */
export interface SubscriptionData {
    /** Code. */
    code?: string;
    /** Name. */
    name?: string;
    /** Expire Date. */
    expireDate?: string;
    /** Quantity. */
    quantity?: number;
}

/**
 * Definition: CommitteePositionData
 */
export interface CommitteePositionData {
    /** Code. */
    code?: string;
    /** Name. */
    name?: string;
    /** Term. */
    term?: number;
    /** Description. */
    description?: string;
    /** Is Admin. */
    isAdmin?: boolean;
}

/**
 * Definition: CommitteeData
 */
export interface CommitteeData {
    /** Name. */
    name?: string;
    /** Code. */
    code?: string;
    /** Website. */
    website?: string;
    /** Id. */
    id?: string;
    /** Parent Committee Id. */
    parentCommitteeId?: string;
    /** Category Name. */
    categoryName?: string;
    /** Category Code. */
    categoryCode?: string;
    /** Description. */
    description?: string;
    /** Start Date. */
    startDate?: string;
    /** End Date. */
    endDate?: string;
    /** Show In Directory. */
    showInDirectory?: boolean;
    /** Term. */
    term?: Record<string, unknown>;
    /** Tags. */
    tags?: Array<string>;
}

/**
 * Definition: CommitteeMemberUpdateData
 */
export interface CommitteeMemberUpdateData {
    /** Record Number. */
    recordNumber?: string;
    /** Member Code. */
    code?: string;
    /** New Position Code. */
    positionCode?: string;
    /** Start Date. */
    startDate?: string;
    /** End Date. */
    endDate?: string;
    /** Representing Organization - Record Number. */
    representingOrganizationRecordNumber?: string;
    /** Representing Organization - Relationship Name. */
    representingOrganizationRelationshipName?: string;
    /** Nominated By - Record Number */
    nominatedByCustomerRecordNumber?: string;
    /** Committee Term. */
    committeeTerm?: number;
    /** Rank. */
    rank?: number;
}

/**
 * Definition: EducationCreditData
 */
export interface EducationCreditData {
    /** Education Credit Type. */
    type: string;
    /** Description. */
    description: string;
    /** Number of Credits Earned. */
    numberOfCredits: number;
    /** Earned Date. */
    dateEarned: string;
    /** Provider Name. */
    providerName?: string;
    /** State or Province Name. */
    stateName?: string;
    /** Reference. */
    reference?: string;
    /** Is Self Reported. */
    isSelfReported?: boolean;
}

/**
 * Definition: NoteData
 */
export interface NoteData {
    /** Is Alert. */
    isAlert?: boolean;
    /** Title. */
    title?: string;
    /** Follow Up Date. */
    followUpDate?: string;
    /** Content. */
    content: string;
    /** Category. */
    category?: string;
    /** Is Internal. */
    isInternal?: boolean;
}

/**
 * Definition: AwardNominationData
 */
export interface AwardNominationData {
    /** Nominee Record Number. */
    nomineeRecordNumber: string;
    /** Nominated By Record Number. */
    nominatedByCustomerRecordNumber: string;
    /** Nomination Date. */
    nominationDate: string;
    /** Awarded Date. */
    awardedDate?: string;
    /** Status. */
    status?: string;
    /** An explanation why the nominee should be considered for the award. */
    description?: string;
}

/**
 * Definition: UpdateAwardNominationData
 */
export interface UpdateAwardNominationData {
    /** Nominee Record Number. */
    nomineeRecordNumber?: string;
    /** Nominated By Record Number. */
    nominatedByCustomerRecordNumber: string;
    /** Nomination Date. */
    nominationDate: string;
    /** Awarded Date. */
    awardedDate?: string;
    /** Status. */
    status?: string;
    /** An explanation why the nominee should be considered for the award. */
    description?: string;
}

/**
 * Definition: AwardRecipientIndividualData
 */
export interface AwardRecipientIndividualData {
    /** Awarded Date. */
    awardedDate?: string;
    /** First Name. */
    firstName?: string;
    /** Last Name. */
    lastName?: string;
    /** Middle Name. */
    middleName?: string;
    /** Preferred First Name. */
    preferredFirstName?: string;
    /** Second Last Name. */
    secondLastName?: string;
    /** Prefix. */
    prefix?: string;
    /** Suffix. */
    suffix?: string;
    /** Gender. */
    gender?: string;
    primaryOrganization?: Record<string, unknown>;
    /** Individual Id. */
    id?: string;
    /** Customer Type. */
    customerType?: string;
    /** Record Number. */
    recordNumber?: string;
    /** Title. */
    title?: string;
    /** Addresses. */
    addresses?: Array<Record<string, unknown>>;
    /** Emails. */
    emails?: Array<Record<string, unknown>>;
    /** Phones. */
    phones?: Array<Record<string, unknown>>;
    /** Show In Directory. */
    showInDirectory?: boolean;
}

/**
 * Definition: AwardRecipientOrganizationData
 */
export interface AwardRecipientOrganizationData {
    /** Awarded Date. */
    awardedDate?: string;
    /** Organization Name. */
    name?: string;
    /** Acronym. */
    acronym?: string;
    /** Branch Name. */
    branchName?: string;
    /** Annual Sales. */
    annualSales?: number;
    /** Employee Range Id. */
    employeeRangeId?: string;
    /** Organization Id. */
    id?: string;
    /** Customer Type. */
    customerType?: string;
    /** Record Number. */
    recordNumber?: string;
    /** Show In Directory. */
    showInDirectory?: boolean;
    /** Addresses. */
    addresses?: Array<Record<string, unknown>>;
    /** Emails. */
    emails?: Array<Record<string, unknown>>;
    /** Phones. */
    phones?: Array<Record<string, unknown>>;
}

/**
 * Definition: OrganizationLookupBasicData
 */
export interface OrganizationLookupBasicData {
    /** Organization Name. */
    name?: string;
    /** Acronym. */
    acronym?: string;
    /** Branch Name. */
    branchName?: string;
    /** Employee Range Id. */
    employeeRangeId?: string;
    /** Organization Id. */
    id?: string;
    /** Record Number. */
    recordNumber?: string;
    /** Addresses. */
    addresses?: Array<Record<string, unknown>>;
}

/**
 * Definition: IndividualLookupBasicData
 */
export interface IndividualLookupBasicData {
    /** First Name. */
    firstName?: string;
    /** Last Name. */
    lastName?: string;
    /** Middle Name. */
    middleName?: string;
    /** Preferred First Name. */
    preferredFirstName?: string;
    /** Second Last Name. */
    secondLastName?: string;
    /** Prefix. */
    prefix?: string;
    /** Suffix. */
    suffix?: string;
    /** Gender. */
    gender?: string;
    /** Title. */
    title?: string;
    primaryOrganization?: Record<string, unknown>;
    /** Individual Id. */
    id?: string;
    /** Record Number. */
    recordNumber?: string;
}

/**
 * Definition: CustomerCustomFieldValuePayload
 */
export interface CustomerCustomFieldValuePayload {
    /** Webhook Id. */
    WebhookId?: string;
    /** Properties. */
    Properties?: Record<string, unknown>;
    Notifications?: Array<Record<string, unknown>>;
}

/**
 * Definition: CustomerMergedPayload
 */
export interface CustomerMergedPayload {
    /** Webhook Id. */
    WebhookId?: string;
    /** Properties. */
    Properties?: Record<string, unknown>;
    Notifications?: Array<Record<string, unknown>>;
}

/**
 * Definition: CustomerRelationshipUpdatedPayload
 */
export interface CustomerRelationshipUpdatedPayload {
    /** Webhook Id. */
    WebhookId?: string;
    /** Properties. */
    Properties?: Record<string, unknown>;
    Notifications?: Array<Record<string, unknown>>;
}

/**
 * Definition: CustomerPhoneUpdatedPayload
 */
export interface CustomerPhoneUpdatedPayload {
    /** Webhook Id. */
    WebhookId?: string;
    /** Properties. */
    Properties?: Record<string, unknown>;
    Notifications?: Array<Record<string, unknown>>;
}

/**
 * Definition: CustomerAddressUpdatedPayload
 */
export interface CustomerAddressUpdatedPayload {
    /** Webhook Id. */
    WebhookId?: string;
    /** Properties. */
    Properties?: Record<string, unknown>;
    Notifications?: Array<Record<string, unknown>>;
}

/**
 * Definition: EventRegistrationSubstitutedPayload
 */
export interface EventRegistrationSubstitutedPayload {
    /** Webhook Id. */
    WebhookId?: string;
    /** Properties. */
    Properties?: Record<string, unknown>;
    Notifications?: Array<Record<string, unknown>>;
}

/**
 * Definition: IndividualRequestToBeForgottenPayload
 */
export interface IndividualRequestToBeForgottenPayload {
    /** Webhook Id. */
    WebhookId?: string;
    /** Properties. */
    Properties?: Record<string, unknown>;
    Notifications?: Array<Record<string, unknown>>;
}

/**
 * Definition: ProductPurchasePayload
 */
export interface ProductPurchasePayload {
    /** Webhook Id. */
    WebhookId?: string;
    /** Properties. */
    Properties?: Record<string, unknown>;
    Notifications?: Array<Record<string, unknown>>;
}

/**
 * Definition: MembershipTypeTerminatedPayload
 */
export interface MembershipTypeTerminatedPayload {
    /** Webhook Id. */
    WebhookId?: string;
    /** Properties. */
    Properties?: Record<string, unknown>;
    Notifications?: Array<Record<string, unknown>>;
}

/**
 * Options for the getAbandonedCheckouts operation.
 */
export interface GetAbandonedCheckoutsOptions extends ConnectorOperationOptions {
    /** Product Code */
    productCode?: string;
    /** Customer Record Number */
    customerRecordNumber?: string;
}

/**
 * Options for the listOfExams operation.
 */
export interface ListOfExamsOptions extends ConnectorOperationOptions {
    /** A specific Exam code. */
    code?: string;
    /** A specific Exam Category name. */
    categoryName?: string;
    /** When set to true, only public Exams will be displayed; If it's not specified will display all. */
    isPublic?: boolean;
    /** When a date (yyyy-MM-ddTHH:mm:ss (UTC)) is specified, only Exams modified after this date will be displayed. */
    changedSince?: string;
    /** Tag */
    tag?: string;
    /** When set to true, add the active price list for every product. */
    includePrices?: boolean;
}

/**
 * Options for the listRegistrants operation.
 */
export interface ListRegistrantsOptions extends ConnectorOperationOptions {
    /** Session Code. */
    sessionCode?: string;
    /** Include Details */
    includeDetails?: boolean;
    /** Registered Since */
    registeredSince?: string;
}

/**
 * Options for the getPurchasesForAnIndividual operation.
 */
export interface GetPurchasesForAnIndividualOptions extends ConnectorOperationOptions {
    /** Product Code. */
    productCode?: string;
    /** yyyy-MM-ddTHH:mm:ss */
    purchasedSince?: string;
    /** Product Category Code. */
    productCategoryCode?: string;
}

/**
 * Options for the listAllEventCancellationsByEvent operation.
 */
export interface ListAllEventCancellationsByEventOptions extends ConnectorOperationOptions {
    /** Indicates if additional information should be returned. */
    includeDetails?: boolean;
    /** The initial date to search cancellations (yyyy-MM-ddTHH:mm:ss) */
    cancelledSince?: string;
}

/**
 * Options for the getAllOpenOrdersForAnIndividual operation.
 */
export interface GetAllOpenOrdersForAnIndividualOptions extends ConnectorOperationOptions {
    /** Include Line Items. */
    includeLineItems?: boolean;
    /** From Date. */
    fromDate?: string;
    /** To Date. */
    toDate?: string;
}

/**
 * Options for the findMembersOrIndividualsByFirstName operation.
 */
export interface FindMembersOrIndividualsByFirstNameOptions extends ConnectorOperationOptions {
    /** Include Email Data */
    includeEmail?: boolean;
}

/**
 * Options for the findMembersOrIndividualsByLastName operation.
 */
export interface FindMembersOrIndividualsByLastNameOptions extends ConnectorOperationOptions {
    /** Include Email Data. */
    includeEmail?: boolean;
}

/**
 * Options for the listOfAllOrganizationMembers operation.
 */
export interface ListOfAllOrganizationMembersOptions extends ConnectorOperationOptions {
    /** Zip Code */
    zipCode?: string;
    /** Radius in Miles */
    radius?: number;
    /** State Abbreviation */
    stateAbbreviation?: string;
    /** Congressional District */
    congressionalDistrict?: number;
    /** A valid code for a Membership Type */
    membershipTypeCode?: string;
    /** A valid Membership Type Category name */
    membershipTypeCategory?: string;
    /** City */
    city?: string;
    /** Name */
    name?: string;
    /** Tag */
    tag?: string;
    /** Latitude */
    latitude?: number;
    /** Longitude */
    longitude?: number;
    /** Domain */
    domain?: string;
    /** Include Membership Data */
    includeMembership?: boolean;
    /** Include Address Data */
    includeAddress?: boolean;
    /** Include Phone Data */
    includePhone?: boolean;
    /** Include Email Data */
    includeEmail?: boolean;
    /** Include Custom Field Data */
    includeCustomFields?: boolean;
    /** Expire Date - From */
    expiringFrom?: string;
    /** Expire Date - To */
    expiringTo?: string;
}

/**
 * Options for the listOfAllIndividualMembers operation.
 */
export interface ListOfAllIndividualMembersOptions extends ConnectorOperationOptions {
    /** Zip Code */
    zipCode?: string;
    /** Radius in Miles */
    radius?: number;
    /** A valid code for a Membership Type */
    membershipTypeCode?: string;
    /** A valid Membership Type Category name */
    membershipTypeCategory?: string;
    /** Tag */
    tag?: string;
    /** Include Membership Data */
    includeMembership?: boolean;
    /** Include Address Data */
    includeAddress?: boolean;
    /** Include Phone Data */
    includePhone?: boolean;
    /** Include Email Data */
    includeEmail?: boolean;
    /** Include Link Data */
    includeLink?: boolean;
    /** Include Custom Field Data */
    includeCustomFields?: boolean;
    /** Include Categories Data */
    includeCategories?: boolean;
    /** Include Membership Renewal URLs */
    includeMembershipRenewalUrl?: boolean;
    /** Expire Date - From */
    expiringFrom?: string;
    /** Expire Date - To */
    expiringTo?: string;
}

/**
 * Options for the registerAnIndividualForAFreeSession operation.
 */
export interface RegisterAnIndividualForAFreeSessionOptions extends ConnectorOperationOptions {
    /** A Valid Registration Number. */
    registrationNumber?: string;
}

/**
 * Options for the listAllAwards operation.
 */
export interface ListAllAwardsOptions extends ConnectorOperationOptions {
    /** Year. */
    year?: number;
}

/**
 * Options for the findMembersOrIndividualsByName operation.
 */
export interface FindMembersOrIndividualsByNameOptions extends ConnectorOperationOptions {
    /** Include Email Data. */
    includeEmail?: boolean;
}

/**
 * Options for the getNomineesByCommittee operation.
 */
export interface GetNomineesByCommitteeOptions extends ConnectorOperationOptions {
    /** Term */
    term?: string;
}

/**
 * Options for the getOrganizationsRelationships operation.
 */
export interface GetOrganizationsRelationshipsOptions extends ConnectorOperationOptions {
    /** Name of the Relationship. */
    relationshipName?: string;
    /** Includes Details. */
    includesDetails?: boolean;
}

/**
 * Options for the getCommitteeInformationForAnIndividual operation.
 */
export interface GetCommitteeInformationForAnIndividualOptions extends ConnectorOperationOptions {
    /** Include Inactive */
    includeInactive?: boolean;
}

/**
 * Options for the getAllEvents operation.
 */
export interface GetAllEventsOptions extends ConnectorOperationOptions {
    /** Code. */
    code?: string;
    /** Name. */
    name?: string;
    /** Tag. */
    tag?: string;
}

/**
 * Options for the getAllEventRegistrationsInformationForAnIndividual operation.
 */
export interface GetAllEventRegistrationsInformationForAnIndividualOptions extends ConnectorOperationOptions {
    /** Event Code. */
    eventCode?: string;
}

/**
 * Options for the getIndividualsRelationships operation.
 */
export interface GetIndividualsRelationshipsOptions extends ConnectorOperationOptions {
    /** Name of the Relationship. */
    relationshipName?: string;
    /** Include Details. */
    includeDetails?: boolean;
}

/**
 * Options for the addIndividual operation.
 */
export interface AddIndividualOptions extends ConnectorOperationOptions {
    /** Choose True if a user account should be created when the individual is created. */
    createUser?: boolean;
}

/**
 * Options for the findMembersOrIndividualsByFirstAndLastName operation.
 */
export interface FindMembersOrIndividualsByFirstAndLastNameOptions extends ConnectorOperationOptions {
    /** Include Email Data. */
    includeEmail?: boolean;
}

/**
 * Options for the getCommitteeMembersByCommitteeIdOrCode operation.
 */
export interface GetCommitteeMembersByCommitteeIdOrCodeOptions extends ConnectorOperationOptions {
    /** Term */
    term?: number;
    /** Position Codes */
    positionCodes?: string;
}

/**
 * Options for the getAllCommittees operation.
 */
export interface GetAllCommitteesOptions extends ConnectorOperationOptions {
    /** Committee Code. */
    code?: string;
    /** Committee Name. */
    name?: string;
    /** Term. */
    term?: number;
    /** Active Only. */
    activeOnly?: boolean;
}

/**
 * Options for the listAllIndividuals operation.
 */
export interface ListAllIndividualsOptions extends ConnectorOperationOptions {
    /** Individual Name. */
    name?: string;
    /** Include Details. */
    includeDetails?: boolean;
    /** Individual Old ID. */
    oldId?: string;
}
// #endregion Types

export const ImpexiumTriggerOperations = {
    OnWhenCommitteeMemberUpdated: "When-Committee-Member-Updated",
    OnWhenCustomerAddressUpdated: "When-Customer-Address-Updated",
    OnWhenCustomerCustomFieldValueUpdated: "When-Customer-Custom-Field-Value-Updated",
    OnWhenCustomerIsMerged: "When-Customer-Is-Merged",
    OnWhenCustomerPhoneUpdated: "When-Customer-Phone-Updated",
    OnWhenCustomerRelationshipUpdated: "When-Customer-Relationship-Updated",
    OnWhenEmailUpdated: "When-Email-Updated",
    OnWhenEventRegistrationSubstituted: "When-Event-Registration-Substituted",
    OnWhenIndividualCreated: "When-Individual-Created",
    OnWhenIndividualDeleted: "When-Individual-Deleted",
    OnWhenIndividualRequestForgotten: "When-Individual-Request-Forgotten",
    OnWhenMembershipTerminated: "When-Membership-Terminated",
    OnWhenProductPurchased: "When-product-purchased",
    OnWhenPurchaseCancelled: "When-purchase-cancelled",
    OnWhenPurchasePaid: "When-Purchase-Paid",
    OnWhenRequestUpdated: "When-Request-Updated",
} as const;

export type ImpexiumTriggerOperation = typeof ImpexiumTriggerOperations[keyof typeof ImpexiumTriggerOperations];

export const ImpexiumTriggerParameters = {
    OnWhenCommitteeMemberUpdated: {
        contentType: {
            name: "Content-Type",
            type: "string",
            required: true,
            defaultValue: "application/json",
        },
    },
    OnWhenCustomerAddressUpdated: {
        contentType: {
            name: "Content-Type",
            type: "string",
            required: true,
            defaultValue: "application/json",
        },
    },
    OnWhenCustomerCustomFieldValueUpdated: {
        contentType: {
            name: "Content-Type",
            type: "string",
            required: true,
            defaultValue: "application/json",
        },
    },
    OnWhenCustomerIsMerged: {
        contentType: {
            name: "Content-Type",
            type: "string",
            required: true,
            defaultValue: "application/json",
        },
    },
    OnWhenCustomerPhoneUpdated: {
        contentType: {
            name: "Content-Type",
            type: "string",
            required: true,
            defaultValue: "application/json",
        },
    },
    OnWhenCustomerRelationshipUpdated: {
        contentType: {
            name: "Content-Type",
            type: "string",
            required: true,
            defaultValue: "application/json",
        },
    },
    OnWhenEmailUpdated: {
        contentType: {
            name: "Content-Type",
            type: "string",
            required: true,
            defaultValue: "application/json",
        },
    },
    OnWhenEventRegistrationSubstituted: {
        contentType: {
            name: "Content-Type",
            type: "string",
            required: true,
            defaultValue: "application/json",
        },
    },
    OnWhenIndividualCreated: {
        contentType: {
            name: "Content-Type",
            type: "string",
            required: true,
            defaultValue: "application/json",
        },
    },
    OnWhenIndividualDeleted: {
        contentType: {
            name: "Content-Type",
            type: "string",
            required: true,
            defaultValue: "application/json",
        },
    },
    OnWhenIndividualRequestForgotten: {
        contentType: {
            name: "Content-Type",
            type: "string",
            required: true,
            defaultValue: "application/json",
        },
    },
    OnWhenMembershipTerminated: {
        contentType: {
            name: "Content-Type",
            type: "string",
            required: true,
            defaultValue: "application/json",
        },
    },
    OnWhenProductPurchased: {
        contentType: {
            name: "Content-Type",
            type: "string",
            required: true,
            defaultValue: "application/json",
        },
    },
    OnWhenPurchaseCancelled: {
        contentType: {
            name: "Content-Type",
            type: "string",
            required: true,
            defaultValue: "application/json",
        },
    },
    OnWhenPurchasePaid: {
        contentType: {
            name: "Content-Type",
            type: "string",
            required: true,
            defaultValue: "application/json",
        },
    },
    OnWhenRequestUpdated: {
        contentType: {
            name: "Content-Type",
            type: "string",
            required: true,
            defaultValue: "application/json",
        },
    },
} as const;

// #region Client

/**
 * Typed client for the impexium connector.
 */
export class ImpexiumClient extends ConnectorClientBase {

    /**
     * Creates a new ImpexiumClient.
     * @param connectionRuntimeUrl The connection runtime URL from Azure Portal.
     * @param credential The credential used for authentication.
     * @param options Optional connector client options.
     */
    constructor(connectionRuntimeUrl: string, credential: TokenCredential, options?: ConnectorClientOptions) {
        super(connectionRuntimeUrl, credential, options);
    }

    public get connectorName(): string {
        return "impexium";
    }

    /**
     * Get Abandoned Checkouts
     * @remarks Get Abandoned Checkouts
     */
    public async getAbandonedCheckouts(pageNumber: number, abandonedFrom: string, contentType: string, options: GetAbandonedCheckoutsOptions = {}): Promise<GetAbandonedCheckoutsResponse> {
        const queryParams: string[] = [];
        if (abandonedFrom !== undefined) {
            queryParams.push(`abandonedFrom=${encodeURIComponent(String(abandonedFrom))}`);
        }
        if (options.productCode !== undefined) {
            queryParams.push(`productCode=${encodeURIComponent(String(options.productCode))}`);
        }
        if (options.customerRecordNumber !== undefined) {
            queryParams.push(`customerRecordNumber=${encodeURIComponent(String(options.customerRecordNumber))}`);
        }
        const requestPath = `/api/v1/Shopping/AbandonedCheckOuts/${encodeURIComponent(String(pageNumber))}` + (queryParams.length > 0 ? "?" + queryParams.join("&") : "");
        const requestHeaders: Record<string, string> = {};
        if (contentType !== undefined) {
            requestHeaders["Content-Type"] = String(contentType);
        }
        const requestUrl = this.resolveUrl(requestPath);
        const httpResponse = await this.sendWithTracingAsync<GetAbandonedCheckoutsResponse>("Impexium.getAbandonedCheckouts", "GetAbandonedCheckouts", "GET", requestUrl, undefined, options, requestHeaders);

        return httpResponse.value as GetAbandonedCheckoutsResponse;
    }

    /**
     * List All Exhibitors
     * @remarks List All Exhibitors
     */
    public listAllExhibitors(exhibitCode: string, pageNumber: number, contentType: string, options: ConnectorOperationOptions = {}): ConnectorPagedAsyncIterableIterator<ExhibitorData> {
        const requestPath = `/api/v1/Exhibits/${encodeURIComponent(String(exhibitCode))}/Exhibitors/${encodeURIComponent(String(pageNumber))}`;
        const requestHeaders: Record<string, string> = {};
        if (contentType !== undefined) {
            requestHeaders["Content-Type"] = String(contentType);
        }
        return this.createPageable<ListAllExhibitorsResponse, ExhibitorData>(
            requestPath,
            async (requestUrl) => {
                const httpResponse = await this.sendWithTracingAsync<ListAllExhibitorsResponse>("Impexium.listAllExhibitors", "List-All-Exhibitors", "GET", requestUrl, undefined, options, requestHeaders);

                return httpResponse.value as ListAllExhibitorsResponse;
            },
            "dataList",
            undefined,
        );
    }

    /**
     * List of Exams
     * @remarks List of Exams
     */
    public listOfExams(pageNumber: number, contentType: string, options: ListOfExamsOptions = {}): ConnectorPagedAsyncIterableIterator<ExamData> {
        const queryParams: string[] = [];
        if (options.code !== undefined) {
            queryParams.push(`Code=${encodeURIComponent(String(options.code))}`);
        }
        if (options.categoryName !== undefined) {
            queryParams.push(`categoryName=${encodeURIComponent(String(options.categoryName))}`);
        }
        if (options.isPublic !== undefined) {
            queryParams.push(`isPublic=${encodeURIComponent(String(options.isPublic))}`);
        }
        if (options.changedSince !== undefined) {
            queryParams.push(`changedSince=${encodeURIComponent(String(options.changedSince))}`);
        }
        if (options.tag !== undefined) {
            queryParams.push(`Tag=${encodeURIComponent(String(options.tag))}`);
        }
        if (options.includePrices !== undefined) {
            queryParams.push(`includePrices=${encodeURIComponent(String(options.includePrices))}`);
        }
        const requestPath = `/api/v1/Products/Exams/${encodeURIComponent(String(pageNumber))}` + (queryParams.length > 0 ? "?" + queryParams.join("&") : "");
        const requestHeaders: Record<string, string> = {};
        if (contentType !== undefined) {
            requestHeaders["Content-Type"] = String(contentType);
        }
        return this.createPageable<ListOfExamsResponse, ExamData>(
            requestPath,
            async (requestUrl) => {
                const httpResponse = await this.sendWithTracingAsync<ListOfExamsResponse>("Impexium.listOfExams", "List-of-Exams", "GET", requestUrl, undefined, options, requestHeaders);

                return httpResponse.value as ListOfExamsResponse;
            },
            "dataList",
            undefined,
        );
    }

    /**
     * List Registrants
     * @remarks List Registrants
     */
    public listRegistrants(eventCode: string, pageNumber: number, contentType: string, options: ListRegistrantsOptions = {}): ConnectorPagedAsyncIterableIterator<RegistrantData> {
        const queryParams: string[] = [];
        if (options.sessionCode !== undefined) {
            queryParams.push(`sessionCode=${encodeURIComponent(String(options.sessionCode))}`);
        }
        if (options.includeDetails !== undefined) {
            queryParams.push(`includeDetails=${encodeURIComponent(String(options.includeDetails))}`);
        }
        if (options.registeredSince !== undefined) {
            queryParams.push(`registeredSince=${encodeURIComponent(String(options.registeredSince))}`);
        }
        const requestPath = `/api/v1/Events/${encodeURIComponent(String(eventCode))}/Registrations/${encodeURIComponent(String(pageNumber))}` + (queryParams.length > 0 ? "?" + queryParams.join("&") : "");
        const requestHeaders: Record<string, string> = {};
        if (contentType !== undefined) {
            requestHeaders["Content-Type"] = String(contentType);
        }
        return this.createPageable<ListRegistrantsResponse, RegistrantData>(
            requestPath,
            async (requestUrl) => {
                const httpResponse = await this.sendWithTracingAsync<ListRegistrantsResponse>("Impexium.listRegistrants", "List-Registrants", "GET", requestUrl, undefined, options, requestHeaders);

                return httpResponse.value as ListRegistrantsResponse;
            },
            "dataList",
            undefined,
        );
    }

    /**
     * Get Course Attendees
     * @remarks Get Course Attendees
     */
    public async getCourseAttendees(code: string, pageNumber: number, contentType: string, options: ConnectorOperationOptions = {}): Promise<GetCourseAttendeesResponse> {
        const requestPath = `/api/v1/Courses/${encodeURIComponent(String(code))}/Attendees/${encodeURIComponent(String(pageNumber))}`;
        const requestHeaders: Record<string, string> = {};
        if (contentType !== undefined) {
            requestHeaders["Content-Type"] = String(contentType);
        }
        const requestUrl = this.resolveUrl(requestPath);
        const httpResponse = await this.sendWithTracingAsync<GetCourseAttendeesResponse>("Impexium.getCourseAttendees", "Get-Course-Attendees", "GET", requestUrl, undefined, options, requestHeaders);

        return httpResponse.value as GetCourseAttendeesResponse;
    }

    /**
     * Add Exam Scores
     * @remarks Add Exam Scores
     */
    public async addExamScores(input: AddExamScoresInput, examCode: string, contentType: string, options: ConnectorOperationOptions = {}): Promise<Array<ExamScoreResultData>> {
        const requestPath = `/api/v1/Exams/${encodeURIComponent(String(examCode))}/Scores`;
        const requestHeaders: Record<string, string> = {};
        if (contentType !== undefined) {
            requestHeaders["Content-Type"] = String(contentType);
        }
        const requestUrl = this.resolveUrl(requestPath);
        const httpResponse = await this.sendWithTracingAsync<Array<ExamScoreResultData>>("Impexium.addExamScores", "Add-Exam-Scores", "POST", requestUrl, input, options, requestHeaders);

        return httpResponse.value as Array<ExamScoreResultData>;
    }

    /**
     * Find Members by Name
     * @remarks Find Members by Name
     */
    public async findMembersByName(name: string, pageNumber: number, contentType: string, options: ConnectorOperationOptions = {}): Promise<FindMembersByNameResponse> {
        const requestPath = `/api/v1/Customers/Members/FindByName/${encodeURIComponent(String(name))}/${encodeURIComponent(String(pageNumber))}`;
        const requestHeaders: Record<string, string> = {};
        if (contentType !== undefined) {
            requestHeaders["Content-Type"] = String(contentType);
        }
        const requestUrl = this.resolveUrl(requestPath);
        const httpResponse = await this.sendWithTracingAsync<FindMembersByNameResponse>("Impexium.findMembersByName", "Find-Members-by-Name", "GET", requestUrl, undefined, options, requestHeaders);

        return httpResponse.value as FindMembersByNameResponse;
    }

    /**
     * Get Purchases for an Individual
     * @remarks Get Purchases for an Individual
     */
    public async getPurchasesForAnIndividual(idOrRecordNumber: string, pageNumber: number, contentType: string, options: GetPurchasesForAnIndividualOptions = {}): Promise<GetPurchasesForAnIndividualResponse> {
        const queryParams: string[] = [];
        if (options.productCode !== undefined) {
            queryParams.push(`productCode=${encodeURIComponent(String(options.productCode))}`);
        }
        if (options.purchasedSince !== undefined) {
            queryParams.push(`purchasedSince=${encodeURIComponent(String(options.purchasedSince))}`);
        }
        if (options.productCategoryCode !== undefined) {
            queryParams.push(`productCategoryCode=${encodeURIComponent(String(options.productCategoryCode))}`);
        }
        const requestPath = `/api/v1/Individuals/${encodeURIComponent(String(idOrRecordNumber))}/Purchases/${encodeURIComponent(String(pageNumber))}` + (queryParams.length > 0 ? "?" + queryParams.join("&") : "");
        const requestHeaders: Record<string, string> = {};
        if (contentType !== undefined) {
            requestHeaders["Content-Type"] = String(contentType);
        }
        const requestUrl = this.resolveUrl(requestPath);
        const httpResponse = await this.sendWithTracingAsync<GetPurchasesForAnIndividualResponse>("Impexium.getPurchasesForAnIndividual", "Get-Purchases-for-an-Individual", "GET", requestUrl, undefined, options, requestHeaders);

        return httpResponse.value as GetPurchasesForAnIndividualResponse;
    }

    /**
     * Add or Update a List of Custom Fields Per Organization
     * @remarks Add or Update a List of Custom Fields Per Organization
     */
    public async addOrUpdateAListOfCustomFieldsPerOrganization(input: AddOrUpdateAListOfCustomFieldsPerOrganizationInput, idOrRecordNumber: string, contentType: string, options: ConnectorOperationOptions = {}): Promise<Array<CustomFieldResultData>> {
        const requestPath = `/api/v1/Organizations/${encodeURIComponent(String(idOrRecordNumber))}/CustomFieldsList`;
        const requestHeaders: Record<string, string> = {};
        if (contentType !== undefined) {
            requestHeaders["Content-Type"] = String(contentType);
        }
        const requestUrl = this.resolveUrl(requestPath);
        const httpResponse = await this.sendWithTracingAsync<Array<CustomFieldResultData>>("Impexium.addOrUpdateAListOfCustomFieldsPerOrganization", "Add-or-Update-a-List-of-Custom-Fields-Per-Organization", "POST", requestUrl, input, options, requestHeaders);

        return httpResponse.value as Array<CustomFieldResultData>;
    }

    /**
     * Add Nominee
     * @remarks Add Nominee
     */
    public async addNominee(input: CommitteeNomineeSaveData, code: string, contentType: string, options: ConnectorOperationOptions = {}): Promise<void> {
        const requestPath = `/api/v1/Committees/${encodeURIComponent(String(code))}/Nominations`;
        const requestHeaders: Record<string, string> = {};
        if (contentType !== undefined) {
            requestHeaders["Content-Type"] = String(contentType);
        }
        const requestUrl = this.resolveUrl(requestPath);
        await this.sendWithTracingAsync<void>("Impexium.addNominee", "Add-Nominee", "POST", requestUrl, input, options, requestHeaders);
    }

    /**
     * Get Individual Custom Field Values
     * @remarks Get Individual Custom Field Values
     */
    public async getIndividualCustomFieldValues(id: string, contentType: string, options: ConnectorOperationOptions = {}): Promise<Array<CustomFieldData>> {
        const requestPath = `/api/v1/Individuals/${encodeURIComponent(String(id))}/CustomFields`;
        const requestHeaders: Record<string, string> = {};
        if (contentType !== undefined) {
            requestHeaders["Content-Type"] = String(contentType);
        }
        const requestUrl = this.resolveUrl(requestPath);
        const httpResponse = await this.sendWithTracingAsync<Array<CustomFieldData>>("Impexium.getIndividualCustomFieldValues", "Get-Individual-Custom-Field-Values", "GET", requestUrl, undefined, options, requestHeaders);

        return httpResponse.value as Array<CustomFieldData>;
    }

    /**
     * Update Individual Custom Field Values
     * @remarks Update Individual Custom Field Values.
     */
    public async updateCustomFieldValue(input: CustomFieldData, id: string, contentType: string, options: ConnectorOperationOptions = {}): Promise<void> {
        const requestPath = `/api/v1/Individuals/${encodeURIComponent(String(id))}/CustomFields`;
        const requestHeaders: Record<string, string> = {};
        if (contentType !== undefined) {
            requestHeaders["Content-Type"] = String(contentType);
        }
        const requestUrl = this.resolveUrl(requestPath);
        await this.sendWithTracingAsync<void>("Impexium.updateCustomFieldValue", "Update-custom-field-value", "POST", requestUrl, input, options, requestHeaders);
    }

    /**
     * List All Event Cancellations by Event
     * @remarks List All Event Cancellations by Event
     */
    public listAllEventCancellationsByEvent(eventCode: string, pageNumber: number, contentType: string, options: ListAllEventCancellationsByEventOptions = {}): ConnectorPagedAsyncIterableIterator<RegistrantCancellationData> {
        const queryParams: string[] = [];
        if (options.includeDetails !== undefined) {
            queryParams.push(`includeDetails=${encodeURIComponent(String(options.includeDetails))}`);
        }
        if (options.cancelledSince !== undefined) {
            queryParams.push(`cancelledSince=${encodeURIComponent(String(options.cancelledSince))}`);
        }
        const requestPath = `/api/v1/Events/${encodeURIComponent(String(eventCode))}/Cancellations/${encodeURIComponent(String(pageNumber))}` + (queryParams.length > 0 ? "?" + queryParams.join("&") : "");
        const requestHeaders: Record<string, string> = {};
        if (contentType !== undefined) {
            requestHeaders["Content-Type"] = String(contentType);
        }
        return this.createPageable<ListAllEventCancellationsByEventResponse, RegistrantCancellationData>(
            requestPath,
            async (requestUrl) => {
                const httpResponse = await this.sendWithTracingAsync<ListAllEventCancellationsByEventResponse>("Impexium.listAllEventCancellationsByEvent", "List-All-Event-Cancellations-by-Event", "GET", requestUrl, undefined, options, requestHeaders);

                return httpResponse.value as ListAllEventCancellationsByEventResponse;
            },
            "dataList",
            undefined,
        );
    }

    /**
     * Get All Open Orders for an Individual
     * @remarks Get All Open Orders for an Individual
     */
    public async getAllOpenOrdersForAnIndividual(idOrRecordNumber: string, pageNumber: number, contentType: string, options: GetAllOpenOrdersForAnIndividualOptions = {}): Promise<GetAllOpenOrdersForAnIndividualResponse> {
        const queryParams: string[] = [];
        if (options.includeLineItems !== undefined) {
            queryParams.push(`includeLineItems=${encodeURIComponent(String(options.includeLineItems))}`);
        }
        if (options.fromDate !== undefined) {
            queryParams.push(`fromDate=${encodeURIComponent(String(options.fromDate))}`);
        }
        if (options.toDate !== undefined) {
            queryParams.push(`toDate=${encodeURIComponent(String(options.toDate))}`);
        }
        const requestPath = `/api/v1/Individuals/${encodeURIComponent(String(idOrRecordNumber))}/Orders/Open/${encodeURIComponent(String(pageNumber))}` + (queryParams.length > 0 ? "?" + queryParams.join("&") : "");
        const requestHeaders: Record<string, string> = {};
        if (contentType !== undefined) {
            requestHeaders["Content-Type"] = String(contentType);
        }
        const requestUrl = this.resolveUrl(requestPath);
        const httpResponse = await this.sendWithTracingAsync<GetAllOpenOrdersForAnIndividualResponse>("Impexium.getAllOpenOrdersForAnIndividual", "Get-All-Open-Orders-for-an-Individual", "GET", requestUrl, undefined, options, requestHeaders);

        return httpResponse.value as GetAllOpenOrdersForAnIndividualResponse;
    }

    /**
     * List Completed User Tasks by User ID or Email
     * @remarks List Completed User Tasks by User ID or Email
     */
    public listCompletedUserTasksByUserIdOrEmail(userIdOrEmail: string, pageNumber: number, contentType: string, options: ConnectorOperationOptions = {}): ConnectorPagedAsyncIterableIterator<UserTaskData> {
        const requestPath = `/api/v1/tasks/Users/${encodeURIComponent(String(userIdOrEmail))}/Completed/${encodeURIComponent(String(pageNumber))}`;
        const requestHeaders: Record<string, string> = {};
        if (contentType !== undefined) {
            requestHeaders["Content-Type"] = String(contentType);
        }
        return this.createPageable<ListCompletedUserTasksByUserIdOrEmailResponse, UserTaskData>(
            requestPath,
            async (requestUrl) => {
                const httpResponse = await this.sendWithTracingAsync<ListCompletedUserTasksByUserIdOrEmailResponse>("Impexium.listCompletedUserTasksByUserIdOrEmail", "List-Completed-User-Tasks-by-User-ID-or-Email", "GET", requestUrl, undefined, options, requestHeaders);

                return httpResponse.value as ListCompletedUserTasksByUserIdOrEmailResponse;
            },
            "dataList",
            undefined,
        );
    }

    /**
     * List Pending User Tasks by User ID or Email
     * @remarks List Pending User Tasks by User ID or Email
     */
    public listPendingUserTasksByUserIdOrEmail(userIdOrEmail: string, pageNumber: number, contentType: string, options: ConnectorOperationOptions = {}): ConnectorPagedAsyncIterableIterator<UserTaskData> {
        const requestPath = `/api/v1/tasks/Users/${encodeURIComponent(String(userIdOrEmail))}/Pending/${encodeURIComponent(String(pageNumber))}`;
        const requestHeaders: Record<string, string> = {};
        if (contentType !== undefined) {
            requestHeaders["Content-Type"] = String(contentType);
        }
        return this.createPageable<ListPendingUserTasksByUserIdOrEmailResponse, UserTaskData>(
            requestPath,
            async (requestUrl) => {
                const httpResponse = await this.sendWithTracingAsync<ListPendingUserTasksByUserIdOrEmailResponse>("Impexium.listPendingUserTasksByUserIdOrEmail", "List-Pending-User-Tasks-by-User-ID-or-Email", "GET", requestUrl, undefined, options, requestHeaders);

                return httpResponse.value as ListPendingUserTasksByUserIdOrEmailResponse;
            },
            "dataList",
            undefined,
        );
    }

    /**
     * Add Note to Sales Opportunity
     * @remarks Add Note to Sales Opportunity
     */
    public async addNoteToSalesOpportunity(input: BaseNoteData, id: string, contentType: string, options: ConnectorOperationOptions = {}): Promise<void> {
        const requestPath = `/api/v1/Sales/Opportunities/${encodeURIComponent(String(id))}/Notes`;
        const requestHeaders: Record<string, string> = {};
        if (contentType !== undefined) {
            requestHeaders["Content-Type"] = String(contentType);
        }
        const requestUrl = this.resolveUrl(requestPath);
        await this.sendWithTracingAsync<void>("Impexium.addNoteToSalesOpportunity", "Add-Note-to-Sales-Opportunity", "POST", requestUrl, input, options, requestHeaders);
    }

    /**
     * Add Activity to Sales Opportunity
     * @remarks Add Activity to Sales Opportunity
     */
    public async addActivityToSalesOpportunity(input: ActivityData, id: string, contentType: string, options: ConnectorOperationOptions = {}): Promise<void> {
        const requestPath = `/api/v1/Sales/Opportunities/${encodeURIComponent(String(id))}/Activities`;
        const requestHeaders: Record<string, string> = {};
        if (contentType !== undefined) {
            requestHeaders["Content-Type"] = String(contentType);
        }
        const requestUrl = this.resolveUrl(requestPath);
        await this.sendWithTracingAsync<void>("Impexium.addActivityToSalesOpportunity", "Add-Activity-to-Sales-Opportunity", "POST", requestUrl, input, options, requestHeaders);
    }

    /**
     * Update Task by Task Number
     * @remarks Update Task by Task Number
     */
    public async updateTaskByTaskNumber(input: TaskSaveData, taskNumber: string, contentType: string, options: ConnectorOperationOptions = {}): Promise<TaskData> {
        const requestPath = `/api/v1/tasks/${encodeURIComponent(String(taskNumber))}`;
        const requestHeaders: Record<string, string> = {};
        if (contentType !== undefined) {
            requestHeaders["Content-Type"] = String(contentType);
        }
        const requestUrl = this.resolveUrl(requestPath);
        const httpResponse = await this.sendWithTracingAsync<TaskData>("Impexium.updateTaskByTaskNumber", "Update-Task-by-Task-Number", "PUT", requestUrl, input, options, requestHeaders);

        return httpResponse.value as TaskData;
    }

    /**
     * List All Countries
     * @remarks List All Countries
     */
    public listAllCountries(pageNumber: number, contentType: string, options: ConnectorOperationOptions = {}): ConnectorPagedAsyncIterableIterator<CountryData> {
        const requestPath = `/api/v1/Countries/All/${encodeURIComponent(String(pageNumber))}`;
        const requestHeaders: Record<string, string> = {};
        if (contentType !== undefined) {
            requestHeaders["Content-Type"] = String(contentType);
        }
        return this.createPageable<ListAllCountriesResponse, CountryData>(
            requestPath,
            async (requestUrl) => {
                const httpResponse = await this.sendWithTracingAsync<ListAllCountriesResponse>("Impexium.listAllCountries", "List-All-Countries", "GET", requestUrl, undefined, options, requestHeaders);

                return httpResponse.value as ListAllCountriesResponse;
            },
            "dataList",
            undefined,
        );
    }

    /**
     * Get All States by Country
     * @remarks Get All States by Country
     */
    public async getAllStatesByCountry(countryId: string, pageNumber: number, contentType: string, options: ConnectorOperationOptions = {}): Promise<GetAllStatesByCountryResponse> {
        const requestPath = `/api/v1/Countries/${encodeURIComponent(String(countryId))}/States/All/${encodeURIComponent(String(pageNumber))}`;
        const requestHeaders: Record<string, string> = {};
        if (contentType !== undefined) {
            requestHeaders["Content-Type"] = String(contentType);
        }
        const requestUrl = this.resolveUrl(requestPath);
        const httpResponse = await this.sendWithTracingAsync<GetAllStatesByCountryResponse>("Impexium.getAllStatesByCountry", "Get-All-States-by-Country", "GET", requestUrl, undefined, options, requestHeaders);

        return httpResponse.value as GetAllStatesByCountryResponse;
    }

    /**
     * List All Exhibits
     * @remarks List All Exhibits
     */
    public listAllExhibits(pageNumber: number, contentType: string, options: ConnectorOperationOptions = {}): ConnectorPagedAsyncIterableIterator<ExhibitData> {
        const requestPath = `/api/v1/Exhibits/${encodeURIComponent(String(pageNumber))}`;
        const requestHeaders: Record<string, string> = {};
        if (contentType !== undefined) {
            requestHeaders["Content-Type"] = String(contentType);
        }
        return this.createPageable<ListAllExhibitsResponse, ExhibitData>(
            requestPath,
            async (requestUrl) => {
                const httpResponse = await this.sendWithTracingAsync<ListAllExhibitsResponse>("Impexium.listAllExhibits", "List-All-Exhibits", "GET", requestUrl, undefined, options, requestHeaders);

                return httpResponse.value as ListAllExhibitsResponse;
            },
            "dataList",
            undefined,
        );
    }

    /**
     * Delete a Category for an Organization
     * @remarks Delete a Category for an Organization
     */
    public async deleteACategoryForAnOrganization(recordNumber: string, categoryCode: string, contentType: string, options: ConnectorOperationOptions = {}): Promise<void> {
        const requestPath = `/api/v1/Organizations/${encodeURIComponent(String(recordNumber))}/Categories/${encodeURIComponent(String(categoryCode))}`;
        const requestHeaders: Record<string, string> = {};
        if (contentType !== undefined) {
            requestHeaders["Content-Type"] = String(contentType);
        }
        const requestUrl = this.resolveUrl(requestPath);
        await this.sendWithTracingAsync<void>("Impexium.deleteACategoryForAnOrganization", "Delete-a-Category-for-an-Organization", "DELETE", requestUrl, undefined, options, requestHeaders);
    }

    /**
     * Add Customer Request
     * @remarks Add Customer Request
     */
    public async addCustomerRequest(input: RequestSaveData, contentType: string, options: ConnectorOperationOptions = {}): Promise<void> {
        const requestPath = `/api/v1/Requests`;
        const requestHeaders: Record<string, string> = {};
        if (contentType !== undefined) {
            requestHeaders["Content-Type"] = String(contentType);
        }
        const requestUrl = this.resolveUrl(requestPath);
        await this.sendWithTracingAsync<void>("Impexium.addCustomerRequest", "Add-Customer-Request", "POST", requestUrl, input, options, requestHeaders);
    }

    /**
     * Update Customer Request
     * @remarks Update Customer Request
     */
    public async updateCustomerRequest(input: RequestUpdateData, contentType: string, options: ConnectorOperationOptions = {}): Promise<RequestUpdateData> {
        const requestPath = `/api/v1/Requests`;
        const requestHeaders: Record<string, string> = {};
        if (contentType !== undefined) {
            requestHeaders["Content-Type"] = String(contentType);
        }
        const requestUrl = this.resolveUrl(requestPath);
        const httpResponse = await this.sendWithTracingAsync<RequestUpdateData>("Impexium.updateCustomerRequest", "Update-Customer-Request", "PUT", requestUrl, input, options, requestHeaders);

        return httpResponse.value as RequestUpdateData;
    }

    /**
     * Add Categories for an Organization
     * @remarks Add Categories for an Organization
     */
    public async addCategoriesForAnOrganization(input: AddCategoriesForAnOrganizationInput, recordNumber: string, contentType: string, options: ConnectorOperationOptions = {}): Promise<void> {
        const requestPath = `/api/v1/Organizations/${encodeURIComponent(String(recordNumber))}/Categories`;
        const requestHeaders: Record<string, string> = {};
        if (contentType !== undefined) {
            requestHeaders["Content-Type"] = String(contentType);
        }
        const requestUrl = this.resolveUrl(requestPath);
        await this.sendWithTracingAsync<void>("Impexium.addCategoriesForAnOrganization", "Add-Categories-for-an-Organization", "POST", requestUrl, input, options, requestHeaders);
    }

    /**
     * List of Customer Relationships
     * @remarks List of Customer Relationships
     */
    public listOfCustomerRelationships(pageNumber: number, contentType: string, options: ConnectorOperationOptions = {}): ConnectorPagedAsyncIterableIterator<RelationshipTypeData> {
        const requestPath = `/api/v1/Customers/RelationshipTypes/${encodeURIComponent(String(pageNumber))}`;
        const requestHeaders: Record<string, string> = {};
        if (contentType !== undefined) {
            requestHeaders["Content-Type"] = String(contentType);
        }
        return this.createPageable<ListOfCustomerRelationshipsResponse, RelationshipTypeData>(
            requestPath,
            async (requestUrl) => {
                const httpResponse = await this.sendWithTracingAsync<ListOfCustomerRelationshipsResponse>("Impexium.listOfCustomerRelationships", "List-of-Customer-Relationships", "GET", requestUrl, undefined, options, requestHeaders);

                return httpResponse.value as ListOfCustomerRelationshipsResponse;
            },
            "dataList",
            undefined,
        );
    }

    /**
     * List All Open Customer Request
     * @remarks List All Open Customer Request
     */
    public listAllOpenCustomerRequest(pageNumber: number, contentType: string, options: ConnectorOperationOptions = {}): ConnectorPagedAsyncIterableIterator<RequestData> {
        const requestPath = `/api/v1/Requests/Open/${encodeURIComponent(String(pageNumber))}`;
        const requestHeaders: Record<string, string> = {};
        if (contentType !== undefined) {
            requestHeaders["Content-Type"] = String(contentType);
        }
        return this.createPageable<ListAllOpenCustomerRequestResponse, RequestData>(
            requestPath,
            async (requestUrl) => {
                const httpResponse = await this.sendWithTracingAsync<ListAllOpenCustomerRequestResponse>("Impexium.listAllOpenCustomerRequest", "List-All-Open-Customer-Request", "GET", requestUrl, undefined, options, requestHeaders);

                return httpResponse.value as ListAllOpenCustomerRequestResponse;
            },
            "dataList",
            undefined,
        );
    }

    /**
     * Get Organization Inactive Memberships
     * @remarks Get Organization Inactive Memberships
     */
    public async getOrganizationInactiveMemberships(id: string, contentType: string, options: ConnectorOperationOptions = {}): Promise<MembershipData> {
        const requestPath = `/api/v1/Organizations/${encodeURIComponent(String(id))}/Memberships/Inactive`;
        const requestHeaders: Record<string, string> = {};
        if (contentType !== undefined) {
            requestHeaders["Content-Type"] = String(contentType);
        }
        const requestUrl = this.resolveUrl(requestPath);
        const httpResponse = await this.sendWithTracingAsync<MembershipData>("Impexium.getOrganizationInactiveMemberships", "Get-Organization-Inactive-Memberships", "GET", requestUrl, undefined, options, requestHeaders);

        return httpResponse.value as MembershipData;
    }

    /**
     * Delete Record From Custom Data Table
     * @remarks Delete Record From Custom Data Table
     */
    public async deleteRecordFromCustomDataTable(tableName: string, id: string, contentType: string, options: ConnectorOperationOptions = {}): Promise<void> {
        const requestPath = `/api/v1/CustomData/${encodeURIComponent(String(tableName))}/${encodeURIComponent(String(id))}`;
        const requestHeaders: Record<string, string> = {};
        if (contentType !== undefined) {
            requestHeaders["Content-Type"] = String(contentType);
        }
        const requestUrl = this.resolveUrl(requestPath);
        await this.sendWithTracingAsync<void>("Impexium.deleteRecordFromCustomDataTable", "Delete-Record-From-Custom-Data-Table", "DELETE", requestUrl, undefined, options, requestHeaders);
    }

    /**
     * Update User Task Progress or Mark as Completed
     * @remarks Update User Task Progress or Mark as Completed
     */
    public async updateUserTaskProgressOrMarkAsCompleted(input: UserTaskData, userIdOrEmail: string, contentType: string, options: ConnectorOperationOptions = {}): Promise<UserTaskData> {
        const requestPath = `/api/v1/tasks/Users/${encodeURIComponent(String(userIdOrEmail))}`;
        const requestHeaders: Record<string, string> = {};
        if (contentType !== undefined) {
            requestHeaders["Content-Type"] = String(contentType);
        }
        const requestUrl = this.resolveUrl(requestPath);
        const httpResponse = await this.sendWithTracingAsync<UserTaskData>("Impexium.updateUserTaskProgressOrMarkAsCompleted", "Update-User-Task-Progress-or-Mark-as-Completed", "PUT", requestUrl, input, options, requestHeaders);

        return httpResponse.value as UserTaskData;
    }

    /**
     * Find Members (Individuals) by First Name
     * @remarks Find Members (Individuals) by First Name
     */
    public async findMembersOrIndividualsByFirstName(firstName: string, pageNumber: number, contentType: string, options: FindMembersOrIndividualsByFirstNameOptions = {}): Promise<IndividualData> {
        const queryParams: string[] = [];
        if (options.includeEmail !== undefined) {
            queryParams.push(`includeEmail=${encodeURIComponent(String(options.includeEmail))}`);
        }
        const requestPath = `/api/v1/Individuals/Members/FindByFirstName/${encodeURIComponent(String(firstName))}/${encodeURIComponent(String(pageNumber))}` + (queryParams.length > 0 ? "?" + queryParams.join("&") : "");
        const requestHeaders: Record<string, string> = {};
        if (contentType !== undefined) {
            requestHeaders["Content-Type"] = String(contentType);
        }
        const requestUrl = this.resolveUrl(requestPath);
        const httpResponse = await this.sendWithTracingAsync<IndividualData>("Impexium.findMembersOrIndividualsByFirstName", "Find-Members-or-Individuals-by-First-Name", "GET", requestUrl, undefined, options, requestHeaders);

        return httpResponse.value as IndividualData;
    }

    /**
     * Find Members (Individuals) by Last Name
     * @remarks Find Members (Individuals) by Last Name
     */
    public async findMembersOrIndividualsByLastName(lastName: string, pageNumber: number, contentType: string, options: FindMembersOrIndividualsByLastNameOptions = {}): Promise<IndividualData> {
        const queryParams: string[] = [];
        if (options.includeEmail !== undefined) {
            queryParams.push(`includeEmail=${encodeURIComponent(String(options.includeEmail))}`);
        }
        const requestPath = `/api/v1/Individuals/Members/FindByLastName/${encodeURIComponent(String(lastName))}/${encodeURIComponent(String(pageNumber))}` + (queryParams.length > 0 ? "?" + queryParams.join("&") : "");
        const requestHeaders: Record<string, string> = {};
        if (contentType !== undefined) {
            requestHeaders["Content-Type"] = String(contentType);
        }
        const requestUrl = this.resolveUrl(requestPath);
        const httpResponse = await this.sendWithTracingAsync<IndividualData>("Impexium.findMembersOrIndividualsByLastName", "Find-Members-or-Individuals-by-Last-Name", "GET", requestUrl, undefined, options, requestHeaders);

        return httpResponse.value as IndividualData;
    }

    /**
     * Create a new task or Assign Task to a User
     * @remarks Create a new task or Assign Task to a User
     */
    public async assignTaskToAUser(input: UserTaskData, userIdOrEmail: string, contentType: string, options: ConnectorOperationOptions = {}): Promise<void> {
        const requestPath = `/api/v1/tasks/Users/${encodeURIComponent(String(userIdOrEmail))}/Task`;
        const requestHeaders: Record<string, string> = {};
        if (contentType !== undefined) {
            requestHeaders["Content-Type"] = String(contentType);
        }
        const requestUrl = this.resolveUrl(requestPath);
        await this.sendWithTracingAsync<void>("Impexium.assignTaskToAUser", "Assign-Task-to-a-User", "POST", requestUrl, input, options, requestHeaders);
    }

    /**
     * Delete a Category for an Individual
     * @remarks Delete a Category for an Individual
     */
    public async deleteACategoryForAnIndividual(recordNumber: string, categoryCode: string, contentType: string, options: ConnectorOperationOptions = {}): Promise<void> {
        const requestPath = `/api/v1/Individuals/${encodeURIComponent(String(recordNumber))}/Categories/${encodeURIComponent(String(categoryCode))}`;
        const requestHeaders: Record<string, string> = {};
        if (contentType !== undefined) {
            requestHeaders["Content-Type"] = String(contentType);
        }
        const requestUrl = this.resolveUrl(requestPath);
        await this.sendWithTracingAsync<void>("Impexium.deleteACategoryForAnIndividual", "Delete-a-Category-for-an-Individual", "DELETE", requestUrl, undefined, options, requestHeaders);
    }

    /**
     * Add Notification to Individual
     * @remarks Add Notification to Individual
     */
    public async addNotificationToIndividual(input: NotificationData, id: string, contentType: string, options: ConnectorOperationOptions = {}): Promise<void> {
        const requestPath = `/api/v1/Individuals/${encodeURIComponent(String(id))}/Notifications`;
        const requestHeaders: Record<string, string> = {};
        if (contentType !== undefined) {
            requestHeaders["Content-Type"] = String(contentType);
        }
        const requestUrl = this.resolveUrl(requestPath);
        await this.sendWithTracingAsync<void>("Impexium.addNotificationToIndividual", "Add-Notification-to-Individual", "POST", requestUrl, input, options, requestHeaders);
    }

    /**
     * Add Categories for an Individual
     * @remarks Add Categories for an Individual
     */
    public async addCategoriesForAnIndividual(input: AddCategoriesForAnIndividualInput, recordNumber: string, contentType: string, options: ConnectorOperationOptions = {}): Promise<void> {
        const requestPath = `/api/v1/Individuals/${encodeURIComponent(String(recordNumber))}/Categories`;
        const requestHeaders: Record<string, string> = {};
        if (contentType !== undefined) {
            requestHeaders["Content-Type"] = String(contentType);
        }
        const requestUrl = this.resolveUrl(requestPath);
        await this.sendWithTracingAsync<void>("Impexium.addCategoriesForAnIndividual", "Add-Categories-for-an-Individual", "POST", requestUrl, input, options, requestHeaders);
    }

    /**
     * Add a New Task
     * @remarks Add a New Task
     */
    public async addANewTask(input: TaskSaveData, contentType: string, options: ConnectorOperationOptions = {}): Promise<TaskData> {
        const requestPath = `/api/v1/tasks`;
        const requestHeaders: Record<string, string> = {};
        if (contentType !== undefined) {
            requestHeaders["Content-Type"] = String(contentType);
        }
        const requestUrl = this.resolveUrl(requestPath);
        const httpResponse = await this.sendWithTracingAsync<TaskData>("Impexium.addANewTask", "Add-a-New-Task", "POST", requestUrl, input, options, requestHeaders);

        return httpResponse.value as TaskData;
    }

    /**
     * List of All Organization Members
     * @remarks List of All Organization Members
     */
    public listOfAllOrganizationMembers(pageNumber: number, contentType: string, options: ListOfAllOrganizationMembersOptions = {}): ConnectorPagedAsyncIterableIterator<OrganizationData> {
        const queryParams: string[] = [];
        if (options.zipCode !== undefined) {
            queryParams.push(`zipCode=${encodeURIComponent(String(options.zipCode))}`);
        }
        if (options.radius !== undefined) {
            queryParams.push(`Radius=${encodeURIComponent(String(options.radius))}`);
        }
        if (options.stateAbbreviation !== undefined) {
            queryParams.push(`stateAbbreviation=${encodeURIComponent(String(options.stateAbbreviation))}`);
        }
        if (options.congressionalDistrict !== undefined) {
            queryParams.push(`congressionalDistrict=${encodeURIComponent(String(options.congressionalDistrict))}`);
        }
        if (options.membershipTypeCode !== undefined) {
            queryParams.push(`membershipTypeCode=${encodeURIComponent(String(options.membershipTypeCode))}`);
        }
        if (options.membershipTypeCategory !== undefined) {
            queryParams.push(`membershipTypeCategory=${encodeURIComponent(String(options.membershipTypeCategory))}`);
        }
        if (options.city !== undefined) {
            queryParams.push(`City=${encodeURIComponent(String(options.city))}`);
        }
        if (options.name !== undefined) {
            queryParams.push(`Name=${encodeURIComponent(String(options.name))}`);
        }
        if (options.tag !== undefined) {
            queryParams.push(`Tag=${encodeURIComponent(String(options.tag))}`);
        }
        if (options.latitude !== undefined) {
            queryParams.push(`Latitude=${encodeURIComponent(String(options.latitude))}`);
        }
        if (options.longitude !== undefined) {
            queryParams.push(`Longitude=${encodeURIComponent(String(options.longitude))}`);
        }
        if (options.domain !== undefined) {
            queryParams.push(`Domain=${encodeURIComponent(String(options.domain))}`);
        }
        if (options.includeMembership !== undefined) {
            queryParams.push(`includeMembership=${encodeURIComponent(String(options.includeMembership))}`);
        }
        if (options.includeAddress !== undefined) {
            queryParams.push(`includeAddress=${encodeURIComponent(String(options.includeAddress))}`);
        }
        if (options.includePhone !== undefined) {
            queryParams.push(`includePhone=${encodeURIComponent(String(options.includePhone))}`);
        }
        if (options.includeEmail !== undefined) {
            queryParams.push(`includeEmail=${encodeURIComponent(String(options.includeEmail))}`);
        }
        if (options.includeCustomFields !== undefined) {
            queryParams.push(`includeCustomFields=${encodeURIComponent(String(options.includeCustomFields))}`);
        }
        if (options.expiringFrom !== undefined) {
            queryParams.push(`expiringFrom=${encodeURIComponent(String(options.expiringFrom))}`);
        }
        if (options.expiringTo !== undefined) {
            queryParams.push(`expiringTo=${encodeURIComponent(String(options.expiringTo))}`);
        }
        const requestPath = `/api/v1/Organizations/Members/All/${encodeURIComponent(String(pageNumber))}` + (queryParams.length > 0 ? "?" + queryParams.join("&") : "");
        const requestHeaders: Record<string, string> = {};
        if (contentType !== undefined) {
            requestHeaders["Content-Type"] = String(contentType);
        }
        return this.createPageable<ListOfAllOrganizationMembersResponse, OrganizationData>(
            requestPath,
            async (requestUrl) => {
                const httpResponse = await this.sendWithTracingAsync<ListOfAllOrganizationMembersResponse>("Impexium.listOfAllOrganizationMembers", "List-of-All-Organization-Members", "GET", requestUrl, undefined, options, requestHeaders);

                return httpResponse.value as ListOfAllOrganizationMembersResponse;
            },
            "dataList",
            undefined,
        );
    }

    /**
     * List of All Individual Members
     * @remarks List of All Individual Members
     */
    public listOfAllIndividualMembers(pageNumber: number, contentType: string, options: ListOfAllIndividualMembersOptions = {}): ConnectorPagedAsyncIterableIterator<IndividualData> {
        const queryParams: string[] = [];
        if (options.zipCode !== undefined) {
            queryParams.push(`zipCode=${encodeURIComponent(String(options.zipCode))}`);
        }
        if (options.radius !== undefined) {
            queryParams.push(`Radius=${encodeURIComponent(String(options.radius))}`);
        }
        if (options.membershipTypeCode !== undefined) {
            queryParams.push(`membershipTypeCode=${encodeURIComponent(String(options.membershipTypeCode))}`);
        }
        if (options.membershipTypeCategory !== undefined) {
            queryParams.push(`membershipTypeCategory=${encodeURIComponent(String(options.membershipTypeCategory))}`);
        }
        if (options.tag !== undefined) {
            queryParams.push(`Tag=${encodeURIComponent(String(options.tag))}`);
        }
        if (options.includeMembership !== undefined) {
            queryParams.push(`includeMembership=${encodeURIComponent(String(options.includeMembership))}`);
        }
        if (options.includeAddress !== undefined) {
            queryParams.push(`includeAddress=${encodeURIComponent(String(options.includeAddress))}`);
        }
        if (options.includePhone !== undefined) {
            queryParams.push(`includePhone=${encodeURIComponent(String(options.includePhone))}`);
        }
        if (options.includeEmail !== undefined) {
            queryParams.push(`includeEmail=${encodeURIComponent(String(options.includeEmail))}`);
        }
        if (options.includeLink !== undefined) {
            queryParams.push(`includeLink=${encodeURIComponent(String(options.includeLink))}`);
        }
        if (options.includeCustomFields !== undefined) {
            queryParams.push(`includeCustomFields=${encodeURIComponent(String(options.includeCustomFields))}`);
        }
        if (options.includeCategories !== undefined) {
            queryParams.push(`includeCategories=${encodeURIComponent(String(options.includeCategories))}`);
        }
        if (options.includeMembershipRenewalUrl !== undefined) {
            queryParams.push(`includeMembershipRenewalUrl=${encodeURIComponent(String(options.includeMembershipRenewalUrl))}`);
        }
        if (options.expiringFrom !== undefined) {
            queryParams.push(`expiringFrom=${encodeURIComponent(String(options.expiringFrom))}`);
        }
        if (options.expiringTo !== undefined) {
            queryParams.push(`expiringTo=${encodeURIComponent(String(options.expiringTo))}`);
        }
        const requestPath = `/api/v1/Individuals/Members/All/${encodeURIComponent(String(pageNumber))}` + (queryParams.length > 0 ? "?" + queryParams.join("&") : "");
        const requestHeaders: Record<string, string> = {};
        if (contentType !== undefined) {
            requestHeaders["Content-Type"] = String(contentType);
        }
        return this.createPageable<ListOfAllIndividualMembersResponse, IndividualData>(
            requestPath,
            async (requestUrl) => {
                const httpResponse = await this.sendWithTracingAsync<ListOfAllIndividualMembersResponse>("Impexium.listOfAllIndividualMembers", "List-of-All-Individual-Members", "GET", requestUrl, undefined, options, requestHeaders);

                return httpResponse.value as ListOfAllIndividualMembersResponse;
            },
            "dataList",
            undefined,
        );
    }

    /**
     * Get List of Active Certifications for an Organization
     * @remarks Get List of Active Certifications for an Organization
     */
    public async getListOfActiveCertificationsForAnOrganization(idOrRecordNumber: string, pageNumber: number, contentType: string, options: ConnectorOperationOptions = {}): Promise<GetListOfActiveCertificationsForAnOrganizationResponse> {
        const requestPath = `/api/v1/Organizations/${encodeURIComponent(String(idOrRecordNumber))}/Certifications/${encodeURIComponent(String(pageNumber))}`;
        const requestHeaders: Record<string, string> = {};
        if (contentType !== undefined) {
            requestHeaders["Content-Type"] = String(contentType);
        }
        const requestUrl = this.resolveUrl(requestPath);
        const httpResponse = await this.sendWithTracingAsync<GetListOfActiveCertificationsForAnOrganizationResponse>("Impexium.getListOfActiveCertificationsForAnOrganization", "Get-List-of-Active-Certifications-for-an-Organization", "GET", requestUrl, undefined, options, requestHeaders);

        return httpResponse.value as GetListOfActiveCertificationsForAnOrganizationResponse;
    }

    /**
     * Get List of Active Certifications for an Individual
     * @remarks Get List of Active Certifications for an Individual
     */
    public async getListOfActiveCertificationsForAnIndividual(idOrRecordNumber: string, pageNumber: number, contentType: string, options: ConnectorOperationOptions = {}): Promise<GetListOfActiveCertificationsForAnIndividualResponse> {
        const requestPath = `/api/v1/Individuals/${encodeURIComponent(String(idOrRecordNumber))}/Certifications/${encodeURIComponent(String(pageNumber))}`;
        const requestHeaders: Record<string, string> = {};
        if (contentType !== undefined) {
            requestHeaders["Content-Type"] = String(contentType);
        }
        const requestUrl = this.resolveUrl(requestPath);
        const httpResponse = await this.sendWithTracingAsync<GetListOfActiveCertificationsForAnIndividualResponse>("Impexium.getListOfActiveCertificationsForAnIndividual", "Get-List-of-Active-Certifications-for-an-Individual", "GET", requestUrl, undefined, options, requestHeaders);

        return httpResponse.value as GetListOfActiveCertificationsForAnIndividualResponse;
    }

    /**
     * Get Individual Inactive Memberships
     * @remarks Get Individual Inactive Memberships
     */
    public async getIndividualInactiveMemberships(id: string, contentType: string, options: ConnectorOperationOptions = {}): Promise<MembershipData> {
        const requestPath = `/api/v1/Individuals/${encodeURIComponent(String(id))}/Memberships/Inactive`;
        const requestHeaders: Record<string, string> = {};
        if (contentType !== undefined) {
            requestHeaders["Content-Type"] = String(contentType);
        }
        const requestUrl = this.resolveUrl(requestPath);
        const httpResponse = await this.sendWithTracingAsync<MembershipData>("Impexium.getIndividualInactiveMemberships", "Get-Individual-Inactive-Memberships", "GET", requestUrl, undefined, options, requestHeaders);

        return httpResponse.value as MembershipData;
    }

    /**
     * Add Activity to Organization
     * @remarks Add Activity to Organization
     */
    public async addActivityToOrganization(input: ActivityData, id: string, contentType: string, options: ConnectorOperationOptions = {}): Promise<void> {
        const requestPath = `/api/v1/Organizations/${encodeURIComponent(String(id))}/Activities`;
        const requestHeaders: Record<string, string> = {};
        if (contentType !== undefined) {
            requestHeaders["Content-Type"] = String(contentType);
        }
        const requestUrl = this.resolveUrl(requestPath);
        await this.sendWithTracingAsync<void>("Impexium.addActivityToOrganization", "Add-Activity-to-Organization", "POST", requestUrl, input, options, requestHeaders);
    }

    /**
     * Register an Individual for a Free Session
     * @remarks Register an Individual for a Free Session
     */
    public async registerAnIndividualForAFreeSession(input: RegisterAnIndividualForAFreeSessionInput, eventCode: string, customerIdOrRecordNumber: string, contentType: string, options: RegisterAnIndividualForAFreeSessionOptions = {}): Promise<void> {
        const queryParams: string[] = [];
        if (options.registrationNumber !== undefined) {
            queryParams.push(`registrationNumber=${encodeURIComponent(String(options.registrationNumber))}`);
        }
        const requestPath = `/api/v1/Events/${encodeURIComponent(String(eventCode))}/Sessions/Register/${encodeURIComponent(String(customerIdOrRecordNumber))}` + (queryParams.length > 0 ? "?" + queryParams.join("&") : "");
        const requestHeaders: Record<string, string> = {};
        if (contentType !== undefined) {
            requestHeaders["Content-Type"] = String(contentType);
        }
        const requestUrl = this.resolveUrl(requestPath);
        await this.sendWithTracingAsync<void>("Impexium.registerAnIndividualForAFreeSession", "Register-an-Individual-for-a-Free-Session", "POST", requestUrl, input, options, requestHeaders);
    }

    /**
     * Get a List of Licenses
     * @remarks Get a List of Licenses
     */
    public async getAListOfLicenses(id: string, pageNumber: number, contentType: string, options: ConnectorOperationOptions = {}): Promise<GetAListOfLicensesResponse> {
        const requestPath = `/api/v1/Individuals/${encodeURIComponent(String(id))}/Licenses/${encodeURIComponent(String(pageNumber))}`;
        const requestHeaders: Record<string, string> = {};
        if (contentType !== undefined) {
            requestHeaders["Content-Type"] = String(contentType);
        }
        const requestUrl = this.resolveUrl(requestPath);
        const httpResponse = await this.sendWithTracingAsync<GetAListOfLicensesResponse>("Impexium.getAListOfLicenses", "Get-a-List-of-Licenses", "GET", requestUrl, undefined, options, requestHeaders);

        return httpResponse.value as GetAListOfLicensesResponse;
    }

    /**
     * List All Awards
     * @remarks List All Awards
     */
    public listAllAwards(pageNumber: number, contentType: string, options: ListAllAwardsOptions = {}): ConnectorPagedAsyncIterableIterator<AwardData> {
        const queryParams: string[] = [];
        if (options.year !== undefined) {
            queryParams.push(`Year=${encodeURIComponent(String(options.year))}`);
        }
        const requestPath = `/api/v1/Awards/${encodeURIComponent(String(pageNumber))}` + (queryParams.length > 0 ? "?" + queryParams.join("&") : "");
        const requestHeaders: Record<string, string> = {};
        if (contentType !== undefined) {
            requestHeaders["Content-Type"] = String(contentType);
        }
        return this.createPageable<ListAllAwardsResponse, AwardData>(
            requestPath,
            async (requestUrl) => {
                const httpResponse = await this.sendWithTracingAsync<ListAllAwardsResponse>("Impexium.listAllAwards", "List-All-Awards", "GET", requestUrl, undefined, options, requestHeaders);

                return httpResponse.value as ListAllAwardsResponse;
            },
            "dataList",
            undefined,
        );
    }

    /**
     * Find Members (Individuals) by Name
     * @remarks Find Members (Individuals) by Name
     */
    public async findMembersOrIndividualsByName(name: string, pageNumber: number, contentType: string, options: FindMembersOrIndividualsByNameOptions = {}): Promise<FindMembersOrIndividualsByNameResponse> {
        const queryParams: string[] = [];
        if (options.includeEmail !== undefined) {
            queryParams.push(`includeEmail=${encodeURIComponent(String(options.includeEmail))}`);
        }
        const requestPath = `/api/v1/Individuals/Members/FindByName/${encodeURIComponent(String(name))}/${encodeURIComponent(String(pageNumber))}` + (queryParams.length > 0 ? "?" + queryParams.join("&") : "");
        const requestHeaders: Record<string, string> = {};
        if (contentType !== undefined) {
            requestHeaders["Content-Type"] = String(contentType);
        }
        const requestUrl = this.resolveUrl(requestPath);
        const httpResponse = await this.sendWithTracingAsync<FindMembersOrIndividualsByNameResponse>("Impexium.findMembersOrIndividualsByName", "Find-Members-or-Individuals-by-Name", "GET", requestUrl, undefined, options, requestHeaders);

        return httpResponse.value as FindMembersOrIndividualsByNameResponse;
    }

    /**
     * Get a List of All Services of an Organization
     * @remarks Get a List of All Services of an Organization
     */
    public async getAListOfAllServicesOfAnOrganization(id: string, contentType: string, options: ConnectorOperationOptions = {}): Promise<GetAListOfAllServicesOfAnOrganizationResponse> {
        const requestPath = `/api/v1/Organizations/${encodeURIComponent(String(id))}/Services`;
        const requestHeaders: Record<string, string> = {};
        if (contentType !== undefined) {
            requestHeaders["Content-Type"] = String(contentType);
        }
        const requestUrl = this.resolveUrl(requestPath);
        const httpResponse = await this.sendWithTracingAsync<GetAListOfAllServicesOfAnOrganizationResponse>("Impexium.getAListOfAllServicesOfAnOrganization", "Get-a-List-of-All-Services-of-an-Organization", "GET", requestUrl, undefined, options, requestHeaders);

        return httpResponse.value as GetAListOfAllServicesOfAnOrganizationResponse;
    }

    /**
     * Add a Service to an Organization
     * @remarks Add a Service to an Organization
     */
    public async addAServiceToAnOrganization(input: ServiceData, id: string, contentType: string, options: ConnectorOperationOptions = {}): Promise<ServiceData> {
        const requestPath = `/api/v1/Organizations/${encodeURIComponent(String(id))}/Services`;
        const requestHeaders: Record<string, string> = {};
        if (contentType !== undefined) {
            requestHeaders["Content-Type"] = String(contentType);
        }
        const requestUrl = this.resolveUrl(requestPath);
        const httpResponse = await this.sendWithTracingAsync<ServiceData>("Impexium.addAServiceToAnOrganization", "Add-a-Service-to-an-Organization", "POST", requestUrl, input, options, requestHeaders);

        return httpResponse.value as ServiceData;
    }

    /**
     * Update Phone for an Individual
     * @remarks Update Phone for an Individual
     */
    public async updatePhoneForAnIndividual(input: PhoneSaveData, idOrRecordNumber: string, id: string, contentType: string, options: ConnectorOperationOptions = {}): Promise<PhoneDataSet> {
        const requestPath = `/api/v1/Individuals/${encodeURIComponent(String(idOrRecordNumber))}/Phones/${encodeURIComponent(String(id))}`;
        const requestHeaders: Record<string, string> = {};
        if (contentType !== undefined) {
            requestHeaders["Content-Type"] = String(contentType);
        }
        const requestUrl = this.resolveUrl(requestPath);
        const httpResponse = await this.sendWithTracingAsync<PhoneDataSet>("Impexium.updatePhoneForAnIndividual", "Update-Phone-for-an-Individual", "PUT", requestUrl, input, options, requestHeaders);

        return httpResponse.value as PhoneDataSet;
    }

    /**
     * Update Phone for an Organization
     * @remarks Update Phone for an Organization
     */
    public async updatePhoneForAnOrganization(input: PhoneSaveData, idOrRecordNumber: string, id: string, contentType: string, options: ConnectorOperationOptions = {}): Promise<PhoneDataSet> {
        const requestPath = `/api/v1/Organizations/${encodeURIComponent(String(idOrRecordNumber))}/Phones/${encodeURIComponent(String(id))}`;
        const requestHeaders: Record<string, string> = {};
        if (contentType !== undefined) {
            requestHeaders["Content-Type"] = String(contentType);
        }
        const requestUrl = this.resolveUrl(requestPath);
        const httpResponse = await this.sendWithTracingAsync<PhoneDataSet>("Impexium.updatePhoneForAnOrganization", "Update-Phone-for-an-Organization", "PUT", requestUrl, input, options, requestHeaders);

        return httpResponse.value as PhoneDataSet;
    }

    /**
     * Delete an Individual Web Link
     * @remarks Delete an Individual Web Link
     */
    public async deleteAnIndividualWebLink(input: DeleteAnIndividualWebLinkInput, idOrRecordNumber: string, contentType: string, options: ConnectorOperationOptions = {}): Promise<void> {
        const requestPath = `/api/v1/Individuals/${encodeURIComponent(String(idOrRecordNumber))}/Links`;
        const requestHeaders: Record<string, string> = {};
        if (contentType !== undefined) {
            requestHeaders["Content-Type"] = String(contentType);
        }
        const requestUrl = this.resolveUrl(requestPath);
        await this.sendWithTracingAsync<void>("Impexium.deleteAnIndividualWebLink", "Delete-an-Individual-Web-Link", "DELETE", requestUrl, input, options, requestHeaders);
    }

    /**
     * Add Web Link for Individual
     * @remarks Add Web Link for Individual
     */
    public async addWebLinkForIndividual(input: AddWebLinkForIndividualInput, idOrRecordNumber: string, contentType: string, options: ConnectorOperationOptions = {}): Promise<void> {
        const requestPath = `/api/v1/Individuals/${encodeURIComponent(String(idOrRecordNumber))}/Links`;
        const requestHeaders: Record<string, string> = {};
        if (contentType !== undefined) {
            requestHeaders["Content-Type"] = String(contentType);
        }
        const requestUrl = this.resolveUrl(requestPath);
        await this.sendWithTracingAsync<void>("Impexium.addWebLinkForIndividual", "Add-Web-Link-for-Individual", "POST", requestUrl, input, options, requestHeaders);
    }

    /**
     * Add Phone to Organization
     * @remarks Add Phone to Organization
     */
    public async addPhoneToOrganization(input: PhoneSaveData, idOrRecordNumber: string, contentType: string, options: ConnectorOperationOptions = {}): Promise<PhoneDataSet> {
        const requestPath = `/api/v1/Organizations/${encodeURIComponent(String(idOrRecordNumber))}/Phones`;
        const requestHeaders: Record<string, string> = {};
        if (contentType !== undefined) {
            requestHeaders["Content-Type"] = String(contentType);
        }
        const requestUrl = this.resolveUrl(requestPath);
        const httpResponse = await this.sendWithTracingAsync<PhoneDataSet>("Impexium.addPhoneToOrganization", "Add-Phone-to-Organization", "POST", requestUrl, input, options, requestHeaders);

        return httpResponse.value as PhoneDataSet;
    }

    /**
     * Get Nominees by Committee
     * @remarks Get Nominees by Committee
     */
    public async getNomineesByCommittee(code: string, pageNumber: number, contentType: string, options: GetNomineesByCommitteeOptions = {}): Promise<GetNomineesByCommitteeResponse> {
        const queryParams: string[] = [];
        if (options.term !== undefined) {
            queryParams.push(`Term=${encodeURIComponent(String(options.term))}`);
        }
        const requestPath = `/api/v1/Committees/${encodeURIComponent(String(code))}/Nominations/${encodeURIComponent(String(pageNumber))}` + (queryParams.length > 0 ? "?" + queryParams.join("&") : "");
        const requestHeaders: Record<string, string> = {};
        if (contentType !== undefined) {
            requestHeaders["Content-Type"] = String(contentType);
        }
        const requestUrl = this.resolveUrl(requestPath);
        const httpResponse = await this.sendWithTracingAsync<GetNomineesByCommitteeResponse>("Impexium.getNomineesByCommittee", "Get-Nominees-by-Committee", "GET", requestUrl, undefined, options, requestHeaders);

        return httpResponse.value as GetNomineesByCommitteeResponse;
    }

    /**
     * Get Organization's Active Subscriptions
     * @remarks Get Organization's Active Subscriptions
     */
    public async getOrganizationsActiveSubscriptions(id: string, pageNumber: number, contentType: string, options: ConnectorOperationOptions = {}): Promise<GetOrganizationsActiveSubscriptionsResponse> {
        const requestPath = `/api/v1/Organizations/${encodeURIComponent(String(id))}/Subscriptions/${encodeURIComponent(String(pageNumber))}`;
        const requestHeaders: Record<string, string> = {};
        if (contentType !== undefined) {
            requestHeaders["Content-Type"] = String(contentType);
        }
        const requestUrl = this.resolveUrl(requestPath);
        const httpResponse = await this.sendWithTracingAsync<GetOrganizationsActiveSubscriptionsResponse>("Impexium.getOrganizationsActiveSubscriptions", "Get-Organizations-Active-Subscriptions", "GET", requestUrl, undefined, options, requestHeaders);

        return httpResponse.value as GetOrganizationsActiveSubscriptionsResponse;
    }

    /**
     * Delete an Organization Web Link
     * @remarks Delete an Organization Web Link
     */
    public async deleteAnOrganizationWebLink(input: DeleteAnOrganizationWebLinkInput, idOrRecordNumber: string, contentType: string, options: ConnectorOperationOptions = {}): Promise<void> {
        const requestPath = `/api/v1/Organizations/${encodeURIComponent(String(idOrRecordNumber))}/Links`;
        const requestHeaders: Record<string, string> = {};
        if (contentType !== undefined) {
            requestHeaders["Content-Type"] = String(contentType);
        }
        const requestUrl = this.resolveUrl(requestPath);
        await this.sendWithTracingAsync<void>("Impexium.deleteAnOrganizationWebLink", "Delete-an-Organization-Web-Link", "DELETE", requestUrl, input, options, requestHeaders);
    }

    /**
     * Add Web Link for Organization
     * @remarks Add Web Link for Organization
     */
    public async addWebLinkForOrganization(input: AddWebLinkForOrganizationInput, idOrRecordNumber: string, contentType: string, options: ConnectorOperationOptions = {}): Promise<void> {
        const requestPath = `/api/v1/Organizations/${encodeURIComponent(String(idOrRecordNumber))}/Links`;
        const requestHeaders: Record<string, string> = {};
        if (contentType !== undefined) {
            requestHeaders["Content-Type"] = String(contentType);
        }
        const requestUrl = this.resolveUrl(requestPath);
        await this.sendWithTracingAsync<void>("Impexium.addWebLinkForOrganization", "Add-Web-Link-for-Organization", "POST", requestUrl, input, options, requestHeaders);
    }

    /**
     * Add Email to Organization
     * @remarks Add Email to Organization
     */
    public async addEmailToOrganization(input: EmailData, id: string, contentType: string, options: ConnectorOperationOptions = {}): Promise<void> {
        const requestPath = `/api/v1/Organizations/${encodeURIComponent(String(id))}/Emails`;
        const requestHeaders: Record<string, string> = {};
        if (contentType !== undefined) {
            requestHeaders["Content-Type"] = String(contentType);
        }
        const requestUrl = this.resolveUrl(requestPath);
        await this.sendWithTracingAsync<void>("Impexium.addEmailToOrganization", "Add-Email-to-Organization", "POST", requestUrl, input, options, requestHeaders);
    }

    /**
     * Get Organization's Relationships
     * @remarks Get Organization's Relationships
     */
    public async getOrganizationsRelationships(id: string, pageNumber: number, contentType: string, options: GetOrganizationsRelationshipsOptions = {}): Promise<GetOrganizationsRelationshipsResponse> {
        const queryParams: string[] = [];
        if (options.relationshipName !== undefined) {
            queryParams.push(`relationshipName.=${encodeURIComponent(String(options.relationshipName))}`);
        }
        if (options.includesDetails !== undefined) {
            queryParams.push(`includesDetails=${encodeURIComponent(String(options.includesDetails))}`);
        }
        const requestPath = `/api/v1/Organizations/${encodeURIComponent(String(id))}/Relationships/${encodeURIComponent(String(pageNumber))}` + (queryParams.length > 0 ? "?" + queryParams.join("&") : "");
        const requestHeaders: Record<string, string> = {};
        if (contentType !== undefined) {
            requestHeaders["Content-Type"] = String(contentType);
        }
        const requestUrl = this.resolveUrl(requestPath);
        const httpResponse = await this.sendWithTracingAsync<GetOrganizationsRelationshipsResponse>("Impexium.getOrganizationsRelationships", "Get-Organizations-Relationships", "GET", requestUrl, undefined, options, requestHeaders);

        return httpResponse.value as GetOrganizationsRelationshipsResponse;
    }

    /**
     * Add or Update Address to Organization
     * @remarks Add or Update Address to Organization
     */
    public async addOrUpdateAddressToOrganization(input: AddressSaveData, idOrRecordNumber: string, contentType: string, options: ConnectorOperationOptions = {}): Promise<AddressSaveData> {
        const requestPath = `/api/v1/Organizations/${encodeURIComponent(String(idOrRecordNumber))}/Addresses`;
        const requestHeaders: Record<string, string> = {};
        if (contentType !== undefined) {
            requestHeaders["Content-Type"] = String(contentType);
        }
        const requestUrl = this.resolveUrl(requestPath);
        const httpResponse = await this.sendWithTracingAsync<AddressSaveData>("Impexium.addOrUpdateAddressToOrganization", "Add-or-Update-Address-to-Organization", "POST", requestUrl, input, options, requestHeaders);

        return httpResponse.value as AddressSaveData;
    }

    /**
     * Add or Update a List of Custom Fields Per Individual
     * @remarks Add or Update a List of Custom Fields Per Individual
     */
    public async addOrUpdateAListOfCustomFieldsPerIndividual(input: AddOrUpdateAListOfCustomFieldsPerIndividualInput, idOrRecordNumber: string, contentType: string, options: ConnectorOperationOptions = {}): Promise<Array<CustomFieldResultData>> {
        const requestPath = `/api/v1/Individuals/${encodeURIComponent(String(idOrRecordNumber))}/CustomFieldsList`;
        const requestHeaders: Record<string, string> = {};
        if (contentType !== undefined) {
            requestHeaders["Content-Type"] = String(contentType);
        }
        const requestUrl = this.resolveUrl(requestPath);
        const httpResponse = await this.sendWithTracingAsync<Array<CustomFieldResultData>>("Impexium.addOrUpdateAListOfCustomFieldsPerIndividual", "Add-or-Update-a-List-of-Custom-Fields-Per-Individual", "POST", requestUrl, input, options, requestHeaders);

        return httpResponse.value as Array<CustomFieldResultData>;
    }

    /**
     * Get Upcoming Events
     * @remarks Get Upcoming Events
     */
    public async getUpcomingEvents(pageNumber: number, contentType: string, options: ConnectorOperationOptions = {}): Promise<GetUpcomingEventsResponse> {
        const requestPath = `/api/v1/Events/Upcoming/${encodeURIComponent(String(pageNumber))}`;
        const requestHeaders: Record<string, string> = {};
        if (contentType !== undefined) {
            requestHeaders["Content-Type"] = String(contentType);
        }
        const requestUrl = this.resolveUrl(requestPath);
        const httpResponse = await this.sendWithTracingAsync<GetUpcomingEventsResponse>("Impexium.getUpcomingEvents", "Get-Upcoming-Events", "GET", requestUrl, undefined, options, requestHeaders);

        return httpResponse.value as GetUpcomingEventsResponse;
    }

    /**
     * Add Phone to Individual
     * @remarks Add Phone to Individual
     */
    public async addPhoneToIndividual(input: PhoneSaveData, idOrRecordNumber: string, contentType: string, options: ConnectorOperationOptions = {}): Promise<PhoneDataSet> {
        const requestPath = `/api/v1/Individuals/${encodeURIComponent(String(idOrRecordNumber))}/Phones`;
        const requestHeaders: Record<string, string> = {};
        if (contentType !== undefined) {
            requestHeaders["Content-Type"] = String(contentType);
        }
        const requestUrl = this.resolveUrl(requestPath);
        const httpResponse = await this.sendWithTracingAsync<PhoneDataSet>("Impexium.addPhoneToIndividual", "Add-Phone-to-Individual", "POST", requestUrl, input, options, requestHeaders);

        return httpResponse.value as PhoneDataSet;
    }

    /**
     * Get Committee Information for an Individual
     * @remarks Get Committee Information for an Individual
     */
    public async getCommitteeInformationForAnIndividual(id: string, pageNumber: number, contentType: string, options: GetCommitteeInformationForAnIndividualOptions = {}): Promise<GetCommitteeInformationForAnIndividualResponse> {
        const queryParams: string[] = [];
        if (options.includeInactive !== undefined) {
            queryParams.push(`includeInactive=${encodeURIComponent(String(options.includeInactive))}`);
        }
        const requestPath = `/api/v1/Individuals/${encodeURIComponent(String(id))}/Committees/${encodeURIComponent(String(pageNumber))}` + (queryParams.length > 0 ? "?" + queryParams.join("&") : "");
        const requestHeaders: Record<string, string> = {};
        if (contentType !== undefined) {
            requestHeaders["Content-Type"] = String(contentType);
        }
        const requestUrl = this.resolveUrl(requestPath);
        const httpResponse = await this.sendWithTracingAsync<GetCommitteeInformationForAnIndividualResponse>("Impexium.getCommitteeInformationForAnIndividual", "Get-Committee-Information-for-an-Individual", "GET", requestUrl, undefined, options, requestHeaders);

        return httpResponse.value as GetCommitteeInformationForAnIndividualResponse;
    }

    /**
     * Get Organization Custom Field Values
     * @remarks Get Organization Custom Field Values
     */
    public async getOrganizationCustomFieldValues(id: string, contentType: string, options: ConnectorOperationOptions = {}): Promise<Array<CustomFieldData>> {
        const requestPath = `/api/v1/Organizations/${encodeURIComponent(String(id))}/CustomFields`;
        const requestHeaders: Record<string, string> = {};
        if (contentType !== undefined) {
            requestHeaders["Content-Type"] = String(contentType);
        }
        const requestUrl = this.resolveUrl(requestPath);
        const httpResponse = await this.sendWithTracingAsync<Array<CustomFieldData>>("Impexium.getOrganizationCustomFieldValues", "Get-Organization-Custom-Field-Values", "GET", requestUrl, undefined, options, requestHeaders);

        return httpResponse.value as Array<CustomFieldData>;
    }

    /**
     * Get Organization Active Memberships
     * @remarks Get Organization Active Memberships
     */
    public async getOrganizationActiveMemberships(id: string, contentType: string, options: ConnectorOperationOptions = {}): Promise<Array<MembershipData>> {
        const requestPath = `/api/v1/Organizations/${encodeURIComponent(String(id))}/Memberships/Active`;
        const requestHeaders: Record<string, string> = {};
        if (contentType !== undefined) {
            requestHeaders["Content-Type"] = String(contentType);
        }
        const requestUrl = this.resolveUrl(requestPath);
        const httpResponse = await this.sendWithTracingAsync<Array<MembershipData>>("Impexium.getOrganizationActiveMemberships", "Get-Organization-Active-Memberships", "GET", requestUrl, undefined, options, requestHeaders);

        return httpResponse.value as Array<MembershipData>;
    }

    /**
     * Get All Events
     * @remarks Get All Events
     */
    public async getAllEvents(pageNumber: number, contentType: string, options: GetAllEventsOptions = {}): Promise<GetAllEventsResponse> {
        const queryParams: string[] = [];
        if (options.code !== undefined) {
            queryParams.push(`Code=${encodeURIComponent(String(options.code))}`);
        }
        if (options.name !== undefined) {
            queryParams.push(`Name=${encodeURIComponent(String(options.name))}`);
        }
        if (options.tag !== undefined) {
            queryParams.push(`Tag=${encodeURIComponent(String(options.tag))}`);
        }
        const requestPath = `/api/v1/Events/All/${encodeURIComponent(String(pageNumber))}` + (queryParams.length > 0 ? "?" + queryParams.join("&") : "");
        const requestHeaders: Record<string, string> = {};
        if (contentType !== undefined) {
            requestHeaders["Content-Type"] = String(contentType);
        }
        const requestUrl = this.resolveUrl(requestPath);
        const httpResponse = await this.sendWithTracingAsync<GetAllEventsResponse>("Impexium.getAllEvents", "Get-All-Events", "GET", requestUrl, undefined, options, requestHeaders);

        return httpResponse.value as GetAllEventsResponse;
    }

    /**
     * Get Individual Active Memberships
     * @remarks Get Individual Active Memberships
     */
    public async getIndividualActiveMemberships(id: string, contentType: string, options: ConnectorOperationOptions = {}): Promise<Array<MembershipData>> {
        const requestPath = `/api/v1/Individuals/${encodeURIComponent(String(id))}/Memberships/Active`;
        const requestHeaders: Record<string, string> = {};
        if (contentType !== undefined) {
            requestHeaders["Content-Type"] = String(contentType);
        }
        const requestUrl = this.resolveUrl(requestPath);
        const httpResponse = await this.sendWithTracingAsync<Array<MembershipData>>("Impexium.getIndividualActiveMemberships", "Get-Individual-Active-Memberships", "GET", requestUrl, undefined, options, requestHeaders);

        return httpResponse.value as Array<MembershipData>;
    }

    /**
     * Get All Event Registrations Information for an Individual
     * @remarks Get All Event Registrations Information for an Individual
     */
    public async getAllEventRegistrationsInformationForAnIndividual(idOrRecordNumber: string, pageNumber: number, contentType: string, options: GetAllEventRegistrationsInformationForAnIndividualOptions = {}): Promise<GetAllEventRegistrationsInformationForAnIndividualResponse> {
        const queryParams: string[] = [];
        if (options.eventCode !== undefined) {
            queryParams.push(`eventCode=${encodeURIComponent(String(options.eventCode))}`);
        }
        const requestPath = `/api/v1/Individuals/${encodeURIComponent(String(idOrRecordNumber))}/Registrations/${encodeURIComponent(String(pageNumber))}` + (queryParams.length > 0 ? "?" + queryParams.join("&") : "");
        const requestHeaders: Record<string, string> = {};
        if (contentType !== undefined) {
            requestHeaders["Content-Type"] = String(contentType);
        }
        const requestUrl = this.resolveUrl(requestPath);
        const httpResponse = await this.sendWithTracingAsync<GetAllEventRegistrationsInformationForAnIndividualResponse>("Impexium.getAllEventRegistrationsInformationForAnIndividual", "Get-All-Event-Registrations-Information-for-an-Individual", "GET", requestUrl, undefined, options, requestHeaders);

        return httpResponse.value as GetAllEventRegistrationsInformationForAnIndividualResponse;
    }

    /**
     * Get Individual's Relationships
     * @remarks Get Individual's Relationships
     */
    public async getIndividualsRelationships(id: string, pageNumber: number, contentType: string, options: GetIndividualsRelationshipsOptions = {}): Promise<GetIndividualsRelationshipsResponse> {
        const queryParams: string[] = [];
        if (options.relationshipName !== undefined) {
            queryParams.push(`relationshipName=${encodeURIComponent(String(options.relationshipName))}`);
        }
        if (options.includeDetails !== undefined) {
            queryParams.push(`includeDetails=${encodeURIComponent(String(options.includeDetails))}`);
        }
        const requestPath = `/api/v1/Individuals/${encodeURIComponent(String(id))}/Relationships/${encodeURIComponent(String(pageNumber))}` + (queryParams.length > 0 ? "?" + queryParams.join("&") : "");
        const requestHeaders: Record<string, string> = {};
        if (contentType !== undefined) {
            requestHeaders["Content-Type"] = String(contentType);
        }
        const requestUrl = this.resolveUrl(requestPath);
        const httpResponse = await this.sendWithTracingAsync<GetIndividualsRelationshipsResponse>("Impexium.getIndividualsRelationships", "Get-Individuals-Relationships", "GET", requestUrl, undefined, options, requestHeaders);

        return httpResponse.value as GetIndividualsRelationshipsResponse;
    }

    /**
     * Update an Individual Email
     * @remarks Update an Individual Email
     */
    public async updateAnIndividualEmail(input: UpdateAnIndividualEmailInput, idOrRecordNumber: string, currentEmailAddress: string, contentType: string, options: ConnectorOperationOptions = {}): Promise<void> {
        const requestPath = `/api/v1/Individuals/${encodeURIComponent(String(idOrRecordNumber))}/Emails/${encodeURIComponent(String(currentEmailAddress))}`;
        const requestHeaders: Record<string, string> = {};
        if (contentType !== undefined) {
            requestHeaders["Content-Type"] = String(contentType);
        }
        const requestUrl = this.resolveUrl(requestPath);
        await this.sendWithTracingAsync<void>("Impexium.updateAnIndividualEmail", "Update-an-Individual-Email", "PUT", requestUrl, input, options, requestHeaders);
    }

    /**
     * Save Relationship for Organization
     * @remarks Save Relationship for Organization
     */
    public async saveRelationshipForOrganization(input: SaveRelationshipForOrganizationInput, id: string, contentType: string, options: ConnectorOperationOptions = {}): Promise<void> {
        const requestPath = `/api/v1/Organizations/${encodeURIComponent(String(id))}/Relationships`;
        const requestHeaders: Record<string, string> = {};
        if (contentType !== undefined) {
            requestHeaders["Content-Type"] = String(contentType);
        }
        const requestUrl = this.resolveUrl(requestPath);
        await this.sendWithTracingAsync<void>("Impexium.saveRelationshipForOrganization", "Save-Relationship-for-Organization", "POST", requestUrl, input, options, requestHeaders);
    }

    /**
     * Add Individual
     * @remarks Adds an Individual.
     */
    public async addIndividual(input: AddIndividualInput, contentType: string, options: AddIndividualOptions = {}): Promise<IndividualData> {
        const queryParams: string[] = [];
        if (options.createUser !== undefined) {
            queryParams.push(`createUser=${encodeURIComponent(String(options.createUser))}`);
        }
        const requestPath = `/api/v1/Individuals` + (queryParams.length > 0 ? "?" + queryParams.join("&") : "");
        const requestHeaders: Record<string, string> = {};
        if (contentType !== undefined) {
            requestHeaders["Content-Type"] = String(contentType);
        }
        const requestUrl = this.resolveUrl(requestPath);
        const httpResponse = await this.sendWithTracingAsync<IndividualData>("Impexium.addIndividual", "Add-Individual", "POST", requestUrl, input, options, requestHeaders);

        return httpResponse.value as IndividualData;
    }

    /**
     * Add Email to Individual
     * @remarks Add Email to Individual
     */
    public async addEmailToIndividual(input: AddEmailToIndividualInput, idOrRecordNumber: string, contentType: string, options: ConnectorOperationOptions = {}): Promise<EmailData> {
        const requestPath = `/api/v1/Individuals/${encodeURIComponent(String(idOrRecordNumber))}/Emails`;
        const requestHeaders: Record<string, string> = {};
        if (contentType !== undefined) {
            requestHeaders["Content-Type"] = String(contentType);
        }
        const requestUrl = this.resolveUrl(requestPath);
        const httpResponse = await this.sendWithTracingAsync<EmailData>("Impexium.addEmailToIndividual", "Add-Email-to-Individual", "POST", requestUrl, input, options, requestHeaders);

        return httpResponse.value as EmailData;
    }

    /**
     * Add or Update Address to Individual
     * @remarks Add or Update Address to Individual
     */
    public async addOrUpdateAddressToIndividual(input: AddressSaveData, idOrRecordNumber: string, contentType: string, options: ConnectorOperationOptions = {}): Promise<AddressSaveData> {
        const requestPath = `/api/v1/Individuals/${encodeURIComponent(String(idOrRecordNumber))}/Addresses`;
        const requestHeaders: Record<string, string> = {};
        if (contentType !== undefined) {
            requestHeaders["Content-Type"] = String(contentType);
        }
        const requestUrl = this.resolveUrl(requestPath);
        const httpResponse = await this.sendWithTracingAsync<AddressSaveData>("Impexium.addOrUpdateAddressToIndividual", "Add-or-Update-Address-to-Individual", "POST", requestUrl, input, options, requestHeaders);

        return httpResponse.value as AddressSaveData;
    }

    /**
     * Find Members (Individuals) by First Name and Last Name
     * @remarks Find Members (Individuals) by First Name and Last Name
     */
    public async findMembersOrIndividualsByFirstAndLastName(firstName: string, lastName: string, pageNumber: number, contentType: string, options: FindMembersOrIndividualsByFirstAndLastNameOptions = {}): Promise<FindMembersOrIndividualsByFirstAndLastNameResponse> {
        const queryParams: string[] = [];
        if (options.includeEmail !== undefined) {
            queryParams.push(`includeEmail=${encodeURIComponent(String(options.includeEmail))}`);
        }
        const requestPath = `/api/v1/Individuals/Members/FindByName/${encodeURIComponent(String(firstName))}/${encodeURIComponent(String(lastName))}/${encodeURIComponent(String(pageNumber))}` + (queryParams.length > 0 ? "?" + queryParams.join("&") : "");
        const requestHeaders: Record<string, string> = {};
        if (contentType !== undefined) {
            requestHeaders["Content-Type"] = String(contentType);
        }
        const requestUrl = this.resolveUrl(requestPath);
        const httpResponse = await this.sendWithTracingAsync<FindMembersOrIndividualsByFirstAndLastNameResponse>("Impexium.findMembersOrIndividualsByFirstAndLastName", "Find-members-or-individuals-by-first-and-last-name", "GET", requestUrl, undefined, options, requestHeaders);

        return httpResponse.value as FindMembersOrIndividualsByFirstAndLastNameResponse;
    }

    /**
     * Get Committee Members by committee ID or code
     * @remarks Get Committee Members by Committee ID or Code
     */
    public async getCommitteeMembersByCommitteeIdOrCode(idOrCode: string, pageNumber: number, contentType: string, options: GetCommitteeMembersByCommitteeIdOrCodeOptions = {}): Promise<void> {
        const queryParams: string[] = [];
        if (options.term !== undefined) {
            queryParams.push(`Term=${encodeURIComponent(String(options.term))}`);
        }
        if (options.positionCodes !== undefined) {
            queryParams.push(`positionCodes=${encodeURIComponent(String(options.positionCodes))}`);
        }
        const requestPath = `/api/v1/Committees/${encodeURIComponent(String(idOrCode))}/Members/${encodeURIComponent(String(pageNumber))}` + (queryParams.length > 0 ? "?" + queryParams.join("&") : "");
        const requestHeaders: Record<string, string> = {};
        if (contentType !== undefined) {
            requestHeaders["Content-Type"] = String(contentType);
        }
        const requestUrl = this.resolveUrl(requestPath);
        await this.sendWithTracingAsync<void>("Impexium.getCommitteeMembersByCommitteeIdOrCode", "Get-Committee-Members-by-committee-ID-or-code", "GET", requestUrl, undefined, options, requestHeaders);
    }

    /**
     * Add Activity
     * @remarks Add Activity.
     */
    public async addActivity(input: AddActivityInput, id: string, contentType: string, options: ConnectorOperationOptions = {}): Promise<void> {
        const requestPath = `/api/v1/Individuals/${encodeURIComponent(String(id))}/Activities`;
        const requestHeaders: Record<string, string> = {};
        if (contentType !== undefined) {
            requestHeaders["Content-Type"] = String(contentType);
        }
        const requestUrl = this.resolveUrl(requestPath);
        await this.sendWithTracingAsync<void>("Impexium.addActivity", "Add-Activity", "POST", requestUrl, input, options, requestHeaders);
    }

    /**
     * Find Individual by Id or Email or Record Number
     * @remarks Find Individual by Id or Email.
     */
    public async findIndividualIdOrEmail(idOrRecordNumberOrEmail: string, pageNumber: number, includeDetails: boolean, contentType: string, options: ConnectorOperationOptions = {}): Promise<FindIndividualIdOrEmailResponse> {
        const queryParams: string[] = [];
        if (includeDetails !== undefined) {
            queryParams.push(`IncludeDetails=${encodeURIComponent(String(includeDetails))}`);
        }
        const requestPath = `/api/v1/Individuals/Profile/${encodeURIComponent(String(idOrRecordNumberOrEmail))}/${encodeURIComponent(String(pageNumber))}/` + (queryParams.length > 0 ? "?" + queryParams.join("&") : "");
        const requestHeaders: Record<string, string> = {};
        if (contentType !== undefined) {
            requestHeaders["Content-Type"] = String(contentType);
        }
        const requestUrl = this.resolveUrl(requestPath);
        const httpResponse = await this.sendWithTracingAsync<FindIndividualIdOrEmailResponse>("Impexium.findIndividualIdOrEmail", "Find-Individual-IDOrEmail", "GET", requestUrl, undefined, options, requestHeaders);

        return httpResponse.value as FindIndividualIdOrEmailResponse;
    }

    /**
     * Add Relationship to Individual
     * @remarks Add Relationship to Individual.
     */
    public async addRelationshipToIndividual(input: AddRelationshipToIndividualInput, id: string, contentType: string, options: ConnectorOperationOptions = {}): Promise<void> {
        const requestPath = `/api/v1/Individuals/${encodeURIComponent(String(id))}/Relationships`;
        const requestHeaders: Record<string, string> = {};
        if (contentType !== undefined) {
            requestHeaders["Content-Type"] = String(contentType);
        }
        const requestUrl = this.resolveUrl(requestPath);
        await this.sendWithTracingAsync<void>("Impexium.addRelationshipToIndividual", "Add-relationship-to-individual", "POST", requestUrl, input, options, requestHeaders);
    }

    /**
     * Add Education Credits to Individual
     * @remarks Add a new Education Credit to an Individual.
     */
    public async addIndividualEducationCredit(input: EducationCreditData, idOrRecordNumber: string, contentType: string, options: ConnectorOperationOptions = {}): Promise<void> {
        const requestPath = `/api/v1/Individuals/${encodeURIComponent(String(idOrRecordNumber))}/EducationCredits`;
        const requestHeaders: Record<string, string> = {};
        if (contentType !== undefined) {
            requestHeaders["Content-Type"] = String(contentType);
        }
        const requestUrl = this.resolveUrl(requestPath);
        await this.sendWithTracingAsync<void>("Impexium.addIndividualEducationCredit", "Individual-Add-Education-Credit", "POST", requestUrl, input, options, requestHeaders);
    }

    /**
     * Add Note to Individual
     * @remarks Add a Note to an Individual.
     */
    public async addIndividualNote(input: NoteData, id: string, contentType: string, options: ConnectorOperationOptions = {}): Promise<void> {
        const requestPath = `/api/v1/Individuals/${encodeURIComponent(String(id))}/Notes`;
        const requestHeaders: Record<string, string> = {};
        if (contentType !== undefined) {
            requestHeaders["Content-Type"] = String(contentType);
        }
        const requestUrl = this.resolveUrl(requestPath);
        await this.sendWithTracingAsync<void>("Impexium.addIndividualNote", "Individual-AddNote", "POST", requestUrl, input, options, requestHeaders);
    }

    /**
     * Lookup Individuals
     * @remarks Lookup Individuals by Name
     */
    public async individualsLookupByName(pageNumber: number, name: string, includeOrgAddresses: boolean, contentType: string, options: ConnectorOperationOptions = {}): Promise<IndividualsLookupByNameResponse> {
        const queryParams: string[] = [];
        if (name !== undefined) {
            queryParams.push(`name=${encodeURIComponent(String(name))}`);
        }
        if (includeOrgAddresses !== undefined) {
            queryParams.push(`includeOrgAddresses=${encodeURIComponent(String(includeOrgAddresses))}`);
        }
        const requestPath = `/api/v1/Individuals/Lookup/${encodeURIComponent(String(pageNumber))}` + (queryParams.length > 0 ? "?" + queryParams.join("&") : "");
        const requestHeaders: Record<string, string> = {};
        if (contentType !== undefined) {
            requestHeaders["Content-Type"] = String(contentType);
        }
        const requestUrl = this.resolveUrl(requestPath);
        const httpResponse = await this.sendWithTracingAsync<IndividualsLookupByNameResponse>("Impexium.individualsLookupByName", "Individuals-LookupByName", "GET", requestUrl, undefined, options, requestHeaders);

        return httpResponse.value as IndividualsLookupByNameResponse;
    }

    /**
     * Add Individual to Committee
     * @remarks Add Individual to Committee.
     */
    public async addToCommittee(input: CommitteeMemberCreateData, code: string, contentType: string, options: ConnectorOperationOptions = {}): Promise<AddToCommitteeResponse> {
        const requestPath = `/api/v1/Committees/${encodeURIComponent(String(code))}/Members`;
        const requestHeaders: Record<string, string> = {};
        if (contentType !== undefined) {
            requestHeaders["Content-Type"] = String(contentType);
        }
        const requestUrl = this.resolveUrl(requestPath);
        const httpResponse = await this.sendWithTracingAsync<AddToCommitteeResponse>("Impexium.addToCommittee", "Add-To-Committee", "POST", requestUrl, input, options, requestHeaders);

        return httpResponse.value as AddToCommitteeResponse;
    }

    /**
     * Update Committee Member
     * @remarks Update Committee Member.
     */
    public async updateCommitteeMember(input: CommitteeMemberUpdateData, code: string, memberRecordNumber: string, currentPositionCode: string, contentType: string, options: ConnectorOperationOptions = {}): Promise<void> {
        const requestPath = `/api/v1/Committees/${encodeURIComponent(String(code))}/Members/${encodeURIComponent(String(memberRecordNumber))}/${encodeURIComponent(String(currentPositionCode))}`;
        const requestHeaders: Record<string, string> = {};
        if (contentType !== undefined) {
            requestHeaders["Content-Type"] = String(contentType);
        }
        const requestUrl = this.resolveUrl(requestPath);
        await this.sendWithTracingAsync<void>("Impexium.updateCommitteeMember", "Update-Committee-Member", "PUT", requestUrl, input, options, requestHeaders);
    }

    /**
     * Add Organization
     * @remarks Add Organization.
     */
    public async addOrganization(input: AddOrganizationInput, contentType: string, options: ConnectorOperationOptions = {}): Promise<OrganizationData> {
        const requestPath = `/api/v1/Organizations`;
        const requestHeaders: Record<string, string> = {};
        if (contentType !== undefined) {
            requestHeaders["Content-Type"] = String(contentType);
        }
        const requestUrl = this.resolveUrl(requestPath);
        const httpResponse = await this.sendWithTracingAsync<OrganizationData>("Impexium.addOrganization", "Add-Organization", "POST", requestUrl, input, options, requestHeaders);

        return httpResponse.value as OrganizationData;
    }

    /**
     * Update Organization
     * @remarks Update Organization.
     */
    public async updateOrganization(input: UpdateOrganizationInput, id: string, contentType: string, options: ConnectorOperationOptions = {}): Promise<OrganizationData> {
        const requestPath = `/api/v1/Organizations/${encodeURIComponent(String(id))}`;
        const requestHeaders: Record<string, string> = {};
        if (contentType !== undefined) {
            requestHeaders["Content-Type"] = String(contentType);
        }
        const requestUrl = this.resolveUrl(requestPath);
        const httpResponse = await this.sendWithTracingAsync<OrganizationData>("Impexium.updateOrganization", "Update-Organization", "PUT", requestUrl, input, options, requestHeaders);

        return httpResponse.value as OrganizationData;
    }

    /**
     * Find Organization by Id or Record Number
     * @remarks Get organization's profile information by ID or Record Number
     */
    public async getOrganizationProfile(idOrRecordnumber: string, pageNumber: number, includeDescription: boolean, contentType: string, options: ConnectorOperationOptions = {}): Promise<OrganizationData> {
        const queryParams: string[] = [];
        if (includeDescription !== undefined) {
            queryParams.push(`includeDescription=${encodeURIComponent(String(includeDescription))}`);
        }
        const requestPath = `/api/v1/Organizations/Profile/${encodeURIComponent(String(idOrRecordnumber))}/${encodeURIComponent(String(pageNumber))}` + (queryParams.length > 0 ? "?" + queryParams.join("&") : "");
        const requestHeaders: Record<string, string> = {};
        if (contentType !== undefined) {
            requestHeaders["Content-Type"] = String(contentType);
        }
        const requestUrl = this.resolveUrl(requestPath);
        const httpResponse = await this.sendWithTracingAsync<OrganizationData>("Impexium.getOrganizationProfile", "Organization-Get-Profile", "GET", requestUrl, undefined, options, requestHeaders);

        return httpResponse.value as OrganizationData;
    }

    /**
     * Add Note to Organization
     * @remarks Add a Note to an Organization.
     */
    public async addOrganizationNote(input: NoteData, id: string, contentType: string, options: ConnectorOperationOptions = {}): Promise<void> {
        const requestPath = `/api/v1/Organizations/${encodeURIComponent(String(id))}/Notes`;
        const requestHeaders: Record<string, string> = {};
        if (contentType !== undefined) {
            requestHeaders["Content-Type"] = String(contentType);
        }
        const requestUrl = this.resolveUrl(requestPath);
        await this.sendWithTracingAsync<void>("Impexium.addOrganizationNote", "Organization-AddNote", "POST", requestUrl, input, options, requestHeaders);
    }

    /**
     * Lookup Organizations
     * @remarks Lookup Organizations by Name or Acronym
     */
    public async organizationLookupByName(pageNumber: number, name: string, includeAddresses: boolean, contentType: string, options: ConnectorOperationOptions = {}): Promise<OrganizationLookupByNameResponse> {
        const queryParams: string[] = [];
        if (name !== undefined) {
            queryParams.push(`name=${encodeURIComponent(String(name))}`);
        }
        if (includeAddresses !== undefined) {
            queryParams.push(`includeAddresses=${encodeURIComponent(String(includeAddresses))}`);
        }
        const requestPath = `/api/v1/Organizations/Lookup/${encodeURIComponent(String(pageNumber))}` + (queryParams.length > 0 ? "?" + queryParams.join("&") : "");
        const requestHeaders: Record<string, string> = {};
        if (contentType !== undefined) {
            requestHeaders["Content-Type"] = String(contentType);
        }
        const requestUrl = this.resolveUrl(requestPath);
        const httpResponse = await this.sendWithTracingAsync<OrganizationLookupByNameResponse>("Impexium.organizationLookupByName", "Organization-LookupByName", "GET", requestUrl, undefined, options, requestHeaders);

        return httpResponse.value as OrganizationLookupByNameResponse;
    }

    /**
     * Get All Committees
     * @remarks Get All Committees
     */
    public async getAllCommittees(pageNumber: number, contentType: string, options: GetAllCommitteesOptions = {}): Promise<GetAllCommitteesResponse> {
        const queryParams: string[] = [];
        if (options.code !== undefined) {
            queryParams.push(`Code=${encodeURIComponent(String(options.code))}`);
        }
        if (options.name !== undefined) {
            queryParams.push(`Name=${encodeURIComponent(String(options.name))}`);
        }
        if (options.term !== undefined) {
            queryParams.push(`Term=${encodeURIComponent(String(options.term))}`);
        }
        if (options.activeOnly !== undefined) {
            queryParams.push(`activeOnly=${encodeURIComponent(String(options.activeOnly))}`);
        }
        const requestPath = `/api/v1/Committees/${encodeURIComponent(String(pageNumber))}` + (queryParams.length > 0 ? "?" + queryParams.join("&") : "");
        const requestHeaders: Record<string, string> = {};
        if (contentType !== undefined) {
            requestHeaders["Content-Type"] = String(contentType);
        }
        const requestUrl = this.resolveUrl(requestPath);
        const httpResponse = await this.sendWithTracingAsync<GetAllCommitteesResponse>("Impexium.getAllCommittees", "Get-All-Committees", "GET", requestUrl, undefined, options, requestHeaders);

        return httpResponse.value as GetAllCommitteesResponse;
    }

    /**
     * Get Positions by Committee
     * @remarks Get Positions by Committee
     */
    public async getPositionsByCommittee(code: string, contentType: string, options: ConnectorOperationOptions = {}): Promise<GetPositionsByCommitteeResponse> {
        const requestPath = `/api/v1/Committees/${encodeURIComponent(String(code))}/Positions`;
        const requestHeaders: Record<string, string> = {};
        if (contentType !== undefined) {
            requestHeaders["Content-Type"] = String(contentType);
        }
        const requestUrl = this.resolveUrl(requestPath);
        const httpResponse = await this.sendWithTracingAsync<GetPositionsByCommitteeResponse>("Impexium.getPositionsByCommittee", "Get-Positions-by-Committee", "GET", requestUrl, undefined, options, requestHeaders);

        return httpResponse.value as GetPositionsByCommitteeResponse;
    }

    /**
     * Get Sub-Committees
     * @remarks Get Sub-Committees
     */
    public async getSubCommittees(code: string, pageNumber: number, contentType: string, options: ConnectorOperationOptions = {}): Promise<GetSubCommitteesResponse> {
        const requestPath = `/api/v1/Committees/${encodeURIComponent(String(code))}/subcommittees/${encodeURIComponent(String(pageNumber))}`;
        const requestHeaders: Record<string, string> = {};
        if (contentType !== undefined) {
            requestHeaders["Content-Type"] = String(contentType);
        }
        const requestUrl = this.resolveUrl(requestPath);
        const httpResponse = await this.sendWithTracingAsync<GetSubCommitteesResponse>("Impexium.getSubCommittees", "Get-Sub-Committees", "GET", requestUrl, undefined, options, requestHeaders);

        return httpResponse.value as GetSubCommitteesResponse;
    }

    /**
     * Get Individual's Active Subscriptions
     * @remarks Get Individual's Active Subscriptions
     */
    public async getIndividualsActiveSubscriptions(id: string, pageNumber: number, contentType: string, options: ConnectorOperationOptions = {}): Promise<GetIndividualsActiveSubscriptionsResponse> {
        const requestPath = `/api/v1/Individuals/${encodeURIComponent(String(id))}/Subscriptions/All/${encodeURIComponent(String(pageNumber))}`;
        const requestHeaders: Record<string, string> = {};
        if (contentType !== undefined) {
            requestHeaders["Content-Type"] = String(contentType);
        }
        const requestUrl = this.resolveUrl(requestPath);
        const httpResponse = await this.sendWithTracingAsync<GetIndividualsActiveSubscriptionsResponse>("Impexium.getIndividualsActiveSubscriptions", "Get-Individuals-Active-Subscriptions", "GET", requestUrl, undefined, options, requestHeaders);

        return httpResponse.value as GetIndividualsActiveSubscriptionsResponse;
    }

    /**
     * List all Individuals
     * @remarks List all Individuals
     */
    public listAllIndividuals(pageNumber: number, contentType: string, options: ListAllIndividualsOptions = {}): ConnectorPagedAsyncIterableIterator<IndividualData> {
        const queryParams: string[] = [];
        if (options.name !== undefined) {
            queryParams.push(`Name=${encodeURIComponent(String(options.name))}`);
        }
        if (options.includeDetails !== undefined) {
            queryParams.push(`includeDetails=${encodeURIComponent(String(options.includeDetails))}`);
        }
        if (options.oldId !== undefined) {
            queryParams.push(`oldID=${encodeURIComponent(String(options.oldId))}`);
        }
        const requestPath = `/api/v1/Individuals/${encodeURIComponent(String(pageNumber))}` + (queryParams.length > 0 ? "?" + queryParams.join("&") : "");
        const requestHeaders: Record<string, string> = {};
        if (contentType !== undefined) {
            requestHeaders["Content-Type"] = String(contentType);
        }
        return this.createPageable<ListAllIndividualsResponse, IndividualData>(
            requestPath,
            async (requestUrl) => {
                const httpResponse = await this.sendWithTracingAsync<ListAllIndividualsResponse>("Impexium.listAllIndividuals", "List-all-Individuals", "GET", requestUrl, undefined, options, requestHeaders);

                return httpResponse.value as ListAllIndividualsResponse;
            },
            "dataList",
            undefined,
        );
    }

    /**
     * Find Customer by Phone Number
     * @remarks Find Customer by Phone Number.
     */
    public async findCustomerPhone(pageNumber: string, phoneNumber: string, includeAddress: boolean, includePhone: boolean, includeEmail: boolean, contentType: string, options: ConnectorOperationOptions = {}): Promise<FindCustomerPhoneResponse> {
        const queryParams: string[] = [];
        if (phoneNumber !== undefined) {
            queryParams.push(`phoneNumber=${encodeURIComponent(String(phoneNumber))}`);
        }
        if (includeAddress !== undefined) {
            queryParams.push(`includeAddress=${encodeURIComponent(String(includeAddress))}`);
        }
        if (includePhone !== undefined) {
            queryParams.push(`includePhone=${encodeURIComponent(String(includePhone))}`);
        }
        if (includeEmail !== undefined) {
            queryParams.push(`includeEmail=${encodeURIComponent(String(includeEmail))}`);
        }
        const requestPath = `/api/v1/Customers/${encodeURIComponent(String(pageNumber))}` + (queryParams.length > 0 ? "?" + queryParams.join("&") : "");
        const requestHeaders: Record<string, string> = {};
        if (contentType !== undefined) {
            requestHeaders["Content-Type"] = String(contentType);
        }
        const requestUrl = this.resolveUrl(requestPath);
        const httpResponse = await this.sendWithTracingAsync<FindCustomerPhoneResponse>("Impexium.findCustomerPhone", "Find-Customer-Phone", "GET", requestUrl, undefined, options, requestHeaders);

        return httpResponse.value as FindCustomerPhoneResponse;
    }

    /**
     * Mark Registrant as Attended
     * @remarks Mark the Registrant as Attended for the specified Events.
     */
    public async markRegistrantAttended(input: MarkRegistrantAttendedInput, recordNumber: string, contentType: string, options: ConnectorOperationOptions = {}): Promise<void> {
        const requestPath = `/api/v1/Events/Registrants/${encodeURIComponent(String(recordNumber))}/Attended`;
        const requestHeaders: Record<string, string> = {};
        if (contentType !== undefined) {
            requestHeaders["Content-Type"] = String(contentType);
        }
        const requestUrl = this.resolveUrl(requestPath);
        await this.sendWithTracingAsync<void>("Impexium.markRegistrantAttended", "Mark-Registrant-Attended", "PUT", requestUrl, input, options, requestHeaders);
    }

    /**
     * Add Award Nomination
     * @remarks Add a new Award Nomination.
     */
    public async addAwardsAwardNomination(input: AwardNominationData, id: string, contentType: string, options: ConnectorOperationOptions = {}): Promise<void> {
        const requestPath = `/api/v1/Awards/${encodeURIComponent(String(id))}/Nominations`;
        const requestHeaders: Record<string, string> = {};
        if (contentType !== undefined) {
            requestHeaders["Content-Type"] = String(contentType);
        }
        const requestUrl = this.resolveUrl(requestPath);
        await this.sendWithTracingAsync<void>("Impexium.addAwardsAwardNomination", "Awards-AddAwardNomination", "POST", requestUrl, input, options, requestHeaders);
    }

    /**
     * Update Award Nomination
     * @remarks Update an existing Award Nomination.
     */
    public async updateAwardAwardNomination(input: UpdateAwardNominationData, id: string, nomineeRecordNumber: string, contentType: string, options: ConnectorOperationOptions = {}): Promise<void> {
        const requestPath = `/api/v1/Awards/${encodeURIComponent(String(id))}/Nominations/${encodeURIComponent(String(nomineeRecordNumber))}`;
        const requestHeaders: Record<string, string> = {};
        if (contentType !== undefined) {
            requestHeaders["Content-Type"] = String(contentType);
        }
        const requestUrl = this.resolveUrl(requestPath);
        await this.sendWithTracingAsync<void>("Impexium.updateAwardAwardNomination", "Awards-UpdateAwardNomination", "PUT", requestUrl, input, options, requestHeaders);
    }

    /**
     * List Award Individual Recipients
     * @remarks List all Individuals that are Award Recipients.
     */
    public getAwardIndividualAwardRecipients(pageNumber: number, id: string, includeDetails: boolean, contentType: string, options: ConnectorOperationOptions = {}): ConnectorPagedAsyncIterableIterator<AwardRecipientIndividualData> {
        const queryParams: string[] = [];
        if (includeDetails !== undefined) {
            queryParams.push(`includeDetails=${encodeURIComponent(String(includeDetails))}`);
        }
        const requestPath = `/api/v1/Awards/${encodeURIComponent(String(id))}/Recipients/Individuals/${encodeURIComponent(String(pageNumber))}` + (queryParams.length > 0 ? "?" + queryParams.join("&") : "");
        const requestHeaders: Record<string, string> = {};
        if (contentType !== undefined) {
            requestHeaders["Content-Type"] = String(contentType);
        }
        return this.createPageable<AwardsGetIndividualAwardRecipientsResponse, AwardRecipientIndividualData>(
            requestPath,
            async (requestUrl) => {
                const httpResponse = await this.sendWithTracingAsync<AwardsGetIndividualAwardRecipientsResponse>("Impexium.getAwardIndividualAwardRecipients", "Awards-GetIndividualAwardRecipients", "GET", requestUrl, undefined, options, requestHeaders);

                return httpResponse.value as AwardsGetIndividualAwardRecipientsResponse;
            },
            "dataList",
            undefined,
        );
    }

    /**
     * List Award Organization Recipients
     * @remarks List all Organizations that are Award Recipients.
     */
    public getAwardOrganizationAwardRecipients(pageNumber: number, id: string, includeDetails: boolean, contentType: string, options: ConnectorOperationOptions = {}): ConnectorPagedAsyncIterableIterator<AwardRecipientOrganizationData> {
        const queryParams: string[] = [];
        if (includeDetails !== undefined) {
            queryParams.push(`includeDetails=${encodeURIComponent(String(includeDetails))}`);
        }
        const requestPath = `/api/v1/Awards/${encodeURIComponent(String(id))}/Recipients/Organizations/${encodeURIComponent(String(pageNumber))}` + (queryParams.length > 0 ? "?" + queryParams.join("&") : "");
        const requestHeaders: Record<string, string> = {};
        if (contentType !== undefined) {
            requestHeaders["Content-Type"] = String(contentType);
        }
        return this.createPageable<AwardsGetOrganizationAwardRecipientsResponse, AwardRecipientOrganizationData>(
            requestPath,
            async (requestUrl) => {
                const httpResponse = await this.sendWithTracingAsync<AwardsGetOrganizationAwardRecipientsResponse>("Impexium.getAwardOrganizationAwardRecipients", "Awards-GetOrganizationAwardRecipients", "GET", requestUrl, undefined, options, requestHeaders);

                return httpResponse.value as AwardsGetOrganizationAwardRecipientsResponse;
            },
            "dataList",
            undefined,
        );
    }

}

// #endregion Client
