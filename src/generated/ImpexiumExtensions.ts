// ImpexiumExtensions.ts - Auto-generated DirectClient SDK for TypeScript
// Do not edit this file directly.

import { ConnectorClientBase } from "../azureConnectors/clientBase.ts";
import { ConnectorException } from "../azureConnectors/connectorException.ts";
import { ConnectorClientOptions } from "../azureConnectors/options.ts";
import { TokenProvider } from "../azureConnectors/authentication.ts";

// #region Types

/**
 * Response for Get Abandoned Checkouts
 */
export interface GetAbandonedCheckoutsResponse {
    /** Page Number */
    pageNumber?: number;
    /** dataList */
    dataList?: Array<AbandonedCheckoutData>;
}

/**
 * Response for List All Exhibitors
 */
export interface ListAllExhibitorsResponse {
    /** Page Number */
    pageNumber?: number;
    /** dataList */
    dataList?: Array<ExhibitorData>;
}

/**
 * Response for List of Exams
 */
export interface ListOfExamsResponse {
    /** Page Number */
    pageNumber?: number;
    /** dataList */
    dataList?: Array<ExamData>;
}

/**
 * Response for List Registrants
 */
export interface ListRegistrantsResponse {
    /** Page Number. */
    pageNumber?: number;
    /** dataList */
    dataList?: Array<RegistrantData>;
}

/**
 * Response for Get Course Attendees
 */
export interface GetCourseAttendeesResponse {
    /** Page Number. */
    pageNumber?: number;
    /** dataList */
    dataList?: Array<CourseAttendeeData>;
}

/**
 * Add Exam Scores
 */
export interface AddExamScoresInput {
    [key: string]: unknown;
}

/**
 * Response for Find Members by Name
 */
export interface FindMembersByNameResponse {
    /** Page Number. */
    pageNumber?: number;
    /** dataList */
    dataList?: Array<ContactData>;
}

/**
 * Response for Get Purchases for an Individual
 */
export interface GetPurchasesForAnIndividualResponse {
    /** Page Number. */
    pageNumber?: number;
    /** dataList */
    dataList?: Array<PurchasedItemData>;
}

/**
 * Add or Update a List of Custom Fields Per Organization
 */
export interface AddOrUpdateAListOfCustomFieldsPerOrganizationInput {
    [key: string]: unknown;
}

/**
 * Response for List All Event Cancellations by Event
 */
export interface ListAllEventCancellationsByEventResponse {
    /** Page Number. */
    pageNumber?: number;
    /** dataList */
    dataList?: Array<RegistrantCancellationData>;
}

/**
 * Response for Get All Open Orders for an Individual
 */
export interface GetAllOpenOrdersForAnIndividualResponse {
    /** Page Number */
    pageNumber?: number;
    /** dataList */
    dataList?: Array<PayableOrderData>;
}

/**
 * Response for List Completed User Tasks by User ID or Email
 */
export interface ListCompletedUserTasksByUserIdOrEmailResponse {
    /** Page Number. */
    pageNumber?: number;
    /** dataList */
    dataList?: Array<UserTaskData>;
}

/**
 * Response for List Pending User Tasks by User ID or Email
 */
export interface ListPendingUserTasksByUserIdOrEmailResponse {
    /** Page Number. */
    pageNumber?: number;
    /** dataList */
    dataList?: Array<UserTaskData>;
}

/**
 * Response for List All Countries
 */
export interface ListAllCountriesResponse {
    /** Page Number */
    pageNumber?: number;
    /** dataList */
    dataList?: Array<CountryData>;
}

/**
 * Response for Get All States by Country
 */
export interface GetAllStatesByCountryResponse {
    /** Page Number */
    pageNumber?: number;
    /** dataList */
    dataList?: Array<StateProvinceData>;
}

/**
 * Response for List All Exhibits
 */
export interface ListAllExhibitsResponse {
    /** Page Number */
    pageNumber?: number;
    /** dataList */
    dataList?: Array<ExhibitData>;
}

/**
 * Add Categories for an Organization
 */
export interface AddCategoriesForAnOrganizationInput {
    [key: string]: unknown;
}

/**
 * Response for List of Customer Relationships
 */
export interface ListOfCustomerRelationshipsResponse {
    /** Page Number. */
    pageNumber?: number;
    /** dataList */
    dataList?: Array<RelationshipTypeData>;
}

/**
 * Response for List All Open Customer Request
 */
export interface ListAllOpenCustomerRequestResponse {
    /** Page Number. */
    pageNumber?: number;
    /** dataList */
    dataList?: Array<RequestData>;
}

/**
 * Add Categories for an Individual
 */
export interface AddCategoriesForAnIndividualInput {
    [key: string]: unknown;
}

/**
 * Response for List of All Organization Members
 */
export interface ListOfAllOrganizationMembersResponse {
    /** Page Number. */
    pageNumber?: number;
    /** dataList */
    dataList?: Array<OrganizationData>;
}

/**
 * Response for List of All Individual Members
 */
export interface ListOfAllIndividualMembersResponse {
    /** Page Number. */
    pageNumber?: number;
    /** dataList */
    dataList?: Array<IndividualData>;
}

/**
 * Response for Get List of Active Certifications for an Organization
 */
export interface GetListOfActiveCertificationsForAnOrganizationResponse {
    /** Page Number. */
    pageNumber?: number;
    /** dataList */
    dataList?: Array<CertificationData>;
}

/**
 * Response for Get List of Active Certifications for an Individual
 */
export interface GetListOfActiveCertificationsForAnIndividualResponse {
    /** Page Number. */
    pageNumber?: number;
    /** dataList */
    dataList?: Array<CertificationData>;
}

/**
 * Register an Individual for a Free Session
 */
export interface RegisterAnIndividualForAFreeSessionInput {
    [key: string]: unknown;
}

/**
 * Response for Get a List of Licenses
 */
export interface GetAListOfLicensesResponse {
    /** Page Number. */
    pageNumber?: number;
    /** dataList */
    dataList?: Array<LicenseData>;
}

/**
 * Response for List All Awards
 */
export interface ListAllAwardsResponse {
    /** Page Number. */
    pageNumber?: number;
    /** dataList */
    dataList?: Array<AwardData>;
}

/**
 * Response for Find Members (Individuals) by Name
 */
export interface FindMembersOrIndividualsByNameResponse {
    /** Page Number. */
    pageNumber?: number;
    /** dataList */
    dataList?: Array<IndividualData>;
}

/**
 * Response for Get a List of All Services of an Organization
 */
export interface GetAListOfAllServicesOfAnOrganizationResponse {
    /** Page Number. */
    pageNumber?: number;
    /** dataList */
    dataList?: Array<ServiceData>;
}

/**
 * Delete an Individual Web Link
 */
export interface DeleteAnIndividualWebLinkInput {
    /** Type. */
    type?: string;
    /** Url. */
    url?: string;
}

/**
 * Add Web Link for Individual
 */
export interface AddWebLinkForIndividualInput {
    /** Type. */
    type?: string;
    /** Url. */
    url?: string;
}

/**
 * Response for Get Nominees by Committee
 */
export interface GetNomineesByCommitteeResponse {
    /** Page Number. */
    pageNumber?: number;
    /** Data List */
    dataList?: Array<CommitteeNomineeData>;
}

/**
 * Response for Get Organization's Active Subscriptions
 */
export interface GetOrganizationsActiveSubscriptionsResponse {
    /** Page Number. */
    pageNumber?: number;
    /** dataList */
    dataList?: Array<SubscriptionData>;
}

/**
 * Delete an Organization Web Link
 */
export interface DeleteAnOrganizationWebLinkInput {
    /** Type. */
    type: string;
    /** Url. */
    url: string;
}

/**
 * Add Web Link for Organization
 */
export interface AddWebLinkForOrganizationInput {
    /** Type. */
    type: string;
    /** Url. */
    url: string;
}

/**
 * Response for Get Organization's Relationships
 */
export interface GetOrganizationsRelationshipsResponse {
    /** Page Number. */
    pageNumber?: number;
    /** DataList */
    dataList?: Array<RelationshipData>;
}

/**
 * Add or Update a List of Custom Fields Per Individual
 */
export interface AddOrUpdateAListOfCustomFieldsPerIndividualInput {
    [key: string]: unknown;
}

/**
 * Response for Get Upcoming Events
 */
export interface GetUpcomingEventsResponse {
    /** Page Number. */
    pageNumber?: number;
    /** DataList */
    dataList?: Array<EventData>;
}

/**
 * Response for Get Committee Information for an Individual
 */
export interface GetCommitteeInformationForAnIndividualResponse {
    /** Page Number */
    pageNumber?: number;
    /** Data List. */
    dataList?: Array<CommitteeMemberData>;
}

/**
 * Response for Get All Events
 */
export interface GetAllEventsResponse {
    /** Page Number. */
    pageNumber?: number;
    /** DataList */
    dataList?: Array<EventData>;
}

/**
 * Response for Get All Event Registrations Information for an Individual
 */
export interface GetAllEventRegistrationsInformationForAnIndividualResponse {
    /** Page Number. */
    pageNumber?: number;
    /** dataList */
    dataList?: Array<RegistrationData>;
}

/**
 * Response for Get Individual's Relationships
 */
export interface GetIndividualsRelationshipsResponse {
    /** Page Number. */
    pageNumber?: number;
    /** Data List. */
    dataList?: Array<RelationshipData>;
}

/**
 * Update an Individual Email
 */
export interface UpdateAnIndividualEmailInput {
    /** Primary. */
    primary?: boolean;
    /** Address. */
    address?: string;
    /** Type. */
    type?: string;
    /** Show In Directory */
    showInDirectory?: boolean;
}

/**
 * Save Relationship for Organization
 */
export interface SaveRelationshipForOrganizationInput {
    /** Relationship Name. */
    relationshipName: string;
    /** Related To Customer Record Number. */
    relatedToCustomerRecordNumber: string;
    /** Reciprocal Relationship Name. */
    reciprocalRelationshipName: string;
    /** Is Primary. */
    isPrimary?: boolean;
    /** Start Date. */
    startDate?: string;
    /** End Date. */
    endDate?: string;
    /** Is Reciprocal Primary. */
    isReciprocalPrimary?: boolean;
    /** Title. */
    title?: string;
}

/**
 * Add Individual
 */
export interface AddIndividualInput {
    /** First Name. */
    firstName: string;
    /** Last Name. */
    lastName: string;
    /** Email Address. */
    email?: string;
    /** Record Number. */
    recordNumber?: string;
}

/**
 * Add Email to Individual
 */
export interface AddEmailToIndividualInput {
    /** Primary */
    primary?: boolean;
    /** Address. */
    address?: string;
    /** Type. */
    type?: string;
    /** Show In Directory */
    showInDirectory?: boolean;
}

/**
 * Response for Find Members (Individuals) by First Name and Last Name
 */
export interface FindMembersOrIndividualsByFirstAndLastNameResponse {
    /** Page Number. */
    pageNumber?: number;
    /** dataList */
    dataList?: Array<IndividualData>;
}

/**
 * Add Activity
 */
export interface AddActivityInput {
    /** Category. */
    category: string;
    /** Text. */
    text: string;
    /** URL. */
    url?: string;
    /** Activity Date. */
    activityDate?: string;
}

/**
 * Response for Find Individual by Id or Email or Record Number
 */
export interface FindIndividualIdOrEmailResponse {
    /** Page Number. */
    pageNumber?: number;
    /** Data List. */
    dataList?: Array<IndividualData>;
}

/**
 * Add Relationship to Individual
 */
export interface AddRelationshipToIndividualInput {
    /** Relationship Name. */
    relationshipName: string;
    /** Related To Customer Record Number. */
    relatedToCustomerRecordNumber: string;
    /** Reciprocal Relationship Name. */
    reciprocalRelationshipName: string;
    /** Is Primary. */
    isPrimary?: boolean;
    /** Is Reciprocal Primary. */
    isReciprocalPrimary?: boolean;
    /** Start Date. */
    startDate?: string;
    /** End Date. */
    endDate?: string;
}

/**
 * Response for Lookup Individuals
 */
export interface IndividualsLookupByNameResponse {
    /** Page Number. */
    pageNumber?: number;
    /** Data List. */
    dataList?: Array<IndividualLookupBasicData>;
}

/**
 * Response for Add Individual to Committee
 */
export interface AddToCommitteeResponse {
    /** Member Id. */
    id?: string;
    /** First Name. */
    firstName?: string;
    /** Last Name. */
    lastName?: string;
    /** Code. */
    code?: string;
    /** Record Number. */
    recordNumber?: string;
    /** Position. */
    position?: string;
    /** Position Code. */
    positionCode?: string;
    /** Start Date. */
    startDate?: string;
    /** End Date. */
    endDate?: string;
    primaryOrganization?: Record<string, unknown>;
    organizationRepresenting?: Record<string, unknown>;
    nominatedByCustomer?: Record<string, unknown>;
    addresses?: Array<Record<string, unknown>>;
    phones?: Array<Record<string, unknown>>;
    emails?: Array<Record<string, unknown>>;
    /** Rank. */
    rank?: number;
}

/**
 * Add Organization
 */
export interface AddOrganizationInput {
    /** Id. */
    id?: string;
    /** Name. */
    name: string;
    /** Show In Directory. */
    showInDirectory: boolean;
    /** Branch Name. */
    branchName?: string;
    /** Website. */
    website?: string;
}

/**
 * Update Organization
 */
export interface UpdateOrganizationInput {
    /** Name. */
    name?: string;
    /** Description. */
    description?: string;
    /** Show In Directory. */
    showInDirectory?: boolean;
    /** Branch Name. */
    branchName?: string;
    /** Website. */
    website?: string;
}

/**
 * Response for Lookup Organizations
 */
export interface OrganizationLookupByNameResponse {
    /** Page Number. */
    pageNumber?: number;
    /** Data List. */
    dataList?: Array<OrganizationLookupBasicData>;
}

/**
 * Response for Get All Committees
 */
export interface GetAllCommitteesResponse {
    /** Page Number. */
    pageNumber?: number;
    /** dataList */
    dataList?: Array<CommitteeData>;
}

/**
 * Response for Get Positions by Committee
 */
export interface GetPositionsByCommitteeResponse {
    /** Page Number. */
    pageNumber?: number;
    /** dataList */
    dataList?: Array<CommitteePositionData>;
}

/**
 * Response for Get Sub-Committees
 */
export interface GetSubCommitteesResponse {
    /** pageNumber */
    pageNumber?: number;
    /** dataList */
    dataList?: Array<CommitteeData>;
}

/**
 * Response for Get Individual's Active Subscriptions
 */
export interface GetIndividualsActiveSubscriptionsResponse {
    /** Page Number. */
    pageNumber?: number;
    /** dataList */
    dataList?: Array<SubscriptionData>;
}

/**
 * Response for List all Individuals
 */
export interface ListAllIndividualsResponse {
    /** Page Number. */
    pageNumber?: number;
    dataList?: Array<IndividualData>;
}

/**
 * Response for Find Customer by Phone Number
 */
export interface FindCustomerPhoneResponse {
    /** Page Number. */
    pageNumber?: number;
    dataList?: Array<Record<string, unknown>>;
}

/**
 * Mark Registrant as Attended
 */
export interface MarkRegistrantAttendedInput {
    [key: string]: unknown;
}

/**
 * Response for List Award Individual Recipients
 */
export interface AwardsGetIndividualAwardRecipientsResponse {
    /** Page Number. */
    pageNumber?: number;
    /** Data List. */
    dataList?: Array<AwardRecipientIndividualData>;
}

/**
 * Response for List Award Organization Recipients
 */
export interface AwardsGetOrganizationAwardRecipientsResponse {
    /** Page Number. */
    pageNumber?: number;
    /** Data List. */
    dataList?: Array<AwardRecipientOrganizationData>;
}

/**
 * When an individual is created
 */
export interface WhenIndividualCreatedInput {
    /** Type Name. */
    typeName: string;
    /** Secret. */
    secret: string;
    /** Callback URL. */
    callBackUrl: string;
    /** Description. */
    description: string;
}

/**
 * When an individual is deleted
 */
export interface WhenIndividualDeletedInput {
    /** Type Name. */
    typeName: string;
    /** Secret. */
    secret: string;
    /** Callback URL. */
    callBackUrl: string;
    /** Description. */
    description: string;
}

/**
 * When an individual requests to be forgotten
 */
export interface WhenIndividualRequestForgottenInput {
    /** Type Name. */
    typeName: string;
    /** Secret. */
    secret: string;
    /** Callback URL. */
    callBackUrl: string;
    /** Description. */
    description: string;
}

/**
 * When a product is purchased
 */
export interface WhenProductPurchasedInput {
    /** Id. */
    id?: string;
    /** Type Name. */
    typeName: string;
    /** Filters. */
    filters: Array<string>;
    /** Secret. */
    secret: string;
    /** Callback URL. */
    callBackUrl: string;
    /** Description. */
    description: string;
}

/**
 * When a committee member is updated
 */
export interface WhenCommitteeMemberUpdatedInput {
    /** Type Name. */
    typeName: string;
    /** Secret. */
    secret: string;
    /** Callback URL. */
    callBackUrl: string;
    /** Description. */
    description: string;
}

/**
 * When a purchase is canceled
 */
export interface WhenPurchaseCancelledInput {
    /** Id. */
    id?: string;
    /** Type Name. */
    typeName: string;
    /** Filters. */
    filters: Array<string>;
    /** Secret. */
    secret: string;
    /** Callback URL. */
    callBackUrl: string;
    /** Description. */
    description: string;
}

/**
 * When a customer request is updated
 */
export interface WhenRequestUpdatedInput {
    /** Type Name. */
    typeName: string;
    /** Secret. */
    secret: string;
    /** Callback URL. */
    callBackUrl: string;
    /** Description. */
    description: string;
}

/**
 * When a customer email is updated
 */
export interface WhenEmailUpdatedInput {
    /** Type Name. */
    typeName: string;
    /** Secret. */
    secret: string;
    /** Callback URL. */
    callBackUrl: string;
    /** Description. */
    description: string;
}

/**
 * When a customer custom field value is updated
 */
export interface WhenCustomerCustomFieldValueUpdatedInput {
    /** Type Name. */
    typeName: string;
    /** Filters. */
    filters: Array<string>;
    /** Secret. */
    secret: string;
    /** Callback URL. */
    callBackUrl: string;
    /** Description. */
    description: string;
}

/**
 * When a customer is merged
 */
export interface WhenCustomerIsMergedInput {
    /** Type Name. */
    typeName: string;
    /** Secret. */
    secret: string;
    /** Callback URL. */
    callBackUrl: string;
    /** Description. */
    description: string;
}

/**
 * When a customer relationship is updated
 */
export interface WhenCustomerRelationshipUpdatedInput {
    /** Type Name. */
    typeName: string;
    /** Secret. */
    secret: string;
    /** Callback URL. */
    callBackUrl: string;
    /** Description. */
    description: string;
}

/**
 * When a customer phone is updated
 */
export interface WhenCustomerPhoneUpdatedInput {
    /** Type Name. */
    typeName: string;
    /** Secret. */
    secret: string;
    /** Callback URL. */
    callBackUrl: string;
    /** Description. */
    description: string;
}

/**
 * When a customer address is updated
 */
export interface WhenCustomerAddressUpdatedInput {
    /** Type Name. */
    typeName: string;
    /** Secret. */
    secret: string;
    /** Callback URL. */
    callBackUrl: string;
    /** Description. */
    description: string;
}

/**
 * When an event registration is substituted
 */
export interface WhenEventRegistrationSubstitutedInput {
    /** Type Name. */
    typeName: string;
    /** Filters. */
    filters: Array<string>;
    /** Secret. */
    secret: string;
    /** Callback URL. */
    callBackUrl: string;
    /** Description. */
    description: string;
}

/**
 * When a purchase is paid
 */
export interface WhenPurchasePaidInput {
    /** Type Name. */
    typeName: string;
    /** Filters. */
    filters: Array<string>;
    /** Secret. */
    secret: string;
    /** Callback URL. */
    callBackUrl: string;
    /** Description. */
    description: string;
}

/**
 * When a membership is terminated
 */
export interface WhenMembershipTerminatedInput {
    /** Id. */
    id?: string;
    /** Type Name. */
    typeName: string;
    /** Secret. */
    secret: string;
    /** Callback URL. */
    callBackUrl: string;
    /** Description. */
    description: string;
}

/**
 * Definition: AbandonedCheckoutData
 */
export interface AbandonedCheckoutData {
    /** Created On */
    createdOn?: string;
    /** Created By Email Address */
    createdByEmailAddress?: string;
    /** Currency */
    currency?: string;
    customer?: Record<string, unknown>;
    /** Line Items */
    lineItems?: Array<Record<string, unknown>>;
}

/**
 * Definition: ExhibitorData
 */
export interface ExhibitorData {
    /** Organization */
    organization?: Record<string, unknown>;
    /** Booths */
    booths?: Array<Record<string, unknown>>;
}

/**
 * Definition: ExamData
 */
export interface ExamData {
    /** Exam Start Date */
    examStartDate?: string;
    /** Exam End Date */
    examEndDate?: string;
    /** Exam Number */
    examNumber?: string;
    /** Education Credits */
    educationCredits?: Array<Record<string, unknown>>;
    /** categories */
    categories?: Array<Record<string, unknown>>;
    /** Active Prices */
    activePrices?: Array<Record<string, unknown>>;
    /** Related Products */
    relatedProducts?: Array<Record<string, unknown>>;
    /** Tags. */
    tags?: Array<string>;
    /** ID. */
    id?: string;
    /** Code. */
    code?: string;
    /** Name. */
    name?: string;
    /** Short Description. */
    shortDescription?: string;
    /** Long Description. */
    longDescription?: string;
    /** Type. */
    type?: string;
    /** Url. */
    url?: string;
    /** Image Url. */
    imageUrl?: Array<string>;
    /** Is Public. */
    isPublic?: boolean;
    /** Available From. */
    availableFrom?: string;
    /** Available Until. */
    availableUntil?: string;
}

/**
 * Definition: RegistrantData
 */
export interface RegistrantData {
    /** Title. */
    title?: string;
    /** Badge Name */
    badgeName?: string;
    /** Badge Organization. */
    badgeOrganization?: string;
    /** Badge City. */
    badgeCity?: string;
    /** Badge State. */
    badgeState?: string;
    /** Attended Date. */
    attendedDate?: string;
    sessions?: Array<Record<string, unknown>>;
    /** Itemized Custom Fields */
    itemizedCustomFields?: Array<Record<string, unknown>>;
    /** Guest Of Record Number. */
    guestOfRecordNumber?: string;
    /** Registrant Type Code. */
    registrantTypeCode?: string;
    /** Registrant Type Name. */
    registrantTypeName?: string;
    /** Registration Number. */
    registrationNumber?: string;
    boughtTogetherWith?: Array<Record<string, unknown>>;
    /** First Name. */
    firstName?: string;
    /** Last Name. */
    lastName?: string;
    /** Middle Name. */
    middleName?: string;
    /** Preferred First Name. */
    preferredFirstName?: string;
    /** Second Last Name. */
    secondLastName?: string;
    /** Prefix. */
    prefix?: string;
    /** Suffix. */
    suffix?: string;
    /** Gender. */
    gender?: string;
    primaryOrganization?: Record<string, unknown>;
    /** ID. */
    id?: string;
    /** Customer Type */
    customerType?: string;
    /** Record Number. */
    recordNumber?: string;
    /** Show In Directory. */
    showInDirectory?: boolean;
    addresses?: Array<Record<string, unknown>>;
    phones?: Array<Record<string, unknown>>;
    emails?: Array<Record<string, unknown>>;
}

/**
 * Definition: CourseAttendeeData
 */
export interface CourseAttendeeData {
    /** Title. */
    title?: string;
    /** Registered Date. */
    registeredDate?: string;
    /** Attended Date. */
    attendedDate?: string;
    /** Itemized Custom Fields */
    itemizedCustomFields?: Array<Record<string, unknown>>;
    /** First Name. */
    firstName?: string;
    /** Last Name. */
    lastName?: string;
    /** Middle Name. */
    middleName?: string;
    /** Preferred First Name. */
    preferredFirstName?: string;
    /** Second Last Name. */
    secondLastName?: string;
    /** Prefix. */
    prefix?: string;
    /** Suffix */
    suffix?: string;
    /** Gender. */
    gender?: string;
    primaryOrganization?: Record<string, unknown>;
    /** ID. */
    id?: string;
    /** Customer Type. */
    customerType?: string;
    /** Record Number. */
    recordNumber?: string;
    /** Show In Directory. */
    showInDirectory?: boolean;
    /** Addresses. */
    addresses?: Array<Record<string, unknown>>;
    /** Emails. */
    emails?: Array<Record<string, unknown>>;
    /** Phones. */
    phones?: Array<Record<string, unknown>>;
}

/**
 * Definition: ExamScoreResultData
 */
export interface ExamScoreResultData {
    /** Individual Record Number. */
    individualRecordNumber?: string;
    /** Status. */
    status?: string;
    /** Message. */
    message?: string;
}

/**
 * Definition: ExamScoreData
 */
export interface ExamScoreData {
    /** Individual Record Number. */
    individualRecordNumber?: string;
    /** Score. */
    score?: string;
}

/**
 * Definition: ContactData
 */
export interface ContactData {
    /** ID. */
    id?: string;
    /** Customer Type. */
    customerType?: string;
    /** Record Number. */
    recordNumber?: string;
    /** Title. */
    title?: string;
    /** Name. */
    name?: string;
    addresses?: Array<Record<string, unknown>>;
    /** Image Uri. */
    imageUri?: string;
    /** Twitter. */
    twitter?: string;
    /** LinkedIn. */
    linkedIn?: string;
    /** emails */
    emails?: Array<Record<string, unknown>>;
    phones?: Array<Record<string, unknown>>;
    /** Website. */
    webSite?: string;
    /** Memberships. */
    memberships?: Array<Record<string, unknown>>;
    /** Category. */
    category?: string;
    /** Categories */
    categories?: Array<Record<string, unknown>>;
    /** Custom Fields */
    customFields?: Array<Record<string, unknown>>;
    /** Show In Directory. */
    showInDirectory?: boolean;
    /** Links */
    links?: Array<Record<string, unknown>>;
    /** Old ID. */
    oldId?: string;
}

/**
 * Definition: PurchasedItemData
 */
export interface PurchasedItemData {
    /** Order Number. */
    orderNumber?: string;
    /** Product Code. */
    productCode?: string;
    /** Product Name. */
    productName?: string;
    /** Product Type. */
    productType?: string;
    /** Purchase Date. */
    purchaseDate?: string;
    /** Additional Info. */
    additionalInfo?: string;
    /** Quantity. */
    quantity?: number;
    /** Itemized Custom Fields */
    itemizedCustomFields?: Array<Record<string, unknown>>;
    /** Product Categories */
    productCategories?: Array<Record<string, unknown>>;
}

/**
 * Definition: CustomFieldValueData
 */
export interface CustomFieldValueData {
    /** Name. */
    name?: string;
    /** Value. */
    value?: string;
}

/**
 * Definition: CommitteeNomineeSaveData
 */
export interface CommitteeNomineeSaveData {
    /** Nominee Record Number. */
    nomineeRecordNumber?: string;
    /** Code. */
    code?: string;
    /** Position Code. */
    positionCode?: string;
    /** Status. */
    status?: string;
    /** Representing Organization Record Number. */
    representingOrganizationRecordNumber?: string;
    /** Representing Organization Relationship Name. */
    representingOrganizationRelationshipName?: string;
    /** Nominated By Customer Record Number. */
    nominatedByCustomerRecordNumber?: string;
    /** Committee Term. */
    committeeTerm?: number;
    /** Rank. */
    rank?: number;
}

/**
 * Definition: RegistrantCancellationData
 */
export interface RegistrantCancellationData {
    /** Title. */
    title?: string;
    /** Registration Number. */
    registrationNumber?: string;
    /** Event Cancellation Date. */
    eventCancellationDate?: string;
    /** Guest Of Record Number. */
    guestOfRecordNumber?: string;
    /** Cancelled Sessions. */
    cancelledSessions?: Array<Record<string, unknown>>;
    /** First Name. */
    firstName?: string;
    /** Last Name. */
    lastName?: string;
    /** Middle Name. */
    middleName?: string;
    /** Preferred First Name. */
    preferredFirstName?: string;
    /** Second Last Name. */
    secondLastName?: string;
    /** Prefix. */
    prefix?: string;
    /** Suffix. */
    suffix?: string;
    /** Gender. */
    gender?: string;
    /** primaryOrganization */
    primaryOrganization?: Record<string, unknown>;
    /** ID. */
    id?: string;
    /** Customer Type. */
    customerType?: string;
    /** Record Number. */
    recordNumber?: string;
    /** Show In Directory. */
    showInDirectory?: boolean;
    /** Addresses. */
    addresses?: Array<Record<string, unknown>>;
    /** Emails */
    emails?: Array<Record<string, unknown>>;
    /** Phones. */
    phones?: Array<Record<string, unknown>>;
}

/**
 * Definition: PayableOrderData
 */
export interface PayableOrderData {
    /** Number. */
    number?: string;
    /** Date. */
    date?: string;
    /** Balance. */
    balance?: number;
    /** lineItems */
    lineItems?: Array<Record<string, unknown>>;
    /** Type. */
    type?: string;
    /** ID. */
    id?: string;
    /** Customer Id. */
    customerId?: string;
    /** Total Amount. */
    totalAmount?: number;
    /** Title. */
    title?: string;
}

/**
 * Definition: BaseNoteData
 */
export interface BaseNoteData {
    /** Content. */
    Content?: string;
    /** Category. */
    Category?: string;
    /** Is Internal. */
    isInternal?: boolean;
}

/**
 * Definition: TaskSaveData
 */
export interface TaskSaveData {
    /** Title. */
    title?: string;
    /** Category. */
    category?: string;
    /** Description. */
    description?: string;
    /** Start Date. */
    startDate?: string;
    /** Due Date. */
    dueDate?: string;
}

/**
 * Definition: CountryData
 */
export interface CountryData {
    /** ID. */
    id?: string;
    /** Name. */
    name?: string;
    /** Two Letter Iso Code. */
    twoLetterIsoCode?: string;
    /** Three Letter Iso Code. */
    threeLetterIsoCode?: string;
    /** Code. */
    code?: number;
}

/**
 * Definition: StateProvinceData
 */
export interface StateProvinceData {
    /** ID. */
    id?: string;
    /** Name. */
    name?: string;
    /** Abbreviation. */
    abbreviation?: string;
    /** Iso Code. */
    isoCode?: string;
}

/**
 * Definition: ExhibitData
 */
export interface ExhibitData {
    /** Code. */
    code?: string;
    /** Name. */
    name?: string;
    /** Start Date. */
    startDate?: string;
    /** Start Time. */
    startTime?: string;
    /** End Date. */
    endDate?: string;
    /** End Time. */
    endTime?: string;
    /** Categories */
    categories?: Array<Record<string, unknown>>;
    /** Short Description. */
    shortDescription?: string;
    /** Long Description. */
    longDescription?: string;
    featuredListing?: Record<string, unknown>;
}

/**
 * Definition: RequestUpdateData
 */
export interface RequestUpdateData {
    /** Request Number. */
    requestNumber?: string;
    /** Closed Date. */
    closedDate?: string;
}

/**
 * Definition: RequestSaveData
 */
export interface RequestSaveData {
    /** Requested By Customer Record Number. */
    requestedByCustomerRecordNumber?: string;
    /** Request Type. */
    requestType?: string;
    /** Description. */
    description?: string;
    /** Source. */
    source?: string;
}

/**
 * Definition: RelationshipTypeData
 */
export interface RelationshipTypeData {
    /** Name. */
    name?: string;
    /** Reciprocal Relationship Name. */
    reciprocalRelationshipName?: string;
    /** Relationship Type. */
    relationshipType?: string;
    /** Allow Primary. */
    allowPrimary?: boolean;
    /** Can Purchase For Organization. */
    canPurchaseForOrganization?: boolean;
    /** Can Manage PAC. */
    canManagePAC?: boolean;
    /** Can Manage Organization. */
    canManageOrganization?: boolean;
}

/**
 * Definition: RequestData
 */
export interface RequestData {
    /** Request Number. */
    requestNumber?: string;
    /** Request Date. */
    requestDate?: string;
    /** Requested By Customer Record Number. */
    requestedByCustomerRecordNumber?: string;
    /** Request Type. */
    requestType?: string;
    /** Description. */
    description?: string;
    /** Source. */
    source?: string;
    /** Closed Date. */
    closedDate?: string;
}

/**
 * Definition: UserTaskData
 */
export interface UserTaskData {
    /** Task Number. */
    taskNumber?: string;
    /** Title. */
    title?: string;
    /** Category. */
    category?: string;
    /** Description. */
    description?: string;
    /** Assigned By. */
    assignedBy?: string;
    /** Start Date. */
    startDate?: string;
    /** Due Date. */
    dueDate?: string;
    /** Progress. */
    progress?: string;
    /** Completed Date. */
    completedDate?: string;
}

/**
 * Definition: NotificationData
 */
export interface NotificationData {
    /** Title. */
    title?: string;
    /** Message. */
    message?: string;
    /** Url. */
    url?: string;
    /** Date. */
    date?: string;
    /** Is Read. */
    isRead?: boolean;
}

/**
 * Definition: SaveCategoryBasicData
 */
export interface SaveCategoryBasicData {
    /** Code. */
    code?: string;
    /** Is Primary. */
    isPrimary?: boolean;
}

/**
 * Definition: TaskData
 */
export interface TaskData {
    /** Task Number. */
    taskNumber?: string;
    /** Title. */
    title?: string;
    /** Category. */
    category?: string;
    /** Description. */
    description?: string;
    /** Start Date. */
    startDate?: string;
    /** Due Date. */
    dueDate?: string;
}

/**
 * Definition: CertificationData
 */
export interface CertificationData {
    /** Code. */
    code?: string;
    /** Name. */
    name?: string;
    /** Expire Date. */
    expireDate?: string;
}

/**
 * Definition: ActivityData
 */
export interface ActivityData {
    /** Category. */
    category?: string;
    /** Text. */
    text?: string;
    /** Url. */
    url?: string;
    /** Activity Date. */
    activityDate?: string;
}

/**
 * Definition: SessionRegistrationData
 */
export interface SessionRegistrationData {
    /** Session Code. */
    sessionCode?: string;
}

/**
 * Definition: AwardData
 */
export interface AwardData {
    /** ID. */
    id?: string;
    /** Name. */
    name?: string;
    /** Description. */
    description?: string;
    /** Year. */
    year?: number;
    /** Customer Type. */
    customerType?: string;
    /** Allow Public Nomination. */
    allowPublicNomination?: boolean;
    /** Nomination Start Date. */
    nominationStartDate?: string;
    /** Nomination End Date. */
    nominationEndDate?: string;
}

/**
 * Definition: LicenseData
 */
export interface LicenseData {
    /** ID. */
    id?: string;
    /** License Type. */
    licenseType?: string;
    /** Number. */
    number?: string;
    /** State. */
    state?: string;
    /** Status. */
    status?: string;
    /** Start Date. */
    startDate?: string;
    /** Experation Date. */
    expirationDate?: string;
}

/**
 * Definition: ServiceData
 */
export interface ServiceData {
    /** Code. */
    code?: string;
    /** Name. */
    name?: string;
}

/**
 * Definition: CommitteeNomineeData
 */
export interface CommitteeNomineeData {
    /** ID. */
    id?: string;
    /** First Name. */
    firstName?: string;
    /** Last Name. */
    lastName?: string;
    /** Code. */
    code?: string;
    /** Position. */
    position?: string;
    /** Position Code. */
    positionCode?: string;
    /** Start Date. */
    startDate?: string;
    /** End Date. */
    endDate?: string;
    primaryOrganization?: Record<string, unknown>;
    organizationRepresenting?: Record<string, unknown>;
    nominatedByCustomer?: Record<string, unknown>;
    addresses?: Array<Record<string, unknown>>;
    phones?: Array<Record<string, unknown>>;
    emails?: Array<Record<string, unknown>>;
    /** Rank. */
    rank?: number;
}

/**
 * Definition: CustomFieldResultData
 */
export interface CustomFieldResultData {
    /** Status. */
    status?: string;
    /** Message. */
    message?: string;
    /** Custom Field Name. */
    customFieldName?: string;
}

/**
 * Definition: RelationshipData
 */
export interface RelationshipData {
    /** Relationship Name. */
    relationshipName?: string;
    /** Related To Customer. */
    relatedToCustomer?: Record<string, unknown>;
    /** Reciprocal Relationship Name. */
    reciprocalRelationshipName?: string;
    /** Is Primary. */
    isPrimary?: boolean;
    /** Start Date. */
    startDate?: string;
    /** End Date. */
    endDate?: string;
}

/**
 * Definition: RegistrationData
 */
export interface RegistrationData {
    /** Individual ID. */
    individualId?: string;
    /** Event. */
    event?: Record<string, unknown>;
    /** Sessions. */
    sessions?: Array<Record<string, unknown>>;
    /** Badge Name. */
    badgeName?: string;
    /** Badge Organization. */
    badgeOrganization?: string;
    /** Badge City. */
    badgeCity?: string;
    /** Badge State. */
    badgeState?: string;
    boughtTogetherWith?: Array<Record<string, unknown>>;
    /** Registrant Type Code. */
    registrantTypeCode?: string;
    /** Registrant Type Name. */
    registrantTypeName?: string;
    /** Registration Number. */
    registrationNumber?: string;
}

/**
 * Definition: CommitteeMemberData
 */
export interface CommitteeMemberData {
    committee?: Record<string, unknown>;
    /** Position. */
    position?: string;
    /** Position Code. */
    positionCode?: string;
    /** Code. */
    code?: string;
    /** Start Date. */
    startDate?: string;
    /** End Date. */
    endDate?: string;
}

/**
 * Definition: CustomFieldData
 */
export interface CustomFieldData {
    /** Name. */
    name?: string;
    /** Caption. */
    caption?: string;
    /** Custom Field Value. For multi-selects, use "|" as delimiter. */
    value?: string;
}

/**
 * Definition: MembershipData
 */
export interface MembershipData {
    /** Membership Type. */
    membershipType?: string;
    /** Membership Type ID. */
    membershipTypeId?: string;
    /** Code. */
    code?: string;
    /** Expire Date. */
    expireDate?: string;
    /** Grace Expire Date. */
    graceExpireDate?: string;
    /** Inherited Membership Benefits. */
    inheritedMembershipBenefits?: boolean;
    /** Renewal URL. */
    renewalUrl?: string;
    /** Join Date. */
    joinDate?: string;
    /** Terminate Date. */
    terminateDate?: string;
    /** Effective Date. */
    effectiveDate?: string;
    /** Membership Unique ID. */
    membershipUniqueId?: string;
    /** Terms List */
    termsList?: Array<Record<string, unknown>>;
}

/**
 * Definition: EventData
 */
export interface EventData {
    /** Event ID. */
    eventId?: string;
    /** Event Code. */
    eventCode?: string;
    /** Event Name. */
    eventName?: string;
    /** Event Start Date. */
    eventStartDate?: string;
    /** Event Start Time. */
    eventStartTime?: string;
    /** Event End Date. */
    eventEndDate?: string;
    /** Event End Time. */
    eventEndTime?: string;
    /** Event Time Zone. */
    eventTimezone?: string;
    /** Event Description. */
    eventDescription?: string;
    /** Short Description. */
    shortDescription?: string;
    /** Public Event. */
    publicEvent?: boolean;
    /** Image Url. */
    imageUrl?: Array<string>;
    /** External Url. */
    externalUrl?: string;
    /** External Code. */
    externalCode?: string;
    /** Event Locations. */
    eventLocations?: Array<Record<string, unknown>>;
    /** Event Locations - Categories. */
    categories?: Array<Record<string, unknown>>;
    /** Event Locations - Education Credits. */
    educationCredits?: Array<Record<string, unknown>>;
    /** Event Locations - Uploads Data */
    uploadsData?: Array<Record<string, unknown>>;
    /** Event Locations - Related Products. */
    relatedProducts?: Array<Record<string, unknown>>;
    /** Event Locations - Tags */
    tags?: Array<string>;
}

/**
 * Definition: PhoneSaveData
 */
export interface PhoneSaveData {
    /** Country Name. */
    countryName: string;
    /** Primary. */
    primary?: boolean;
    /** Show in Directory. */
    showInDirectory?: boolean;
    /** Number. */
    number: string;
    /** Extension. */
    extension?: string;
    /** Type Name. */
    typeName: string;
}

/**
 * Definition: PhoneDataSet
 */
export interface PhoneDataSet {
    /** ID. */
    id?: string;
    /** Is Primary. */
    primary?: boolean;
    /** Type. */
    type?: number;
    country?: Record<string, unknown>;
    /** Show In Directory. */
    showInDirectory?: boolean;
    /** Number. */
    number?: string;
    /** Extension. */
    extension?: string;
    /** Type Name. */
    typeName?: string;
}

/**
 * Definition: EmailData
 */
export interface EmailData {
    /** Is Primary Email. */
    primary?: boolean;
    /** Address. */
    address?: string;
    /** Type. */
    type?: string;
    /** Show In Directory. */
    showInDirectory?: boolean;
}

/**
 * Definition: AddressSaveData
 */
export interface AddressSaveData {
    /** ID. */
    id?: string;
    /** Primary. */
    primary?: boolean;
    /** Line 1. */
    line1?: string;
    /** Line 2. */
    line2?: string;
    /** Line 3. */
    line3?: string;
    /** City. */
    city?: string;
    /** State. */
    state?: string;
    /** Zipcode. */
    zipcode?: string;
    /** Country. */
    country?: string;
    /** Type. */
    type?: string;
    /** Show In Directory. */
    showInDirectory?: boolean;
    /** Is Preferred Shipping. */
    isPreferredShipping?: boolean;
    /** Is Preferred Billing. */
    isPreferredBilling?: boolean;
    /** Is Bad Address. */
    isBadAddress?: boolean;
}

/**
 * Definition: IndividualData
 */
export interface IndividualData {
    /** First Name. */
    firstName?: string;
    /** Last Name. */
    lastName?: string;
    /** Middle Name. */
    middleName?: string;
    /** Preferred First Name. */
    preferredFirstName?: string;
    /** Second Last Name. */
    secondLastName?: string;
    /** Prefix. */
    prefix?: string;
    /** Suffix. */
    suffix?: string;
    /** Gender. */
    gender?: string;
    primaryOrganization?: Record<string, unknown>;
    /** Committees. */
    committees?: Array<Record<string, unknown>>;
    /** Security Roles. */
    securityRoles?: Array<Record<string, unknown>>;
    user?: Record<string, unknown>;
    /** Designations. */
    designationData?: Array<Record<string, unknown>>;
    /** Relationships. */
    relationships?: Array<Record<string, unknown>>;
    /** Job Roles. */
    jobRoles?: Array<Record<string, unknown>>;
    /** Tags. */
    tags?: Array<string>;
    /** Individual Id. */
    id?: string;
    /** Customer Type. */
    customerType?: string;
    /** Record Number. */
    recordNumber?: string;
    /** Title. */
    title?: string;
    /** Name. */
    name?: string;
    /** Addresses. */
    addresses?: Array<Record<string, unknown>>;
    /** Email. */
    email?: string;
    /** Image URI. */
    imageUri?: string;
    /** Twitter. */
    twitter?: string;
    /** LinkedIn. */
    linkedIn?: string;
    /** Emails. */
    emails?: Array<Record<string, unknown>>;
    /** Phones. */
    phones?: Array<Record<string, unknown>>;
    /** Website. */
    webSite?: string;
    /** Memberships. */
    memberships?: Array<Record<string, unknown>>;
    /** Category. */
    category?: string;
    /** Categories. */
    categories?: Array<Record<string, unknown>>;
    /** Custom Fields. */
    customFields?: Array<Record<string, unknown>>;
    /** Show In Directory. */
    showInDirectory?: boolean;
    /** Links. */
    links?: Array<Record<string, unknown>>;
    /** Old Id. */
    oldId?: string;
    /** Is Deceased. */
    isDeceased?: boolean;
}

/**
 * Definition: IndividualPayload
 */
export interface IndividualPayload {
    /** Webhook Id. */
    WebhookId?: string;
    /** Properties. */
    Properties?: Record<string, unknown>;
    /** Notifications. */
    Notifications?: Array<Record<string, unknown>>;
}

/**
 * Definition: OrganizationData
 */
export interface OrganizationData {
    /** Contact Ids. */
    contactIds?: Array<string>;
    /** Parent Company Id. */
    parentCompanyId?: string;
    /** Description. */
    description?: string;
    /** Branch Name. */
    branchName?: string;
    /** Tags. */
    tags?: Array<string>;
    /** Organization Id. */
    id?: string;
    /** Customer Type. */
    customerType?: string;
    /** Record Number. */
    recordNumber?: string;
    /** Title. */
    title?: string;
    /** Organization Name. */
    name?: string;
    /** Addresses. */
    addresses?: Array<Record<string, unknown>>;
    /** Email. */
    email?: string;
    /** Image URI. */
    imageUri?: string;
    /** Twitter. */
    twitter?: string;
    /** LinkedIn. */
    linkedIn?: string;
    /** Emails. */
    emails?: Array<Record<string, unknown>>;
    /** Phones. */
    phones?: Array<Record<string, unknown>>;
    /** Website. */
    webSite?: string;
    /** Memberships. */
    memberships?: Array<Record<string, unknown>>;
    /** Category. */
    category?: string;
    /** Categories. */
    categories?: Array<Record<string, unknown>>;
    /** Custom Fields. */
    customFields?: Array<Record<string, unknown>>;
    /** Show In Directory. */
    showInDirectory?: boolean;
    /** Links. */
    links?: Array<Record<string, unknown>>;
    /** Old Id. */
    oldId?: string;
}

/**
 * Definition: CommitteeMemberPayload
 */
export interface CommitteeMemberPayload {
    /** Webhook Id. */
    WebhookId?: string;
    /** Properties. */
    Properties?: Record<string, unknown>;
    /** Notifications. */
    Notifications?: Array<Record<string, unknown>>;
}

/**
 * Definition: PurchaseCancelledPayload
 */
export interface PurchaseCancelledPayload {
    /** Webhook Id. */
    WebhookId?: string;
    /** Properties. */
    Properties?: Record<string, unknown>;
    /** Notifications. */
    Notifications?: Array<Record<string, unknown>>;
}

/**
 * Definition: CustomerRequestPayload
 */
export interface CustomerRequestPayload {
    /** Webhook Id,. */
    WebhookId?: string;
    /** Properties. */
    Properties?: Record<string, unknown>;
    /** Notifications. */
    Notifications?: Array<Record<string, unknown>>;
}

/**
 * Definition: EmailPayload
 */
export interface EmailPayload {
    /** Webhook Id. */
    WebhookId?: string;
    /** Properties. */
    Properties?: Record<string, unknown>;
    /** Notifications. */
    Notifications?: Array<Record<string, unknown>>;
}

/**
 * Definition: CommitteeMemberCreateData
 */
export interface CommitteeMemberCreateData {
    /** Record Number. */
    recordNumber: string;
    /** Member Code. */
    code?: string;
    /** Position Code. */
    positionCode: string;
    /** Start Date. */
    startDate: string;
    /** End Date. */
    endDate?: string;
    /** Representing Organization - Record Number. */
    representingOrganizationRecordNumber?: string;
    /** Representing Organization - Relationship Name. */
    representingOrganizationRelationshipName?: string;
    /** Nominated By - Record Number */
    nominatedByCustomerRecordNumber?: string;
    /** Committee Term. */
    committeeTerm?: number;
    /** Rank. */
    rank?: number;
}

/**
 * Definition: SubscriptionData
 */
export interface SubscriptionData {
    /** Code. */
    code?: string;
    /** Name. */
    name?: string;
    /** Expire Date. */
    expireDate?: string;
    /** Quantity. */
    quantity?: number;
}

/**
 * Definition: CommitteePositionData
 */
export interface CommitteePositionData {
    /** Code. */
    code?: string;
    /** Name. */
    name?: string;
    /** Term. */
    term?: number;
    /** Description. */
    description?: string;
    /** Is Admin. */
    isAdmin?: boolean;
}

/**
 * Definition: CommitteeData
 */
export interface CommitteeData {
    /** Name. */
    name?: string;
    /** Code. */
    code?: string;
    /** Website. */
    website?: string;
    /** Id. */
    id?: string;
    /** Parent Committee Id. */
    parentCommitteeId?: string;
    /** Category Name. */
    categoryName?: string;
    /** Category Code. */
    categoryCode?: string;
    /** Description. */
    description?: string;
    /** Start Date. */
    startDate?: string;
    /** End Date. */
    endDate?: string;
    /** Show In Directory. */
    showInDirectory?: boolean;
    /** Term. */
    term?: Record<string, unknown>;
    /** Tags. */
    tags?: Array<string>;
}

/**
 * Definition: CommitteeMemberUpdateData
 */
export interface CommitteeMemberUpdateData {
    /** Record Number. */
    recordNumber?: string;
    /** Member Code. */
    code?: string;
    /** New Position Code. */
    positionCode?: string;
    /** Start Date. */
    startDate?: string;
    /** End Date. */
    endDate?: string;
    /** Representing Organization - Record Number. */
    representingOrganizationRecordNumber?: string;
    /** Representing Organization - Relationship Name. */
    representingOrganizationRelationshipName?: string;
    /** Nominated By - Record Number */
    nominatedByCustomerRecordNumber?: string;
    /** Committee Term. */
    committeeTerm?: number;
    /** Rank. */
    rank?: number;
}

/**
 * Definition: EducationCreditData
 */
export interface EducationCreditData {
    /** Education Credit Type. */
    type: string;
    /** Description. */
    description: string;
    /** Number of Credits Earned. */
    numberOfCredits: number;
    /** Earned Date. */
    dateEarned: string;
    /** Provider Name. */
    providerName?: string;
    /** State or Province Name. */
    stateName?: string;
    /** Reference. */
    reference?: string;
    /** Is Self Reported. */
    isSelfReported?: boolean;
}

/**
 * Definition: NoteData
 */
export interface NoteData {
    /** Is Alert. */
    isAlert?: boolean;
    /** Title. */
    title?: string;
    /** Follow Up Date. */
    followUpDate?: string;
    /** Content. */
    content: string;
    /** Category. */
    category?: string;
    /** Is Internal. */
    isInternal?: boolean;
}

/**
 * Definition: AwardNominationData
 */
export interface AwardNominationData {
    /** Nominee Record Number. */
    nomineeRecordNumber: string;
    /** Nominated By Record Number. */
    nominatedByCustomerRecordNumber: string;
    /** Nomination Date. */
    nominationDate: string;
    /** Awarded Date. */
    awardedDate?: string;
    /** Status. */
    status?: string;
    /** An explanation why the nominee should be considered for the award. */
    description?: string;
}

/**
 * Definition: UpdateAwardNominationData
 */
export interface UpdateAwardNominationData {
    /** Nominee Record Number. */
    nomineeRecordNumber?: string;
    /** Nominated By Record Number. */
    nominatedByCustomerRecordNumber: string;
    /** Nomination Date. */
    nominationDate: string;
    /** Awarded Date. */
    awardedDate?: string;
    /** Status. */
    status?: string;
    /** An explanation why the nominee should be considered for the award. */
    description?: string;
}

/**
 * Definition: AwardRecipientIndividualData
 */
export interface AwardRecipientIndividualData {
    /** Awarded Date. */
    awardedDate?: string;
    /** First Name. */
    firstName?: string;
    /** Last Name. */
    lastName?: string;
    /** Middle Name. */
    middleName?: string;
    /** Preferred First Name. */
    preferredFirstName?: string;
    /** Second Last Name. */
    secondLastName?: string;
    /** Prefix. */
    prefix?: string;
    /** Suffix. */
    suffix?: string;
    /** Gender. */
    gender?: string;
    primaryOrganization?: Record<string, unknown>;
    /** Individual Id. */
    id?: string;
    /** Customer Type. */
    customerType?: string;
    /** Record Number. */
    recordNumber?: string;
    /** Title. */
    title?: string;
    /** Addresses. */
    addresses?: Array<Record<string, unknown>>;
    /** Emails. */
    emails?: Array<Record<string, unknown>>;
    /** Phones. */
    phones?: Array<Record<string, unknown>>;
    /** Show In Directory. */
    showInDirectory?: boolean;
}

/**
 * Definition: AwardRecipientOrganizationData
 */
export interface AwardRecipientOrganizationData {
    /** Awarded Date. */
    awardedDate?: string;
    /** Organization Name. */
    name?: string;
    /** Acronym. */
    acronym?: string;
    /** Branch Name. */
    branchName?: string;
    /** Annual Sales. */
    annualSales?: number;
    /** Employee Range Id. */
    employeeRangeId?: string;
    /** Organization Id. */
    id?: string;
    /** Customer Type. */
    customerType?: string;
    /** Record Number. */
    recordNumber?: string;
    /** Show In Directory. */
    showInDirectory?: boolean;
    /** Addresses. */
    addresses?: Array<Record<string, unknown>>;
    /** Emails. */
    emails?: Array<Record<string, unknown>>;
    /** Phones. */
    phones?: Array<Record<string, unknown>>;
}

/**
 * Definition: OrganizationLookupBasicData
 */
export interface OrganizationLookupBasicData {
    /** Organization Name. */
    name?: string;
    /** Acronym. */
    acronym?: string;
    /** Branch Name. */
    branchName?: string;
    /** Employee Range Id. */
    employeeRangeId?: string;
    /** Organization Id. */
    id?: string;
    /** Record Number. */
    recordNumber?: string;
    /** Addresses. */
    addresses?: Array<Record<string, unknown>>;
}

/**
 * Definition: IndividualLookupBasicData
 */
export interface IndividualLookupBasicData {
    /** First Name. */
    firstName?: string;
    /** Last Name. */
    lastName?: string;
    /** Middle Name. */
    middleName?: string;
    /** Preferred First Name. */
    preferredFirstName?: string;
    /** Second Last Name. */
    secondLastName?: string;
    /** Prefix. */
    prefix?: string;
    /** Suffix. */
    suffix?: string;
    /** Gender. */
    gender?: string;
    /** Title. */
    title?: string;
    primaryOrganization?: Record<string, unknown>;
    /** Individual Id. */
    id?: string;
    /** Record Number. */
    recordNumber?: string;
}

/**
 * Definition: CustomerCustomFieldValuePayload
 */
export interface CustomerCustomFieldValuePayload {
    /** Webhook Id. */
    WebhookId?: string;
    /** Properties. */
    Properties?: Record<string, unknown>;
    Notifications?: Array<Record<string, unknown>>;
}

/**
 * Definition: CustomerMergedPayload
 */
export interface CustomerMergedPayload {
    /** Webhook Id. */
    WebhookId?: string;
    /** Properties. */
    Properties?: Record<string, unknown>;
    Notifications?: Array<Record<string, unknown>>;
}

/**
 * Definition: CustomerRelationshipUpdatedPayload
 */
export interface CustomerRelationshipUpdatedPayload {
    /** Webhook Id. */
    WebhookId?: string;
    /** Properties. */
    Properties?: Record<string, unknown>;
    Notifications?: Array<Record<string, unknown>>;
}

/**
 * Definition: CustomerPhoneUpdatedPayload
 */
export interface CustomerPhoneUpdatedPayload {
    /** Webhook Id. */
    WebhookId?: string;
    /** Properties. */
    Properties?: Record<string, unknown>;
    Notifications?: Array<Record<string, unknown>>;
}

/**
 * Definition: CustomerAddressUpdatedPayload
 */
export interface CustomerAddressUpdatedPayload {
    /** Webhook Id. */
    WebhookId?: string;
    /** Properties. */
    Properties?: Record<string, unknown>;
    Notifications?: Array<Record<string, unknown>>;
}

/**
 * Definition: EventRegistrationSubstitutedPayload
 */
export interface EventRegistrationSubstitutedPayload {
    /** Webhook Id. */
    WebhookId?: string;
    /** Properties. */
    Properties?: Record<string, unknown>;
    Notifications?: Array<Record<string, unknown>>;
}

/**
 * Definition: IndividualRequestToBeForgottenPayload
 */
export interface IndividualRequestToBeForgottenPayload {
    /** Webhook Id. */
    WebhookId?: string;
    /** Properties. */
    Properties?: Record<string, unknown>;
    Notifications?: Array<Record<string, unknown>>;
}

/**
 * Definition: ProductPurchasePayload
 */
export interface ProductPurchasePayload {
    /** Webhook Id. */
    WebhookId?: string;
    /** Properties. */
    Properties?: Record<string, unknown>;
    Notifications?: Array<Record<string, unknown>>;
}

/**
 * Definition: MembershipTypeTerminatedPayload
 */
export interface MembershipTypeTerminatedPayload {
    /** Webhook Id. */
    WebhookId?: string;
    /** Properties. */
    Properties?: Record<string, unknown>;
    Notifications?: Array<Record<string, unknown>>;
}
// #endregion Types

export const ImpexiumTriggerOperations = {
    OnWhenCommitteeMemberUpdated: "When-Committee-Member-Updated",
    OnWhenCustomerAddressUpdated: "When-Customer-Address-Updated",
    OnWhenCustomerCustomFieldValueUpdated: "When-Customer-Custom-Field-Value-Updated",
    OnWhenCustomerIsMerged: "When-Customer-Is-Merged",
    OnWhenCustomerPhoneUpdated: "When-Customer-Phone-Updated",
    OnWhenCustomerRelationshipUpdated: "When-Customer-Relationship-Updated",
    OnWhenEmailUpdated: "When-Email-Updated",
    OnWhenEventRegistrationSubstituted: "When-Event-Registration-Substituted",
    OnWhenIndividualCreated: "When-Individual-Created",
    OnWhenIndividualDeleted: "When-Individual-Deleted",
    OnWhenIndividualRequestForgotten: "When-Individual-Request-Forgotten",
    OnWhenMembershipTerminated: "When-Membership-Terminated",
    OnWhenProductPurchased: "When-product-purchased",
    OnWhenPurchaseCancelled: "When-purchase-cancelled",
    OnWhenPurchasePaid: "When-Purchase-Paid",
    OnWhenRequestUpdated: "When-Request-Updated",
} as const;

export type ImpexiumTriggerOperation = typeof ImpexiumTriggerOperations[keyof typeof ImpexiumTriggerOperations];

export const ImpexiumTriggerParameters = {
    OnWhenCommitteeMemberUpdated: {
        contentType: {
            name: "Content-Type",
            type: "string",
            required: true,
            defaultValue: "application/json",
        },
    },
    OnWhenCustomerAddressUpdated: {
        contentType: {
            name: "Content-Type",
            type: "string",
            required: true,
            defaultValue: "application/json",
        },
    },
    OnWhenCustomerCustomFieldValueUpdated: {
        contentType: {
            name: "Content-Type",
            type: "string",
            required: true,
            defaultValue: "application/json",
        },
    },
    OnWhenCustomerIsMerged: {
        contentType: {
            name: "Content-Type",
            type: "string",
            required: true,
            defaultValue: "application/json",
        },
    },
    OnWhenCustomerPhoneUpdated: {
        contentType: {
            name: "Content-Type",
            type: "string",
            required: true,
            defaultValue: "application/json",
        },
    },
    OnWhenCustomerRelationshipUpdated: {
        contentType: {
            name: "Content-Type",
            type: "string",
            required: true,
            defaultValue: "application/json",
        },
    },
    OnWhenEmailUpdated: {
        contentType: {
            name: "Content-Type",
            type: "string",
            required: true,
            defaultValue: "application/json",
        },
    },
    OnWhenEventRegistrationSubstituted: {
        contentType: {
            name: "Content-Type",
            type: "string",
            required: true,
            defaultValue: "application/json",
        },
    },
    OnWhenIndividualCreated: {
        contentType: {
            name: "Content-Type",
            type: "string",
            required: true,
            defaultValue: "application/json",
        },
    },
    OnWhenIndividualDeleted: {
        contentType: {
            name: "Content-Type",
            type: "string",
            required: true,
            defaultValue: "application/json",
        },
    },
    OnWhenIndividualRequestForgotten: {
        contentType: {
            name: "Content-Type",
            type: "string",
            required: true,
            defaultValue: "application/json",
        },
    },
    OnWhenMembershipTerminated: {
        contentType: {
            name: "Content-Type",
            type: "string",
            required: true,
            defaultValue: "application/json",
        },
    },
    OnWhenProductPurchased: {
        contentType: {
            name: "Content-Type",
            type: "string",
            required: true,
            defaultValue: "application/json",
        },
    },
    OnWhenPurchaseCancelled: {
        contentType: {
            name: "Content-Type",
            type: "string",
            required: true,
            defaultValue: "application/json",
        },
    },
    OnWhenPurchasePaid: {
        contentType: {
            name: "Content-Type",
            type: "string",
            required: true,
            defaultValue: "application/json",
        },
    },
    OnWhenRequestUpdated: {
        contentType: {
            name: "Content-Type",
            type: "string",
            required: true,
            defaultValue: "application/json",
        },
    },
} as const;

// #region Client

/**
 * Typed client for the impexium connector.
 */
export class ImpexiumClient extends ConnectorClientBase {

    /**
     * Creates a new ImpexiumClient.
     * @param connectionRuntimeUrl The connection runtime URL from Azure Portal.
     * @param tokenProvider The token provider for authentication.
     * @param options Optional connector client options.
     */
    constructor(connectionRuntimeUrl: string, tokenProvider: TokenProvider, options?: ConnectorClientOptions) {
        super(connectionRuntimeUrl, tokenProvider, options);
    }

    public get connectorName(): string {
        return "impexium";
    }

    /**
     * Get Abandoned Checkouts
     * @remarks Get Abandoned Checkouts
     */
    public async getAbandonedCheckoutsAsync(pageNumber: string, abandonedFrom?: string, productCode?: string, customerRecordNumber?: string, abortSignal?: AbortSignal): Promise<GetAbandonedCheckoutsResponse> {
        const queryParams: string[] = [];
        if (abandonedFrom !== undefined) {
            queryParams.push(`abandonedFrom=${encodeURIComponent(String(abandonedFrom))}`);
        }
        if (productCode !== undefined) {
            queryParams.push(`productCode=${encodeURIComponent(String(productCode))}`);
        }
        if (customerRecordNumber !== undefined) {
            queryParams.push(`customerRecordNumber=${encodeURIComponent(String(customerRecordNumber))}`);
        }
        const requestPath = `/api/v1/Shopping/AbandonedCheckOuts/${pageNumber}` + (queryParams.length > 0 ? "?" + queryParams.join("&") : "");
        const requestUrl = this.resolveUrl(requestPath);
        const httpResponse = await this.httpClient.sendAsync<GetAbandonedCheckoutsResponse>("GET", requestUrl, undefined, undefined, abortSignal);

        if (!httpResponse.isSuccessStatusCode) {
            throw new ConnectorException(this.connectorName, `GET ${requestPath}`, httpResponse.statusCode, httpResponse.text);
        }

        return httpResponse.value as GetAbandonedCheckoutsResponse;
    }

    /**
     * List All Exhibitors
     * @remarks List All Exhibitors
     */
    public async listAllExhibitorsAsync(exhibitCode: string, pageNumber: string, abortSignal?: AbortSignal): Promise<ListAllExhibitorsResponse> {
        const requestPath = `/api/v1/Exhibits/${exhibitCode}/Exhibitors/${pageNumber}`;
        const requestUrl = this.resolveUrl(requestPath);
        const httpResponse = await this.httpClient.sendAsync<ListAllExhibitorsResponse>("GET", requestUrl, undefined, undefined, abortSignal);

        if (!httpResponse.isSuccessStatusCode) {
            throw new ConnectorException(this.connectorName, `GET ${requestPath}`, httpResponse.statusCode, httpResponse.text);
        }

        return httpResponse.value as ListAllExhibitorsResponse;
    }

    /**
     * List of Exams
     * @remarks List of Exams
     */
    public async listOfExamsAsync(pageNumber: string, code?: string, categoryName?: string, isPublic?: string, changedSince?: string, tag?: string, includePrices?: string, abortSignal?: AbortSignal): Promise<ListOfExamsResponse> {
        const queryParams: string[] = [];
        if (code !== undefined) {
            queryParams.push(`Code=${encodeURIComponent(String(code))}`);
        }
        if (categoryName !== undefined) {
            queryParams.push(`categoryName=${encodeURIComponent(String(categoryName))}`);
        }
        if (isPublic !== undefined) {
            queryParams.push(`isPublic=${encodeURIComponent(String(isPublic))}`);
        }
        if (changedSince !== undefined) {
            queryParams.push(`changedSince=${encodeURIComponent(String(changedSince))}`);
        }
        if (tag !== undefined) {
            queryParams.push(`Tag=${encodeURIComponent(String(tag))}`);
        }
        if (includePrices !== undefined) {
            queryParams.push(`includePrices=${encodeURIComponent(String(includePrices))}`);
        }
        const requestPath = `/api/v1/Products/Exams/${pageNumber}` + (queryParams.length > 0 ? "?" + queryParams.join("&") : "");
        const requestUrl = this.resolveUrl(requestPath);
        const httpResponse = await this.httpClient.sendAsync<ListOfExamsResponse>("GET", requestUrl, undefined, undefined, abortSignal);

        if (!httpResponse.isSuccessStatusCode) {
            throw new ConnectorException(this.connectorName, `GET ${requestPath}`, httpResponse.statusCode, httpResponse.text);
        }

        return httpResponse.value as ListOfExamsResponse;
    }

    /**
     * List Registrants
     * @remarks List Registrants
     */
    public async listRegistrantsAsync(eventCode: string, pageNumber: string, sessionCode?: string, includeDetails?: string, registeredSince?: string, abortSignal?: AbortSignal): Promise<ListRegistrantsResponse> {
        const queryParams: string[] = [];
        if (sessionCode !== undefined) {
            queryParams.push(`sessionCode=${encodeURIComponent(String(sessionCode))}`);
        }
        if (includeDetails !== undefined) {
            queryParams.push(`includeDetails=${encodeURIComponent(String(includeDetails))}`);
        }
        if (registeredSince !== undefined) {
            queryParams.push(`registeredSince=${encodeURIComponent(String(registeredSince))}`);
        }
        const requestPath = `/api/v1/Events/${eventCode}/Registrations/${pageNumber}` + (queryParams.length > 0 ? "?" + queryParams.join("&") : "");
        const requestUrl = this.resolveUrl(requestPath);
        const httpResponse = await this.httpClient.sendAsync<ListRegistrantsResponse>("GET", requestUrl, undefined, undefined, abortSignal);

        if (!httpResponse.isSuccessStatusCode) {
            throw new ConnectorException(this.connectorName, `GET ${requestPath}`, httpResponse.statusCode, httpResponse.text);
        }

        return httpResponse.value as ListRegistrantsResponse;
    }

    /**
     * Get Course Attendees
     * @remarks Get Course Attendees
     */
    public async getCourseAttendeesAsync(code: string, pageNumber: string, abortSignal?: AbortSignal): Promise<GetCourseAttendeesResponse> {
        const requestPath = `/api/v1/Courses/${code}/Attendees/${pageNumber}`;
        const requestUrl = this.resolveUrl(requestPath);
        const httpResponse = await this.httpClient.sendAsync<GetCourseAttendeesResponse>("GET", requestUrl, undefined, undefined, abortSignal);

        if (!httpResponse.isSuccessStatusCode) {
            throw new ConnectorException(this.connectorName, `GET ${requestPath}`, httpResponse.statusCode, httpResponse.text);
        }

        return httpResponse.value as GetCourseAttendeesResponse;
    }

    /**
     * Add Exam Scores
     * @remarks Add Exam Scores
     */
    public async addExamScoresAsync(input: AddExamScoresInput, examCode: string, abortSignal?: AbortSignal): Promise<Array<ExamScoreResultData>> {
        const requestPath = `/api/v1/Exams/${examCode}/Scores`;
        const requestUrl = this.resolveUrl(requestPath);
        const httpResponse = await this.httpClient.sendAsync<Array<ExamScoreResultData>>("POST", requestUrl, undefined, input, abortSignal);

        if (!httpResponse.isSuccessStatusCode) {
            throw new ConnectorException(this.connectorName, `POST ${requestPath}`, httpResponse.statusCode, httpResponse.text);
        }

        return httpResponse.value as Array<ExamScoreResultData>;
    }

    /**
     * Find Members by Name
     * @remarks Find Members by Name
     */
    public async findMembersByNameAsync(name: string, pageNumber: string, abortSignal?: AbortSignal): Promise<FindMembersByNameResponse> {
        const requestPath = `/api/v1/Customers/Members/FindByName/${name}/${pageNumber}`;
        const requestUrl = this.resolveUrl(requestPath);
        const httpResponse = await this.httpClient.sendAsync<FindMembersByNameResponse>("GET", requestUrl, undefined, undefined, abortSignal);

        if (!httpResponse.isSuccessStatusCode) {
            throw new ConnectorException(this.connectorName, `GET ${requestPath}`, httpResponse.statusCode, httpResponse.text);
        }

        return httpResponse.value as FindMembersByNameResponse;
    }

    /**
     * Get Purchases for an Individual
     * @remarks Get Purchases for an Individual
     */
    public async getPurchasesForAnIndividualAsync(idOrRecordNumber: string, pageNumber: string, productCode?: string, purchasedSince?: string, productCategoryCode?: string, abortSignal?: AbortSignal): Promise<GetPurchasesForAnIndividualResponse> {
        const queryParams: string[] = [];
        if (productCode !== undefined) {
            queryParams.push(`productCode=${encodeURIComponent(String(productCode))}`);
        }
        if (purchasedSince !== undefined) {
            queryParams.push(`purchasedSince=${encodeURIComponent(String(purchasedSince))}`);
        }
        if (productCategoryCode !== undefined) {
            queryParams.push(`productCategoryCode=${encodeURIComponent(String(productCategoryCode))}`);
        }
        const requestPath = `/api/v1/Individuals/${idOrRecordNumber}/Purchases/${pageNumber}` + (queryParams.length > 0 ? "?" + queryParams.join("&") : "");
        const requestUrl = this.resolveUrl(requestPath);
        const httpResponse = await this.httpClient.sendAsync<GetPurchasesForAnIndividualResponse>("GET", requestUrl, undefined, undefined, abortSignal);

        if (!httpResponse.isSuccessStatusCode) {
            throw new ConnectorException(this.connectorName, `GET ${requestPath}`, httpResponse.statusCode, httpResponse.text);
        }

        return httpResponse.value as GetPurchasesForAnIndividualResponse;
    }

    /**
     * Add or Update a List of Custom Fields Per Organization
     * @remarks Add or Update a List of Custom Fields Per Organization
     */
    public async addOrUpdateAListOfCustomFieldsPerOrganizationAsync(input: AddOrUpdateAListOfCustomFieldsPerOrganizationInput, idOrRecordNumber: string, abortSignal?: AbortSignal): Promise<Array<CustomFieldResultData>> {
        const requestPath = `/api/v1/Organizations/${idOrRecordNumber}/CustomFieldsList`;
        const requestUrl = this.resolveUrl(requestPath);
        const httpResponse = await this.httpClient.sendAsync<Array<CustomFieldResultData>>("POST", requestUrl, undefined, input, abortSignal);

        if (!httpResponse.isSuccessStatusCode) {
            throw new ConnectorException(this.connectorName, `POST ${requestPath}`, httpResponse.statusCode, httpResponse.text);
        }

        return httpResponse.value as Array<CustomFieldResultData>;
    }

    /**
     * Add Nominee
     * @remarks Add Nominee
     */
    public async addNomineeAsync(input: CommitteeNomineeSaveData, code: string, abortSignal?: AbortSignal): Promise<void> {
        const requestPath = `/api/v1/Committees/${code}/Nominations`;
        const requestUrl = this.resolveUrl(requestPath);
        const httpResponse = await this.httpClient.sendAsync<void>("POST", requestUrl, undefined, input, abortSignal);

        if (!httpResponse.isSuccessStatusCode) {
            throw new ConnectorException(this.connectorName, `POST ${requestPath}`, httpResponse.statusCode, httpResponse.text);
        }
    }

    /**
     * Get Individual Custom Field Values
     * @remarks Get Individual Custom Field Values
     */
    public async getIndividualCustomFieldValuesAsync(id: string, abortSignal?: AbortSignal): Promise<Array<CustomFieldData>> {
        const requestPath = `/api/v1/Individuals/${id}/CustomFields`;
        const requestUrl = this.resolveUrl(requestPath);
        const httpResponse = await this.httpClient.sendAsync<Array<CustomFieldData>>("GET", requestUrl, undefined, undefined, abortSignal);

        if (!httpResponse.isSuccessStatusCode) {
            throw new ConnectorException(this.connectorName, `GET ${requestPath}`, httpResponse.statusCode, httpResponse.text);
        }

        return httpResponse.value as Array<CustomFieldData>;
    }

    /**
     * Update Individual Custom Field Values
     * @remarks Update Individual Custom Field Values.
     */
    public async updateCustomFieldValueAsync(input: CustomFieldData, id: string, abortSignal?: AbortSignal): Promise<void> {
        const requestPath = `/api/v1/Individuals/${id}/CustomFields`;
        const requestUrl = this.resolveUrl(requestPath);
        const httpResponse = await this.httpClient.sendAsync<void>("POST", requestUrl, undefined, input, abortSignal);

        if (!httpResponse.isSuccessStatusCode) {
            throw new ConnectorException(this.connectorName, `POST ${requestPath}`, httpResponse.statusCode, httpResponse.text);
        }
    }

    /**
     * List All Event Cancellations by Event
     * @remarks List All Event Cancellations by Event
     */
    public async listAllEventCancellationsByEventAsync(eventCode: string, pageNumber: string, includeDetails?: string, cancelledSince?: string, abortSignal?: AbortSignal): Promise<ListAllEventCancellationsByEventResponse> {
        const queryParams: string[] = [];
        if (includeDetails !== undefined) {
            queryParams.push(`includeDetails=${encodeURIComponent(String(includeDetails))}`);
        }
        if (cancelledSince !== undefined) {
            queryParams.push(`cancelledSince=${encodeURIComponent(String(cancelledSince))}`);
        }
        const requestPath = `/api/v1/Events/${eventCode}/Cancellations/${pageNumber}` + (queryParams.length > 0 ? "?" + queryParams.join("&") : "");
        const requestUrl = this.resolveUrl(requestPath);
        const httpResponse = await this.httpClient.sendAsync<ListAllEventCancellationsByEventResponse>("GET", requestUrl, undefined, undefined, abortSignal);

        if (!httpResponse.isSuccessStatusCode) {
            throw new ConnectorException(this.connectorName, `GET ${requestPath}`, httpResponse.statusCode, httpResponse.text);
        }

        return httpResponse.value as ListAllEventCancellationsByEventResponse;
    }

    /**
     * Get All Open Orders for an Individual
     * @remarks Get All Open Orders for an Individual
     */
    public async getAllOpenOrdersForAnIndividualAsync(idOrRecordNumber: string, pageNumber: string, includeLineItems?: string, fromDate?: string, toDate?: string, abortSignal?: AbortSignal): Promise<GetAllOpenOrdersForAnIndividualResponse> {
        const queryParams: string[] = [];
        if (includeLineItems !== undefined) {
            queryParams.push(`includeLineItems=${encodeURIComponent(String(includeLineItems))}`);
        }
        if (fromDate !== undefined) {
            queryParams.push(`fromDate=${encodeURIComponent(String(fromDate))}`);
        }
        if (toDate !== undefined) {
            queryParams.push(`toDate=${encodeURIComponent(String(toDate))}`);
        }
        const requestPath = `/api/v1/Individuals/${idOrRecordNumber}/Orders/Open/${pageNumber}` + (queryParams.length > 0 ? "?" + queryParams.join("&") : "");
        const requestUrl = this.resolveUrl(requestPath);
        const httpResponse = await this.httpClient.sendAsync<GetAllOpenOrdersForAnIndividualResponse>("GET", requestUrl, undefined, undefined, abortSignal);

        if (!httpResponse.isSuccessStatusCode) {
            throw new ConnectorException(this.connectorName, `GET ${requestPath}`, httpResponse.statusCode, httpResponse.text);
        }

        return httpResponse.value as GetAllOpenOrdersForAnIndividualResponse;
    }

    /**
     * List Completed User Tasks by User ID or Email
     * @remarks List Completed User Tasks by User ID or Email
     */
    public async listCompletedUserTasksByUserIdOrEmailAsync(userIdOrEmail: string, pageNumber: string, abortSignal?: AbortSignal): Promise<ListCompletedUserTasksByUserIdOrEmailResponse> {
        const requestPath = `/api/v1/tasks/Users/${userIdOrEmail}/Completed/${pageNumber}`;
        const requestUrl = this.resolveUrl(requestPath);
        const httpResponse = await this.httpClient.sendAsync<ListCompletedUserTasksByUserIdOrEmailResponse>("GET", requestUrl, undefined, undefined, abortSignal);

        if (!httpResponse.isSuccessStatusCode) {
            throw new ConnectorException(this.connectorName, `GET ${requestPath}`, httpResponse.statusCode, httpResponse.text);
        }

        return httpResponse.value as ListCompletedUserTasksByUserIdOrEmailResponse;
    }

    /**
     * List Pending User Tasks by User ID or Email
     * @remarks List Pending User Tasks by User ID or Email
     */
    public async listPendingUserTasksByUserIdOrEmailAsync(userIdOrEmail: string, pageNumber: string, abortSignal?: AbortSignal): Promise<ListPendingUserTasksByUserIdOrEmailResponse> {
        const requestPath = `/api/v1/tasks/Users/${userIdOrEmail}/Pending/${pageNumber}`;
        const requestUrl = this.resolveUrl(requestPath);
        const httpResponse = await this.httpClient.sendAsync<ListPendingUserTasksByUserIdOrEmailResponse>("GET", requestUrl, undefined, undefined, abortSignal);

        if (!httpResponse.isSuccessStatusCode) {
            throw new ConnectorException(this.connectorName, `GET ${requestPath}`, httpResponse.statusCode, httpResponse.text);
        }

        return httpResponse.value as ListPendingUserTasksByUserIdOrEmailResponse;
    }

    /**
     * Add Note to Sales Opportunity
     * @remarks Add Note to Sales Opportunity
     */
    public async addNoteToSalesOpportunityAsync(input: BaseNoteData, id: string, abortSignal?: AbortSignal): Promise<void> {
        const requestPath = `/api/v1/Sales/Opportunities/${id}/Notes`;
        const requestUrl = this.resolveUrl(requestPath);
        const httpResponse = await this.httpClient.sendAsync<void>("POST", requestUrl, undefined, input, abortSignal);

        if (!httpResponse.isSuccessStatusCode) {
            throw new ConnectorException(this.connectorName, `POST ${requestPath}`, httpResponse.statusCode, httpResponse.text);
        }
    }

    /**
     * Add Activity to Sales Opportunity
     * @remarks Add Activity to Sales Opportunity
     */
    public async addActivityToSalesOpportunityAsync(input: ActivityData, id: string, abortSignal?: AbortSignal): Promise<void> {
        const requestPath = `/api/v1/Sales/Opportunities/${id}/Activities`;
        const requestUrl = this.resolveUrl(requestPath);
        const httpResponse = await this.httpClient.sendAsync<void>("POST", requestUrl, undefined, input, abortSignal);

        if (!httpResponse.isSuccessStatusCode) {
            throw new ConnectorException(this.connectorName, `POST ${requestPath}`, httpResponse.statusCode, httpResponse.text);
        }
    }

    /**
     * Update Task by Task Number
     * @remarks Update Task by Task Number
     */
    public async updateTaskByTaskNumberAsync(input: TaskSaveData, taskNumber: string, abortSignal?: AbortSignal): Promise<TaskData> {
        const requestPath = `/api/v1/tasks/${taskNumber}`;
        const requestUrl = this.resolveUrl(requestPath);
        const httpResponse = await this.httpClient.sendAsync<TaskData>("PUT", requestUrl, undefined, input, abortSignal);

        if (!httpResponse.isSuccessStatusCode) {
            throw new ConnectorException(this.connectorName, `PUT ${requestPath}`, httpResponse.statusCode, httpResponse.text);
        }

        return httpResponse.value as TaskData;
    }

    /**
     * List All Countries
     * @remarks List All Countries
     */
    public async listAllCountriesAsync(pageNumber: string, abortSignal?: AbortSignal): Promise<ListAllCountriesResponse> {
        const requestPath = `/api/v1/Countries/All/${pageNumber}`;
        const requestUrl = this.resolveUrl(requestPath);
        const httpResponse = await this.httpClient.sendAsync<ListAllCountriesResponse>("GET", requestUrl, undefined, undefined, abortSignal);

        if (!httpResponse.isSuccessStatusCode) {
            throw new ConnectorException(this.connectorName, `GET ${requestPath}`, httpResponse.statusCode, httpResponse.text);
        }

        return httpResponse.value as ListAllCountriesResponse;
    }

    /**
     * Get All States by Country
     * @remarks Get All States by Country
     */
    public async getAllStatesByCountryAsync(countryId: string, pageNumber: string, abortSignal?: AbortSignal): Promise<GetAllStatesByCountryResponse> {
        const requestPath = `/api/v1/Countries/${countryId}/States/All/${pageNumber}`;
        const requestUrl = this.resolveUrl(requestPath);
        const httpResponse = await this.httpClient.sendAsync<GetAllStatesByCountryResponse>("GET", requestUrl, undefined, undefined, abortSignal);

        if (!httpResponse.isSuccessStatusCode) {
            throw new ConnectorException(this.connectorName, `GET ${requestPath}`, httpResponse.statusCode, httpResponse.text);
        }

        return httpResponse.value as GetAllStatesByCountryResponse;
    }

    /**
     * List All Exhibits
     * @remarks List All Exhibits
     */
    public async listAllExhibitsAsync(pageNumber: string, abortSignal?: AbortSignal): Promise<ListAllExhibitsResponse> {
        const requestPath = `/api/v1/Exhibits/${pageNumber}`;
        const requestUrl = this.resolveUrl(requestPath);
        const httpResponse = await this.httpClient.sendAsync<ListAllExhibitsResponse>("GET", requestUrl, undefined, undefined, abortSignal);

        if (!httpResponse.isSuccessStatusCode) {
            throw new ConnectorException(this.connectorName, `GET ${requestPath}`, httpResponse.statusCode, httpResponse.text);
        }

        return httpResponse.value as ListAllExhibitsResponse;
    }

    /**
     * Delete a Category for an Organization
     * @remarks Delete a Category for an Organization
     */
    public async deleteACategoryForAnOrganizationAsync(recordNumber: string, categoryCode: string, abortSignal?: AbortSignal): Promise<void> {
        const requestPath = `/api/v1/Organizations/${recordNumber}/Categories/${categoryCode}`;
        const requestUrl = this.resolveUrl(requestPath);
        const httpResponse = await this.httpClient.sendAsync<void>("DELETE", requestUrl, undefined, undefined, abortSignal);

        if (!httpResponse.isSuccessStatusCode) {
            throw new ConnectorException(this.connectorName, `DELETE ${requestPath}`, httpResponse.statusCode, httpResponse.text);
        }
    }

    /**
     * Add Customer Request
     * @remarks Add Customer Request
     */
    public async addCustomerRequestAsync(input: RequestSaveData, abortSignal?: AbortSignal): Promise<void> {
        const requestPath = `/api/v1/Requests`;
        const requestUrl = this.resolveUrl(requestPath);
        const httpResponse = await this.httpClient.sendAsync<void>("POST", requestUrl, undefined, input, abortSignal);

        if (!httpResponse.isSuccessStatusCode) {
            throw new ConnectorException(this.connectorName, `POST ${requestPath}`, httpResponse.statusCode, httpResponse.text);
        }
    }

    /**
     * Update Customer Request
     * @remarks Update Customer Request
     */
    public async updateCustomerRequestAsync(input: RequestUpdateData, abortSignal?: AbortSignal): Promise<RequestUpdateData> {
        const requestPath = `/api/v1/Requests`;
        const requestUrl = this.resolveUrl(requestPath);
        const httpResponse = await this.httpClient.sendAsync<RequestUpdateData>("PUT", requestUrl, undefined, input, abortSignal);

        if (!httpResponse.isSuccessStatusCode) {
            throw new ConnectorException(this.connectorName, `PUT ${requestPath}`, httpResponse.statusCode, httpResponse.text);
        }

        return httpResponse.value as RequestUpdateData;
    }

    /**
     * Add Categories for an Organization
     * @remarks Add Categories for an Organization
     */
    public async addCategoriesForAnOrganizationAsync(input: AddCategoriesForAnOrganizationInput, recordNumber: string, abortSignal?: AbortSignal): Promise<void> {
        const requestPath = `/api/v1/Organizations/${recordNumber}/Categories`;
        const requestUrl = this.resolveUrl(requestPath);
        const httpResponse = await this.httpClient.sendAsync<void>("POST", requestUrl, undefined, input, abortSignal);

        if (!httpResponse.isSuccessStatusCode) {
            throw new ConnectorException(this.connectorName, `POST ${requestPath}`, httpResponse.statusCode, httpResponse.text);
        }
    }

    /**
     * List of Customer Relationships
     * @remarks List of Customer Relationships
     */
    public async listOfCustomerRelationshipsAsync(pageNumber: string, abortSignal?: AbortSignal): Promise<ListOfCustomerRelationshipsResponse> {
        const requestPath = `/api/v1/Customers/RelationshipTypes/${pageNumber}`;
        const requestUrl = this.resolveUrl(requestPath);
        const httpResponse = await this.httpClient.sendAsync<ListOfCustomerRelationshipsResponse>("GET", requestUrl, undefined, undefined, abortSignal);

        if (!httpResponse.isSuccessStatusCode) {
            throw new ConnectorException(this.connectorName, `GET ${requestPath}`, httpResponse.statusCode, httpResponse.text);
        }

        return httpResponse.value as ListOfCustomerRelationshipsResponse;
    }

    /**
     * List All Open Customer Request
     * @remarks List All Open Customer Request
     */
    public async listAllOpenCustomerRequestAsync(pageNumber: string, abortSignal?: AbortSignal): Promise<ListAllOpenCustomerRequestResponse> {
        const requestPath = `/api/v1/Requests/Open/${pageNumber}`;
        const requestUrl = this.resolveUrl(requestPath);
        const httpResponse = await this.httpClient.sendAsync<ListAllOpenCustomerRequestResponse>("GET", requestUrl, undefined, undefined, abortSignal);

        if (!httpResponse.isSuccessStatusCode) {
            throw new ConnectorException(this.connectorName, `GET ${requestPath}`, httpResponse.statusCode, httpResponse.text);
        }

        return httpResponse.value as ListAllOpenCustomerRequestResponse;
    }

    /**
     * Get Organization Inactive Memberships
     * @remarks Get Organization Inactive Memberships
     */
    public async getOrganizationInactiveMembershipsAsync(id: string, abortSignal?: AbortSignal): Promise<MembershipData> {
        const requestPath = `/api/v1/Organizations/${id}/Memberships/Inactive`;
        const requestUrl = this.resolveUrl(requestPath);
        const httpResponse = await this.httpClient.sendAsync<MembershipData>("GET", requestUrl, undefined, undefined, abortSignal);

        if (!httpResponse.isSuccessStatusCode) {
            throw new ConnectorException(this.connectorName, `GET ${requestPath}`, httpResponse.statusCode, httpResponse.text);
        }

        return httpResponse.value as MembershipData;
    }

    /**
     * Delete Record From Custom Data Table
     * @remarks Delete Record From Custom Data Table
     */
    public async deleteRecordFromCustomDataTableAsync(tableName: string, id: string, abortSignal?: AbortSignal): Promise<void> {
        const requestPath = `/api/v1/CustomData/${tableName}/${id}`;
        const requestUrl = this.resolveUrl(requestPath);
        const httpResponse = await this.httpClient.sendAsync<void>("DELETE", requestUrl, undefined, undefined, abortSignal);

        if (!httpResponse.isSuccessStatusCode) {
            throw new ConnectorException(this.connectorName, `DELETE ${requestPath}`, httpResponse.statusCode, httpResponse.text);
        }
    }

    /**
     * Update User Task Progress or Mark as Completed
     * @remarks Update User Task Progress or Mark as Completed
     */
    public async updateUserTaskProgressOrMarkAsCompletedAsync(input: UserTaskData, userIdOrEmail: string, abortSignal?: AbortSignal): Promise<UserTaskData> {
        const requestPath = `/api/v1/tasks/Users/${userIdOrEmail}`;
        const requestUrl = this.resolveUrl(requestPath);
        const httpResponse = await this.httpClient.sendAsync<UserTaskData>("PUT", requestUrl, undefined, input, abortSignal);

        if (!httpResponse.isSuccessStatusCode) {
            throw new ConnectorException(this.connectorName, `PUT ${requestPath}`, httpResponse.statusCode, httpResponse.text);
        }

        return httpResponse.value as UserTaskData;
    }

    /**
     * Find Members (Individuals) by First Name
     * @remarks Find Members (Individuals) by First Name
     */
    public async findMembersOrIndividualsByFirstNameAsync(firstName: string, pageNumber: string, includeEmail?: string, abortSignal?: AbortSignal): Promise<IndividualData> {
        const queryParams: string[] = [];
        if (includeEmail !== undefined) {
            queryParams.push(`includeEmail=${encodeURIComponent(String(includeEmail))}`);
        }
        const requestPath = `/api/v1/Individuals/Members/FindByFirstName/${firstName}/${pageNumber}` + (queryParams.length > 0 ? "?" + queryParams.join("&") : "");
        const requestUrl = this.resolveUrl(requestPath);
        const httpResponse = await this.httpClient.sendAsync<IndividualData>("GET", requestUrl, undefined, undefined, abortSignal);

        if (!httpResponse.isSuccessStatusCode) {
            throw new ConnectorException(this.connectorName, `GET ${requestPath}`, httpResponse.statusCode, httpResponse.text);
        }

        return httpResponse.value as IndividualData;
    }

    /**
     * Find Members (Individuals) by Last Name
     * @remarks Find Members (Individuals) by Last Name
     */
    public async findMembersOrIndividualsByLastNameAsync(lastName: string, pageNumber: string, includeEmail?: string, abortSignal?: AbortSignal): Promise<IndividualData> {
        const queryParams: string[] = [];
        if (includeEmail !== undefined) {
            queryParams.push(`includeEmail=${encodeURIComponent(String(includeEmail))}`);
        }
        const requestPath = `/api/v1/Individuals/Members/FindByLastName/${lastName}/${pageNumber}` + (queryParams.length > 0 ? "?" + queryParams.join("&") : "");
        const requestUrl = this.resolveUrl(requestPath);
        const httpResponse = await this.httpClient.sendAsync<IndividualData>("GET", requestUrl, undefined, undefined, abortSignal);

        if (!httpResponse.isSuccessStatusCode) {
            throw new ConnectorException(this.connectorName, `GET ${requestPath}`, httpResponse.statusCode, httpResponse.text);
        }

        return httpResponse.value as IndividualData;
    }

    /**
     * Create a new task or Assign Task to a User
     * @remarks Create a new task or Assign Task to a User
     */
    public async assignTaskToAUserAsync(input: UserTaskData, userIdOrEmail: string, abortSignal?: AbortSignal): Promise<void> {
        const requestPath = `/api/v1/tasks/Users/${userIdOrEmail}/Task`;
        const requestUrl = this.resolveUrl(requestPath);
        const httpResponse = await this.httpClient.sendAsync<void>("POST", requestUrl, undefined, input, abortSignal);

        if (!httpResponse.isSuccessStatusCode) {
            throw new ConnectorException(this.connectorName, `POST ${requestPath}`, httpResponse.statusCode, httpResponse.text);
        }
    }

    /**
     * Delete a Category for an Individual
     * @remarks Delete a Category for an Individual
     */
    public async deleteACategoryForAnIndividualAsync(recordNumber: string, categoryCode: string, abortSignal?: AbortSignal): Promise<void> {
        const requestPath = `/api/v1/Individuals/${recordNumber}/Categories/${categoryCode}`;
        const requestUrl = this.resolveUrl(requestPath);
        const httpResponse = await this.httpClient.sendAsync<void>("DELETE", requestUrl, undefined, undefined, abortSignal);

        if (!httpResponse.isSuccessStatusCode) {
            throw new ConnectorException(this.connectorName, `DELETE ${requestPath}`, httpResponse.statusCode, httpResponse.text);
        }
    }

    /**
     * Add Notification to Individual
     * @remarks Add Notification to Individual
     */
    public async addNotificationToIndividualAsync(input: NotificationData, id: string, abortSignal?: AbortSignal): Promise<void> {
        const requestPath = `/api/v1/Individuals/${id}/Notifications`;
        const requestUrl = this.resolveUrl(requestPath);
        const httpResponse = await this.httpClient.sendAsync<void>("POST", requestUrl, undefined, input, abortSignal);

        if (!httpResponse.isSuccessStatusCode) {
            throw new ConnectorException(this.connectorName, `POST ${requestPath}`, httpResponse.statusCode, httpResponse.text);
        }
    }

    /**
     * Add Categories for an Individual
     * @remarks Add Categories for an Individual
     */
    public async addCategoriesForAnIndividualAsync(input: AddCategoriesForAnIndividualInput, recordNumber: string, abortSignal?: AbortSignal): Promise<void> {
        const requestPath = `/api/v1/Individuals/${recordNumber}/Categories`;
        const requestUrl = this.resolveUrl(requestPath);
        const httpResponse = await this.httpClient.sendAsync<void>("POST", requestUrl, undefined, input, abortSignal);

        if (!httpResponse.isSuccessStatusCode) {
            throw new ConnectorException(this.connectorName, `POST ${requestPath}`, httpResponse.statusCode, httpResponse.text);
        }
    }

    /**
     * Add a New Task
     * @remarks Add a New Task
     */
    public async addANewTaskAsync(input: TaskSaveData, abortSignal?: AbortSignal): Promise<TaskData> {
        const requestPath = `/api/v1/tasks`;
        const requestUrl = this.resolveUrl(requestPath);
        const httpResponse = await this.httpClient.sendAsync<TaskData>("POST", requestUrl, undefined, input, abortSignal);

        if (!httpResponse.isSuccessStatusCode) {
            throw new ConnectorException(this.connectorName, `POST ${requestPath}`, httpResponse.statusCode, httpResponse.text);
        }

        return httpResponse.value as TaskData;
    }

    /**
     * List of All Organization Members
     * @remarks List of All Organization Members
     */
    public async listOfAllOrganizationMembersAsync(pageNumber: string, zipCode?: string, radius?: string, stateAbbreviation?: string, congressionalDistrict?: string, membershipTypeCode?: string, membershipTypeCategory?: string, city?: string, name?: string, tag?: string, latitude?: string, longitude?: string, domain?: string, includeMembership?: string, includeAddress?: string, includePhone?: string, includeEmail?: string, includeCustomFields?: string, expiringFrom?: string, expiringTo?: string, abortSignal?: AbortSignal): Promise<ListOfAllOrganizationMembersResponse> {
        const queryParams: string[] = [];
        if (zipCode !== undefined) {
            queryParams.push(`zipCode=${encodeURIComponent(String(zipCode))}`);
        }
        if (radius !== undefined) {
            queryParams.push(`Radius=${encodeURIComponent(String(radius))}`);
        }
        if (stateAbbreviation !== undefined) {
            queryParams.push(`stateAbbreviation=${encodeURIComponent(String(stateAbbreviation))}`);
        }
        if (congressionalDistrict !== undefined) {
            queryParams.push(`congressionalDistrict=${encodeURIComponent(String(congressionalDistrict))}`);
        }
        if (membershipTypeCode !== undefined) {
            queryParams.push(`membershipTypeCode=${encodeURIComponent(String(membershipTypeCode))}`);
        }
        if (membershipTypeCategory !== undefined) {
            queryParams.push(`membershipTypeCategory=${encodeURIComponent(String(membershipTypeCategory))}`);
        }
        if (city !== undefined) {
            queryParams.push(`City=${encodeURIComponent(String(city))}`);
        }
        if (name !== undefined) {
            queryParams.push(`Name=${encodeURIComponent(String(name))}`);
        }
        if (tag !== undefined) {
            queryParams.push(`Tag=${encodeURIComponent(String(tag))}`);
        }
        if (latitude !== undefined) {
            queryParams.push(`Latitude=${encodeURIComponent(String(latitude))}`);
        }
        if (longitude !== undefined) {
            queryParams.push(`Longitude=${encodeURIComponent(String(longitude))}`);
        }
        if (domain !== undefined) {
            queryParams.push(`Domain=${encodeURIComponent(String(domain))}`);
        }
        if (includeMembership !== undefined) {
            queryParams.push(`includeMembership=${encodeURIComponent(String(includeMembership))}`);
        }
        if (includeAddress !== undefined) {
            queryParams.push(`includeAddress=${encodeURIComponent(String(includeAddress))}`);
        }
        if (includePhone !== undefined) {
            queryParams.push(`includePhone=${encodeURIComponent(String(includePhone))}`);
        }
        if (includeEmail !== undefined) {
            queryParams.push(`includeEmail=${encodeURIComponent(String(includeEmail))}`);
        }
        if (includeCustomFields !== undefined) {
            queryParams.push(`includeCustomFields=${encodeURIComponent(String(includeCustomFields))}`);
        }
        if (expiringFrom !== undefined) {
            queryParams.push(`expiringFrom=${encodeURIComponent(String(expiringFrom))}`);
        }
        if (expiringTo !== undefined) {
            queryParams.push(`expiringTo=${encodeURIComponent(String(expiringTo))}`);
        }
        const requestPath = `/api/v1/Organizations/Members/All/${pageNumber}` + (queryParams.length > 0 ? "?" + queryParams.join("&") : "");
        const requestUrl = this.resolveUrl(requestPath);
        const httpResponse = await this.httpClient.sendAsync<ListOfAllOrganizationMembersResponse>("GET", requestUrl, undefined, undefined, abortSignal);

        if (!httpResponse.isSuccessStatusCode) {
            throw new ConnectorException(this.connectorName, `GET ${requestPath}`, httpResponse.statusCode, httpResponse.text);
        }

        return httpResponse.value as ListOfAllOrganizationMembersResponse;
    }

    /**
     * List of All Individual Members
     * @remarks List of All Individual Members
     */
    public async listOfAllIndividualMembersAsync(pageNumber: string, zipCode?: string, radius?: string, membershipTypeCode?: string, membershipTypeCategory?: string, tag?: string, includeMembership?: string, includeAddress?: string, includePhone?: string, includeEmail?: string, includeLink?: string, includeCustomFields?: string, includeCategories?: string, includeMembershipRenewalUrl?: string, expiringFrom?: string, expiringTo?: string, abortSignal?: AbortSignal): Promise<ListOfAllIndividualMembersResponse> {
        const queryParams: string[] = [];
        if (zipCode !== undefined) {
            queryParams.push(`zipCode=${encodeURIComponent(String(zipCode))}`);
        }
        if (radius !== undefined) {
            queryParams.push(`Radius=${encodeURIComponent(String(radius))}`);
        }
        if (membershipTypeCode !== undefined) {
            queryParams.push(`membershipTypeCode=${encodeURIComponent(String(membershipTypeCode))}`);
        }
        if (membershipTypeCategory !== undefined) {
            queryParams.push(`membershipTypeCategory=${encodeURIComponent(String(membershipTypeCategory))}`);
        }
        if (tag !== undefined) {
            queryParams.push(`Tag=${encodeURIComponent(String(tag))}`);
        }
        if (includeMembership !== undefined) {
            queryParams.push(`includeMembership=${encodeURIComponent(String(includeMembership))}`);
        }
        if (includeAddress !== undefined) {
            queryParams.push(`includeAddress=${encodeURIComponent(String(includeAddress))}`);
        }
        if (includePhone !== undefined) {
            queryParams.push(`includePhone=${encodeURIComponent(String(includePhone))}`);
        }
        if (includeEmail !== undefined) {
            queryParams.push(`includeEmail=${encodeURIComponent(String(includeEmail))}`);
        }
        if (includeLink !== undefined) {
            queryParams.push(`includeLink=${encodeURIComponent(String(includeLink))}`);
        }
        if (includeCustomFields !== undefined) {
            queryParams.push(`includeCustomFields=${encodeURIComponent(String(includeCustomFields))}`);
        }
        if (includeCategories !== undefined) {
            queryParams.push(`includeCategories=${encodeURIComponent(String(includeCategories))}`);
        }
        if (includeMembershipRenewalUrl !== undefined) {
            queryParams.push(`includeMembershipRenewalUrl=${encodeURIComponent(String(includeMembershipRenewalUrl))}`);
        }
        if (expiringFrom !== undefined) {
            queryParams.push(`expiringFrom=${encodeURIComponent(String(expiringFrom))}`);
        }
        if (expiringTo !== undefined) {
            queryParams.push(`expiringTo=${encodeURIComponent(String(expiringTo))}`);
        }
        const requestPath = `/api/v1/Individuals/Members/All/${pageNumber}` + (queryParams.length > 0 ? "?" + queryParams.join("&") : "");
        const requestUrl = this.resolveUrl(requestPath);
        const httpResponse = await this.httpClient.sendAsync<ListOfAllIndividualMembersResponse>("GET", requestUrl, undefined, undefined, abortSignal);

        if (!httpResponse.isSuccessStatusCode) {
            throw new ConnectorException(this.connectorName, `GET ${requestPath}`, httpResponse.statusCode, httpResponse.text);
        }

        return httpResponse.value as ListOfAllIndividualMembersResponse;
    }

    /**
     * Get List of Active Certifications for an Organization
     * @remarks Get List of Active Certifications for an Organization
     */
    public async getListOfActiveCertificationsForAnOrganizationAsync(idOrRecordNumber: string, pageNumber: string, abortSignal?: AbortSignal): Promise<GetListOfActiveCertificationsForAnOrganizationResponse> {
        const requestPath = `/api/v1/Organizations/${idOrRecordNumber}/Certifications/${pageNumber}`;
        const requestUrl = this.resolveUrl(requestPath);
        const httpResponse = await this.httpClient.sendAsync<GetListOfActiveCertificationsForAnOrganizationResponse>("GET", requestUrl, undefined, undefined, abortSignal);

        if (!httpResponse.isSuccessStatusCode) {
            throw new ConnectorException(this.connectorName, `GET ${requestPath}`, httpResponse.statusCode, httpResponse.text);
        }

        return httpResponse.value as GetListOfActiveCertificationsForAnOrganizationResponse;
    }

    /**
     * Get List of Active Certifications for an Individual
     * @remarks Get List of Active Certifications for an Individual
     */
    public async getListOfActiveCertificationsForAnIndividualAsync(idOrRecordNumber: string, pageNumber: string, abortSignal?: AbortSignal): Promise<GetListOfActiveCertificationsForAnIndividualResponse> {
        const requestPath = `/api/v1/Individuals/${idOrRecordNumber}/Certifications/${pageNumber}`;
        const requestUrl = this.resolveUrl(requestPath);
        const httpResponse = await this.httpClient.sendAsync<GetListOfActiveCertificationsForAnIndividualResponse>("GET", requestUrl, undefined, undefined, abortSignal);

        if (!httpResponse.isSuccessStatusCode) {
            throw new ConnectorException(this.connectorName, `GET ${requestPath}`, httpResponse.statusCode, httpResponse.text);
        }

        return httpResponse.value as GetListOfActiveCertificationsForAnIndividualResponse;
    }

    /**
     * Get Individual Inactive Memberships
     * @remarks Get Individual Inactive Memberships
     */
    public async getIndividualInactiveMembershipsAsync(id: string, abortSignal?: AbortSignal): Promise<MembershipData> {
        const requestPath = `/api/v1/Individuals/${id}/Memberships/Inactive`;
        const requestUrl = this.resolveUrl(requestPath);
        const httpResponse = await this.httpClient.sendAsync<MembershipData>("GET", requestUrl, undefined, undefined, abortSignal);

        if (!httpResponse.isSuccessStatusCode) {
            throw new ConnectorException(this.connectorName, `GET ${requestPath}`, httpResponse.statusCode, httpResponse.text);
        }

        return httpResponse.value as MembershipData;
    }

    /**
     * Add Activity to Organization
     * @remarks Add Activity to Organization
     */
    public async addActivityToOrganizationAsync(input: ActivityData, id: string, abortSignal?: AbortSignal): Promise<void> {
        const requestPath = `/api/v1/Organizations/${id}/Activities`;
        const requestUrl = this.resolveUrl(requestPath);
        const httpResponse = await this.httpClient.sendAsync<void>("POST", requestUrl, undefined, input, abortSignal);

        if (!httpResponse.isSuccessStatusCode) {
            throw new ConnectorException(this.connectorName, `POST ${requestPath}`, httpResponse.statusCode, httpResponse.text);
        }
    }

    /**
     * Register an Individual for a Free Session
     * @remarks Register an Individual for a Free Session
     */
    public async registerAnIndividualForAFreeSessionAsync(input: RegisterAnIndividualForAFreeSessionInput, eventCode: string, customerIdOrRecordNumber: string, registrationNumber?: string, abortSignal?: AbortSignal): Promise<void> {
        const queryParams: string[] = [];
        if (registrationNumber !== undefined) {
            queryParams.push(`registrationNumber=${encodeURIComponent(String(registrationNumber))}`);
        }
        const requestPath = `/api/v1/Events/${eventCode}/Sessions/Register/${customerIdOrRecordNumber}` + (queryParams.length > 0 ? "?" + queryParams.join("&") : "");
        const requestUrl = this.resolveUrl(requestPath);
        const httpResponse = await this.httpClient.sendAsync<void>("POST", requestUrl, undefined, input, abortSignal);

        if (!httpResponse.isSuccessStatusCode) {
            throw new ConnectorException(this.connectorName, `POST ${requestPath}`, httpResponse.statusCode, httpResponse.text);
        }
    }

    /**
     * Get a List of Licenses
     * @remarks Get a List of Licenses
     */
    public async getAListOfLicensesAsync(id: string, pageNumber: string, abortSignal?: AbortSignal): Promise<GetAListOfLicensesResponse> {
        const requestPath = `/api/v1/Individuals/${id}/Licenses/${pageNumber}`;
        const requestUrl = this.resolveUrl(requestPath);
        const httpResponse = await this.httpClient.sendAsync<GetAListOfLicensesResponse>("GET", requestUrl, undefined, undefined, abortSignal);

        if (!httpResponse.isSuccessStatusCode) {
            throw new ConnectorException(this.connectorName, `GET ${requestPath}`, httpResponse.statusCode, httpResponse.text);
        }

        return httpResponse.value as GetAListOfLicensesResponse;
    }

    /**
     * List All Awards
     * @remarks List All Awards
     */
    public async listAllAwardsAsync(pageNumber: string, year?: string, abortSignal?: AbortSignal): Promise<ListAllAwardsResponse> {
        const queryParams: string[] = [];
        if (year !== undefined) {
            queryParams.push(`Year=${encodeURIComponent(String(year))}`);
        }
        const requestPath = `/api/v1/Awards/${pageNumber}` + (queryParams.length > 0 ? "?" + queryParams.join("&") : "");
        const requestUrl = this.resolveUrl(requestPath);
        const httpResponse = await this.httpClient.sendAsync<ListAllAwardsResponse>("GET", requestUrl, undefined, undefined, abortSignal);

        if (!httpResponse.isSuccessStatusCode) {
            throw new ConnectorException(this.connectorName, `GET ${requestPath}`, httpResponse.statusCode, httpResponse.text);
        }

        return httpResponse.value as ListAllAwardsResponse;
    }

    /**
     * Find Members (Individuals) by Name
     * @remarks Find Members (Individuals) by Name
     */
    public async findMembersOrIndividualsByNameAsync(name: string, pageNumber: string, includeEmail?: string, abortSignal?: AbortSignal): Promise<FindMembersOrIndividualsByNameResponse> {
        const queryParams: string[] = [];
        if (includeEmail !== undefined) {
            queryParams.push(`includeEmail=${encodeURIComponent(String(includeEmail))}`);
        }
        const requestPath = `/api/v1/Individuals/Members/FindByName/${name}/${pageNumber}` + (queryParams.length > 0 ? "?" + queryParams.join("&") : "");
        const requestUrl = this.resolveUrl(requestPath);
        const httpResponse = await this.httpClient.sendAsync<FindMembersOrIndividualsByNameResponse>("GET", requestUrl, undefined, undefined, abortSignal);

        if (!httpResponse.isSuccessStatusCode) {
            throw new ConnectorException(this.connectorName, `GET ${requestPath}`, httpResponse.statusCode, httpResponse.text);
        }

        return httpResponse.value as FindMembersOrIndividualsByNameResponse;
    }

    /**
     * Get a List of All Services of an Organization
     * @remarks Get a List of All Services of an Organization
     */
    public async getAListOfAllServicesOfAnOrganizationAsync(id: string, abortSignal?: AbortSignal): Promise<GetAListOfAllServicesOfAnOrganizationResponse> {
        const requestPath = `/api/v1/Organizations/${id}/Services`;
        const requestUrl = this.resolveUrl(requestPath);
        const httpResponse = await this.httpClient.sendAsync<GetAListOfAllServicesOfAnOrganizationResponse>("GET", requestUrl, undefined, undefined, abortSignal);

        if (!httpResponse.isSuccessStatusCode) {
            throw new ConnectorException(this.connectorName, `GET ${requestPath}`, httpResponse.statusCode, httpResponse.text);
        }

        return httpResponse.value as GetAListOfAllServicesOfAnOrganizationResponse;
    }

    /**
     * Add a Service to an Organization
     * @remarks Add a Service to an Organization
     */
    public async addAServiceToAnOrganizationAsync(input: ServiceData, id: string, abortSignal?: AbortSignal): Promise<ServiceData> {
        const requestPath = `/api/v1/Organizations/${id}/Services`;
        const requestUrl = this.resolveUrl(requestPath);
        const httpResponse = await this.httpClient.sendAsync<ServiceData>("POST", requestUrl, undefined, input, abortSignal);

        if (!httpResponse.isSuccessStatusCode) {
            throw new ConnectorException(this.connectorName, `POST ${requestPath}`, httpResponse.statusCode, httpResponse.text);
        }

        return httpResponse.value as ServiceData;
    }

    /**
     * Update Phone for an Individual
     * @remarks Update Phone for an Individual
     */
    public async updatePhoneForAnIndividualAsync(input: PhoneSaveData, idOrRecordNumber: string, id: string, abortSignal?: AbortSignal): Promise<PhoneDataSet> {
        const requestPath = `/api/v1/Individuals/${idOrRecordNumber}/Phones/${id}`;
        const requestUrl = this.resolveUrl(requestPath);
        const httpResponse = await this.httpClient.sendAsync<PhoneDataSet>("PUT", requestUrl, undefined, input, abortSignal);

        if (!httpResponse.isSuccessStatusCode) {
            throw new ConnectorException(this.connectorName, `PUT ${requestPath}`, httpResponse.statusCode, httpResponse.text);
        }

        return httpResponse.value as PhoneDataSet;
    }

    /**
     * Update Phone for an Organization
     * @remarks Update Phone for an Organization
     */
    public async updatePhoneForAnOrganizationAsync(input: PhoneSaveData, idOrRecordNumber: string, id: string, abortSignal?: AbortSignal): Promise<PhoneDataSet> {
        const requestPath = `/api/v1/Organizations/${idOrRecordNumber}/Phones/${id}`;
        const requestUrl = this.resolveUrl(requestPath);
        const httpResponse = await this.httpClient.sendAsync<PhoneDataSet>("PUT", requestUrl, undefined, input, abortSignal);

        if (!httpResponse.isSuccessStatusCode) {
            throw new ConnectorException(this.connectorName, `PUT ${requestPath}`, httpResponse.statusCode, httpResponse.text);
        }

        return httpResponse.value as PhoneDataSet;
    }

    /**
     * Delete an Individual Web Link
     * @remarks Delete an Individual Web Link
     */
    public async deleteAnIndividualWebLinkAsync(input: DeleteAnIndividualWebLinkInput, idOrRecordNumber: string, abortSignal?: AbortSignal): Promise<void> {
        const requestPath = `/api/v1/Individuals/${idOrRecordNumber}/Links`;
        const requestUrl = this.resolveUrl(requestPath);
        const httpResponse = await this.httpClient.sendAsync<void>("DELETE", requestUrl, undefined, input, abortSignal);

        if (!httpResponse.isSuccessStatusCode) {
            throw new ConnectorException(this.connectorName, `DELETE ${requestPath}`, httpResponse.statusCode, httpResponse.text);
        }
    }

    /**
     * Add Web Link for Individual
     * @remarks Add Web Link for Individual
     */
    public async addWebLinkForIndividualAsync(input: AddWebLinkForIndividualInput, idOrRecordNumber: string, abortSignal?: AbortSignal): Promise<void> {
        const requestPath = `/api/v1/Individuals/${idOrRecordNumber}/Links`;
        const requestUrl = this.resolveUrl(requestPath);
        const httpResponse = await this.httpClient.sendAsync<void>("POST", requestUrl, undefined, input, abortSignal);

        if (!httpResponse.isSuccessStatusCode) {
            throw new ConnectorException(this.connectorName, `POST ${requestPath}`, httpResponse.statusCode, httpResponse.text);
        }
    }

    /**
     * Add Phone to Organization
     * @remarks Add Phone to Organization
     */
    public async addPhoneToOrganizationAsync(input: PhoneSaveData, idOrRecordNumber: string, abortSignal?: AbortSignal): Promise<PhoneDataSet> {
        const requestPath = `/api/v1/Organizations/${idOrRecordNumber}/Phones`;
        const requestUrl = this.resolveUrl(requestPath);
        const httpResponse = await this.httpClient.sendAsync<PhoneDataSet>("POST", requestUrl, undefined, input, abortSignal);

        if (!httpResponse.isSuccessStatusCode) {
            throw new ConnectorException(this.connectorName, `POST ${requestPath}`, httpResponse.statusCode, httpResponse.text);
        }

        return httpResponse.value as PhoneDataSet;
    }

    /**
     * Get Nominees by Committee
     * @remarks Get Nominees by Committee
     */
    public async getNomineesByCommitteeAsync(code: string, pageNumber: string, term?: string, abortSignal?: AbortSignal): Promise<GetNomineesByCommitteeResponse> {
        const queryParams: string[] = [];
        if (term !== undefined) {
            queryParams.push(`Term=${encodeURIComponent(String(term))}`);
        }
        const requestPath = `/api/v1/Committees/${code}/Nominations/${pageNumber}` + (queryParams.length > 0 ? "?" + queryParams.join("&") : "");
        const requestUrl = this.resolveUrl(requestPath);
        const httpResponse = await this.httpClient.sendAsync<GetNomineesByCommitteeResponse>("GET", requestUrl, undefined, undefined, abortSignal);

        if (!httpResponse.isSuccessStatusCode) {
            throw new ConnectorException(this.connectorName, `GET ${requestPath}`, httpResponse.statusCode, httpResponse.text);
        }

        return httpResponse.value as GetNomineesByCommitteeResponse;
    }

    /**
     * Get Organization's Active Subscriptions
     * @remarks Get Organization's Active Subscriptions
     */
    public async getOrganizationsActiveSubscriptionsAsync(id: string, pageNumber: string, abortSignal?: AbortSignal): Promise<GetOrganizationsActiveSubscriptionsResponse> {
        const requestPath = `/api/v1/Organizations/${id}/Subscriptions/${pageNumber}`;
        const requestUrl = this.resolveUrl(requestPath);
        const httpResponse = await this.httpClient.sendAsync<GetOrganizationsActiveSubscriptionsResponse>("GET", requestUrl, undefined, undefined, abortSignal);

        if (!httpResponse.isSuccessStatusCode) {
            throw new ConnectorException(this.connectorName, `GET ${requestPath}`, httpResponse.statusCode, httpResponse.text);
        }

        return httpResponse.value as GetOrganizationsActiveSubscriptionsResponse;
    }

    /**
     * Delete an Organization Web Link
     * @remarks Delete an Organization Web Link
     */
    public async deleteAnOrganizationWebLinkAsync(input: DeleteAnOrganizationWebLinkInput, idOrRecordNumber: string, abortSignal?: AbortSignal): Promise<void> {
        const requestPath = `/api/v1/Organizations/${idOrRecordNumber}/Links`;
        const requestUrl = this.resolveUrl(requestPath);
        const httpResponse = await this.httpClient.sendAsync<void>("DELETE", requestUrl, undefined, input, abortSignal);

        if (!httpResponse.isSuccessStatusCode) {
            throw new ConnectorException(this.connectorName, `DELETE ${requestPath}`, httpResponse.statusCode, httpResponse.text);
        }
    }

    /**
     * Add Web Link for Organization
     * @remarks Add Web Link for Organization
     */
    public async addWebLinkForOrganizationAsync(input: AddWebLinkForOrganizationInput, idOrRecordNumber: string, abortSignal?: AbortSignal): Promise<void> {
        const requestPath = `/api/v1/Organizations/${idOrRecordNumber}/Links`;
        const requestUrl = this.resolveUrl(requestPath);
        const httpResponse = await this.httpClient.sendAsync<void>("POST", requestUrl, undefined, input, abortSignal);

        if (!httpResponse.isSuccessStatusCode) {
            throw new ConnectorException(this.connectorName, `POST ${requestPath}`, httpResponse.statusCode, httpResponse.text);
        }
    }

    /**
     * Add Email to Organization
     * @remarks Add Email to Organization
     */
    public async addEmailToOrganizationAsync(input: EmailData, id: string, abortSignal?: AbortSignal): Promise<void> {
        const requestPath = `/api/v1/Organizations/${id}/Emails`;
        const requestUrl = this.resolveUrl(requestPath);
        const httpResponse = await this.httpClient.sendAsync<void>("POST", requestUrl, undefined, input, abortSignal);

        if (!httpResponse.isSuccessStatusCode) {
            throw new ConnectorException(this.connectorName, `POST ${requestPath}`, httpResponse.statusCode, httpResponse.text);
        }
    }

    /**
     * Get Organization's Relationships
     * @remarks Get Organization's Relationships
     */
    public async getOrganizationsRelationshipsAsync(id: string, pageNumber: string, relationshipName?: string, includesDetails?: string, abortSignal?: AbortSignal): Promise<GetOrganizationsRelationshipsResponse> {
        const queryParams: string[] = [];
        if (relationshipName !== undefined) {
            queryParams.push(`relationshipName.=${encodeURIComponent(String(relationshipName))}`);
        }
        if (includesDetails !== undefined) {
            queryParams.push(`includesDetails=${encodeURIComponent(String(includesDetails))}`);
        }
        const requestPath = `/api/v1/Organizations/${id}/Relationships/${pageNumber}` + (queryParams.length > 0 ? "?" + queryParams.join("&") : "");
        const requestUrl = this.resolveUrl(requestPath);
        const httpResponse = await this.httpClient.sendAsync<GetOrganizationsRelationshipsResponse>("GET", requestUrl, undefined, undefined, abortSignal);

        if (!httpResponse.isSuccessStatusCode) {
            throw new ConnectorException(this.connectorName, `GET ${requestPath}`, httpResponse.statusCode, httpResponse.text);
        }

        return httpResponse.value as GetOrganizationsRelationshipsResponse;
    }

    /**
     * Add or Update Address to Organization
     * @remarks Add or Update Address to Organization
     */
    public async addOrUpdateAddressToOrganizationAsync(input: AddressSaveData, idOrRecordNumber: string, abortSignal?: AbortSignal): Promise<AddressSaveData> {
        const requestPath = `/api/v1/Organizations/${idOrRecordNumber}/Addresses`;
        const requestUrl = this.resolveUrl(requestPath);
        const httpResponse = await this.httpClient.sendAsync<AddressSaveData>("POST", requestUrl, undefined, input, abortSignal);

        if (!httpResponse.isSuccessStatusCode) {
            throw new ConnectorException(this.connectorName, `POST ${requestPath}`, httpResponse.statusCode, httpResponse.text);
        }

        return httpResponse.value as AddressSaveData;
    }

    /**
     * Add or Update a List of Custom Fields Per Individual
     * @remarks Add or Update a List of Custom Fields Per Individual
     */
    public async addOrUpdateAListOfCustomFieldsPerIndividualAsync(input: AddOrUpdateAListOfCustomFieldsPerIndividualInput, idOrRecordNumber: string, abortSignal?: AbortSignal): Promise<Array<CustomFieldResultData>> {
        const requestPath = `/api/v1/Individuals/${idOrRecordNumber}/CustomFieldsList`;
        const requestUrl = this.resolveUrl(requestPath);
        const httpResponse = await this.httpClient.sendAsync<Array<CustomFieldResultData>>("POST", requestUrl, undefined, input, abortSignal);

        if (!httpResponse.isSuccessStatusCode) {
            throw new ConnectorException(this.connectorName, `POST ${requestPath}`, httpResponse.statusCode, httpResponse.text);
        }

        return httpResponse.value as Array<CustomFieldResultData>;
    }

    /**
     * Get Upcoming Events
     * @remarks Get Upcoming Events
     */
    public async getUpcomingEventsAsync(pageNumber: string, abortSignal?: AbortSignal): Promise<GetUpcomingEventsResponse> {
        const requestPath = `/api/v1/Events/Upcoming/${pageNumber}`;
        const requestUrl = this.resolveUrl(requestPath);
        const httpResponse = await this.httpClient.sendAsync<GetUpcomingEventsResponse>("GET", requestUrl, undefined, undefined, abortSignal);

        if (!httpResponse.isSuccessStatusCode) {
            throw new ConnectorException(this.connectorName, `GET ${requestPath}`, httpResponse.statusCode, httpResponse.text);
        }

        return httpResponse.value as GetUpcomingEventsResponse;
    }

    /**
     * Add Phone to Individual
     * @remarks Add Phone to Individual
     */
    public async addPhoneToIndividualAsync(input: PhoneSaveData, idOrRecordNumber: string, abortSignal?: AbortSignal): Promise<PhoneDataSet> {
        const requestPath = `/api/v1/Individuals/${idOrRecordNumber}/Phones`;
        const requestUrl = this.resolveUrl(requestPath);
        const httpResponse = await this.httpClient.sendAsync<PhoneDataSet>("POST", requestUrl, undefined, input, abortSignal);

        if (!httpResponse.isSuccessStatusCode) {
            throw new ConnectorException(this.connectorName, `POST ${requestPath}`, httpResponse.statusCode, httpResponse.text);
        }

        return httpResponse.value as PhoneDataSet;
    }

    /**
     * Get Committee Information for an Individual
     * @remarks Get Committee Information for an Individual
     */
    public async getCommitteeInformationForAnIndividualAsync(id: string, pageNumber: string, includeInactive?: string, abortSignal?: AbortSignal): Promise<GetCommitteeInformationForAnIndividualResponse> {
        const queryParams: string[] = [];
        if (includeInactive !== undefined) {
            queryParams.push(`includeInactive=${encodeURIComponent(String(includeInactive))}`);
        }
        const requestPath = `/api/v1/Individuals/${id}/Committees/${pageNumber}` + (queryParams.length > 0 ? "?" + queryParams.join("&") : "");
        const requestUrl = this.resolveUrl(requestPath);
        const httpResponse = await this.httpClient.sendAsync<GetCommitteeInformationForAnIndividualResponse>("GET", requestUrl, undefined, undefined, abortSignal);

        if (!httpResponse.isSuccessStatusCode) {
            throw new ConnectorException(this.connectorName, `GET ${requestPath}`, httpResponse.statusCode, httpResponse.text);
        }

        return httpResponse.value as GetCommitteeInformationForAnIndividualResponse;
    }

    /**
     * Get Organization Custom Field Values
     * @remarks Get Organization Custom Field Values
     */
    public async getOrganizationCustomFieldValuesAsync(id: string, abortSignal?: AbortSignal): Promise<Array<CustomFieldData>> {
        const requestPath = `/api/v1/Organizations/${id}/CustomFields`;
        const requestUrl = this.resolveUrl(requestPath);
        const httpResponse = await this.httpClient.sendAsync<Array<CustomFieldData>>("GET", requestUrl, undefined, undefined, abortSignal);

        if (!httpResponse.isSuccessStatusCode) {
            throw new ConnectorException(this.connectorName, `GET ${requestPath}`, httpResponse.statusCode, httpResponse.text);
        }

        return httpResponse.value as Array<CustomFieldData>;
    }

    /**
     * Get Organization Active Memberships
     * @remarks Get Organization Active Memberships
     */
    public async getOrganizationActiveMembershipsAsync(id: string, abortSignal?: AbortSignal): Promise<Array<MembershipData>> {
        const requestPath = `/api/v1/Organizations/${id}/Memberships/Active`;
        const requestUrl = this.resolveUrl(requestPath);
        const httpResponse = await this.httpClient.sendAsync<Array<MembershipData>>("GET", requestUrl, undefined, undefined, abortSignal);

        if (!httpResponse.isSuccessStatusCode) {
            throw new ConnectorException(this.connectorName, `GET ${requestPath}`, httpResponse.statusCode, httpResponse.text);
        }

        return httpResponse.value as Array<MembershipData>;
    }

    /**
     * Get All Events
     * @remarks Get All Events
     */
    public async getAllEventsAsync(pageNumber: string, code?: string, name?: string, tag?: string, abortSignal?: AbortSignal): Promise<GetAllEventsResponse> {
        const queryParams: string[] = [];
        if (code !== undefined) {
            queryParams.push(`Code=${encodeURIComponent(String(code))}`);
        }
        if (name !== undefined) {
            queryParams.push(`Name=${encodeURIComponent(String(name))}`);
        }
        if (tag !== undefined) {
            queryParams.push(`Tag=${encodeURIComponent(String(tag))}`);
        }
        const requestPath = `/api/v1/Events/All/${pageNumber}` + (queryParams.length > 0 ? "?" + queryParams.join("&") : "");
        const requestUrl = this.resolveUrl(requestPath);
        const httpResponse = await this.httpClient.sendAsync<GetAllEventsResponse>("GET", requestUrl, undefined, undefined, abortSignal);

        if (!httpResponse.isSuccessStatusCode) {
            throw new ConnectorException(this.connectorName, `GET ${requestPath}`, httpResponse.statusCode, httpResponse.text);
        }

        return httpResponse.value as GetAllEventsResponse;
    }

    /**
     * Get Individual Active Memberships
     * @remarks Get Individual Active Memberships
     */
    public async getIndividualActiveMembershipsAsync(id: string, abortSignal?: AbortSignal): Promise<Array<MembershipData>> {
        const requestPath = `/api/v1/Individuals/${id}/Memberships/Active`;
        const requestUrl = this.resolveUrl(requestPath);
        const httpResponse = await this.httpClient.sendAsync<Array<MembershipData>>("GET", requestUrl, undefined, undefined, abortSignal);

        if (!httpResponse.isSuccessStatusCode) {
            throw new ConnectorException(this.connectorName, `GET ${requestPath}`, httpResponse.statusCode, httpResponse.text);
        }

        return httpResponse.value as Array<MembershipData>;
    }

    /**
     * Get All Event Registrations Information for an Individual
     * @remarks Get All Event Registrations Information for an Individual
     */
    public async getAllEventRegistrationsInformationForAnIndividualAsync(idOrRecordNumber: string, pageNumber: string, eventCode?: string, abortSignal?: AbortSignal): Promise<GetAllEventRegistrationsInformationForAnIndividualResponse> {
        const queryParams: string[] = [];
        if (eventCode !== undefined) {
            queryParams.push(`eventCode=${encodeURIComponent(String(eventCode))}`);
        }
        const requestPath = `/api/v1/Individuals/${idOrRecordNumber}/Registrations/${pageNumber}` + (queryParams.length > 0 ? "?" + queryParams.join("&") : "");
        const requestUrl = this.resolveUrl(requestPath);
        const httpResponse = await this.httpClient.sendAsync<GetAllEventRegistrationsInformationForAnIndividualResponse>("GET", requestUrl, undefined, undefined, abortSignal);

        if (!httpResponse.isSuccessStatusCode) {
            throw new ConnectorException(this.connectorName, `GET ${requestPath}`, httpResponse.statusCode, httpResponse.text);
        }

        return httpResponse.value as GetAllEventRegistrationsInformationForAnIndividualResponse;
    }

    /**
     * Get Individual's Relationships
     * @remarks Get Individual's Relationships
     */
    public async getIndividualsRelationshipsAsync(id: string, pageNumber: string, relationshipName?: string, includeDetails?: string, abortSignal?: AbortSignal): Promise<GetIndividualsRelationshipsResponse> {
        const queryParams: string[] = [];
        if (relationshipName !== undefined) {
            queryParams.push(`relationshipName=${encodeURIComponent(String(relationshipName))}`);
        }
        if (includeDetails !== undefined) {
            queryParams.push(`includeDetails=${encodeURIComponent(String(includeDetails))}`);
        }
        const requestPath = `/api/v1/Individuals/${id}/Relationships/${pageNumber}` + (queryParams.length > 0 ? "?" + queryParams.join("&") : "");
        const requestUrl = this.resolveUrl(requestPath);
        const httpResponse = await this.httpClient.sendAsync<GetIndividualsRelationshipsResponse>("GET", requestUrl, undefined, undefined, abortSignal);

        if (!httpResponse.isSuccessStatusCode) {
            throw new ConnectorException(this.connectorName, `GET ${requestPath}`, httpResponse.statusCode, httpResponse.text);
        }

        return httpResponse.value as GetIndividualsRelationshipsResponse;
    }

    /**
     * Update an Individual Email
     * @remarks Update an Individual Email
     */
    public async updateAnIndividualEmailAsync(input: UpdateAnIndividualEmailInput, idOrRecordNumber: string, currentEmailAddress: string, abortSignal?: AbortSignal): Promise<void> {
        const requestPath = `/api/v1/Individuals/${idOrRecordNumber}/Emails/${currentEmailAddress}`;
        const requestUrl = this.resolveUrl(requestPath);
        const httpResponse = await this.httpClient.sendAsync<void>("PUT", requestUrl, undefined, input, abortSignal);

        if (!httpResponse.isSuccessStatusCode) {
            throw new ConnectorException(this.connectorName, `PUT ${requestPath}`, httpResponse.statusCode, httpResponse.text);
        }
    }

    /**
     * Save Relationship for Organization
     * @remarks Save Relationship for Organization
     */
    public async saveRelationshipForOrganizationAsync(input: SaveRelationshipForOrganizationInput, id: string, abortSignal?: AbortSignal): Promise<void> {
        const requestPath = `/api/v1/Organizations/${id}/Relationships`;
        const requestUrl = this.resolveUrl(requestPath);
        const httpResponse = await this.httpClient.sendAsync<void>("POST", requestUrl, undefined, input, abortSignal);

        if (!httpResponse.isSuccessStatusCode) {
            throw new ConnectorException(this.connectorName, `POST ${requestPath}`, httpResponse.statusCode, httpResponse.text);
        }
    }

    /**
     * Add Individual
     * @remarks Adds an Individual.
     */
    public async addIndividualAsync(input: AddIndividualInput, createUser?: string, abortSignal?: AbortSignal): Promise<IndividualData> {
        const queryParams: string[] = [];
        if (createUser !== undefined) {
            queryParams.push(`createUser=${encodeURIComponent(String(createUser))}`);
        }
        const requestPath = `/api/v1/Individuals` + (queryParams.length > 0 ? "?" + queryParams.join("&") : "");
        const requestUrl = this.resolveUrl(requestPath);
        const httpResponse = await this.httpClient.sendAsync<IndividualData>("POST", requestUrl, undefined, input, abortSignal);

        if (!httpResponse.isSuccessStatusCode) {
            throw new ConnectorException(this.connectorName, `POST ${requestPath}`, httpResponse.statusCode, httpResponse.text);
        }

        return httpResponse.value as IndividualData;
    }

    /**
     * Add Email to Individual
     * @remarks Add Email to Individual
     */
    public async addEmailToIndividualAsync(input: AddEmailToIndividualInput, idOrRecordNumber: string, abortSignal?: AbortSignal): Promise<EmailData> {
        const requestPath = `/api/v1/Individuals/${idOrRecordNumber}/Emails`;
        const requestUrl = this.resolveUrl(requestPath);
        const httpResponse = await this.httpClient.sendAsync<EmailData>("POST", requestUrl, undefined, input, abortSignal);

        if (!httpResponse.isSuccessStatusCode) {
            throw new ConnectorException(this.connectorName, `POST ${requestPath}`, httpResponse.statusCode, httpResponse.text);
        }

        return httpResponse.value as EmailData;
    }

    /**
     * Add or Update Address to Individual
     * @remarks Add or Update Address to Individual
     */
    public async addOrUpdateAddressToIndividualAsync(input: AddressSaveData, idOrRecordNumber: string, abortSignal?: AbortSignal): Promise<AddressSaveData> {
        const requestPath = `/api/v1/Individuals/${idOrRecordNumber}/Addresses`;
        const requestUrl = this.resolveUrl(requestPath);
        const httpResponse = await this.httpClient.sendAsync<AddressSaveData>("POST", requestUrl, undefined, input, abortSignal);

        if (!httpResponse.isSuccessStatusCode) {
            throw new ConnectorException(this.connectorName, `POST ${requestPath}`, httpResponse.statusCode, httpResponse.text);
        }

        return httpResponse.value as AddressSaveData;
    }

    /**
     * Find Members (Individuals) by First Name and Last Name
     * @remarks Find Members (Individuals) by First Name and Last Name
     */
    public async findMembersOrIndividualsByFirstAndLastNameAsync(firstName: string, lastName: string, pageNumber: string, includeEmail?: string, abortSignal?: AbortSignal): Promise<FindMembersOrIndividualsByFirstAndLastNameResponse> {
        const queryParams: string[] = [];
        if (includeEmail !== undefined) {
            queryParams.push(`includeEmail=${encodeURIComponent(String(includeEmail))}`);
        }
        const requestPath = `/api/v1/Individuals/Members/FindByName/${firstName}/${lastName}/${pageNumber}` + (queryParams.length > 0 ? "?" + queryParams.join("&") : "");
        const requestUrl = this.resolveUrl(requestPath);
        const httpResponse = await this.httpClient.sendAsync<FindMembersOrIndividualsByFirstAndLastNameResponse>("GET", requestUrl, undefined, undefined, abortSignal);

        if (!httpResponse.isSuccessStatusCode) {
            throw new ConnectorException(this.connectorName, `GET ${requestPath}`, httpResponse.statusCode, httpResponse.text);
        }

        return httpResponse.value as FindMembersOrIndividualsByFirstAndLastNameResponse;
    }

    /**
     * Get Committee Members by committee ID or code
     * @remarks Get Committee Members by Committee ID or Code
     */
    public async getCommitteeMembersByCommitteeIdOrCodeAsync(idOrCode: string, pageNumber: string, term?: string, positionCodes?: string, abortSignal?: AbortSignal): Promise<void> {
        const queryParams: string[] = [];
        if (term !== undefined) {
            queryParams.push(`Term=${encodeURIComponent(String(term))}`);
        }
        if (positionCodes !== undefined) {
            queryParams.push(`positionCodes=${encodeURIComponent(String(positionCodes))}`);
        }
        const requestPath = `/api/v1/Committees/${idOrCode}/Members/${pageNumber}` + (queryParams.length > 0 ? "?" + queryParams.join("&") : "");
        const requestUrl = this.resolveUrl(requestPath);
        const httpResponse = await this.httpClient.sendAsync<void>("GET", requestUrl, undefined, undefined, abortSignal);

        if (!httpResponse.isSuccessStatusCode) {
            throw new ConnectorException(this.connectorName, `GET ${requestPath}`, httpResponse.statusCode, httpResponse.text);
        }
    }

    /**
     * Add Activity
     * @remarks Add Activity.
     */
    public async addActivityAsync(input: AddActivityInput, id: string, abortSignal?: AbortSignal): Promise<void> {
        const requestPath = `/api/v1/Individuals/${id}/Activities`;
        const requestUrl = this.resolveUrl(requestPath);
        const httpResponse = await this.httpClient.sendAsync<void>("POST", requestUrl, undefined, input, abortSignal);

        if (!httpResponse.isSuccessStatusCode) {
            throw new ConnectorException(this.connectorName, `POST ${requestPath}`, httpResponse.statusCode, httpResponse.text);
        }
    }

    /**
     * Find Individual by Id or Email or Record Number
     * @remarks Find Individual by Id or Email.
     */
    public async findIndividualIdOrEmailAsync(idOrRecordNumberOrEmail: string, pageNumber: string, includeDetails?: string, abortSignal?: AbortSignal): Promise<FindIndividualIdOrEmailResponse> {
        const queryParams: string[] = [];
        if (includeDetails !== undefined) {
            queryParams.push(`IncludeDetails=${encodeURIComponent(String(includeDetails))}`);
        }
        const requestPath = `/api/v1/Individuals/Profile/${idOrRecordNumberOrEmail}/${pageNumber}/` + (queryParams.length > 0 ? "?" + queryParams.join("&") : "");
        const requestUrl = this.resolveUrl(requestPath);
        const httpResponse = await this.httpClient.sendAsync<FindIndividualIdOrEmailResponse>("GET", requestUrl, undefined, undefined, abortSignal);

        if (!httpResponse.isSuccessStatusCode) {
            throw new ConnectorException(this.connectorName, `GET ${requestPath}`, httpResponse.statusCode, httpResponse.text);
        }

        return httpResponse.value as FindIndividualIdOrEmailResponse;
    }

    /**
     * Add Relationship to Individual
     * @remarks Add Relationship to Individual.
     */
    public async addRelationshipToIndividualAsync(input: AddRelationshipToIndividualInput, id: string, abortSignal?: AbortSignal): Promise<void> {
        const requestPath = `/api/v1/Individuals/${id}/Relationships`;
        const requestUrl = this.resolveUrl(requestPath);
        const httpResponse = await this.httpClient.sendAsync<void>("POST", requestUrl, undefined, input, abortSignal);

        if (!httpResponse.isSuccessStatusCode) {
            throw new ConnectorException(this.connectorName, `POST ${requestPath}`, httpResponse.statusCode, httpResponse.text);
        }
    }

    /**
     * Add Education Credits to Individual
     * @remarks Add a new Education Credit to an Individual.
     */
    public async individualAddEducationCreditAsync(input: EducationCreditData, idOrRecordNumber: string, abortSignal?: AbortSignal): Promise<void> {
        const requestPath = `/api/v1/Individuals/${idOrRecordNumber}/EducationCredits`;
        const requestUrl = this.resolveUrl(requestPath);
        const httpResponse = await this.httpClient.sendAsync<void>("POST", requestUrl, undefined, input, abortSignal);

        if (!httpResponse.isSuccessStatusCode) {
            throw new ConnectorException(this.connectorName, `POST ${requestPath}`, httpResponse.statusCode, httpResponse.text);
        }
    }

    /**
     * Add Note to Individual
     * @remarks Add a Note to an Individual.
     */
    public async individualAddNoteAsync(input: NoteData, id: string, abortSignal?: AbortSignal): Promise<void> {
        const requestPath = `/api/v1/Individuals/${id}/Notes`;
        const requestUrl = this.resolveUrl(requestPath);
        const httpResponse = await this.httpClient.sendAsync<void>("POST", requestUrl, undefined, input, abortSignal);

        if (!httpResponse.isSuccessStatusCode) {
            throw new ConnectorException(this.connectorName, `POST ${requestPath}`, httpResponse.statusCode, httpResponse.text);
        }
    }

    /**
     * Lookup Individuals
     * @remarks Lookup Individuals by Name
     */
    public async individualsLookupByNameAsync(pageNumber: string, name?: string, includeOrgAddresses?: string, abortSignal?: AbortSignal): Promise<IndividualsLookupByNameResponse> {
        const queryParams: string[] = [];
        if (name !== undefined) {
            queryParams.push(`name=${encodeURIComponent(String(name))}`);
        }
        if (includeOrgAddresses !== undefined) {
            queryParams.push(`includeOrgAddresses=${encodeURIComponent(String(includeOrgAddresses))}`);
        }
        const requestPath = `/api/v1/Individuals/Lookup/${pageNumber}` + (queryParams.length > 0 ? "?" + queryParams.join("&") : "");
        const requestUrl = this.resolveUrl(requestPath);
        const httpResponse = await this.httpClient.sendAsync<IndividualsLookupByNameResponse>("GET", requestUrl, undefined, undefined, abortSignal);

        if (!httpResponse.isSuccessStatusCode) {
            throw new ConnectorException(this.connectorName, `GET ${requestPath}`, httpResponse.statusCode, httpResponse.text);
        }

        return httpResponse.value as IndividualsLookupByNameResponse;
    }

    /**
     * Add Individual to Committee
     * @remarks Add Individual to Committee.
     */
    public async addToCommitteeAsync(input: CommitteeMemberCreateData, code: string, abortSignal?: AbortSignal): Promise<AddToCommitteeResponse> {
        const requestPath = `/api/v1/Committees/${code}/Members`;
        const requestUrl = this.resolveUrl(requestPath);
        const httpResponse = await this.httpClient.sendAsync<AddToCommitteeResponse>("POST", requestUrl, undefined, input, abortSignal);

        if (!httpResponse.isSuccessStatusCode) {
            throw new ConnectorException(this.connectorName, `POST ${requestPath}`, httpResponse.statusCode, httpResponse.text);
        }

        return httpResponse.value as AddToCommitteeResponse;
    }

    /**
     * Update Committee Member
     * @remarks Update Committee Member.
     */
    public async updateCommitteeMemberAsync(input: CommitteeMemberUpdateData, code: string, memberRecordNumber: string, currentPositionCode: string, abortSignal?: AbortSignal): Promise<void> {
        const requestPath = `/api/v1/Committees/${code}/Members/${memberRecordNumber}/${currentPositionCode}`;
        const requestUrl = this.resolveUrl(requestPath);
        const httpResponse = await this.httpClient.sendAsync<void>("PUT", requestUrl, undefined, input, abortSignal);

        if (!httpResponse.isSuccessStatusCode) {
            throw new ConnectorException(this.connectorName, `PUT ${requestPath}`, httpResponse.statusCode, httpResponse.text);
        }
    }

    /**
     * Add Organization
     * @remarks Add Organization.
     */
    public async addOrganizationAsync(input: AddOrganizationInput, abortSignal?: AbortSignal): Promise<OrganizationData> {
        const requestPath = `/api/v1/Organizations`;
        const requestUrl = this.resolveUrl(requestPath);
        const httpResponse = await this.httpClient.sendAsync<OrganizationData>("POST", requestUrl, undefined, input, abortSignal);

        if (!httpResponse.isSuccessStatusCode) {
            throw new ConnectorException(this.connectorName, `POST ${requestPath}`, httpResponse.statusCode, httpResponse.text);
        }

        return httpResponse.value as OrganizationData;
    }

    /**
     * Update Organization
     * @remarks Update Organization.
     */
    public async updateOrganizationAsync(input: UpdateOrganizationInput, id: string, abortSignal?: AbortSignal): Promise<OrganizationData> {
        const requestPath = `/api/v1/Organizations/${id}`;
        const requestUrl = this.resolveUrl(requestPath);
        const httpResponse = await this.httpClient.sendAsync<OrganizationData>("PUT", requestUrl, undefined, input, abortSignal);

        if (!httpResponse.isSuccessStatusCode) {
            throw new ConnectorException(this.connectorName, `PUT ${requestPath}`, httpResponse.statusCode, httpResponse.text);
        }

        return httpResponse.value as OrganizationData;
    }

    /**
     * Find Organization by Id or Record Number
     * @remarks Get organization's profile information by ID or Record Number
     */
    public async organizationGetProfileAsync(idOrRecordnumber: string, pageNumber: string, includeDescription?: string, abortSignal?: AbortSignal): Promise<OrganizationData> {
        const queryParams: string[] = [];
        if (includeDescription !== undefined) {
            queryParams.push(`includeDescription=${encodeURIComponent(String(includeDescription))}`);
        }
        const requestPath = `/api/v1/Organizations/Profile/${idOrRecordnumber}/${pageNumber}` + (queryParams.length > 0 ? "?" + queryParams.join("&") : "");
        const requestUrl = this.resolveUrl(requestPath);
        const httpResponse = await this.httpClient.sendAsync<OrganizationData>("GET", requestUrl, undefined, undefined, abortSignal);

        if (!httpResponse.isSuccessStatusCode) {
            throw new ConnectorException(this.connectorName, `GET ${requestPath}`, httpResponse.statusCode, httpResponse.text);
        }

        return httpResponse.value as OrganizationData;
    }

    /**
     * Add Note to Organization
     * @remarks Add a Note to an Organization.
     */
    public async organizationAddNoteAsync(input: NoteData, id: string, abortSignal?: AbortSignal): Promise<void> {
        const requestPath = `/api/v1/Organizations/${id}/Notes`;
        const requestUrl = this.resolveUrl(requestPath);
        const httpResponse = await this.httpClient.sendAsync<void>("POST", requestUrl, undefined, input, abortSignal);

        if (!httpResponse.isSuccessStatusCode) {
            throw new ConnectorException(this.connectorName, `POST ${requestPath}`, httpResponse.statusCode, httpResponse.text);
        }
    }

    /**
     * Lookup Organizations
     * @remarks Lookup Organizations by Name or Acronym
     */
    public async organizationLookupByNameAsync(pageNumber: string, name?: string, includeAddresses?: string, abortSignal?: AbortSignal): Promise<OrganizationLookupByNameResponse> {
        const queryParams: string[] = [];
        if (name !== undefined) {
            queryParams.push(`name=${encodeURIComponent(String(name))}`);
        }
        if (includeAddresses !== undefined) {
            queryParams.push(`includeAddresses=${encodeURIComponent(String(includeAddresses))}`);
        }
        const requestPath = `/api/v1/Organizations/Lookup/${pageNumber}` + (queryParams.length > 0 ? "?" + queryParams.join("&") : "");
        const requestUrl = this.resolveUrl(requestPath);
        const httpResponse = await this.httpClient.sendAsync<OrganizationLookupByNameResponse>("GET", requestUrl, undefined, undefined, abortSignal);

        if (!httpResponse.isSuccessStatusCode) {
            throw new ConnectorException(this.connectorName, `GET ${requestPath}`, httpResponse.statusCode, httpResponse.text);
        }

        return httpResponse.value as OrganizationLookupByNameResponse;
    }

    /**
     * Get All Committees
     * @remarks Get All Committees
     */
    public async getAllCommitteesAsync(pageNumber: string, code?: string, name?: string, term?: string, activeOnly?: string, abortSignal?: AbortSignal): Promise<GetAllCommitteesResponse> {
        const queryParams: string[] = [];
        if (code !== undefined) {
            queryParams.push(`Code=${encodeURIComponent(String(code))}`);
        }
        if (name !== undefined) {
            queryParams.push(`Name=${encodeURIComponent(String(name))}`);
        }
        if (term !== undefined) {
            queryParams.push(`Term=${encodeURIComponent(String(term))}`);
        }
        if (activeOnly !== undefined) {
            queryParams.push(`activeOnly=${encodeURIComponent(String(activeOnly))}`);
        }
        const requestPath = `/api/v1/Committees/${pageNumber}` + (queryParams.length > 0 ? "?" + queryParams.join("&") : "");
        const requestUrl = this.resolveUrl(requestPath);
        const httpResponse = await this.httpClient.sendAsync<GetAllCommitteesResponse>("GET", requestUrl, undefined, undefined, abortSignal);

        if (!httpResponse.isSuccessStatusCode) {
            throw new ConnectorException(this.connectorName, `GET ${requestPath}`, httpResponse.statusCode, httpResponse.text);
        }

        return httpResponse.value as GetAllCommitteesResponse;
    }

    /**
     * Get Positions by Committee
     * @remarks Get Positions by Committee
     */
    public async getPositionsByCommitteeAsync(code: string, abortSignal?: AbortSignal): Promise<GetPositionsByCommitteeResponse> {
        const requestPath = `/api/v1/Committees/${code}/Positions`;
        const requestUrl = this.resolveUrl(requestPath);
        const httpResponse = await this.httpClient.sendAsync<GetPositionsByCommitteeResponse>("GET", requestUrl, undefined, undefined, abortSignal);

        if (!httpResponse.isSuccessStatusCode) {
            throw new ConnectorException(this.connectorName, `GET ${requestPath}`, httpResponse.statusCode, httpResponse.text);
        }

        return httpResponse.value as GetPositionsByCommitteeResponse;
    }

    /**
     * Get Sub-Committees
     * @remarks Get Sub-Committees
     */
    public async getSubCommitteesAsync(code: string, pageNumber: string, abortSignal?: AbortSignal): Promise<GetSubCommitteesResponse> {
        const requestPath = `/api/v1/Committees/${code}/subcommittees/${pageNumber}`;
        const requestUrl = this.resolveUrl(requestPath);
        const httpResponse = await this.httpClient.sendAsync<GetSubCommitteesResponse>("GET", requestUrl, undefined, undefined, abortSignal);

        if (!httpResponse.isSuccessStatusCode) {
            throw new ConnectorException(this.connectorName, `GET ${requestPath}`, httpResponse.statusCode, httpResponse.text);
        }

        return httpResponse.value as GetSubCommitteesResponse;
    }

    /**
     * Get Individual's Active Subscriptions
     * @remarks Get Individual's Active Subscriptions
     */
    public async getIndividualsActiveSubscriptionsAsync(id: string, pageNumber: string, abortSignal?: AbortSignal): Promise<GetIndividualsActiveSubscriptionsResponse> {
        const requestPath = `/api/v1/Individuals/${id}/Subscriptions/All/${pageNumber}`;
        const requestUrl = this.resolveUrl(requestPath);
        const httpResponse = await this.httpClient.sendAsync<GetIndividualsActiveSubscriptionsResponse>("GET", requestUrl, undefined, undefined, abortSignal);

        if (!httpResponse.isSuccessStatusCode) {
            throw new ConnectorException(this.connectorName, `GET ${requestPath}`, httpResponse.statusCode, httpResponse.text);
        }

        return httpResponse.value as GetIndividualsActiveSubscriptionsResponse;
    }

    /**
     * List all Individuals
     * @remarks List all Individuals
     */
    public async listAllIndividualsAsync(pageNumber: string, name?: string, includeDetails?: string, oldId?: string, abortSignal?: AbortSignal): Promise<ListAllIndividualsResponse> {
        const queryParams: string[] = [];
        if (name !== undefined) {
            queryParams.push(`Name=${encodeURIComponent(String(name))}`);
        }
        if (includeDetails !== undefined) {
            queryParams.push(`includeDetails=${encodeURIComponent(String(includeDetails))}`);
        }
        if (oldId !== undefined) {
            queryParams.push(`oldID=${encodeURIComponent(String(oldId))}`);
        }
        const requestPath = `/api/v1/Individuals/${pageNumber}` + (queryParams.length > 0 ? "?" + queryParams.join("&") : "");
        const requestUrl = this.resolveUrl(requestPath);
        const httpResponse = await this.httpClient.sendAsync<ListAllIndividualsResponse>("GET", requestUrl, undefined, undefined, abortSignal);

        if (!httpResponse.isSuccessStatusCode) {
            throw new ConnectorException(this.connectorName, `GET ${requestPath}`, httpResponse.statusCode, httpResponse.text);
        }

        return httpResponse.value as ListAllIndividualsResponse;
    }

    /**
     * Find Customer by Phone Number
     * @remarks Find Customer by Phone Number.
     */
    public async findCustomerPhoneAsync(pageNumber: string, phoneNumber?: string, includeAddress?: string, includePhone?: string, includeEmail?: string, abortSignal?: AbortSignal): Promise<FindCustomerPhoneResponse> {
        const queryParams: string[] = [];
        if (phoneNumber !== undefined) {
            queryParams.push(`phoneNumber=${encodeURIComponent(String(phoneNumber))}`);
        }
        if (includeAddress !== undefined) {
            queryParams.push(`includeAddress=${encodeURIComponent(String(includeAddress))}`);
        }
        if (includePhone !== undefined) {
            queryParams.push(`includePhone=${encodeURIComponent(String(includePhone))}`);
        }
        if (includeEmail !== undefined) {
            queryParams.push(`includeEmail=${encodeURIComponent(String(includeEmail))}`);
        }
        const requestPath = `/api/v1/Customers/${pageNumber}` + (queryParams.length > 0 ? "?" + queryParams.join("&") : "");
        const requestUrl = this.resolveUrl(requestPath);
        const httpResponse = await this.httpClient.sendAsync<FindCustomerPhoneResponse>("GET", requestUrl, undefined, undefined, abortSignal);

        if (!httpResponse.isSuccessStatusCode) {
            throw new ConnectorException(this.connectorName, `GET ${requestPath}`, httpResponse.statusCode, httpResponse.text);
        }

        return httpResponse.value as FindCustomerPhoneResponse;
    }

    /**
     * Mark Registrant as Attended
     * @remarks Mark the Registrant as Attended for the specified Events.
     */
    public async markRegistrantAttendedAsync(input: MarkRegistrantAttendedInput, recordNumber: string, abortSignal?: AbortSignal): Promise<void> {
        const requestPath = `/api/v1/Events/Registrants/${recordNumber}/Attended`;
        const requestUrl = this.resolveUrl(requestPath);
        const httpResponse = await this.httpClient.sendAsync<void>("PUT", requestUrl, undefined, input, abortSignal);

        if (!httpResponse.isSuccessStatusCode) {
            throw new ConnectorException(this.connectorName, `PUT ${requestPath}`, httpResponse.statusCode, httpResponse.text);
        }
    }

    /**
     * Add Award Nomination
     * @remarks Add a new Award Nomination.
     */
    public async awardsAddAwardNominationAsync(input: AwardNominationData, id: string, abortSignal?: AbortSignal): Promise<void> {
        const requestPath = `/api/v1/Awards/${id}/Nominations`;
        const requestUrl = this.resolveUrl(requestPath);
        const httpResponse = await this.httpClient.sendAsync<void>("POST", requestUrl, undefined, input, abortSignal);

        if (!httpResponse.isSuccessStatusCode) {
            throw new ConnectorException(this.connectorName, `POST ${requestPath}`, httpResponse.statusCode, httpResponse.text);
        }
    }

    /**
     * Update Award Nomination
     * @remarks Update an existing Award Nomination.
     */
    public async awardsUpdateAwardNominationAsync(input: UpdateAwardNominationData, id: string, nomineeRecordNumber: string, abortSignal?: AbortSignal): Promise<void> {
        const requestPath = `/api/v1/Awards/${id}/Nominations/${nomineeRecordNumber}`;
        const requestUrl = this.resolveUrl(requestPath);
        const httpResponse = await this.httpClient.sendAsync<void>("PUT", requestUrl, undefined, input, abortSignal);

        if (!httpResponse.isSuccessStatusCode) {
            throw new ConnectorException(this.connectorName, `PUT ${requestPath}`, httpResponse.statusCode, httpResponse.text);
        }
    }

    /**
     * List Award Individual Recipients
     * @remarks List all Individuals that are Award Recipients.
     */
    public async awardsGetIndividualAwardRecipientsAsync(pageNumber: string, id: string, includeDetails?: string, abortSignal?: AbortSignal): Promise<AwardsGetIndividualAwardRecipientsResponse> {
        const queryParams: string[] = [];
        if (includeDetails !== undefined) {
            queryParams.push(`includeDetails=${encodeURIComponent(String(includeDetails))}`);
        }
        const requestPath = `/api/v1/Awards/${id}/Recipients/Individuals/${pageNumber}` + (queryParams.length > 0 ? "?" + queryParams.join("&") : "");
        const requestUrl = this.resolveUrl(requestPath);
        const httpResponse = await this.httpClient.sendAsync<AwardsGetIndividualAwardRecipientsResponse>("GET", requestUrl, undefined, undefined, abortSignal);

        if (!httpResponse.isSuccessStatusCode) {
            throw new ConnectorException(this.connectorName, `GET ${requestPath}`, httpResponse.statusCode, httpResponse.text);
        }

        return httpResponse.value as AwardsGetIndividualAwardRecipientsResponse;
    }

    /**
     * List Award Organization Recipients
     * @remarks List all Organizations that are Award Recipients.
     */
    public async awardsGetOrganizationAwardRecipientsAsync(pageNumber: string, id: string, includeDetails?: string, abortSignal?: AbortSignal): Promise<AwardsGetOrganizationAwardRecipientsResponse> {
        const queryParams: string[] = [];
        if (includeDetails !== undefined) {
            queryParams.push(`includeDetails=${encodeURIComponent(String(includeDetails))}`);
        }
        const requestPath = `/api/v1/Awards/${id}/Recipients/Organizations/${pageNumber}` + (queryParams.length > 0 ? "?" + queryParams.join("&") : "");
        const requestUrl = this.resolveUrl(requestPath);
        const httpResponse = await this.httpClient.sendAsync<AwardsGetOrganizationAwardRecipientsResponse>("GET", requestUrl, undefined, undefined, abortSignal);

        if (!httpResponse.isSuccessStatusCode) {
            throw new ConnectorException(this.connectorName, `GET ${requestPath}`, httpResponse.statusCode, httpResponse.text);
        }

        return httpResponse.value as AwardsGetOrganizationAwardRecipientsResponse;
    }

}

// #endregion Client
