// TwitterExtensions.ts - Auto-generated DirectClient SDK for TypeScript
// Do not edit this file directly.

import { ConnectorClientBase } from "../azureConnectors/clientBase.ts";
import { ConnectorException } from "../azureConnectors/connectorException.ts";
import { ConnectorClientOptions } from "../azureConnectors/options.ts";
import { TokenProvider } from "../azureConnectors/authentication.ts";
import { TriggerCallbackPayload } from "../azureConnectors/triggerPayload.ts";

// #region Types

/**
 * Post a tweet
 */
export type TweetInput = string;

/**
 * Definition: TweetModel
 */
export interface TweetModel {
    /** Text content of the tweet */
    TweetText: string;
    /** Id of the tweet */
    TweetId?: string;
    /** Time at which the tweet was posted */
    CreatedAt?: string;
    /** Time at which the tweet was posted */
    CreatedAtIso?: string;
    /** Total number of re-tweets for the tweet */
    RetweetCount: number;
    /** Name of the user who has posted the tweet */
    TweetedBy: string;
    /** Url of the media posted along with the tweet */
    MediaUrls?: Array<string>;
    /** Language code of the tweet */
    TweetLanguageCode?: string;
    /** User Id of the author of the tweet that the current tweet is a reply to */
    TweetInReplyToUserId?: string;
    /** Indicates whether the tweet is marked as favorited or not */
    Favorited?: boolean;
    /** List of users mentioned in the tweet */
    UserMentions?: Array<UserMentionsModel>;
    OriginalTweet?: OriginalTweetModel;
    UserDetails?: UserDetailsModel;
}

/**
 * Definition: UserMentionsModel
 */
export interface UserMentionsModel {
    /** X id (handle) of the user */
    Id: number;
    /** Full name of the user */
    FullName?: string;
    /** Screen name of the user */
    UserName?: string;
}

/**
 * Definition: OriginalTweetModel
 */
export interface OriginalTweetModel {
    /** Text content of the original retrieved tweet */
    TweetText: string;
    /** Id of the original tweet */
    TweetId?: string;
    /** Time at which original tweet was posted */
    CreatedAt?: string;
    /** Time at which original tweet was posted */
    CreatedAtIso?: string;
    /** Total number of re-tweets for this original tweet */
    RetweetCount: number;
    /** User name of the person who posted the original tweet */
    TweetedBy: string;
    /** Url of the media posted along with the original tweet */
    MediaUrls?: Array<string>;
    /** Language code for the original tweet */
    TweetLanguageCode?: string;
    /** User Id of the author of the tweet that the current original tweet is a reply to */
    TweetInReplyToUserId?: string;
    /** Indicates whether the original tweet is marked as favorited */
    Favorited?: boolean;
    /** The list of user mentioned in the tweet */
    UserMentions?: Array<OriginalTweetUserMentionsModel>;
    UserDetails?: OriginalTweetUserDetailsModel;
}

/**
 * Definition: UserDetailsModel
 */
export interface UserDetailsModel {
    /** Name of the user */
    FullName: string;
    /** Location of the user */
    Location: string;
    /** X id (handle) of the user */
    Id?: number;
    /** Screen name of the user */
    UserName: string;
    /** Number of followers */
    FollowersCount?: number;
    /** User description */
    Description: string;
    /** User status count */
    StatusesCount?: number;
    /** Number of friends */
    FriendsCount?: number;
    /** Number of tweets that the user has favorited */
    FavouritesCount?: number;
    /** Url of the profile image */
    ProfileImageUrl?: string;
}

/**
 * Definition: OriginalTweetUserMentionsModel
 */
export interface OriginalTweetUserMentionsModel {
    /** X id (handle) of the user */
    Id: number;
    /** Full name of the user */
    FullName?: string;
    /** Screen name of the user */
    UserName?: string;
}

/**
 * Definition: OriginalTweetUserDetailsModel
 */
export interface OriginalTweetUserDetailsModel {
    /** Name of the user */
    FullName: string;
    /** Location of the user */
    Location: string;
    /** X id (handle) of the user */
    Id?: number;
    /** Screen name of the user */
    UserName: string;
    /** Number of followers */
    FollowersCount?: number;
    /** User description */
    Description: string;
    /** User status count */
    StatusesCount?: number;
    /** Number of friends */
    FriendsCount?: number;
    /** Number of tweets that the user has favorited */
    FavouritesCount?: number;
    /** Url of the profile image */
    ProfileImageUrl?: string;
}

/**
 * Definition: TweetResponseModel
 */
export interface TweetResponseModel {
    /** ID of the tweet posted */
    TweetId: string;
}

/**
 * Definition: TriggerBatchResponse[TweetModel]
 */
export interface TriggerBatchResponseTweetModel {
    /** A list of the response objects */
    value?: Array<TweetModel>;
}

/**
 * Typed callback payload for trigger operation 'OnNewTweet'.
 */
export type TwitterOnNewTweetTriggerPayload = TriggerCallbackPayload<TweetModel>;

// #endregion Types

export const TwitterTriggerOperations = {
    OnNewTweet: "OnNewTweet",
} as const;

export type TwitterTriggerOperation = typeof TwitterTriggerOperations[keyof typeof TwitterTriggerOperations];

export const TwitterTriggerParameters = {
    OnNewTweet: {
        searchQuery: {
            name: "searchQuery",
            type: "string",
            required: true,
            description: "Search term like \"happy hour\", #haiku, from:@username, love OR hate.",
            summary: "Search text",
        },
    },
} as const;

// #region Client

/**
 * Typed client for the twitter connector.
 */
export class TwitterClient extends ConnectorClientBase {

    /**
     * Creates a new TwitterClient.
     * @param connectionRuntimeUrl The connection runtime URL from Azure Portal.
     * @param tokenProvider The token provider for authentication.
     * @param options Optional connector client options.
     */
    constructor(connectionRuntimeUrl: string, tokenProvider: TokenProvider, options?: ConnectorClientOptions) {
        super(connectionRuntimeUrl, tokenProvider, options);
    }

    public get connectorName(): string {
        return "twitter";
    }

    /**
     * Get user timeline
     * @remarks This operation gets a list of the most recent tweets posted by a given user.
     */
    public async userTimelineAsync(userName?: string, maxResults?: string, abortSignal?: AbortSignal): Promise<Array<TweetModel>> {
        const queryParams: string[] = [];
        if (userName !== undefined) {
            queryParams.push(`userName=${encodeURIComponent(String(userName))}`);
        }
        if (maxResults !== undefined) {
            queryParams.push(`maxResults=${encodeURIComponent(String(maxResults))}`);
        }
        const requestPath = `/usertimeline` + (queryParams.length > 0 ? "?" + queryParams.join("&") : "");
        const requestUrl = this.resolveUrl(requestPath);
        const httpResponse = await this.httpClient.sendAsync<Array<TweetModel>>("GET", requestUrl, undefined, undefined, abortSignal);

        if (!httpResponse.isSuccessStatusCode) {
            throw new ConnectorException(this.connectorName, `GET ${requestPath}`, httpResponse.statusCode, httpResponse.text);
        }

        return httpResponse.value as Array<TweetModel>;
    }

    /**
     * Get home timeline
     * @remarks This operation gets the most recent tweets and re-tweets posted by me and my followers.
     */
    public async homeTimelineAsync(maxResults?: string, abortSignal?: AbortSignal): Promise<Array<TweetModel>> {
        const queryParams: string[] = [];
        if (maxResults !== undefined) {
            queryParams.push(`maxResults=${encodeURIComponent(String(maxResults))}`);
        }
        const requestPath = `/hometimeline` + (queryParams.length > 0 ? "?" + queryParams.join("&") : "");
        const requestUrl = this.resolveUrl(requestPath);
        const httpResponse = await this.httpClient.sendAsync<Array<TweetModel>>("GET", requestUrl, undefined, undefined, abortSignal);

        if (!httpResponse.isSuccessStatusCode) {
            throw new ConnectorException(this.connectorName, `GET ${requestPath}`, httpResponse.statusCode, httpResponse.text);
        }

        return httpResponse.value as Array<TweetModel>;
    }

    /**
     * Search tweets
     * @remarks This operation gets a list of relevant tweets matching the search query.
     */
    public async searchTweetAsync(searchQuery?: string, maxResults?: string, sinceId?: string, abortSignal?: AbortSignal): Promise<Array<TweetModel>> {
        const queryParams: string[] = [];
        if (searchQuery !== undefined) {
            queryParams.push(`searchQuery=${encodeURIComponent(String(searchQuery))}`);
        }
        if (maxResults !== undefined) {
            queryParams.push(`maxResults=${encodeURIComponent(String(maxResults))}`);
        }
        if (sinceId !== undefined) {
            queryParams.push(`sinceId=${encodeURIComponent(String(sinceId))}`);
        }
        const requestPath = `/searchtweets` + (queryParams.length > 0 ? "?" + queryParams.join("&") : "");
        const requestUrl = this.resolveUrl(requestPath);
        const httpResponse = await this.httpClient.sendAsync<Array<TweetModel>>("GET", requestUrl, undefined, undefined, abortSignal);

        if (!httpResponse.isSuccessStatusCode) {
            throw new ConnectorException(this.connectorName, `GET ${requestPath}`, httpResponse.statusCode, httpResponse.text);
        }

        return httpResponse.value as Array<TweetModel>;
    }

    /**
     * Get followers
     * @remarks This operation gets the list of users that follow a given user.
     */
    public async followersAsync(userName?: string, maxResults?: string, abortSignal?: AbortSignal): Promise<Array<UserDetailsModel>> {
        const queryParams: string[] = [];
        if (userName !== undefined) {
            queryParams.push(`userName=${encodeURIComponent(String(userName))}`);
        }
        if (maxResults !== undefined) {
            queryParams.push(`maxResults=${encodeURIComponent(String(maxResults))}`);
        }
        const requestPath = `/followers` + (queryParams.length > 0 ? "?" + queryParams.join("&") : "");
        const requestUrl = this.resolveUrl(requestPath);
        const httpResponse = await this.httpClient.sendAsync<Array<UserDetailsModel>>("GET", requestUrl, undefined, undefined, abortSignal);

        if (!httpResponse.isSuccessStatusCode) {
            throw new ConnectorException(this.connectorName, `GET ${requestPath}`, httpResponse.statusCode, httpResponse.text);
        }

        return httpResponse.value as Array<UserDetailsModel>;
    }

    /**
     * Get my followers
     * @remarks This operation gets the list of users who are following me.
     */
    public async myFollowersAsync(maxResults?: string, abortSignal?: AbortSignal): Promise<Array<UserDetailsModel>> {
        const queryParams: string[] = [];
        if (maxResults !== undefined) {
            queryParams.push(`maxResults=${encodeURIComponent(String(maxResults))}`);
        }
        const requestPath = `/myfollowers` + (queryParams.length > 0 ? "?" + queryParams.join("&") : "");
        const requestUrl = this.resolveUrl(requestPath);
        const httpResponse = await this.httpClient.sendAsync<Array<UserDetailsModel>>("GET", requestUrl, undefined, undefined, abortSignal);

        if (!httpResponse.isSuccessStatusCode) {
            throw new ConnectorException(this.connectorName, `GET ${requestPath}`, httpResponse.statusCode, httpResponse.text);
        }

        return httpResponse.value as Array<UserDetailsModel>;
    }

    /**
     * Get following
     * @remarks The operation gets the list of people the given user follows.
     */
    public async followingAsync(userName?: string, maxResults?: string, abortSignal?: AbortSignal): Promise<Array<UserDetailsModel>> {
        const queryParams: string[] = [];
        if (userName !== undefined) {
            queryParams.push(`userName=${encodeURIComponent(String(userName))}`);
        }
        if (maxResults !== undefined) {
            queryParams.push(`maxResults=${encodeURIComponent(String(maxResults))}`);
        }
        const requestPath = `/friends` + (queryParams.length > 0 ? "?" + queryParams.join("&") : "");
        const requestUrl = this.resolveUrl(requestPath);
        const httpResponse = await this.httpClient.sendAsync<Array<UserDetailsModel>>("GET", requestUrl, undefined, undefined, abortSignal);

        if (!httpResponse.isSuccessStatusCode) {
            throw new ConnectorException(this.connectorName, `GET ${requestPath}`, httpResponse.statusCode, httpResponse.text);
        }

        return httpResponse.value as Array<UserDetailsModel>;
    }

    /**
     * Get my following
     * @remarks This operation gets the list of users that I am following.
     */
    public async myFollowingAsync(maxResults?: string, abortSignal?: AbortSignal): Promise<Array<UserDetailsModel>> {
        const queryParams: string[] = [];
        if (maxResults !== undefined) {
            queryParams.push(`maxResults=${encodeURIComponent(String(maxResults))}`);
        }
        const requestPath = `/myfriends` + (queryParams.length > 0 ? "?" + queryParams.join("&") : "");
        const requestUrl = this.resolveUrl(requestPath);
        const httpResponse = await this.httpClient.sendAsync<Array<UserDetailsModel>>("GET", requestUrl, undefined, undefined, abortSignal);

        if (!httpResponse.isSuccessStatusCode) {
            throw new ConnectorException(this.connectorName, `GET ${requestPath}`, httpResponse.statusCode, httpResponse.text);
        }

        return httpResponse.value as Array<UserDetailsModel>;
    }

    /**
     * Get user
     * @remarks This operation gets the profile details for a given user, such as user name, description, followers count, and more.
     */
    public async userAsync(userName?: string, abortSignal?: AbortSignal): Promise<UserDetailsModel> {
        const queryParams: string[] = [];
        if (userName !== undefined) {
            queryParams.push(`userName=${encodeURIComponent(String(userName))}`);
        }
        const requestPath = `/user` + (queryParams.length > 0 ? "?" + queryParams.join("&") : "");
        const requestUrl = this.resolveUrl(requestPath);
        const httpResponse = await this.httpClient.sendAsync<UserDetailsModel>("GET", requestUrl, undefined, undefined, abortSignal);

        if (!httpResponse.isSuccessStatusCode) {
            throw new ConnectorException(this.connectorName, `GET ${requestPath}`, httpResponse.statusCode, httpResponse.text);
        }

        return httpResponse.value as UserDetailsModel;
    }

    /**
     * Post a tweet
     * @remarks This operation posts a new tweet.
     */
    public async tweetAsync(input: TweetInput, tweetText?: string, abortSignal?: AbortSignal): Promise<TweetResponseModel> {
        const queryParams: string[] = [];
        if (tweetText !== undefined) {
            queryParams.push(`tweetText=${encodeURIComponent(String(tweetText))}`);
        }
        const requestPath = `/posttweet` + (queryParams.length > 0 ? "?" + queryParams.join("&") : "");
        const requestUrl = this.resolveUrl(requestPath);
        const httpResponse = await this.httpClient.sendAsync<TweetResponseModel>("POST", requestUrl, undefined, input, abortSignal);

        if (!httpResponse.isSuccessStatusCode) {
            throw new ConnectorException(this.connectorName, `POST ${requestPath}`, httpResponse.statusCode, httpResponse.text);
        }

        return httpResponse.value as TweetResponseModel;
    }

    /**
     * Retweet
     * @remarks This operation retweets a tweet.
     */
    public async retweetAsync(tweetId?: string, trimUser?: string, abortSignal?: AbortSignal): Promise<TweetResponseModel> {
        const queryParams: string[] = [];
        if (tweetId !== undefined) {
            queryParams.push(`tweetId=${encodeURIComponent(String(tweetId))}`);
        }
        if (trimUser !== undefined) {
            queryParams.push(`trimUser=${encodeURIComponent(String(trimUser))}`);
        }
        const requestPath = `/retweet` + (queryParams.length > 0 ? "?" + queryParams.join("&") : "");
        const requestUrl = this.resolveUrl(requestPath);
        const httpResponse = await this.httpClient.sendAsync<TweetResponseModel>("POST", requestUrl, undefined, undefined, abortSignal);

        if (!httpResponse.isSuccessStatusCode) {
            throw new ConnectorException(this.connectorName, `POST ${requestPath}`, httpResponse.statusCode, httpResponse.text);
        }

        return httpResponse.value as TweetResponseModel;
    }

}

// #endregion Client
