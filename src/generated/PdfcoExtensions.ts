// PdfcoExtensions.ts - Auto-generated DirectClient SDK for TypeScript
// Do not edit this file directly.

import type { TokenCredential } from "@azure/core-auth";
import { ConnectorClientBase } from "../azureConnectors/clientBase.ts";
import type { ConnectorClientOptions, ConnectorOperationOptions } from "../azureConnectors/options.ts";

// #region Types

/**
 * HTML to PDF
 */
export interface HtmlToPdfInput {
    /** Input HTML code to be converted. */
    html: string;
    /** Base file name for new PDF files. */
    name?: string;
    /** Override default margins with custom values (the order is `top right bottom left`). You can als set all margins: `10px`, `5mm`, `5in` for all sides. */
    margins?: string;
    /** Select PDF paper size. Can be Letter, Legal, Tabloid, Ledger, A0, A1, A2, A3, A4, A5, A6 or a custom size. Custom size can be set in px (pixels), mm or in (inches) with width and height separated by space like this: 200 300, 200px 300px, 200mm 300mm, 20cm 30cm or 6in 8in. */
    paperSize?: string;
    /** PDF page orientation. */
    orientation?: string;
    /** Enable or disable printing of document background */
    printBackground?: boolean;
    /** Set to HTML for header to be applied on every page at the header. */
    header?: string;
    /** Set to HTML for footer to be applied on every page at the bottom. */
    footer?: string;
    /** Set the expiration time for the output link in minutes (default is 60 i.e 60 minutes or 1 hour), After this specified duration, any generated output file(s) will be automatically deleted from PDF.co temporary files storage. The maximum duration for link expiration varies based on your current subscription plan. */
    expiration?: number;
    /** This parameter can be used to set additional configurations for fine-tuning and to enable more options. Visit PDF.co knowledgebase (https://developer.pdf.co/api/profiles/index.html) for profile examples and more. */
    profiles?: string;
    /** Set to true for long processes to run in the background, API will then return a jobId which you can use with JobCheck endpoint to check the status of the process and retrieve the output while you can proceed with other tasks without waiting for this process to finish. */
    async?: boolean;
}

/**
 * Response for HTML to PDF
 */
export interface HtmlToPdfResponse {
    /** Output document URL. */
    url?: string;
    /** jobId */
    jobId?: string;
    /** Number of pages in the output document. */
    pageCount?: number;
    /** Indicates whether the operation is successful or not. */
    error?: boolean;
    /** Status code for HTTP response */
    status?: number;
    /** Output document name */
    name?: string;
    /** PDF.co credits used for this request */
    credits?: number;
    /** Request execution duration */
    duration?: number;
    /** PDF.co account remaining credits */
    remainingCredits?: number;
}

/**
 * URL to PDF
 */
export interface UrlToPdfInput {
    /** Link to input HTML file or website page to be converted. You can pass link to file from Google Drive, Dropbox or another online file service that can generate shareable links. */
    url: string;
    /** Margins to be used in output document. Set to css style margins like 10px, 5mm, 5in for all sides or 5px 5px 5px 5px (the order of margins is top, right, bottom, left). */
    margins?: string;
    /** Letter is set by default. Can be Letter, Legal, Tabloid, Ledger, A0, A1, A2, A3, A4, A5, A6 or a custom size. Custom size can be set in px (pixels), mm or in (inches) with width and height separated by space like this: 200 300, 200px 300px, 200mm 300mm, 20cm 30cm or 6in 8in. */
    paperSize?: string;
    /** Output page orientation */
    orientation?: string;
    /** Enable or disable printing of document background */
    printBackground?: boolean;
    /** Set to HTML for header to be applied on every page at the header. */
    header?: string;
    /** Set to HTML for footer to be applied on every page at the bottom. */
    footer?: string;
    /** Set the expiration time for the output link in minutes (default is 60 i.e 60 minutes or 1 hour), After this specified duration, any generated output file(s) will be automatically deleted from PDF.co temporary files storage. The maximum duration for link expiration varies based on your current subscription plan. */
    expiration?: number;
    /** This parameter can be used to set additional configurations for fine-tuning and to enable more options. Visit PDF.co knowledgebase (https://developer.pdf.co/api/profiles/index.html) for profile examples and more. */
    profiles?: string;
    /** Set to true for long processes to run in the background, API will then return a jobId which you can use with JobCheck endpoint to check the status of the process and retrieve the output while you can proceed with other tasks without waiting for this process to finish. */
    async?: boolean;
}

/**
 * Response for URL to PDF
 */
export interface UrlToPdfResponse {
    /** Output document URL. */
    url?: string;
    /** jobId */
    jobId?: string;
    /** Number of pages in the output document. */
    pageCount?: number;
    /** Indicates whether the operation is successful or not. */
    error?: boolean;
    /** Status code for HTTP response */
    status?: number;
    /** Output document name */
    name?: string;
    /** PDF.co credits used for this request */
    credits?: number;
    /** Request execution duration */
    duration?: number;
    /** PDF.co account remaining credits */
    remainingCredits?: number;
}

/**
 * Add Text, Images, Form Fields to PDF
 */
export interface PdfFillerInput {
    /** Url to the input PDF document */
    url: string;
    /** Annotation strings to be added to the PDF document. Format should be like “x;y;page;text”. Refer to PDF.co documentations for more advanced formats and examples. */
    annotationsString?: string;
    /** Images to be added to the PDF document. Format should be like “x;y;page;urltoimage;link;width;height”. Refer to PDF.co documentations for more advanced formats and examples. */
    imagesString?: string;
    /** Fill PDF form fields. Format should be like “page;fieldName;value”. Refer to PDF.co documentations for more advanced formats and examples. */
    fieldsString?: string;
    /** Output PDF Name */
    name?: string;
    /** Determines whether to return output in response or link to output file. */
    inline?: boolean;
    /** Set the expiration time for the output link in minutes (default is 60 i.e 60 minutes or 1 hour), After this specified duration, any generated output file(s) will be automatically deleted from PDF.co temporary files storage. The maximum duration for link expiration varies based on your current subscription plan. */
    expiration?: number;
    /** This parameter can be used to set additional configurations for fine-tuning and to enable more options. Visit PDF.co knowledgebase (https://developer.pdf.co/api/profiles/index.html) for profile examples and more. */
    profiles?: string;
    /** Set to true for long processes to run in the background, API will then return a jobId which you can use with JobCheck endpoint to check the status of the process and retrieve the output while you can proceed with other tasks without waiting for this process to finish. */
    async?: boolean;
}

/**
 * Response for Add Text, Images, Form Fields to PDF
 */
export interface PdfFillerResponse {
    /** Output document URL. */
    url?: string;
    /** jobId */
    jobId?: string;
    /** Number of pages in the output document. */
    pageCount?: number;
    /** Indicates whether the operation is successful or not. */
    error?: boolean;
    /** Status code for HTTP response */
    status?: number;
    /** Output document name */
    name?: string;
    /** PDF.co credits used for this request */
    credits?: number;
    /** Request execution duration */
    duration?: number;
    /** PDF.co account remaining credits */
    remainingCredits?: number;
}

/**
 * Merge PDF
 */
export interface MergePdfSimplifiedInput {
    /** The comma-separated list of links to PDFs files. Supports links to external sites, `filetoken://..` from PDF.co Files storage and links to G Drive, Dropbox and other services. Links should be accessible without password. For password protected PDF files, use PDF Security module first to unlock input pdf. */
    url: string;
    /** Set the expiration time for the output link in minutes (default is 60 i.e 60 minutes or 1 hour), After this specified duration, any generated output file(s) will be automatically deleted from PDF.co temporary files storage. The maximum duration for link expiration varies based on your current subscription plan. */
    expiration?: number;
    /** Use this parameter to set additional configurations for fine-tuning and extra options. Explore PDF.co knowledgebase (https://developer.pdf.co/api/profiles/index.html) for profile examples. */
    profiles?: string;
    /** Set async to true for long processes to run in the background, API will then return a jobId which you can use with JobCheck endpoint to check the status of the process and retrieve the output while you can proceed with other tasks without waiting for this process to finish. */
    async?: boolean;
}

/**
 * Response for Merge PDF
 */
export interface MergePdfSimplifiedResponse {
    /** Output document URL. */
    url?: string;
    /** jobId */
    jobId?: string;
    /** Number of pages in the output document. */
    pageCount?: number;
    /** Indicates whether the operation is successful or not. */
    error?: boolean;
    /** Status code for HTTP response */
    status?: number;
    /** Output document name */
    name?: string;
    /** PDF.co credits used for this request */
    credits?: number;
    /** Request execution duration */
    duration?: number;
    /** PDF.co account remaining credits */
    remainingCredits?: number;
}

/**
 * Merge PDF (Advanced)
 */
export interface MergePdfInput {
    /** The comma-separated list of links to PDFs, documents, spreadsheets, images. Supports links to external sites, `filetoken://..` from PDF.co Files storage and links to G Drive, Dropbox and other services. Links should be accessible without password. For password protected PDF files, use PDF Security module first to unlock input pdf. */
    url: string;
    /** Set the expiration time for the output link in minutes (default is 60 i.e 60 minutes or 1 hour), After this specified duration, any generated output file(s) will be automatically deleted from PDF.co temporary files storage. The maximum duration for link expiration varies based on your current subscription plan. */
    expiration?: number;
    /** Use this parameter to set additional configurations for fine-tuning and extra options. Explore PDF.co knowledgebase (https://developer.pdf.co/api/profiles/index.html) for profile examples. */
    profiles?: string;
    /** Set async to true for long processes to run in the background, API will then return a jobId which you can use with JobCheck endpoint to check the status of the process and retrieve the output while you can proceed with other tasks without waiting for this process to finish. */
    async?: boolean;
}

/**
 * Response for Merge PDF (Advanced)
 */
export interface MergePdfResponse {
    /** Output document URL. */
    url?: string;
    /** jobId */
    jobId?: string;
    /** Number of pages in the output document. */
    pageCount?: number;
    /** Indicates whether the operation is successful or not. */
    error?: boolean;
    /** Status code for HTTP response */
    status?: number;
    /** Output document name */
    name?: string;
    /** PDF.co credits used for this request */
    credits?: number;
    /** Request execution duration */
    duration?: number;
    /** PDF.co account remaining credits */
    remainingCredits?: number;
}

/**
 * Split PDF
 */
export interface SplitPdfInput {
    /** Url to the source PDF document */
    url: string;
    /** Comma-separated list of page indices (or ranges) to split into. First page is 1 (one). Example: "1,2-5,7-". Special cases: to extract each page into separate PDF use to `*`. */
    pages: string;
    /** Determines whether to return output in response or link to output file. */
    inline?: boolean;
    /** Base file name for new PDF files */
    name?: string;
    /** Set the expiration time for the output link in minutes (default is 60 i.e 60 minutes or 1 hour), After this specified duration, any generated output file(s) will be automatically deleted from PDF.co temporary files storage. The maximum duration for link expiration varies based on your current subscription plan. */
    expiration?: number;
    /** Use this parameter to set additional configurations for fine-tuning and extra options. Explore PDF.co knowledgebase (https://developer.pdf.co/api/profiles/index.html) for profile examples. */
    profiles?: string;
    /** Creates a background processing job. */
    async?: boolean;
}

/**
 * Response for Split PDF
 */
export interface SplitPdfResponse {
    /** urls */
    urls?: Array<string>;
    /** Output document URL. */
    url?: string;
    /** jobId */
    jobId?: string;
    /** Number of pages in the output document. */
    pageCount?: number;
    /** Indicates whether the operation is successful or not. */
    error?: boolean;
    /** Status code for HTTP response */
    status?: number;
    /** Output document name */
    name?: string;
    /** PDF.co credits used for this request */
    credits?: number;
    /** Request execution duration */
    duration?: number;
    /** PDF.co account remaining credits */
    remainingCredits?: number;
}

/**
 * Split PDF (Advanced)
 */
export interface SplitPdf2Input {
    /** Url to the source PDF document */
    url: string;
    /** Text to search for on pages. Can also search by barcode, for more info visit https://developer.pdf.co/api/pdf-split/index.html?utm_source=api-docs#post-tag-pdf-split2 */
    searchString: string;
    /** Set to true if you want to exclude pages where text was found */
    excludeKeyPages?: boolean;
    /** Set to true to enable regular expressions for the search string */
    regexSearch?: boolean;
    /** Set to true to enable case-sensitive search. */
    caseSensitive?: boolean;
    /** the language for OCR (text from image) to use for scanned PDF, PNG, and JPG documents input when extracting text. The default is “eng”. Other languages are also supported: deu, spa, chi_sim, jpn, and many others (full list of supported OCR languages is at https://developer.pdf.co/api/pdf-make-text-searchable-or-unsearchable/index.html#language-support). You can also use 2 languages simultaneously like this: eng+deu or jpn+kor (any combination). */
    lang?: string;
    /** Determines whether to return output in response or link to output file. */
    inline?: boolean;
    /** Base file name for new PDF files */
    name?: string;
    /** Set the expiration time for the output link in minutes (default is 60 i.e 60 minutes or 1 hour), After this specified duration, any generated output file(s) will be automatically deleted from PDF.co temporary files storage. The maximum duration for link expiration varies based on your current subscription plan. */
    expiration?: number;
    /** Use this parameter to set additional configurations for fine-tuning and extra options. Explore PDF.co knowledgebase (https://developer.pdf.co/api/profiles/index.html) for profile examples. */
    profiles?: string;
    /** Creates a background processing job. */
    async?: boolean;
}

/**
 * Response for Split PDF (Advanced)
 */
export interface SplitPdf2Response {
    /** urls */
    urls?: Array<string>;
    /** Output document URL. */
    url?: string;
    /** jobId */
    jobId?: string;
    /** Number of pages in the output document. */
    pageCount?: number;
    /** Indicates whether the operation is successful or not. */
    error?: boolean;
    /** Status code for HTTP response */
    status?: number;
    /** Output document name */
    name?: string;
    /** PDF.co credits used for this request */
    credits?: number;
    /** Request execution duration */
    duration?: number;
    /** PDF.co account remaining credits */
    remainingCredits?: number;
}

/**
 * PDF Search Text
 */
export interface PDFSearchTextInput {
    /** URL to the source file. Supports links from Google Drive, Dropbox, and PDF.co built-in files storage. */
    url: string;
    /** Text to search can support regular expressions if you set the "Regex Search" parameter to true. */
    searchString: string;
    regexSearch?: boolean;
    /** name */
    name?: string;
    /** Comma-separated list of page indices (or ranges) to process. IMPORTANT: the very first page starts at 0 (zero). To set a range use the dash -, for example: 0,2-5,7-. To set a range from index to the last page use range like this: 2- (from page #3 as the index starts at zero and till the of the document) for ALL pages just leave this param empty. Example: 0,2-5,7- means the first page, then the 3rd page to the 6th page, and then the range from the 8th (index = 7) page till the end of the document, The input must be in string format. */
    pages?: string;
    inline?: boolean;
    /** Values can be either ‘SmartMatch’, ‘ExactMatch’, or ‘None’. */
    wordMatchingMode?: string;
    /** Password of the PDF file */
    password?: string;
    /** Set async to true for long processes to run in the background, API will then return a jobId which you can use with JobCheck module to check the status of the process and retrieve the output while you can proceed with other tasks without waiting for this process to finish. */
    async?: boolean;
    /** Use this parameter to set additional configurations for fine-tuning and extra options. Explore PDF.co knowledgebase (https://developer.pdf.co/api/profiles/index.html) for profile examples. */
    profiles?: string;
}

/**
 * Response for PDF Search Text
 */
export interface PDFSearchTextResponse {
    /** body */
    body?: Array<Record<string, unknown>>;
    /** url */
    url?: string;
    jobId?: string;
    /** pageCount */
    pageCount?: number;
    /** error */
    error?: boolean;
    /** status */
    status?: number;
    /** name */
    name?: string;
    /** credits */
    credits?: number;
    /** remainingCredits */
    remainingCredits?: number;
    /** duration */
    duration?: number;
}

/**
 * Document Parser
 */
export interface DocumentParserInput {
    /** URL to the source file. Supports links from Google Drive, Dropbox, and PDF.co built-in files storage. To upload files via API, Check out the Files Upload section at https://developer.pdf.co/api/file-upload/index.html */
    url: string;
    /** Set ID of document parser template to be used. View and manage your templates at https://app.pdf.co/document-parser */
    templateId?: string;
    /** Default is JSON. You can override the default output format to CSV or XML to generate CSV or XML output accordingly. */
    outputFormat: string;
    /** Set to true to return results inside the response. Otherwise, the endpoint will return a link to the output file generated */
    inline?: boolean;
    /** Password of PDF file */
    password?: string;
    /** Use this parameter to set additional configurations for fine-tuning and extra options. Explore PDF.co knowledgebase https://developer.pdf.co/api/profiles/index.html for profile examples, */
    profiles?: string;
    /** Set the expiration time for the output link in minutes (default is 60 i.e 60 minutes or 1 hour), After this specified duration, any generated output file(s) will be automatically deleted from PDF.co temporary files storage. The maximum duration for link expiration varies based on your current subscription plan. */
    expiration?: number;
    /** Set async to true for long processes to run in the background, API will then return a jobId which you can use with "Job Check"  to check the status of the process and retrieve the output while you can proceed with other tasks without waiting for this process to finish. */
    async?: boolean;
}

/**
 * Response for Document Parser
 */
export interface DocumentParserResponse {
    /** Output URL */
    url?: string;
    /** body */
    body?: Record<string, unknown>;
    pageCount?: number;
    error?: boolean;
    jobId?: string;
    status?: number;
    name?: string;
    credits?: number;
    remainingCredits?: number;
    duration?: number;
    message?: string;
}

/**
 * Background Jobs Check
 */
export interface JobCheckInput {
    /** PDF.co background Job Id */
    jobid: string;
}

/**
 * Response for Background Jobs Check
 */
export interface JobCheckResponse {
    /** status */
    status?: string;
    /** message */
    message?: string;
    /** pageCount */
    pageCount?: number;
    /** url */
    url?: string;
    /** jobId */
    jobId?: string;
    /** credits */
    credits?: number;
    /** remainingCredits */
    remainingCredits?: number;
    /** jobDuration */
    jobDuration?: number;
    /** duration */
    duration?: number;
    /** error */
    error?: boolean;
    /** errorCode */
    errorCode?: number;
}

/**
 * Barcode Generator
 */
export interface BarcodeGeneratorInput {
    /** Set async to true for long processes to run in the background, API will then return a jobId which you can use with /job/check endpoint to check the status of the process and retrieve the output while you can proceed with other tasks without waiting for this process to finish. */
    name?: string;
    /** Set the string value to encode inside the barcode. */
    value: string;
    /** Set the barcode type, QRCode By default. */
    type: string;
    /** Set this to the image that you want to be inserted the logo inside the QR-Code barcode. */
    decorationImage?: string;
    /** Set async to true for long processes to run in the background, API will then return a jobId which you can use with JobCheck Module to check the status of the process and retrieve the output while you can proceed with other tasks without waiting for this process to finish. */
    async?: boolean;
    /** Set the expiration time for the output link in minutes (default is 60 i.e 60 minutes or 1 hour), After this specified duration, any generated output file(s) will be automatically deleted from PDF.co temporary files storage. The maximum duration for link expiration varies based on your current subscription plan. */
    expiration?: number;
    /** Use this parameter to set additional configurations for fine-tuning and extra options. Explore PDF.co Knowledgebase https://developer.pdf.co/api/profiles/index.html for profile examples */
    profiles?: string;
}

/**
 * Response for Barcode Generator
 */
export interface BarcodeGeneratorResponse {
    /** jobId */
    jobId?: string;
    /** url */
    url?: string;
    /** error */
    error?: boolean;
    /** status */
    status?: number;
    /** name */
    name?: string;
    /** credits */
    credits?: number;
    /** remainingCredits */
    remainingCredits?: number;
    /** duration */
    duration?: number;
}

/**
 * Barcode Reader
 */
export interface BarcodeReaderInput {
    /** URL to the source file. Supports links from Google Drive, Dropbox, and PDF.co built-in files storage. */
    url: string;
    /** Set the barcode type, QRCode is default. You can also combine types by comma, example "QRCode,Code128,Code39,Interleaved2of5,EAN13".  All supported barcode types are AustralianPostCode, Aztec, BorderedTable, Checkbox, Circle, CircularI2of5, Codabar, CodablockF, Code128, Code16K, Code39, Code39Extended, Code39Mod43, Code39Mod43Extended, Code93, DataMatrix, DPMDataMatrix, EAN13, EAN2, EAN5, EAN8, GS1, GS1DataBarExpanded, GS1DataBarExpandedStacked, GS1DataBarLimited, GS1DataBarOmnidirectional, GS1DataBarStacked, GTIN12, GTIN13, GTIN14, GTIN8, HorizontalLine, IntelligentMail, Interleaved2of5, ITF14, MaxiCode, MICR, MicroPDF, MSI, Oval, PatchCode, PDF417, Pharmacode, PostNet, PZN, QRCode, Rectangle, RoyalMail, RoyalMailKIX, Segment, Trioptic, UnderlinedField, UPCA, UPCE, UPU, VerticalLine. */
    types: string;
    /** Comma-Separated list of page indices (or ranges) to process. IMPORTANT: the very first page starts at 0 (zero). To set a range use the dash -, for example: 0,2-5,7-. To set a range from the index to the last page use range like this: 2- (from page #3 as the index starts at zero and till the end of the document). For ALL pages just leave this param empty. Example: 0,2-5,7- means the first page, then the 3rd page to the 6th page, and then the range from the 8th (index = 7) page till the end of the document. */
    pages?: string;
    /** encrypt */
    encrypt?: boolean;
    /** Use this parameter to set additional configurations for fine-tuning and extra options. Explore PDF.co knowledgebase (https://developer.pdf.co/api/profiles/index.html) for profile examples. */
    profiles?: string;
    /** Set async to true for long processes to run in the background, API will then return a jobId which you can use with JobCheck module to check the status of the process and retrieve the output while you can proceed with other tasks without waiting for this process to finish. */
    async?: boolean;
}

/**
 * Response for Barcode Reader
 */
export interface BarcodeReaderResponse {
    /** jobId */
    jobId?: string;
    /** url */
    url?: string;
    /** barcodes */
    barcodes?: Array<Record<string, unknown>>;
    /** pageCount */
    pageCount?: number;
    /** error */
    error?: boolean;
    /** status */
    status?: number;
    /** credits */
    credits?: number;
    /** remainingCredits */
    remainingCredits?: number;
    /** duration */
    duration?: number;
}

/**
 * PDF Info Reader
 */
export interface PDFInfoReaderInput {
    /** URL to the source file. Supports links from Google Drive, Dropbox, and PDF.co built-in files storage. */
    url: string;
    /** Set to true for long processes to run in the background, API will then return a jobId which you can use with JobCheck module to check the status of the process and retrieve the output while you can proceed with other tasks without waiting for this process to finish. */
    async?: boolean;
    /** Password of the PDF file */
    password?: string;
    /** Use this parameter to set additional configuration for fine-tuning and extra options, Explore PDF.co knowledgebase (https://developer.pdf.co/api/profiles/index.html) for profile examples. */
    profiles?: string;
}

/**
 * Response for PDF Info Reader
 */
export interface PDFInfoReaderResponse {
    /** jobId */
    jobId?: string;
    /** url */
    url?: string;
    /** info */
    info?: Record<string, unknown>;
    /** error */
    error?: boolean;
    /** status */
    status?: number;
    /** credits */
    credits?: number;
    /** remainingCredits */
    remainingCredits?: number;
    /** duration */
    duration?: number;
}

/**
 * PDF Forms Info Reader
 */
export interface PDFFormsInfoReaderInput {
    /** URL to the source file. Supports links from Google Drive, Dropbox, and PDF.co built-in files storage. */
    url: string;
    /** Set to true for long processes to run in the background, API will then return a jobId which you can use with JobCheck module to check the status of the process and retrieve the output while you can proceed with other tasks without waiting for this process to finish. */
    async?: boolean;
    /** Password of the PDF file */
    password?: string;
    /** Use this parameter to set additional configuration for fine-tuning and extra options, Explore PDF.co knowledgebase (https://developer.pdf.co/api/profiles/index.html) for profile examples. */
    profiles?: string;
}

/**
 * Response for PDF Forms Info Reader
 */
export interface PDFFormsInfoReaderResponse {
    /** jobId */
    jobId?: string;
    /** url */
    url?: string;
    /** info */
    info?: Record<string, unknown>;
    /** error */
    error?: boolean;
    /** status */
    status?: number;
    /** credits */
    credits?: number;
    /** remainingCredits */
    remainingCredits?: number;
    /** duration */
    duration?: number;
}

/**
 * PDF Find Table
 */
export interface PDFFindTableInput {
    /** URL to the source file. Supports links from Google Drive, Dropbox, and PDF.co built-in files storage. */
    url: string;
    /** Comma-separated list of page indices (or ranges) to process. IMPORTANT: the very first page starts at 0 (zero). To set a range use the dash -, for example: 0,2-5,7-. To set a range from the index to the last page use range like this: 2- (from page #3 as the index starts at zero and till the of the document). For ALL pages just leave this param empty. Example: 0,2-5,7- means first page, then 3rd page to 6th page, and then the range from 8th (index = 7) page till the end of the document. */
    pages?: string;
    inline?: boolean;
    /** Password of PDF file */
    password?: string;
    /** Use this parameter to set additional configurations for fine-tuning and extra options. Explore PDF.co knowledgebase (https://developer.pdf.co/api/profiles/index.html) for profile examples. */
    profiles?: string;
    /** Set the expiration time for the output link in minutes (default is 60 i.e 60 minutes or 1 hour), After this specified duration, any generated output file(s) will be automatically deleted from PDF.co temporary files storage. The maximum duration for link expiration varies based on your current subscription plan. */
    expiration?: number;
    /** Set async to true for long processes to run in the background, API will then return a jobId which you can use with /job/check endpoint to check the status of the process and retrieve the output while you can proceed with other tasks without waiting for this process to finish. */
    async?: boolean;
}

/**
 * Response for PDF Find Table
 */
export interface PDFFindTableResponse {
    /** jobId */
    jobId?: string;
    /** url */
    url?: string;
    /** body */
    body?: Record<string, unknown>;
    /** pageCount */
    pageCount?: number;
    /** error */
    error?: boolean;
    /** status */
    status?: number;
    /** name */
    name?: string;
    /** Credits */
    credits?: number;
    /** remainingCredits */
    remainingCredits?: number;
    /** duration */
    duration?: number;
}

/**
 * PDF Search and Replace Text
 */
export interface SearchAndReplaceInput {
    /** URL to the source file. Supports links from Google Drive, Dropbox, and PDF.co built-in files storage. */
    url: string;
    /** Search Strings */
    searchStrings: Array<string>;
    /** Replacement Strings */
    replaceStrings?: Array<string>;
    caseSensitive?: boolean;
    /** Limit the number of searches & replacements for every item. The default value is 0 which means unlimited searches and replacements so every found occurrence will be replaced. */
    replacementLimit?: number;
    /** Set to true to use regular expression for a search string(s). */
    regex?: boolean;
    /** Comma-separated list of page indices (or ranges) to process. The very first page starts at 0 (zero). To set a range use the dash -, for example: 0,2-5,7-. To set a range from the index to the last page use range like this: 2- (from page #3 as the index starts at zero and till the of the document). For ALL pages just leave this param empty. Example: 0,2-5,7- means first page, then 3rd page to 6th page, and then the range from 8th (index = 7) page till the end of the document. */
    pages?: string;
    /** The password of the PDF file */
    password?: string;
    /** File name for the generated output */
    name?: string;
    /** Set the expiration time for the output link in minutes (default is 60 i.e 60 minutes or 1 hour), After this specified duration, any generated output file(s) will be automatically deleted from PDF.co temporary files storage. The maximum duration for link expiration varies based on your current subscription plan. */
    expiration?: number;
    /** Set async to true for long processes to run in the background, API will then return a jobId which you can use with JobCheck module to check the status of the process and retrieve the output while you can proceed with other tasks without waiting for this process to finish. */
    async?: boolean;
}

/**
 * Response for PDF Search and Replace Text
 */
export interface SearchAndReplaceResponse {
    /** url */
    url?: string;
    pageCount?: number;
    /** jobId */
    jobId?: string;
    /** error */
    error?: boolean;
    /** status */
    status?: number;
    /** name */
    name?: string;
    /** credits */
    credits?: number;
    /** remainingCredits */
    remainingCredits?: number;
    duration?: number;
}

/**
 * PDF Search and Replace Text with Image
 */
export interface SearchAndReplaceWithImageInput {
    /** URL to the source file. Supports links from Google Drive, Dropbox, and PDF.co built-in files storage. */
    url: string;
    caseSensitive?: boolean;
    /** Single text replacement. Word or phrase to be replaced */
    searchString: string;
    /** Image URL to be inserted in the doc. */
    replaceImage: string;
    /** Comma-separated list of page indices (or ranges) to process. The very first page starts at 0 (zero). To set a range use the dash -, for example: 0,2-5,7-. To set a range from the index to the last page use range like this: 2- (from page #3 as the index starts at zero and till the of the document). For ALL pages just leave this param empty. Example: 0,2-5,7- means first page, then 3rd page to 6th page, and then the range from 8th (index = 7) page till the end of the document. */
    pages?: string;
    /** Set async to true for long processes to run in the background, API will then return a jobId which you can use with Job Check module to check the status of the process and retrieve the output while you can proceed with other tasks without waiting for this process to finish.. */
    async?: boolean;
    regex?: boolean;
    /** Limit the number of searches & replacements for every item. The default value is 0 which means unlimited searches and replacements so every found occurrence will be replaced. */
    replacementLimit?: number;
    /** Set the expiration time for the output link in minutes (default is 60 i.e 60 minutes or 1 hour), After this specified duration, any generated output file(s) will be automatically deleted from PDF.co temporary files storage. The maximum duration for link expiration varies based on your current subscription plan. */
    expiration?: number;
    /** Password of the PDF file */
    password?: string;
}

/**
 * Response for PDF Search and Replace Text with Image
 */
export interface SearchAndReplaceWithImageResponse {
    /** jobId */
    jobId?: string;
    /** url */
    url?: string;
    /** pageCount */
    pageCount?: number;
    /** error */
    error?: boolean;
    /** status */
    status?: number;
    /** name */
    name?: string;
    /** credits */
    credits?: number;
    /** remainingCredits */
    remainingCredits?: number;
    /** duration */
    duration?: number;
}

/**
 * PDF Search and Delete Text from PDF
 */
export interface SearchAndDeleteTextInput {
    /** URL to the source file. Supports links from Google Drive, Dropbox, and PDF.co built-in files storage. */
    url: string;
    /** searchStrings */
    searchStrings: Array<string>;
    caseSensitive?: boolean;
    /** Set to true to use regular expression for a search string(s). */
    regex?: boolean;
    /** Limit the number of searches & replacements for every item. The default value is 0 which means unlimited searches and replacements so every found occurrence will be replaced. */
    replacementLimit?: number;
    /** Set the expiration time for the output link in minutes (default is 60 i.e 60 minutes or 1 hour), After this specified duration, any generated output file(s) will be automatically deleted from PDF.co temporary files storage. The maximum duration for link expiration varies based on your current subscription plan. */
    expiration?: number;
    /** File name for the generated output */
    name?: string;
    /** Password of the PDF file */
    password?: string;
    /** Comma-separated list of page indices (or ranges) to process. The very first page starts at 0 (zero). To set a range use the dash -, for example: 0,2-5,7-. To set a range from the index to the last page use range like this: 2- (from page #3 as the index starts at zero and till the of the document). For ALL pages just leave this param empty. Example: 0,2-5,7- means first page, then 3rd page to 6th page, and then the range from 8th (index = 7) page till the end of the document. */
    pages?: string;
    /** Use this parameter to set additional configurations for fine-tuning and extra options. Explore PDF.co knowledgebase (https://developer.pdf.co/api/profiles/index.html) for profile examples. */
    profiles?: string;
    /** Set async to true for long processes to run in the background, API will then return a jobId which you can use with JobCheck endpoint to check the status of the process and retrieve the output while you can proceed with other tasks without waiting for this process to finish. */
    async?: boolean;
}

/**
 * Response for PDF Search and Delete Text from PDF
 */
export interface SearchAndDeleteTextResponse {
    /** url */
    url?: string;
    /** jobId */
    jobId?: string;
    /** pageCount */
    pageCount?: number;
    /** error */
    error?: boolean;
    /** status */
    status?: number;
    /** name */
    name?: string;
    /** credits */
    credits?: number;
    /** remainingCredits */
    remainingCredits?: number;
    /** duration */
    duration?: number;
}

/**
 * Make Scanned PDF to Text Searchable
 */
export interface PDFSearchableInput {
    /** URL to the source file. Supports links from Google Drive, Dropbox, and PDF.co built-in files storage. */
    url: string;
    /** the language for OCR (text from image) to use for scanned PDF, PNG, and JPG documents input when extracting text. The default is “eng”. Other languages are also supported: deu, spa, chi_sim, jpn, and many others (full list of supported OCR languages is at https://developer.pdf.co/api/pdf-make-text-searchable-or-unsearchable/index.html#language-support). You can also use 2 languages simultaneously like this: eng+deu or jpn+kor (any combination). */
    lang?: string;
    /** Comma-separated list of page indices (or ranges) to process. The very first page starts at 0 (zero). To set a range use the dash -, for example: 0,2-5,7-. To set a range from the index to the last page use range like this: 2- (from page #3 as the index starts at zero and till the of the document). For ALL pages just leave this param empty. Example: 0,2-5,7- means first page, then 3rd page to 6th page, and then the range from 8th (index = 7) page till the end of the document. */
    pages?: string;
    /** Password of PDF file */
    password?: string;
    /** Set Async to true for long processes to run in the background, API will then return a jobId which you can use with JobCheck module to check the status of the process and retrieve the output while you can proceed with other tasks without waiting for this process to finish. */
    async?: boolean;
    /** Use this parameter to set additional configurations for fine-tuning and extra options. Explore PDF.co knowledgebase (https://developer.pdf.co/api/profiles/index.html) for profile examples. */
    profiles?: string;
}

/**
 * Response for Make Scanned PDF to Text Searchable
 */
export interface PDFSearchableResponse {
    /** url */
    url?: string;
    /** jobId */
    jobId?: string;
    /** pageCount */
    pageCount?: number;
    /** error */
    error?: boolean;
    /** status */
    status?: number;
    /** name */
    name?: string;
    /** credits */
    credits?: number;
    /** remainingCredits */
    remainingCredits?: number;
    /** duration */
    duration?: number;
}

/**
 * PDF to Scanned PDF
 */
export interface PDFUnSearchableInput {
    /** URL to the source file. Supports links from Google Drive, Dropbox, and PDF.co built-in files storage. */
    url: string;
    /** Comma-separated list of page indices (or ranges) to process. The very first page starts at 0 (zero). To set a range use the dash -, for example: 0,2-5,7-. To set a range from the index to the last page use range like this: 2- (from page #3 as the index starts at zero and till the of the document). For ALL pages just leave this param empty. Example: 0,2-5,7- means first page, then 3rd page to 6th page, and then the range from 8th (index = 7) page till the end of the document. */
    pages?: string;
    /** Password of PDF file */
    password?: string;
    /** Set Async to true for long processes to run in the background, API will then return a jobId which you can use with JobCheck module to check the status of the process and retrieve the output while you can proceed with other tasks without waiting for this process to finish. */
    async?: boolean;
    /** Use this parameter to set additional configurations for fine-tuning and extra options. Explore PDF.co knowledgebase (https://developer.pdf.co/api/profiles/index.html) for profile examples. */
    profiles?: string;
}

/**
 * Response for PDF to Scanned PDF
 */
export interface PDFUnSearchableResponse {
    /** url */
    url?: string;
    /** jobId */
    jobId?: string;
    /** pageCount */
    pageCount?: number;
    /** error */
    error?: boolean;
    /** status */
    status?: number;
    /** name */
    name?: string;
    /** credits */
    credits?: number;
    /** remainingCredits */
    remainingCredits?: number;
    /** duration */
    duration?: number;
}

/**
 * PDF To CSV
 */
export interface PDFToCSVInput {
    /** URL to the source file. Supports links from Google Drive, Dropbox, and PDF.co built-in files storage. */
    url: string;
    /** Set the language for OCR (text from image) to use for scanned PDF, PNG, and JPG documents input when extracting text. The default is “eng”. Other languages are also supported: deu, spa, chi_sim, jpn, and many others (full list of supported OCR languages is here (https://developer.pdf.co/api/pdf-make-text-searchable-or-unsearchable/index.html#language-support). You can also use 2 languages simultaneously like this: eng+deu or jpn+kor (any combination). */
    lang?: string;
    inline?: boolean;
    /** Unwrap lines into a single line within table cells when lineGrouping is enabled. */
    unwrap?: string;
    /** Comma-separated list of page indices (or ranges) to process. The very first page starts at 0 (zero). To set a range use the dash -, for example: 0,2-5,7-. To set a range from the index to the last page use range like this: 2- (from page #3 as the index starts at zero and till the of the document). For ALL pages just leave this param empty. Example: 0,2-5,7- means first page, then 3rd page to 6th page, and then the range from 8th (index = 7) page till the end of the document. */
    pages?: string;
    /** Defines coordinates for extraction, e.g. 51.8, 114.8, 235.5, 204.0. Use PDF.co PDF Edit Add Helper (https://app.pdf.co/pdf-edit-add-helper) to get or measure pdf coordinates. */
    rect?: string;
    /** Set to true for long processes to run in the background, API will then return a jobId which you can use with JobCheck endpoint to check the status of the process and retrieve the output while you can proceed with other tasks without waiting for this process to finish. */
    async?: boolean;
    /** File name for the generated output */
    name?: string;
    /** password */
    password?: string;
    /** Line grouping within table cells. Set to 1 to enable the grouping. */
    lineGrouping?: string;
    /** Set the expiration time for the output link in minutes (default is 60 i.e 60 minutes or 1 hour), After this specified duration, any generated output file(s) will be automatically deleted from PDF.co temporary files storage. The maximum duration for link expiration varies based on your current subscription plan. */
    expiration?: number;
    /** This parameter can be used to set additional configurations for fine-tuning and to enable more options. Visit PDF.co knowledgebase (https://developer.pdf.co/api/profiles/index.html) for profile examples and more. */
    profiles?: string;
}

/**
 * Response for PDF To CSV
 */
export interface PDFToCSVResponse {
    /** body */
    body?: string;
    /** url */
    url?: string;
    /** jobId */
    jobId?: string;
    /** error */
    error?: boolean;
    /** status */
    status?: number;
    /** name */
    name?: string;
    /** credits */
    credits?: number;
    /** remainingCredits */
    remainingCredits?: number;
    /** duration */
    duration?: number;
}

/**
 * PDF To JSON
 */
export interface PDFToJSONInput {
    /** URL to the source file. Supports links from Google Drive, Dropbox, and PDF.co built-in files storage. */
    url: string;
    /** Set the language for OCR (text from image) to use for scanned PDF, PNG, and JPG documents input when extracting text. The default is “eng”. Other languages are also supported: deu, spa, chi_sim, jpn, and many others (full list of supported OCR languages is here (https://developer.pdf.co/api/pdf-make-text-searchable-or-unsearchable/index.html#language-support). You can also use 2 languages simultaneously like this: eng+deu or jpn+kor (any combination). */
    lang?: string;
    inline?: boolean;
    /** Unwrap lines into a single line within table cells when lineGrouping is enabled. */
    unwrap?: string;
    /** Comma-separated list of page indices (or ranges) to process. The very first page starts at 0 (zero). To set a range use the dash -, for example: 0,2-5,7-. To set a range from the index to the last page use range like this: 2- (from page #3 as the index starts at zero and till the of the document). For ALL pages just leave this param empty. Example: 0,2-5,7- means first page, then 3rd page to 6th page, and then the range from 8th (index = 7) page till the end of the document. */
    pages?: string;
    /** Defines coordinates for extraction, e.g. 51.8, 114.8, 235.5, 204.0. Use PDF.co PDF Edit Add Helper (https://app.pdf.co/pdf-edit-add-helper) to get or measure pdf coordinates. */
    rect?: string;
    /** Set to true for long processes to run in the background, API will then return a jobId which you can use with JobCheck endpoint to check the status of the process and retrieve the output while you can proceed with other tasks without waiting for this process to finish. */
    async?: boolean;
    /** File name for the generated output */
    name?: string;
    /** password */
    password?: string;
    /** Line grouping within table cells. Set to 1 to enable the grouping. */
    lineGrouping?: string;
    /** Set the expiration time for the output link in minutes (default is 60 i.e 60 minutes or 1 hour), After this specified duration, any generated output file(s) will be automatically deleted from PDF.co temporary files storage. The maximum duration for link expiration varies based on your current subscription plan. */
    expiration?: number;
    /** This parameter can be used to set additional configurations for fine-tuning and to enable more options. Visit PDF.co knowledgebase (https://developer.pdf.co/api/profiles/index.html) for profile examples and more. */
    profiles?: string;
}

/**
 * Response for PDF To JSON
 */
export interface PDFToJSONResponse {
    /** body */
    body?: Record<string, unknown>;
    /** url */
    url?: string;
    /** jobId */
    jobId?: string;
    /** pageCount */
    pageCount?: number;
    /** error */
    error?: boolean;
    /** status */
    status?: number;
    /** name */
    name?: string;
    /** credits */
    credits?: number;
    /** remainingCredits */
    remainingCredits?: number;
    /** duration */
    duration?: number;
}

/**
 * PDF To JSON (AI Powered)
 */
export interface PDFToJSONMetaInput {
    /** URL to the source file. Supports links from Google Drive, Dropbox, and PDF.co built-in files storage. */
    url: string;
    /** Set the language for OCR (text from image) to use for scanned PDF, PNG, and JPG documents input when extracting text. The default is “eng”. Other languages are also supported: deu, spa, chi_sim, jpn, and many others (full list of supported OCR languages is here (https://developer.pdf.co/api/pdf-make-text-searchable-or-unsearchable/index.html#language-support). You can also use 2 languages simultaneously like this: eng+deu or jpn+kor (any combination). */
    lang?: string;
    inline?: boolean;
    /** Unwrap lines into a single line within table cells when lineGrouping is enabled. */
    unwrap?: string;
    /** Comma-separated list of page indices (or ranges) to process. The very first page starts at 0 (zero). To set a range use the dash -, for example: 0,2-5,7-. To set a range from the index to the last page use range like this: 2- (from page #3 as the index starts at zero and till the of the document). For ALL pages just leave this param empty. Example: 0,2-5,7- means first page, then 3rd page to 6th page, and then the range from 8th (index = 7) page till the end of the document. */
    pages?: string;
    /** Defines coordinates for extraction, e.g. 51.8, 114.8, 235.5, 204.0. Use PDF.co PDF Edit Add Helper (https://app.pdf.co/pdf-edit-add-helper) to get or measure pdf coordinates. */
    rect?: string;
    /** Set to true for long processes to run in the background, API will then return a jobId which you can use with JobCheck endpoint to check the status of the process and retrieve the output while you can proceed with other tasks without waiting for this process to finish. */
    async?: boolean;
    /** File name for the generated output */
    name?: string;
    /** password */
    password?: string;
    /** Line grouping within table cells. Set to 1 to enable the grouping. */
    lineGrouping?: string;
    /** Set the expiration time for the output link in minutes (default is 60 i.e 60 minutes or 1 hour), After this specified duration, any generated output file(s) will be automatically deleted from PDF.co temporary files storage. The maximum duration for link expiration varies based on your current subscription plan. */
    expiration?: number;
    /** This parameter can be used to set additional configurations for fine-tuning and to enable more options. Visit PDF.co knowledgebase (https://developer.pdf.co/api/profiles/index.html) for profile examples and more. */
    profiles?: string;
}

/**
 * Response for PDF To JSON (AI Powered)
 */
export interface PDFToJSONMetaResponse {
    /** body */
    body?: Record<string, unknown>;
    /** url */
    url?: string;
    /** jobId */
    jobId?: string;
    /** pageCount */
    pageCount?: number;
    /** error */
    error?: boolean;
    /** status */
    status?: number;
    /** name */
    name?: string;
    /** credits */
    credits?: number;
    /** remainingCredits */
    remainingCredits?: number;
    /** duration */
    duration?: number;
}

/**
 * PDF To Text (with layout and ocr)
 */
export interface PDFToTextInput {
    /** URL to the source file. Supports links from Google Drive, Dropbox, and PDF.co built-in files storage. */
    url: string;
    /** Set the language for OCR (text from image) to use for scanned PDF, PNG, and JPG documents input when extracting text. The default is “eng”. Other languages are also supported: deu, spa, chi_sim, jpn, and many others (full list of supported OCR languages is here (https://developer.pdf.co/api/pdf-make-text-searchable-or-unsearchable/index.html#language-support). You can also use 2 languages simultaneously like this: eng+deu or jpn+kor (any combination). */
    lang?: string;
    inline?: boolean;
    /** Unwrap lines into a single line within table cells when lineGrouping is enabled. */
    unwrap?: string;
    /** Comma-separated list of page indices (or ranges) to process. The very first page starts at 0 (zero). To set a range use the dash -, for example: 0,2-5,7-. To set a range from the index to the last page use range like this: 2- (from page #3 as the index starts at zero and till the of the document). For ALL pages just leave this param empty. Example: 0,2-5,7- means first page, then 3rd page to 6th page, and then the range from 8th (index = 7) page till the end of the document. */
    pages?: string;
    /** Defines coordinates for extraction, e.g. 51.8, 114.8, 235.5, 204.0. Use PDF.co PDF Edit Add Helper (https://app.pdf.co/pdf-edit-add-helper) to get or measure pdf coordinates. */
    rect?: string;
    /** Set to true for long processes to run in the background, API will then return a jobId which you can use with JobCheck endpoint to check the status of the process and retrieve the output while you can proceed with other tasks without waiting for this process to finish. */
    async?: boolean;
    /** File name for the generated output */
    name?: string;
    /** password */
    password?: string;
    /** Line grouping within table cells. Set to 1 to enable the grouping. */
    lineGrouping?: string;
    /** Set the expiration time for the output link in minutes (default is 60 i.e 60 minutes or 1 hour), After this specified duration, any generated output file(s) will be automatically deleted from PDF.co temporary files storage. The maximum duration for link expiration varies based on your current subscription plan. */
    expiration?: number;
    /** This parameter can be used to set additional configurations for fine-tuning and to enable more options. Visit PDF.co knowledgebase (https://developer.pdf.co/api/profiles/index.html) for profile examples and more. */
    profiles?: string;
}

/**
 * Response for PDF To Text (with layout and ocr)
 */
export interface PDFToTextResponse {
    /** body */
    body?: string;
    /** url */
    url?: string;
    /** jobId */
    jobId?: string;
    /** pageCount */
    pageCount?: number;
    /** error */
    error?: boolean;
    /** status */
    status?: number;
    /** name */
    name?: string;
    /** credits */
    credits?: number;
    /** remainingCredits */
    remainingCredits?: number;
    /** duration */
    duration?: number;
}

/**
 * PDF To Text (without layout and ocr)
 */
export interface PDFToTextSimpleInput {
    /** URL to the source file. Supports links from Google Drive, Dropbox, and PDF.co built-in files storage. */
    url: string;
    inline?: boolean;
    /** Comma-separated list of page indices (or ranges) to process. The very first page starts at 0 (zero). To set a range use the dash -, for example: 0,2-5,7-. To set a range from the index to the last page use range like this: 2- (from page #3 as the index starts at zero and till the of the document). For ALL pages just leave this param empty. Example: 0,2-5,7- means first page, then 3rd page to 6th page, and then the range from 8th (index = 7) page till the end of the document. */
    pages?: string;
    /** Set to true for long processes to run in the background, API will then return a jobId which you can use with JobCheck endpoint to check the status of the process and retrieve the output while you can proceed with other tasks without waiting for this process to finish. */
    async?: boolean;
    /** File name for the generated output */
    name?: string;
    /** password */
    password?: string;
    /** Set the expiration time for the output link in minutes (default is 60 i.e 60 minutes or 1 hour), After this specified duration, any generated output file(s) will be automatically deleted from PDF.co temporary files storage. The maximum duration for link expiration varies based on your current subscription plan. */
    expiration?: number;
}

/**
 * Response for PDF To Text (without layout and ocr)
 */
export interface PDFToTextSimpleResponse {
    /** body */
    body?: string;
    /** url */
    url?: string;
    /** jobId */
    jobId?: string;
    /** pageCount */
    pageCount?: number;
    /** error */
    error?: boolean;
    /** status */
    status?: number;
    /** name */
    name?: string;
    /** credits */
    credits?: number;
    /** remainingCredits */
    remainingCredits?: number;
    /** duration */
    duration?: number;
}

/**
 * PDF To XLS
 */
export interface PDFToXLSInput {
    /** URL to the source file. Supports links from Google Drive, Dropbox, and PDF.co built-in files storage. */
    url: string;
    /** Set the language for OCR (text from image) to use for scanned PDF, PNG, and JPG documents input when extracting text. The default is “eng”. Other languages are also supported: deu, spa, chi_sim, jpn, and many others (full list of supported OCR languages is here (https://developer.pdf.co/api/pdf-make-text-searchable-or-unsearchable/index.html#language-support). You can also use 2 languages simultaneously like this: eng+deu or jpn+kor (any combination). */
    lang?: string;
    /** Unwrap lines into a single line within table cells when lineGrouping is enabled. */
    unwrap?: string;
    /** Comma-separated list of page indices (or ranges) to process. The very first page starts at 0 (zero). To set a range use the dash -, for example: 0,2-5,7-. To set a range from the index to the last page use range like this: 2- (from page #3 as the index starts at zero and till the of the document). For ALL pages just leave this param empty. Example: 0,2-5,7- means first page, then 3rd page to 6th page, and then the range from 8th (index = 7) page till the end of the document. */
    pages?: string;
    /** Defines coordinates for extraction, e.g. 51.8, 114.8, 235.5, 204.0. Use PDF.co PDF Edit Add Helper (https://app.pdf.co/pdf-edit-add-helper) to get or measure pdf coordinates. */
    rect?: string;
    /** Set to true for long processes to run in the background, API will then return a jobId which you can use with JobCheck endpoint to check the status of the process and retrieve the output while you can proceed with other tasks without waiting for this process to finish. */
    async?: boolean;
    /** File name for the generated output */
    name?: string;
    /** password */
    password?: string;
    /** Line grouping within table cells. Set to 1 to enable the grouping. */
    lineGrouping?: string;
    /** Set the expiration time for the output link in minutes (default is 60 i.e 60 minutes or 1 hour), After this specified duration, any generated output file(s) will be automatically deleted from PDF.co temporary files storage. The maximum duration for link expiration varies based on your current subscription plan. */
    expiration?: number;
    /** This parameter can be used to set additional configurations for fine-tuning and to enable more options. Visit PDF.co knowledgebase (https://developer.pdf.co/api/profiles/index.html) for profile examples and more. */
    profiles?: string;
}

/**
 * Response for PDF To XLS
 */
export interface PDFToXLSResponse {
    /** url */
    url?: string;
    /** jobId */
    jobId?: string;
    /** pageCount */
    pageCount?: number;
    /** error */
    error?: boolean;
    /** status */
    status?: number;
    /** name */
    name?: string;
    /** credits */
    credits?: number;
    /** remainingCredits */
    remainingCredits?: number;
    /** duration */
    duration?: number;
}

/**
 * PDF To XLSX
 */
export interface PDFToXLSXInput {
    /** URL to the source file. Supports links from Google Drive, Dropbox, and PDF.co built-in files storage. */
    url: string;
    /** Set the language for OCR (text from image) to use for scanned PDF, PNG, and JPG documents input when extracting text. The default is “eng”. Other languages are also supported: deu, spa, chi_sim, jpn, and many others (full list of supported OCR languages is here (https://developer.pdf.co/api/pdf-make-text-searchable-or-unsearchable/index.html#language-support). You can also use 2 languages simultaneously like this: eng+deu or jpn+kor (any combination). */
    lang?: string;
    /** Unwrap lines into a single line within table cells when lineGrouping is enabled. */
    unwrap?: string;
    /** Comma-separated list of page indices (or ranges) to process. The very first page starts at 0 (zero). To set a range use the dash -, for example: 0,2-5,7-. To set a range from the index to the last page use range like this: 2- (from page #3 as the index starts at zero and till the of the document). For ALL pages just leave this param empty. Example: 0,2-5,7- means first page, then 3rd page to 6th page, and then the range from 8th (index = 7) page till the end of the document. */
    pages?: string;
    /** Defines coordinates for extraction, e.g. 51.8, 114.8, 235.5, 204.0. Use PDF.co PDF Edit Add Helper (https://app.pdf.co/pdf-edit-add-helper) to get or measure pdf coordinates. */
    rect?: string;
    /** Set to true for long processes to run in the background, API will then return a jobId which you can use with JobCheck endpoint to check the status of the process and retrieve the output while you can proceed with other tasks without waiting for this process to finish. */
    async?: boolean;
    /** File name for the generated output */
    name?: string;
    /** password */
    password?: string;
    /** Line grouping within table cells. Set to 1 to enable the grouping. */
    lineGrouping?: string;
    /** Set the expiration time for the output link in minutes (default is 60 i.e 60 minutes or 1 hour), After this specified duration, any generated output file(s) will be automatically deleted from PDF.co temporary files storage. The maximum duration for link expiration varies based on your current subscription plan. */
    expiration?: number;
    /** This parameter can be used to set additional configurations for fine-tuning and to enable more options. Visit PDF.co knowledgebase (https://developer.pdf.co/api/profiles/index.html) for profile examples and more. */
    profiles?: string;
}

/**
 * Response for PDF To XLSX
 */
export interface PDFToXLSXResponse {
    /** url */
    url?: string;
    /** jobId */
    jobId?: string;
    /** pageCount */
    pageCount?: number;
    /** error */
    error?: boolean;
    /** status */
    status?: number;
    /** name */
    name?: string;
    /** credits */
    credits?: number;
    /** remainingCredits */
    remainingCredits?: number;
    /** duration */
    duration?: number;
}

/**
 * PDF To XML
 */
export interface PDFToXMLInput {
    /** URL to the source file. Supports links from Google Drive, Dropbox, and PDF.co built-in files storage. */
    url: string;
    inline?: boolean;
    /** Set the language for OCR (text from image) to use for scanned PDF, PNG, and JPG documents input when extracting text. The default is “eng”. Other languages are also supported: deu, spa, chi_sim, jpn, and many others (full list of supported OCR languages is here (https://developer.pdf.co/api/pdf-make-text-searchable-or-unsearchable/index.html#language-support). You can also use 2 languages simultaneously like this: eng+deu or jpn+kor (any combination). */
    lang?: string;
    /** Unwrap lines into a single line within table cells when lineGrouping is enabled. */
    unwrap?: string;
    /** Comma-separated list of page indices (or ranges) to process. The very first page starts at 0 (zero). To set a range use the dash -, for example: 0,2-5,7-. To set a range from the index to the last page use range like this: 2- (from page #3 as the index starts at zero and till the of the document). For ALL pages just leave this param empty. Example: 0,2-5,7- means first page, then 3rd page to 6th page, and then the range from 8th (index = 7) page till the end of the document. */
    pages?: string;
    /** Defines coordinates for extraction, e.g. 51.8, 114.8, 235.5, 204.0. Use PDF.co PDF Edit Add Helper (https://app.pdf.co/pdf-edit-add-helper) to get or measure pdf coordinates. */
    rect?: string;
    /** Set to true for long processes to run in the background, API will then return a jobId which you can use with JobCheck endpoint to check the status of the process and retrieve the output while you can proceed with other tasks without waiting for this process to finish. */
    async?: boolean;
    /** File name for the generated output */
    name?: string;
    /** password */
    password?: string;
    /** Line grouping within table cells. Set to 1 to enable the grouping. */
    lineGrouping?: string;
    /** Set the expiration time for the output link in minutes (default is 60 i.e 60 minutes or 1 hour), After this specified duration, any generated output file(s) will be automatically deleted from PDF.co temporary files storage. The maximum duration for link expiration varies based on your current subscription plan. */
    expiration?: number;
    /** This parameter can be used to set additional configurations for fine-tuning and to enable more options. Visit PDF.co knowledgebase (https://developer.pdf.co/api/profiles/index.html) for profile examples and more. */
    profiles?: string;
}

/**
 * Response for PDF To XML
 */
export interface PDFToXMLResponse {
    /** url */
    url?: string;
    /** body */
    body?: string;
    /** jobId */
    jobId?: string;
    /** pageCount */
    pageCount?: number;
    /** error */
    error?: boolean;
    /** status */
    status?: number;
    /** name */
    name?: string;
    /** credits */
    credits?: number;
    /** remainingCredits */
    remainingCredits?: number;
    /** duration */
    duration?: number;
}

/**
 * PDF to JPG
 */
export interface PDFToJPGInput {
    /** URL to the source file. Supports links from Google Drive, Dropbox, and PDF.co built-in files storage. */
    url: string;
    /** inline */
    inline?: boolean;
    /** Comma-separated list of page indices (or ranges) to process. The very first page starts at 0 (zero). To set a range use the dash -, for example: 0,2-5,7-. To set a range from the index to the last page use range like this: 2- (from page #3 as the index starts at zero and till the of the document). For ALL pages just leave this param empty. Example: 0,2-5,7- means first page, then 3rd page to 6th page, and then the range from 8th (index = 7) page till the end of the document. */
    pages?: string;
    /** Defines coordinates for extraction, e.g. 51.8, 114.8, 235.5, 204.0. Use PDF.co PDF Edit Add Helper (https://app.pdf.co/pdf-edit-add-helper) to get or measure pdf coordinates. */
    rect?: string;
    /** Set the expiration time for the output link in minutes (default is 60 i.e 60 minutes or 1 hour), After this specified duration, any generated output file(s) will be automatically deleted from PDF.co temporary files storage. The maximum duration for link expiration varies based on your current subscription plan. */
    expiration?: number;
    /** Set to true for long processes to run in the background, API will then return a jobId which you can use with JobCheck endpoint to check the status of the process and retrieve the output while you can proceed with other tasks without waiting for this process to finish. */
    async?: boolean;
    /** You can set additional and extra options using this parameter that allows you to set custom configuration. Explore PDF.co knowledgebase (https://developer.pdf.co/api/profiles/index.html) for profile examples. */
    profiles?: string;
}

/**
 * Response for PDF to JPG
 */
export interface PDFToJPGResponse {
    /** urls */
    urls?: Array<string>;
    /** jobId */
    jobId?: string;
    /** pageCount */
    pageCount?: number;
    /** error */
    error?: boolean;
    /** status */
    status?: number;
    /** name */
    name?: string;
    /** credits */
    credits?: number;
    /** remainingCredits */
    remainingCredits?: number;
    /** duration */
    duration?: number;
}

/**
 * PDF to PNG
 */
export interface PDFToPNGInput {
    /** URL to the source file. Supports links from Google Drive, Dropbox, and PDF.co built-in files storage. */
    url: string;
    /** inline */
    inline?: boolean;
    /** Comma-separated list of page indices (or ranges) to process. The very first page starts at 0 (zero). To set a range use the dash -, for example: 0,2-5,7-. To set a range from the index to the last page use range like this: 2- (from page #3 as the index starts at zero and till the of the document). For ALL pages just leave this param empty. Example: 0,2-5,7- means first page, then 3rd page to 6th page, and then the range from 8th (index = 7) page till the end of the document. */
    pages?: string;
    /** Defines coordinates for extraction, e.g. 51.8, 114.8, 235.5, 204.0. Use PDF.co PDF Edit Add Helper (https://app.pdf.co/pdf-edit-add-helper) to get or measure pdf coordinates. */
    rect?: string;
    /** Set the expiration time for the output link in minutes (default is 60 i.e 60 minutes or 1 hour), After this specified duration, any generated output file(s) will be automatically deleted from PDF.co temporary files storage. The maximum duration for link expiration varies based on your current subscription plan. */
    expiration?: number;
    /** Set to true for long processes to run in the background, API will then return a jobId which you can use with JobCheck endpoint to check the status of the process and retrieve the output while you can proceed with other tasks without waiting for this process to finish. */
    async?: boolean;
    /** You can set additional and extra options using this parameter that allows you to set custom configuration. Explore PDF.co knowledgebase (https://developer.pdf.co/api/profiles/index.html) for profile examples. */
    profiles?: string;
}

/**
 * Response for PDF to PNG
 */
export interface PDFToPNGResponse {
    /** urls */
    urls?: Array<string>;
    /** jobId */
    jobId?: string;
    /** pageCount */
    pageCount?: number;
    /** error */
    error?: boolean;
    /** status */
    status?: number;
    /** name */
    name?: string;
    /** credits */
    credits?: number;
    /** remainingCredits */
    remainingCredits?: number;
    /** duration */
    duration?: number;
}

/**
 * PDF to WEBP
 */
export interface PDFToWEBPInput {
    /** URL to the source file. Supports links from Google Drive, Dropbox, and PDF.co built-in files storage. */
    url: string;
    /** inline */
    inline?: boolean;
    /** Comma-separated list of page indices (or ranges) to process. The very first page starts at 0 (zero). To set a range use the dash -, for example: 0,2-5,7-. To set a range from the index to the last page use range like this: 2- (from page #3 as the index starts at zero and till the of the document). For ALL pages just leave this param empty. Example: 0,2-5,7- means first page, then 3rd page to 6th page, and then the range from 8th (index = 7) page till the end of the document. */
    pages?: string;
    /** Defines coordinates for extraction, e.g. 51.8, 114.8, 235.5, 204.0. Use PDF.co PDF Edit Add Helper (https://app.pdf.co/pdf-edit-add-helper) to get or measure pdf coordinates. */
    rect?: string;
    /** Set the expiration time for the output link in minutes (default is 60 i.e 60 minutes or 1 hour), After this specified duration, any generated output file(s) will be automatically deleted from PDF.co temporary files storage. The maximum duration for link expiration varies based on your current subscription plan. */
    expiration?: number;
    /** Set to true for long processes to run in the background, API will then return a jobId which you can use with JobCheck endpoint to check the status of the process and retrieve the output while you can proceed with other tasks without waiting for this process to finish. */
    async?: boolean;
    /** You can set additional and extra options using this parameter that allows you to set custom configuration. Explore PDF.co knowledgebase (https://developer.pdf.co/api/profiles/index.html) for profile examples. */
    profiles?: string;
}

/**
 * Response for PDF to WEBP
 */
export interface PDFToWEBPResponse {
    /** urls */
    urls?: Array<string>;
    /** jobId */
    jobId?: string;
    /** pageCount */
    pageCount?: number;
    /** error */
    error?: boolean;
    /** status */
    status?: number;
    /** name */
    name?: string;
    /** credits */
    credits?: number;
    /** remainingCredits */
    remainingCredits?: number;
    /** duration */
    duration?: number;
}

/**
 * PDF to TIFF
 */
export interface PDFToTIFFInput {
    /** URL to the source file. Supports links from Google Drive, Dropbox, and PDF.co built-in files storage. */
    url: string;
    /** inline */
    inline?: boolean;
    /** Comma-separated list of page indices (or ranges) to process. The very first page starts at 0 (zero). To set a range use the dash -, for example: 0,2-5,7-. To set a range from the index to the last page use range like this: 2- (from page #3 as the index starts at zero and till the of the document). For ALL pages just leave this param empty. Example: 0,2-5,7- means first page, then 3rd page to 6th page, and then the range from 8th (index = 7) page till the end of the document. */
    pages?: string;
    /** Defines coordinates for extraction, e.g. 51.8, 114.8, 235.5, 204.0. Use PDF.co PDF Edit Add Helper (https://app.pdf.co/pdf-edit-add-helper) to get or measure pdf coordinates. */
    rect?: string;
    /** Set the expiration time for the output link in minutes (default is 60 i.e 60 minutes or 1 hour), After this specified duration, any generated output file(s) will be automatically deleted from PDF.co temporary files storage. The maximum duration for link expiration varies based on your current subscription plan. */
    expiration?: number;
    /** Set to true for long processes to run in the background, API will then return a jobId which you can use with JobCheck endpoint to check the status of the process and retrieve the output while you can proceed with other tasks without waiting for this process to finish. */
    async?: boolean;
    /** You can set additional and extra options using this parameter that allows you to set custom configuration. Explore PDF.co knowledgebase (https://developer.pdf.co/api/profiles/index.html) for profile examples. */
    profiles?: string;
}

/**
 * Response for PDF to TIFF
 */
export interface PDFToTIFFResponse {
    url?: string;
    /** jobId */
    jobId?: string;
    /** pageCount */
    pageCount?: number;
    /** error */
    error?: boolean;
    /** status */
    status?: number;
    /** name */
    name?: string;
    /** credits */
    credits?: number;
    /** remainingCredits */
    remainingCredits?: number;
    /** duration */
    duration?: number;
}

/**
 * PDF from CSV
 */
export interface PDFFromCSVInput {
    /** URL to the source file. Supports links from Google Drive, Dropbox, and PDF.co built-in files storage. */
    url: string;
    /** Comma-separated list of page indices (or ranges) to process. The very first page starts at 0 (zero). To set a range use the dash -, for example: 0,2-5,7-. To set a range from the index to the last page use range like this: 2- (from page #3 as the index starts at zero and till the of the document). For ALL pages just leave this param empty. Example: 0,2-5,7- means first page, then 3rd page to 6th page, and then the range from 8th (index = 7) page till the end of the document. */
    pages?: string;
    /** File name for the generated output. */
    name?: string;
    /** Set the expiration time for the output link in minutes (default is 60 i.e 60 minutes or 1 hour), After this specified duration, any generated output file(s) will be automatically deleted from PDF.co temporary files storage. The maximum duration for link expiration varies based on your current subscription plan. */
    expiration?: number;
    /** Set to true for long processes to run in the background, API will then return a jobId which you can use with JobCheck endpoint to check the status of the process and retrieve the output while you can proceed with other tasks without waiting for this process to finish. */
    async?: boolean;
    /** You can set additional and extra options using this parameter that allows you to set custom configuration. Explore PDF.co knowledgebase (https://developer.pdf.co/api/profiles/index.html) for profile examples. */
    profiles?: string;
}

/**
 * Response for PDF from CSV
 */
export interface PDFFromCSVResponse {
    url?: string;
    /** jobId */
    jobId?: string;
    /** pageCount */
    pageCount?: number;
    /** error */
    error?: boolean;
    /** status */
    status?: number;
    /** name */
    name?: string;
    /** credits */
    credits?: number;
    /** remainingCredits */
    remainingCredits?: number;
    /** duration */
    duration?: number;
}

/**
 * PDF from Document (Doc, DocX, RTF, TXT, XPS)
 */
export interface PDFFromDocInput {
    /** URL to the source file. Supports links from Google Drive, Dropbox, and PDF.co built-in files storage. */
    url: string;
    /** Comma-separated list of page indices (or ranges) to process. The very first page starts at 0 (zero). To set a range use the dash -, for example: 0,2-5,7-. To set a range from the index to the last page use range like this: 2- (from page #3 as the index starts at zero and till the of the document). For ALL pages just leave this param empty. Example: 0,2-5,7- means first page, then 3rd page to 6th page, and then the range from 8th (index = 7) page till the end of the document. */
    pages?: string;
    /** File name for the generated output. */
    name?: string;
    /** Set the expiration time for the output link in minutes (default is 60 i.e 60 minutes or 1 hour), After this specified duration, any generated output file(s) will be automatically deleted from PDF.co temporary files storage. The maximum duration for link expiration varies based on your current subscription plan. */
    expiration?: number;
    /** Set to true for long processes to run in the background, API will then return a jobId which you can use with JobCheck endpoint to check the status of the process and retrieve the output while you can proceed with other tasks without waiting for this process to finish. */
    async?: boolean;
    /** You can set additional and extra options using this parameter that allows you to set custom configuration. Explore PDF.co knowledgebase (https://developer.pdf.co/api/profiles/index.html) for profile examples. */
    profiles?: string;
}

/**
 * Response for PDF from Document (Doc, DocX, RTF, TXT, XPS)
 */
export interface PDFFromDocResponse {
    url?: string;
    /** jobId */
    jobId?: string;
    /** pageCount */
    pageCount?: number;
    /** error */
    error?: boolean;
    /** status */
    status?: number;
    /** name */
    name?: string;
    /** credits */
    credits?: number;
    /** remainingCredits */
    remainingCredits?: number;
    /** duration */
    duration?: number;
}

/**
 * PDF from Image
 */
export interface PDFFromImagesInput {
    /** URL to the source file. Provide multiple URLS in comma seperated format. Supports links from Google Drive, Dropbox, and PDF.co built-in files storage. */
    url: string;
    /** File name for the generated output. */
    name?: string;
    /** Set the expiration time for the output link in minutes (default is 60 i.e 60 minutes or 1 hour), After this specified duration, any generated output file(s) will be automatically deleted from PDF.co temporary files storage. The maximum duration for link expiration varies based on your current subscription plan. */
    expiration?: number;
    /** Set to true for long processes to run in the background, API will then return a jobId which you can use with JobCheck endpoint to check the status of the process and retrieve the output while you can proceed with other tasks without waiting for this process to finish. */
    async?: boolean;
    /** You can set additional and extra options using this parameter that allows you to set custom configuration. Explore PDF.co knowledgebase (https://developer.pdf.co/api/profiles/index.html) for profile examples. */
    profiles?: string;
}

/**
 * Response for PDF from Image
 */
export interface PDFFromImagesResponse {
    url?: string;
    /** jobId */
    jobId?: string;
    /** pageCount */
    pageCount?: number;
    /** error */
    error?: boolean;
    /** status */
    status?: number;
    /** name */
    name?: string;
    /** credits */
    credits?: number;
    /** remainingCredits */
    remainingCredits?: number;
    /** duration */
    duration?: number;
}

/**
 * PDF from Email
 */
export interface PDFFromEmailInput {
    /** URL to the source file. Supports links from Google Drive, Dropbox, and PDF.co built-in files storage. */
    url: string;
    /** Set to true to automatically embeds all attachments from original input email MSG or EML files into the final output PDF. Set it to false if you don’t want to embed attachments so it will convert only the body of the input email. */
    embedAttachments?: boolean;
    /** Set to false if you don’t want to convert attachments from the original email and want to embed them as original files (as embedded pdf attachments). Converts attachments that are supported by API (DOC, DOCx, HTML, PNG, JPG .etc) into PDF and merges into output final PDF. Non-supported file types are added as PDF attachments (Adobe Reader or another viewer may be required to view PDF attachments). */
    convertAttachments?: boolean;
    /** set to CSS style margins like 10px, 5mm, 5in for all sides or 5px 5px 5px 5px (the order of margins is top, right, bottom, left). */
    margins?: string;
    /** Letter is set by default. Can be Letter, Legal, Tabloid, Ledger, A0, A1, A2, A3, A4, A5, A6 or a custom size. Custom size can be set in px (pixels), mm or in (inches) with width and height separated by space like this: 200 300, 200px 300px, 200mm 300mm, 20cm 30cm or 6in 8in. */
    paperSize?: string;
    orientation?: string;
    /** File name for the generated output. */
    name?: string;
    /** Set the expiration time for the output link in minutes (default is 60 i.e 60 minutes or 1 hour), After this specified duration, any generated output file(s) will be automatically deleted from PDF.co temporary files storage. The maximum duration for link expiration varies based on your current subscription plan. */
    expiration?: number;
    /** Set to true for long processes to run in the background, API will then return a jobId which you can use with JobCheck endpoint to check the status of the process and retrieve the output while you can proceed with other tasks without waiting for this process to finish. */
    async?: boolean;
    /** You can set additional and extra options using this parameter that allows you to set custom configuration. Explore PDF.co knowledgebase (https://developer.pdf.co/api/profiles/index.html) for profile examples. */
    profiles?: string;
}

/**
 * Response for PDF from Email
 */
export interface PDFFromEmailResponse {
    url?: string;
    /** jobId */
    jobId?: string;
    /** pageCount */
    pageCount?: number;
    /** error */
    error?: boolean;
    /** status */
    status?: number;
    /** name */
    name?: string;
    /** credits */
    credits?: number;
    /** remainingCredits */
    remainingCredits?: number;
    /** duration */
    duration?: number;
}

/**
 * PDF Add Security
 */
export interface PDFAddSecurityInput {
    /** URL to the source file. Supports links from Google Drive, Dropbox, and PDF.co built-in files storage. */
    url: string;
    /** The main owner password that is used for document encryption and for setting/removing restrictions. */
    ownerPassword: string;
    /** The optional user password will be asked for viewing and printing document. */
    userPassword?: string;
    /** Encryption algorithm. Valid values: RC4_40bit, RC4_128bit, AES_128bit, AES_256bit. AES_128bit or higher is recommended */
    encryptionAlgorithm?: string;
    /** Allow or prohibit content extraction for accessibility needs. Note: this restriction applies when userPassword (if any) is entered. This restriction does not apply if the user enters Owner Password. */
    allowAccessibilitySupport?: boolean;
    /** Allow or prohibit assembling the document. Note: this restriction applies when userPassword (if any) is entered. This restriction does not apply if the user enters Owner Password. */
    allowAssemblyDocument?: boolean;
    /** Allow or prohibit printing PDF document. Note: this restriction applies when userPassword (if any) is entered. This restriction does not apply if the user enters Owner Password. */
    allowPrintDocument?: boolean;
    /** Allow or prohibit the filling of interactive form fields (including signature fields) in the PDF documents. Note: this restriction applies when userPassword (if any) is entered. This restriction does not apply if the user enters Owner Password. */
    allowFillForms?: boolean;
    /** Allow or prohibit modification of PDF document. Note: this restriction applies when userPassword (if any) is entered. This restriction does not apply if the user enters Owner Password. */
    allowModifyDocument?: boolean;
    /** Allow or prohibit copying content from PDF document. Note: this restriction applies when userPassword (if any) is entered. This restriction does not apply if the user enters Owner Password. */
    allowContentExtraction?: boolean;
    /** Allow or prohibit interacting with text annotations and forms in PDF document. Note: this restriction applies when userPassword (if any) is entered. This restriction does not apply if the user enters Owner Password. */
    allowModifyAnnotations?: boolean;
    /** Allowed printing quality. Valid values: HighResolution, LowResolution. Note: this restriction applies when userPassword (if any) is entered. This restriction does not apply if the user enters Owner Password. */
    printQuality?: string;
    /** File name for the generated output. */
    name?: string;
    /** Set the expiration time for the output link in minutes (default is 60 i.e 60 minutes or 1 hour), After this specified duration, any generated output file(s) will be automatically deleted from PDF.co temporary files storage. The maximum duration for link expiration varies based on your current subscription plan. */
    expiration?: number;
    /** Set to true for long processes to run in the background, API will then return a jobId which you can use with JobCheck endpoint to check the status of the process and retrieve the output while you can proceed with other tasks without waiting for this process to finish. */
    async?: boolean;
    /** You can set additional and extra options using this parameter that allows you to set custom configuration. Explore PDF.co knowledgebase (https://developer.pdf.co/api/profiles/index.html) for profile examples. */
    profiles?: string;
}

/**
 * Response for PDF Add Security
 */
export interface PDFAddSecurityResponse {
    url?: string;
    /** jobId */
    jobId?: string;
    /** pageCount */
    pageCount?: number;
    /** error */
    error?: boolean;
    /** status */
    status?: number;
    /** name */
    name?: string;
    /** credits */
    credits?: number;
    /** remainingCredits */
    remainingCredits?: number;
    /** duration */
    duration?: number;
}

/**
 * PDF Remove Security
 */
export interface PDFSecurityRemoveInput {
    /** URL to the source file. Supports links from Google Drive, Dropbox, and PDF.co built-in files storage. */
    url: string;
    /** The owner/user password to open files and remove security features. */
    password: string;
    /** File name for the generated output. */
    name?: string;
    /** Set the expiration time for the output link in minutes (default is 60 i.e 60 minutes or 1 hour), After this specified duration, any generated output file(s) will be automatically deleted from PDF.co temporary files storage. The maximum duration for link expiration varies based on your current subscription plan. */
    expiration?: number;
    /** Set to true for long processes to run in the background, API will then return a jobId which you can use with JobCheck endpoint to check the status of the process and retrieve the output while you can proceed with other tasks without waiting for this process to finish. */
    async?: boolean;
    /** You can set additional and extra options using this parameter that allows you to set custom configuration. Explore PDF.co knowledgebase (https://developer.pdf.co/api/profiles/index.html) for profile examples. */
    profiles?: string;
}

/**
 * Response for PDF Remove Security
 */
export interface PDFSecurityRemoveResponse {
    url?: string;
    /** jobId */
    jobId?: string;
    /** pageCount */
    pageCount?: number;
    /** error */
    error?: boolean;
    /** status */
    status?: number;
    /** name */
    name?: string;
    /** credits */
    credits?: number;
    /** remainingCredits */
    remainingCredits?: number;
    /** duration */
    duration?: number;
}

/**
 * PDF From Spreadsheet
 */
export interface PDFFromXLSXLSXInput {
    /** URL to the source file. Supports links from Google Drive, Dropbox, and PDF.co built-in files storage. */
    url: string;
    /** Set the WorkSheet index (Default is 0). */
    worksheetIndex?: string;
    /** File name for the generated output. */
    name?: string;
    /** Set the expiration time for the output link in minutes (default is 60 i.e 60 minutes or 1 hour), After this specified duration, any generated output file(s) will be automatically deleted from PDF.co temporary files storage. The maximum duration for link expiration varies based on your current subscription plan. */
    expiration?: number;
    /** Set to true for long processes to run in the background, API will then return a jobId which you can use with JobCheck endpoint to check the status of the process and retrieve the output while you can proceed with other tasks without waiting for this process to finish. */
    async?: boolean;
    /** You can set additional and extra options using this parameter that allows you to set custom configuration. Explore PDF.co knowledgebase (https://developer.pdf.co/api/profiles/index.html) for profile examples. */
    profiles?: string;
}

/**
 * Response for PDF From Spreadsheet
 */
export interface PDFFromXLSXLSXResponse {
    url?: string;
    /** jobId */
    jobId?: string;
    /** pageCount */
    pageCount?: number;
    /** error */
    error?: boolean;
    /** status */
    status?: number;
    /** name */
    name?: string;
    /** credits */
    credits?: number;
    /** remainingCredits */
    remainingCredits?: number;
    /** duration */
    duration?: number;
}

/**
 * Spreadsheet to CSV
 */
export interface XLStoCSVInput {
    /** URL to the source file. Supports links from Google Drive, Dropbox, and PDF.co built-in files storage. */
    url: string;
    /** Set the WorkSheet index (Default is 0). */
    worksheetIndex?: string;
    /** Set the CSV Quotation Symbol */
    quotationSymbol?: string;
    /** Set the CSV Separator Symbol */
    separatorSymbol?: string;
    /** File name for the generated output. */
    name?: string;
    /** Set the expiration time for the output link in minutes (default is 60 i.e 60 minutes or 1 hour), After this specified duration, any generated output file(s) will be automatically deleted from PDF.co temporary files storage. The maximum duration for link expiration varies based on your current subscription plan. */
    expiration?: number;
    /** Set to true for long processes to run in the background, API will then return a jobId which you can use with JobCheck endpoint to check the status of the process and retrieve the output while you can proceed with other tasks without waiting for this process to finish. */
    async?: boolean;
    /** You can set additional and extra options using this parameter that allows you to set custom configuration. Explore PDF.co knowledgebase (https://developer.pdf.co/api/profiles/index.html) for profile examples. */
    profiles?: string;
}

/**
 * Response for Spreadsheet to CSV
 */
export interface XLStoCSVResponse {
    /** url */
    url?: string;
    /** jobId */
    jobId?: string;
    /** error */
    error?: boolean;
    /** status */
    status?: number;
    /** name */
    name?: string;
    /** credits */
    credits?: number;
    /** remainingCredits */
    remainingCredits?: number;
    /** duration */
    duration?: number;
}

/**
 * Spreadsheet to JSON
 */
export interface XLStoJSONInput {
    /** URL to the source file. Supports links from Google Drive, Dropbox, and PDF.co built-in files storage. */
    url: string;
    /** Set the WorkSheet index (Default is 0). */
    worksheetIndex?: string;
    /** File name for the generated output. */
    name?: string;
    /** Set the expiration time for the output link in minutes (default is 60 i.e 60 minutes or 1 hour), After this specified duration, any generated output file(s) will be automatically deleted from PDF.co temporary files storage. The maximum duration for link expiration varies based on your current subscription plan. */
    expiration?: number;
    /** Set to true for long processes to run in the background, API will then return a jobId which you can use with JobCheck endpoint to check the status of the process and retrieve the output while you can proceed with other tasks without waiting for this process to finish. */
    async?: boolean;
    /** You can set additional and extra options using this parameter that allows you to set custom configuration. Explore PDF.co knowledgebase (https://developer.pdf.co/api/profiles/index.html) for profile examples. */
    profiles?: string;
}

/**
 * Response for Spreadsheet to JSON
 */
export interface XLStoJSONResponse {
    /** url */
    url?: string;
    /** jobId */
    jobId?: string;
    /** error */
    error?: boolean;
    /** status */
    status?: number;
    /** name */
    name?: string;
    /** credits */
    credits?: number;
    /** remainingCredits */
    remainingCredits?: number;
    /** duration */
    duration?: number;
}

/**
 * Spreadsheet to HTML
 */
export interface XLStoHTMLInput {
    /** URL to the source file. Supports links from Google Drive, Dropbox, and PDF.co built-in files storage. */
    url: string;
    /** Set the WorkSheet index (Default is 0). */
    worksheetIndex?: string;
    /** File name for the generated output. */
    name?: string;
    /** Set the expiration time for the output link in minutes (default is 60 i.e 60 minutes or 1 hour), After this specified duration, any generated output file(s) will be automatically deleted from PDF.co temporary files storage. The maximum duration for link expiration varies based on your current subscription plan. */
    expiration?: number;
    /** Set to true for long processes to run in the background, API will then return a jobId which you can use with JobCheck endpoint to check the status of the process and retrieve the output while you can proceed with other tasks without waiting for this process to finish. */
    async?: boolean;
    /** You can set additional and extra options using this parameter that allows you to set custom configuration. Explore PDF.co knowledgebase (https://developer.pdf.co/api/profiles/index.html) for profile examples. */
    profiles?: string;
}

/**
 * Response for Spreadsheet to HTML
 */
export interface XLStoHTMLResponse {
    /** url */
    url?: string;
    /** jobId */
    jobId?: string;
    /** error */
    error?: boolean;
    /** status */
    status?: number;
    /** name */
    name?: string;
    /** credits */
    credits?: number;
    /** remainingCredits */
    remainingCredits?: number;
    /** duration */
    duration?: number;
}

/**
 * Spreadsheet to TXT
 */
export interface XLStoTXTInput {
    /** URL to the source file. Supports links from Google Drive, Dropbox, and PDF.co built-in files storage. */
    url: string;
    /** Set the WorkSheet index (Default is 0). */
    worksheetIndex?: string;
    /** File name for the generated output. */
    name?: string;
    /** Set the expiration time for the output link in minutes (default is 60 i.e 60 minutes or 1 hour), After this specified duration, any generated output file(s) will be automatically deleted from PDF.co temporary files storage. The maximum duration for link expiration varies based on your current subscription plan. */
    expiration?: number;
    /** Set to true for long processes to run in the background, API will then return a jobId which you can use with JobCheck endpoint to check the status of the process and retrieve the output while you can proceed with other tasks without waiting for this process to finish. */
    async?: boolean;
    /** You can set additional and extra options using this parameter that allows you to set custom configuration. Explore PDF.co knowledgebase (https://developer.pdf.co/api/profiles/index.html) for profile examples. */
    profiles?: string;
}

/**
 * Response for Spreadsheet to TXT
 */
export interface XLStoTXTResponse {
    /** url */
    url?: string;
    /** jobId */
    jobId?: string;
    /** error */
    error?: boolean;
    /** status */
    status?: number;
    /** name */
    name?: string;
    /** credits */
    credits?: number;
    /** remainingCredits */
    remainingCredits?: number;
    /** duration */
    duration?: number;
}

/**
 * Spreadsheet to XML
 */
export interface XLStoXMLInput {
    /** URL to the source file. Supports links from Google Drive, Dropbox, and PDF.co built-in files storage. */
    url: string;
    /** Set the WorkSheet index (Default is 0). */
    worksheetIndex?: string;
    /** File name for the generated output. */
    name?: string;
    /** Set the expiration time for the output link in minutes (default is 60 i.e 60 minutes or 1 hour), After this specified duration, any generated output file(s) will be automatically deleted from PDF.co temporary files storage. The maximum duration for link expiration varies based on your current subscription plan. */
    expiration?: number;
    /** Set to true for long processes to run in the background, API will then return a jobId which you can use with JobCheck endpoint to check the status of the process and retrieve the output while you can proceed with other tasks without waiting for this process to finish. */
    async?: boolean;
    /** You can set additional and extra options using this parameter that allows you to set custom configuration. Explore PDF.co knowledgebase (https://developer.pdf.co/api/profiles/index.html) for profile examples. */
    profiles?: string;
}

/**
 * Response for Spreadsheet to XML
 */
export interface XLStoXMLResponse {
    /** url */
    url?: string;
    /** jobId */
    jobId?: string;
    /** error */
    error?: boolean;
    /** status */
    status?: number;
    /** name */
    name?: string;
    /** credits */
    credits?: number;
    /** remainingCredits */
    remainingCredits?: number;
    /** duration */
    duration?: number;
}

/**
 * Rotate PDF Pages
 */
export interface PDFRotatePagesInput {
    /** URL to the source file. Supports links from Google Drive, Dropbox, and PDF.co built-in files storage. */
    url: string;
    /** Angle in degrees. Supported values 90, 180, and 270. */
    angle?: number;
    /** Comma-separated list of page indices (or ranges) to process. The very first page starts at 0 (zero). To set a range use the dash -, for example: 0,2-5,7-. To set a range from the index to the last page use range like this: 2- (from page #3 as the index starts at zero and till the of the document). For ALL pages just leave this param empty. Example: 0,2-5,7- means first page, then 3rd page to 6th page, and then the range from 8th (index = 7) page till the end of the document. */
    pages?: string;
    /** Set the expiration time for the output link in minutes (default is 60 i.e 60 minutes or 1 hour), After this specified duration, any generated output file(s) will be automatically deleted from PDF.co temporary files storage. The maximum duration for link expiration varies based on your current subscription plan. */
    expiration?: number;
    /** Password of the PDF file */
    password?: string;
    /** Use this parameter to set additional configurations for fine-tuning and extra options. Explore PDF.co knowledgebase (https://developer.pdf.co/api/profiles/index.html) for profile examples. */
    profiles?: string;
    /** Set async to true for long processes to run in the background, API will then return a jobId which you can use with JobCheck endpoint to check the status of the process and retrieve the output while you can proceed with other tasks without waiting for this process to finish. */
    async?: boolean;
}

/**
 * Response for Rotate PDF Pages
 */
export interface PDFRotatePagesResponse {
    /** url */
    url?: string;
    /** jobId */
    jobId?: string;
    /** pageCount */
    pageCount?: number;
    /** error */
    error?: boolean;
    /** status */
    status?: number;
    /** name */
    name?: string;
    /** credits */
    credits?: number;
    /** remainingCredits */
    remainingCredits?: number;
    /** duration */
    duration?: number;
}

/**
 * Rotate PDF Pages (Auto)
 */
export interface PDFAutoRotatePagesInput {
    /** URL to the source file. Supports links from Google Drive, Dropbox, and PDF.co built-in files storage. */
    url: string;
    /** the language for OCR (text from image) to use for scanned PDF, PNG, and JPG documents input when extracting text. The default is “eng”. Other languages are also supported: deu, spa, chi_sim, jpn, and many others (full list of supported OCR languages is at https://developer.pdf.co/api/pdf-make-text-searchable-or-unsearchable/index.html#language-support). You can also use 2 languages simultaneously like this: eng+deu or jpn+kor (any combination). */
    lang?: string;
    /** Set the expiration time for the output link in minutes (default is 60 i.e 60 minutes or 1 hour), After this specified duration, any generated output file(s) will be automatically deleted from PDF.co temporary files storage. The maximum duration for link expiration varies based on your current subscription plan. */
    expiration?: number;
    /** Password of the PDF file */
    password?: string;
    /** Use this parameter to set additional configurations for fine-tuning and extra options. Explore PDF.co knowledgebase (https://developer.pdf.co/api/profiles/index.html) for profile examples. */
    profiles?: string;
    /** Set async to true for long processes to run in the background, API will then return a jobId which you can use with JobCheck endpoint to check the status of the process and retrieve the output while you can proceed with other tasks without waiting for this process to finish. */
    async?: boolean;
}

/**
 * Response for Rotate PDF Pages (Auto)
 */
export interface PDFAutoRotatePagesResponse {
    /** url */
    url?: string;
    /** jobId */
    jobId?: string;
    /** pageCount */
    pageCount?: number;
    /** error */
    error?: boolean;
    /** status */
    status?: number;
    /** name */
    name?: string;
    /** credits */
    credits?: number;
    /** remainingCredits */
    remainingCredits?: number;
    /** duration */
    duration?: number;
}

/**
 * PDF Delete Pages
 */
export interface PDFDeletePagesInput {
    /** URL to the source file. Supports links from Google Drive, Dropbox, and PDF.co built-in files storage. */
    url: string;
    /** Comma-separated list of page indices (or ranges) to process. The very first page starts at 0 (zero). To set a range use the dash -, for example: 0,2-5,7-. To set a range from the index to the last page use range like this: 2- (from page #3 as the index starts at zero and till the of the document). For ALL pages just leave this param empty. Example: 0,2-5,7- means first page, then 3rd page to 6th page, and then the range from 8th (index = 7) page till the end of the document. */
    pages?: string;
    /** File name for the generated output */
    name?: string;
    /** Set the expiration time for the output link in minutes (default is 60 i.e 60 minutes or 1 hour), After this specified duration, any generated output file(s) will be automatically deleted from PDF.co temporary files storage. The maximum duration for link expiration varies based on your current subscription plan. */
    expiration?: number;
    /** Use this parameter to set additional configurations for fine-tuning and extra options. Explore PDF.co knowledgebase (https://developer.pdf.co/api/profiles/index.html) for profile examples. */
    profiles?: string;
    /** Set async to true for long processes to run in the background, API will then return a jobId which you can use with JobCheck endpoint to check the status of the process and retrieve the output while you can proceed with other tasks without waiting for this process to finish. */
    async?: boolean;
}

/**
 * Response for PDF Delete Pages
 */
export interface PDFDeletePagesResponse {
    /** url */
    url?: string;
    /** jobId */
    jobId?: string;
    /** pageCount */
    pageCount?: number;
    /** error */
    error?: boolean;
    /** status */
    status?: number;
    /** name */
    name?: string;
    /** credits */
    credits?: number;
    /** remainingCredits */
    remainingCredits?: number;
    /** duration */
    duration?: number;
}

/**
 * PDF Compress and Optimize
 */
export interface PDFCompressInput {
    /** URL to the source file. Supports links from Google Drive, Dropbox, and PDF.co built-in files storage. */
    url: string;
    /** File name for the generated output */
    name?: string;
    /** Set the expiration time for the output link in minutes (default is 60 i.e 60 minutes or 1 hour), After this specified duration, any generated output file(s) will be automatically deleted from PDF.co temporary files storage. The maximum duration for link expiration varies based on your current subscription plan. */
    expiration?: number;
    /** Password of the PDF file */
    password?: string;
    /** Use this parameter to set additional configurations for fine-tuning and extra options. Explore PDF.co knowledgebase (https://developer.pdf.co/api/profiles/index.html) for profile examples. */
    profiles?: string;
    /** Set async to true for long processes to run in the background, API will then return a jobId which you can use with JobCheck endpoint to check the status of the process and retrieve the output while you can proceed with other tasks without waiting for this process to finish. */
    async?: boolean;
}

/**
 * Response for PDF Compress and Optimize
 */
export interface PDFCompressResponse {
    /** url */
    url?: string;
    /** jobId */
    jobId?: string;
    /** pageCount */
    pageCount?: number;
    /** error */
    error?: boolean;
    /** status */
    status?: number;
    /** name */
    name?: string;
    /** credits */
    credits?: number;
    /** remainingCredits */
    remainingCredits?: number;
    /** duration */
    duration?: number;
}

/**
 * Document Classifier
 */
export interface PDFClassifierInput {
    /** URL to the source file. Supports links from Google Drive, Dropbox, and PDF.co built-in files storage. */
    url: string;
    /** Define custom classification rules in CSV format. Rules are in CSV format where each row contains: "class name, logic (AND or OR (default)), and keywords" separated by a comma. Each row is separated by the \n symbol. */
    rulescsv?: string;
    /** Instead of inline CSV you can use this parameter and set the URL to a CSV file with classification rules. This is useful if you have a separate developer working on CSV rules. */
    rulescsvurl?: string;
    /** Defines if keywords in rules are case-sensitive or not. */
    caseSensitive?: boolean;
    /** Set to true to return results inside the response. Otherwise, the endpoint will return a link to the output file generated. */
    inline?: boolean;
    /** Set the expiration time for the output link in minutes (default is 60 i.e 60 minutes or 1 hour), After this specified duration, any generated output file(s) will be automatically deleted from PDF.co temporary files storage. The maximum duration for link expiration varies based on your current subscription plan. */
    expiration?: number;
    /** Password of the PDF file */
    password?: string;
    /** Use this parameter to set additional configurations for fine-tuning and extra options. Explore PDF.co knowledgebase (https://developer.pdf.co/api/profiles/index.html) for profile examples. */
    profiles?: string;
    /** Set async to true for long processes to run in the background, API will then return a jobId which you can use with JobCheck endpoint to check the status of the process and retrieve the output while you can proceed with other tasks without waiting for this process to finish. */
    async?: boolean;
}

/**
 * Response for Document Classifier
 */
export interface PDFClassifierResponse {
    /** url */
    url?: string;
    /** body */
    body?: Record<string, unknown>;
    /** jobId */
    jobId?: string;
    /** pageCount */
    pageCount?: number;
    /** error */
    error?: boolean;
    /** status */
    status?: number;
    /** name */
    name?: string;
    /** credits */
    credits?: number;
    /** remainingCredits */
    remainingCredits?: number;
    /** duration */
    duration?: number;
}

/**
 * Send Email with Attachments
 */
export interface EmailSendInput {
    /** URL to the source file. Supports links from Google Drive, Dropbox, and PDF.co built-in files storage. */
    url: string;
    /** The “From” field with sender name and email. Example: John Doe <john@example.com> */
    from: string;
    /** The “To” field with receiver name and email. Example: Partner <partner@example.com> */
    to: string;
    /** The subject for the outgoing email */
    subject: string;
    /** Plain text version of the outgoing email message. */
    bodytext?: string;
    /** HTML version of the outgoing email message. */
    bodyhtml?: string;
    /** SMTP server address */
    smtpserver: string;
    /** SMTP port */
    smtpport: string;
    /** SMTP server username */
    smtpusername: string;
    /** SMTP server password, If you use Gmail then you need to generate an app-specific password as instructed at https://support.google.com/accounts/answer/185833. */
    smtppassword: string;
    /** Use this parameter to set additional configurations for fine-tuning and extra options. Explore PDF.co knowledgebase (https://developer.pdf.co/api/profiles/index.html) for profile examples. */
    profiles?: string;
    /** Set async to true for long processes to run in the background, API will then return a jobId which you can use with JobCheck endpoint to check the status of the process and retrieve the output while you can proceed with other tasks without waiting for this process to finish. */
    async?: boolean;
}

/**
 * Response for Send Email with Attachments
 */
export interface EmailSendResponse {
    /** jobId */
    jobId?: string;
    /** error */
    error?: boolean;
    /** status */
    status?: number;
    /** credits */
    credits?: number;
    /** remainingCredits */
    remainingCredits?: number;
    /** duration */
    duration?: number;
}

/**
 * Email Information
 */
export interface EmailDecodeInput {
    /** URL to the source file. Supports links from Google Drive, Dropbox, and PDF.co built-in files storage. */
    url: string;
    /** Use this parameter to set additional configurations for fine-tuning and extra options. Explore PDF.co knowledgebase (https://developer.pdf.co/api/profiles/index.html) for profile examples. */
    profiles?: string;
    /** Set async to true for long processes to run in the background, API will then return a jobId which you can use with JobCheck endpoint to check the status of the process and retrieve the output while you can proceed with other tasks without waiting for this process to finish. */
    async?: boolean;
}

/**
 * Response for Email Information
 */
export interface EmailDecodeResponse {
    /** url */
    url?: string;
    /** body */
    body?: Record<string, unknown>;
    /** jobId */
    jobId?: string;
    /** error */
    error?: boolean;
    /** status */
    status?: number;
    /** name */
    name?: string;
    /** credits */
    credits?: number;
    /** remainingCredits */
    remainingCredits?: number;
    /** duration */
    duration?: number;
}

/**
 * Email Attachment Extraction
 */
export interface EmailAttachmentExtractionInput {
    /** URL to the source file. Supports links from Google Drive, Dropbox, and PDF.co built-in files storage. */
    url: string;
    /** Use this parameter to set additional configurations for fine-tuning and extra options. Explore PDF.co knowledgebase (https://developer.pdf.co/api/profiles/index.html) for profile examples. */
    profiles?: string;
    /** Set async to true for long processes to run in the background, API will then return a jobId which you can use with JobCheck endpoint to check the status of the process and retrieve the output while you can proceed with other tasks without waiting for this process to finish. */
    async?: boolean;
}

/**
 * Response for Email Attachment Extraction
 */
export interface EmailAttachmentExtractionResponse {
    /** url */
    url?: string;
    /** body */
    body?: Record<string, unknown>;
    /** jobId */
    jobId?: string;
    /** error */
    error?: boolean;
    /** status */
    status?: number;
    /** name */
    name?: string;
    /** credits */
    credits?: number;
    /** remainingCredits */
    remainingCredits?: number;
    /** duration */
    duration?: number;
}

/**
 * PDF Attachment Extraction
 */
export interface PDFAttachmentExtractionInput {
    /** Url to the source PDF document */
    url: string;
    /** Determines whether to return output in response or link to output file. */
    inline?: boolean;
    /** Base file name for new PDF files */
    name?: string;
    /** Set the expiration time for the output link in minutes (default is 60 i.e 60 minutes or 1 hour), After this specified duration, any generated output file(s) will be automatically deleted from PDF.co temporary files storage. The maximum duration for link expiration varies based on your current subscription plan. */
    expiration?: number;
    /** Use this parameter to set additional configurations for fine-tuning and extra options. Explore PDF.co knowledgebase (https://developer.pdf.co/api/profiles/index.html) for profile examples. */
    profiles?: string;
    /** Creates a background processing job. */
    async?: boolean;
}

/**
 * Response for PDF Attachment Extraction
 */
export interface PDFAttachmentExtractionResponse {
    /** urls */
    urls?: Array<string>;
    /** Output document URL. */
    url?: string;
    /** jobId */
    jobId?: string;
    /** Number of pages in the output document. */
    pageCount?: number;
    /** Indicates whether the operation is successful or not. */
    error?: boolean;
    /** Status code for HTTP response */
    status?: number;
    /** Output document name */
    name?: string;
    /** PDF.co credits used for this request */
    credits?: number;
    /** Request execution duration */
    duration?: number;
    /** PDF.co account remaining credits */
    remainingCredits?: number;
}
// #endregion Types

// #region Client

/**
 * Typed client for the pdfco connector.
 */
export class PdfcoClient extends ConnectorClientBase {

    /**
     * Creates a new PdfcoClient.
     * @param connectionRuntimeUrl The connection runtime URL from Azure Portal.
     * @param credential The credential used for authentication.
     * @param options Optional connector client options.
     */
    constructor(connectionRuntimeUrl: string, credential: TokenCredential, options?: ConnectorClientOptions) {
        super(connectionRuntimeUrl, credential, options);
    }

    public get connectorName(): string {
        return "pdfco";
    }

    /**
     * HTML to PDF
     * @remarks Convert HTML code snippet into full featured PDF. Please see api documentation at https://developer.pdf.co/api/pdf-to-html/index.html for comprehensive guidelines.
     */
    public async htmlToPdf(input: HtmlToPdfInput, options: ConnectorOperationOptions = {}): Promise<HtmlToPdfResponse> {
        const requestPath = `/v1/pdf/convert/from/html`;
        const requestUrl = this.resolveUrl(requestPath);
        const httpResponse = await this.sendWithTracingAsync<HtmlToPdfResponse>("Pdfco.htmlToPdf", "HtmlToPdf", "POST", requestUrl, input, options);

        return httpResponse.value as HtmlToPdfResponse;
    }

    /**
     * URL to PDF
     * @remarks Create PDF from URL. Please see api documentation at https://developer.pdf.co/api/pdf-from-url/index.html for comprehensive guidelines.
     */
    public async urlToPdf(input: UrlToPdfInput, options: ConnectorOperationOptions = {}): Promise<UrlToPdfResponse> {
        const requestPath = `/v1/pdf/convert/from/url`;
        const requestUrl = this.resolveUrl(requestPath);
        const httpResponse = await this.sendWithTracingAsync<UrlToPdfResponse>("Pdfco.urlToPdf", "UrlToPdf", "POST", requestUrl, input, options);

        return httpResponse.value as UrlToPdfResponse;
    }

    /**
     * Add Text, Images, Form Fields to PDF
     * @remarks Add text, images, signatures to pdf. Fill PDF form fields and create new pdf from templates. Please see api documentation at https://developer.pdf.co/api/pdf-add/index.html for comprehensive guidelines.
     */
    public async pdfFiller(input: PdfFillerInput, options: ConnectorOperationOptions = {}): Promise<PdfFillerResponse> {
        const requestPath = `/v1/pdf/edit/add`;
        const requestUrl = this.resolveUrl(requestPath);
        const httpResponse = await this.sendWithTracingAsync<PdfFillerResponse>("Pdfco.pdfFiller", "PdfFiller", "POST", requestUrl, input, options);

        return httpResponse.value as PdfFillerResponse;
    }

    /**
     * Merge PDF
     * @remarks Merge PDF from two or more PDF files into a new one. Please see api documentation at https://developer.pdf.co/api/pdf-merge/index.html for comprehensive guidelines.
     */
    public async mergePdfSimplified(input: MergePdfSimplifiedInput, options: ConnectorOperationOptions = {}): Promise<MergePdfSimplifiedResponse> {
        const requestPath = `/v1/pdf/merge`;
        const requestUrl = this.resolveUrl(requestPath);
        const httpResponse = await this.sendWithTracingAsync<MergePdfSimplifiedResponse>("Pdfco.mergePdfSimplified", "MergePdfSimplified", "POST", requestUrl, input, options);

        return httpResponse.value as MergePdfSimplifiedResponse;
    }

    /**
     * Merge PDF (Advanced)
     * @remarks Merge PDF from two or more pdf, doc, xls, images, even zip with documents and images into a new PDF. Supports zip, doc, docx, xls, xlsx, rtf, txt, png, jpg. Please see api documentation at https://developer.pdf.co/api/pdf-merge/index.html#post-tag-pdf-merge2 for comprehensive guidelines.
     */
    public async mergePdf(input: MergePdfInput, options: ConnectorOperationOptions = {}): Promise<MergePdfResponse> {
        const requestPath = `/v1/pdf/merge2`;
        const requestUrl = this.resolveUrl(requestPath);
        const httpResponse = await this.sendWithTracingAsync<MergePdfResponse>("Pdfco.mergePdf", "MergePdf", "POST", requestUrl, input, options);

        return httpResponse.value as MergePdfResponse;
    }

    /**
     * Split PDF
     * @remarks Splits pages from PDF document into multiple new PDF files using page numbers and page ranges. Please see api documentation at https://developer.pdf.co/api/pdf-split/index.html for comprehensive guidelines.
     */
    public async splitPdf(input: SplitPdfInput, options: ConnectorOperationOptions = {}): Promise<SplitPdfResponse> {
        const requestPath = `/v1/pdf/split`;
        const requestUrl = this.resolveUrl(requestPath);
        const httpResponse = await this.sendWithTracingAsync<SplitPdfResponse>("Pdfco.splitPdf", "SplitPdf", "POST", requestUrl, input, options);

        return httpResponse.value as SplitPdfResponse;
    }

    /**
     * Split PDF (Advanced)
     * @remarks Split PDF into multiple PDF files by text search (support regular expressions) or barcode. Please see api documentation at https://developer.pdf.co/api/pdf-split/index.html#post-tag-pdf-split2 for comprehensive guidelines.
     */
    public async splitPdf2(input: SplitPdf2Input, options: ConnectorOperationOptions = {}): Promise<SplitPdf2Response> {
        const requestPath = `/v1/pdf/split2`;
        const requestUrl = this.resolveUrl(requestPath);
        const httpResponse = await this.sendWithTracingAsync<SplitPdf2Response>("Pdfco.splitPdf2", "SplitPdf2", "POST", requestUrl, input, options);

        return httpResponse.value as SplitPdf2Response;
    }

    /**
     * PDF Search Text
     * @remarks Search text in PDF and get coordinates. Supports regular expressions. Please see api documentation at https://developer.pdf.co/api/pdf-find/index.html for comprehensive guidelines.
     */
    public async searchPDFText(input: PDFSearchTextInput, options: ConnectorOperationOptions = {}): Promise<PDFSearchTextResponse> {
        const requestPath = `/v1/pdf/find`;
        const requestUrl = this.resolveUrl(requestPath);
        const httpResponse = await this.sendWithTracingAsync<PDFSearchTextResponse>("Pdfco.searchPDFText", "PDFSearchText", "POST", requestUrl, input, options);

        return httpResponse.value as PDFSearchTextResponse;
    }

    /**
     * Document Parser
     * @remarks Document Parser can automatically parse PDF, JPG, and PNG documents to extract fields, tables, values, and barcodes from invoices, statements, orders, and other PDF and scanned documents. Please see api documentation at https://developer.pdf.co/api/document-parser/index.html for comprehensive guidelines.
     */
    public async documentParser(input: DocumentParserInput, options: ConnectorOperationOptions = {}): Promise<DocumentParserResponse> {
        const requestPath = `/v1/pdf/documentparser`;
        const requestUrl = this.resolveUrl(requestPath);
        const httpResponse = await this.sendWithTracingAsync<DocumentParserResponse>("Pdfco.documentParser", "DocumentParser", "POST", requestUrl, input, options);

        return httpResponse.value as DocumentParserResponse;
    }

    /**
     * Background Jobs Check
     * @remarks Checks status of background job that was previously created with PDF.co API. Please see api documentation at https://developer.pdf.co/api/background-job-check/index.html for comprehensive guidelines.
     */
    public async jobCheck(input: JobCheckInput, options: ConnectorOperationOptions = {}): Promise<JobCheckResponse> {
        const requestPath = `/v1/job/check`;
        const requestUrl = this.resolveUrl(requestPath);
        const httpResponse = await this.sendWithTracingAsync<JobCheckResponse>("Pdfco.jobCheck", "JobCheck", "POST", requestUrl, input, options);

        return httpResponse.value as JobCheckResponse;
    }

    /**
     * Barcode Generator
     * @remarks Generate high quality barcode images. Supports QR Code, Datamatrix, Code 39, Code 128, PDF417 and many other barcode types. Please see api documentation at https://developer.pdf.co/api/barcode-generator/index.html for comprehensive guidelines.
     */
    public async barcodeGenerator(input: BarcodeGeneratorInput, options: ConnectorOperationOptions = {}): Promise<BarcodeGeneratorResponse> {
        const requestPath = `/v1/barcode/generate`;
        const requestUrl = this.resolveUrl(requestPath);
        const httpResponse = await this.sendWithTracingAsync<BarcodeGeneratorResponse>("Pdfco.barcodeGenerator", "BarcodeGenerator", "POST", requestUrl, input, options);

        return httpResponse.value as BarcodeGeneratorResponse;
    }

    /**
     * Barcode Reader
     * @remarks Read barcodes from images and PDF. Can read all popular barcode types from QR Code and Code 128, EAN to Datamatrix, PDF417, GS1 and many other barcodes. Please see api documentation at https://developer.pdf.co/api/barcode-reader/index.html for comprehensive guidelines.
     */
    public async barcodeReader(input: BarcodeReaderInput, options: ConnectorOperationOptions = {}): Promise<BarcodeReaderResponse> {
        const requestPath = `/v1/barcode/read/from/url`;
        const requestUrl = this.resolveUrl(requestPath);
        const httpResponse = await this.sendWithTracingAsync<BarcodeReaderResponse>("Pdfco.barcodeReader", "BarcodeReader", "POST", requestUrl, input, options);

        return httpResponse.value as BarcodeReaderResponse;
    }

    /**
     * PDF Info Reader
     * @remarks Get detailed information about the PDF document, it's properties and security permissions. Please see api documentation at https://developer.pdf.co/api/pdf-info-reader/index.html for comprehensive guidelines.
     */
    public async pdfinfoReader(input: PDFInfoReaderInput, options: ConnectorOperationOptions = {}): Promise<PDFInfoReaderResponse> {
        const requestPath = `/v1/pdf/info`;
        const requestUrl = this.resolveUrl(requestPath);
        const httpResponse = await this.sendWithTracingAsync<PDFInfoReaderResponse>("Pdfco.pdfinfoReader", "PDFInfoReader", "POST", requestUrl, input, options);

        return httpResponse.value as PDFInfoReaderResponse;
    }

    /**
     * PDF Forms Info Reader
     * @remarks Get information about fillable fields inside PDF form file. Please see api documentation at https://developer.pdf.co/api/pdf-forms-info-reader/index.html for comprehensive guidelines.
     */
    public async pdfformsInfoReader(input: PDFFormsInfoReaderInput, options: ConnectorOperationOptions = {}): Promise<PDFFormsInfoReaderResponse> {
        const requestPath = `/v1/pdf/info/fields`;
        const requestUrl = this.resolveUrl(requestPath);
        const httpResponse = await this.sendWithTracingAsync<PDFFormsInfoReaderResponse>("Pdfco.pdfformsInfoReader", "PDFFormsInfoReader", "POST", requestUrl, input, options);

        return httpResponse.value as PDFFormsInfoReaderResponse;
    }

    /**
     * PDF Find Table
     * @remarks AI powered document analysis can scan your document for tables and return the array of tables on pages with coordinates and information about columns detected in these tables. Please see api documentation at https://developer.pdf.co/api/pdf-find/index.html#post-tag-pdf-find-table for comprehensive guidelines.
     */
    public async pdffindTable(input: PDFFindTableInput, options: ConnectorOperationOptions = {}): Promise<PDFFindTableResponse> {
        const requestPath = `/v1/pdf/find/table`;
        const requestUrl = this.resolveUrl(requestPath);
        const httpResponse = await this.sendWithTracingAsync<PDFFindTableResponse>("Pdfco.pdffindTable", "PDFFindTable", "POST", requestUrl, input, options);

        return httpResponse.value as PDFFindTableResponse;
    }

    /**
     * PDF Search and Replace Text
     * @remarks Modify a PDF file by searching for specific text and replacing it with new text. Please see api documentation at https://developer.pdf.co/api/pdf-search-and-replace-text/index.html#post-tag-pdf-edit-replace-text for comprehensive guidelines.
     */
    public async searchAndReplace(input: SearchAndReplaceInput, options: ConnectorOperationOptions = {}): Promise<SearchAndReplaceResponse> {
        const requestPath = `/v1/pdf/edit/replace-text`;
        const requestUrl = this.resolveUrl(requestPath);
        const httpResponse = await this.sendWithTracingAsync<SearchAndReplaceResponse>("Pdfco.searchAndReplace", "SearchAndReplace", "POST", requestUrl, input, options);

        return httpResponse.value as SearchAndReplaceResponse;
    }

    /**
     * PDF Search and Replace Text with Image
     * @remarks Replace specific text in a PDF document with an image. Please see api documentation at https://developer.pdf.co/api/pdf-search-and-replace-text/index.html#post-tag-pdf-edit-replace-text-with-image for comprehensive guidelines.
     */
    public async searchAndReplaceWithImage(input: SearchAndReplaceWithImageInput, options: ConnectorOperationOptions = {}): Promise<SearchAndReplaceWithImageResponse> {
        const requestPath = `/v1/pdf/edit/replace-text-with-image`;
        const requestUrl = this.resolveUrl(requestPath);
        const httpResponse = await this.sendWithTracingAsync<SearchAndReplaceWithImageResponse>("Pdfco.searchAndReplaceWithImage", "SearchAndReplaceWithImage", "POST", requestUrl, input, options);

        return httpResponse.value as SearchAndReplaceWithImageResponse;
    }

    /**
     * PDF Search and Delete Text from PDF
     * @remarks Search text in PDF and remove found text from PDF. Please see api documentation at https://developer.pdf.co/api/pdf-search-and-delete-text/index.html for comprehensive guidelines.
     */
    public async searchAndDeleteText(input: SearchAndDeleteTextInput, options: ConnectorOperationOptions = {}): Promise<SearchAndDeleteTextResponse> {
        const requestPath = `/v1/pdf/edit/delete-text`;
        const requestUrl = this.resolveUrl(requestPath);
        const httpResponse = await this.sendWithTracingAsync<SearchAndDeleteTextResponse>("Pdfco.searchAndDeleteText", "SearchAndDeleteText", "POST", requestUrl, input, options);

        return httpResponse.value as SearchAndDeleteTextResponse;
    }

    /**
     * Make Scanned PDF to Text Searchable
     * @remarks Turn PDF and scanned JPG, PNG images into text-searchable PDF. Please see api documentation at https://developer.pdf.co/api/pdf-make-text-searchable-or-unsearchable/index.html#post-tag-pdf-makesearchable for comprehensive guidelines.
     */
    public async pdfsearchable(input: PDFSearchableInput, options: ConnectorOperationOptions = {}): Promise<PDFSearchableResponse> {
        const requestPath = `/v1/pdf/makesearchable`;
        const requestUrl = this.resolveUrl(requestPath);
        const httpResponse = await this.sendWithTracingAsync<PDFSearchableResponse>("Pdfco.pdfsearchable", "PDFSearchable", "POST", requestUrl, input, options);

        return httpResponse.value as PDFSearchableResponse;
    }

    /**
     * PDF to Scanned PDF
     * @remarks Convert PDF to Scanned PDF. Result PDF is same as if you printed and then scanned your PDF. Please see api documentation at https://developer.pdf.co/api/pdf-make-text-searchable-or-unsearchable/index.html#post-tag-pdf-makeunsearchable for comprehensive guidelines.
     */
    public async pdfunSearchable(input: PDFUnSearchableInput, options: ConnectorOperationOptions = {}): Promise<PDFUnSearchableResponse> {
        const requestPath = `/v1/pdf/makeunsearchable`;
        const requestUrl = this.resolveUrl(requestPath);
        const httpResponse = await this.sendWithTracingAsync<PDFUnSearchableResponse>("Pdfco.pdfunSearchable", "PDFUnSearchable", "POST", requestUrl, input, options);

        return httpResponse.value as PDFUnSearchableResponse;
    }

    /**
     * PDF To CSV
     * @remarks Convert PDF and scanned images into CSV representation with layout, columns, rows, and tables. Please see api documentation at https://developer.pdf.co/api/pdf-to-csv/index.html for comprehensive guidelines.
     */
    public async pdftoCSV(input: PDFToCSVInput, options: ConnectorOperationOptions = {}): Promise<PDFToCSVResponse> {
        const requestPath = `/v1/pdf/convert/to/csv`;
        const requestUrl = this.resolveUrl(requestPath);
        const httpResponse = await this.sendWithTracingAsync<PDFToCSVResponse>("Pdfco.pdftoCSV", "PDFToCSV", "POST", requestUrl, input, options);

        return httpResponse.value as PDFToCSVResponse;
    }

    /**
     * PDF To JSON
     * @remarks Convert PDF and scanned images into JSON representation with text, fonts, images, vectors, and formatting preserved. Please see api documentation at https://developer.pdf.co/api/pdf-to-json/index.html for comprehensive guidelines.
     */
    public async pdftoJSON(input: PDFToJSONInput, options: ConnectorOperationOptions = {}): Promise<PDFToJSONResponse> {
        const requestPath = `/v1/pdf/convert/to/json2`;
        const requestUrl = this.resolveUrl(requestPath);
        const httpResponse = await this.sendWithTracingAsync<PDFToJSONResponse>("Pdfco.pdftoJSON", "PDFToJSON", "POST", requestUrl, input, options);

        return httpResponse.value as PDFToJSONResponse;
    }

    /**
     * PDF To JSON (AI Powered)
     * @remarks Convert PDF and scanned images into JSON representation with text, fonts, images, vectors, and formatting preserved. Please see api documentation at https://developer.pdf.co/api/pdf-to-json/index.html#post-tag-pdf-convert-to-json-meta for comprehensive guidelines.
     */
    public async pdftoJSONMeta(input: PDFToJSONMetaInput, options: ConnectorOperationOptions = {}): Promise<PDFToJSONMetaResponse> {
        const requestPath = `/v1/pdf/convert/to/json-meta`;
        const requestUrl = this.resolveUrl(requestPath);
        const httpResponse = await this.sendWithTracingAsync<PDFToJSONMetaResponse>("Pdfco.pdftoJSONMeta", "PDFToJSONMeta", "POST", requestUrl, input, options);

        return httpResponse.value as PDFToJSONMetaResponse;
    }

    /**
     * PDF To Text (with layout and ocr)
     * @remarks Convert PDF and scanned images to Text with layout preserved. Please see api documentation at https://developer.pdf.co/api/pdf-to-text/index.html for comprehensive guidelines.
     */
    public async pdftoText(input: PDFToTextInput, options: ConnectorOperationOptions = {}): Promise<PDFToTextResponse> {
        const requestPath = `/v1/pdf/convert/to/text`;
        const requestUrl = this.resolveUrl(requestPath);
        const httpResponse = await this.sendWithTracingAsync<PDFToTextResponse>("Pdfco.pdftoText", "PDFToText", "POST", requestUrl, input, options);

        return httpResponse.value as PDFToTextResponse;
    }

    /**
     * PDF To Text (without layout and ocr)
     * @remarks Convert PDF to plain text without layout preserved and OCR support. Please see api documentation at https://developer.pdf.co/api/pdf-to-text/index.html#post-tag-pdf-convert-to-text-simple for comprehensive guidelines.
     */
    public async pdftoTextSimple(input: PDFToTextSimpleInput, options: ConnectorOperationOptions = {}): Promise<PDFToTextSimpleResponse> {
        const requestPath = `/v1/pdf/convert/to/text-simple`;
        const requestUrl = this.resolveUrl(requestPath);
        const httpResponse = await this.sendWithTracingAsync<PDFToTextSimpleResponse>("Pdfco.pdftoTextSimple", "PDFToTextSimple", "POST", requestUrl, input, options);

        return httpResponse.value as PDFToTextSimpleResponse;
    }

    /**
     * PDF To XLS
     * @remarks Convert PDF and scanned images to spreadsheet with layout and fonts preserved. Please see api documentation at https://developer.pdf.co/api/pdf-to-excel/index.html#post-tag-pdf-convert-to-xls for comprehensive guidelines.
     */
    public async pdftoXLS(input: PDFToXLSInput, options: ConnectorOperationOptions = {}): Promise<PDFToXLSResponse> {
        const requestPath = `/v1/pdf/convert/to/xls`;
        const requestUrl = this.resolveUrl(requestPath);
        const httpResponse = await this.sendWithTracingAsync<PDFToXLSResponse>("Pdfco.pdftoXLS", "PDFToXLS", "POST", requestUrl, input, options);

        return httpResponse.value as PDFToXLSResponse;
    }

    /**
     * PDF To XLSX
     * @remarks Convert PDF and scanned images to spreadsheet with layout and fonts preserved. Please see api documentation at https://developer.pdf.co/api/pdf-to-excel/index.html#post-tag-pdf-convert-to-xlsx for comprehensive guidelines.
     */
    public async pdftoXLSX(input: PDFToXLSXInput, options: ConnectorOperationOptions = {}): Promise<PDFToXLSXResponse> {
        const requestPath = `/v1/pdf/convert/to/xlsx`;
        const requestUrl = this.resolveUrl(requestPath);
        const httpResponse = await this.sendWithTracingAsync<PDFToXLSXResponse>("Pdfco.pdftoXLSX", "PDFToXLSX", "POST", requestUrl, input, options);

        return httpResponse.value as PDFToXLSXResponse;
    }

    /**
     * PDF To XML
     * @remarks Convert PDF to XML with information about text value, tables, fonts, images, objects positions. Please see api documentation at https://developer.pdf.co/api/pdf-to-xml/index.html for comprehensive guidelines.
     */
    public async pdftoXML(input: PDFToXMLInput, options: ConnectorOperationOptions = {}): Promise<PDFToXMLResponse> {
        const requestPath = `/v1/pdf/convert/to/xml`;
        const requestUrl = this.resolveUrl(requestPath);
        const httpResponse = await this.sendWithTracingAsync<PDFToXMLResponse>("Pdfco.pdftoXML", "PDFToXML", "POST", requestUrl, input, options);

        return httpResponse.value as PDFToXMLResponse;
    }

    /**
     * PDF to JPG
     * @remarks PDF to JPG conversion. High-quality rendering. Also works great for thumbnail generation and previews. Please see api documentation at https://developer.pdf.co/api/pdf-to-image/index.html#post-tag-pdf-convert-to-jpg for comprehensive guidelines.
     */
    public async pdftoJPG(input: PDFToJPGInput, options: ConnectorOperationOptions = {}): Promise<PDFToJPGResponse> {
        const requestPath = `/v1/pdf/convert/to/jpg`;
        const requestUrl = this.resolveUrl(requestPath);
        const httpResponse = await this.sendWithTracingAsync<PDFToJPGResponse>("Pdfco.pdftoJPG", "PDFToJPG", "POST", requestUrl, input, options);

        return httpResponse.value as PDFToJPGResponse;
    }

    /**
     * PDF to PNG
     * @remarks PDF to PNG conversion. High-quality rendering. Also works great for thumbnail generation and previews. Please see api documentation at https://developer.pdf.co/api/pdf-to-image/index.html#post-tag-pdf-convert-to-png for comprehensive guidelines.
     */
    public async pdftoPNG(input: PDFToPNGInput, options: ConnectorOperationOptions = {}): Promise<PDFToPNGResponse> {
        const requestPath = `/v1/pdf/convert/to/png`;
        const requestUrl = this.resolveUrl(requestPath);
        const httpResponse = await this.sendWithTracingAsync<PDFToPNGResponse>("Pdfco.pdftoPNG", "PDFToPNG", "POST", requestUrl, input, options);

        return httpResponse.value as PDFToPNGResponse;
    }

    /**
     * PDF to WEBP
     * @remarks PDF to WEBP conversion. High-quality rendering. Also works great for thumbnail generation and previews. Please see api documentation at https://developer.pdf.co/api/pdf-to-image/index.html#post-tag-pdf-convert-to-webp for comprehensive guidelines.
     */
    public async pdftoWEBP(input: PDFToWEBPInput, options: ConnectorOperationOptions = {}): Promise<PDFToWEBPResponse> {
        const requestPath = `/v1/pdf/convert/to/webp`;
        const requestUrl = this.resolveUrl(requestPath);
        const httpResponse = await this.sendWithTracingAsync<PDFToWEBPResponse>("Pdfco.pdftoWEBP", "PDFToWEBP", "POST", requestUrl, input, options);

        return httpResponse.value as PDFToWEBPResponse;
    }

    /**
     * PDF to TIFF
     * @remarks PDF to high quality TIFF images conversion. High quality rendering. Also works great for thumbnail generation and previews. Please see api documentation at https://developer.pdf.co/api/pdf-to-image/index.html#post-tag-pdf-convert-to-tiff for comprehensive guidelines.
     */
    public async pdftoTIFF(input: PDFToTIFFInput, options: ConnectorOperationOptions = {}): Promise<PDFToTIFFResponse> {
        const requestPath = `/v1/pdf/convert/to/tiff`;
        const requestUrl = this.resolveUrl(requestPath);
        const httpResponse = await this.sendWithTracingAsync<PDFToTIFFResponse>("Pdfco.pdftoTIFF", "PDFToTIFF", "POST", requestUrl, input, options);

        return httpResponse.value as PDFToTIFFResponse;
    }

    /**
     * PDF from CSV
     * @remarks Convert CSV document to PDF. Works also with XLS or XLSX input. Please see api documentation at https://developer.pdf.co/api/pdf-from-document/index.html#post-tag-pdf-convert-from-csv for comprehensive guidelines.
     */
    public async pdffromCSV(input: PDFFromCSVInput, options: ConnectorOperationOptions = {}): Promise<PDFFromCSVResponse> {
        const requestPath = `/v1/pdf/convert/from/csv`;
        const requestUrl = this.resolveUrl(requestPath);
        const httpResponse = await this.sendWithTracingAsync<PDFFromCSVResponse>("Pdfco.pdffromCSV", "PDFFromCSV", "POST", requestUrl, input, options);

        return httpResponse.value as PDFFromCSVResponse;
    }

    /**
     * PDF from Document (Doc, DocX, RTF, TXT, XPS)
     * @remarks Convert DOC, DOCX, RTF, TXT, XPS files into PDF. Please see api documentation at https://developer.pdf.co/api/pdf-from-document/index.html#post-tag-pdf-convert-from-doc for comprehensive guidelines.
     */
    public async pdffromDoc(input: PDFFromDocInput, options: ConnectorOperationOptions = {}): Promise<PDFFromDocResponse> {
        const requestPath = `/v1/pdf/convert/from/doc`;
        const requestUrl = this.resolveUrl(requestPath);
        const httpResponse = await this.sendWithTracingAsync<PDFFromDocResponse>("Pdfco.pdffromDoc", "PDFFromDoc", "POST", requestUrl, input, options);

        return httpResponse.value as PDFFromDocResponse;
    }

    /**
     * PDF from Image
     * @remarks Create a PDF file from one or more JPG, PNG, TIFF images. Please see api documentation at https://developer.pdf.co/api/pdf-from-image/index.html for comprehensive guidelines.
     */
    public async pdffromImages(input: PDFFromImagesInput, options: ConnectorOperationOptions = {}): Promise<PDFFromImagesResponse> {
        const requestPath = `/v1/pdf/convert/from/image`;
        const requestUrl = this.resolveUrl(requestPath);
        const httpResponse = await this.sendWithTracingAsync<PDFFromImagesResponse>("Pdfco.pdffromImages", "PDFFromImages", "POST", requestUrl, input, options);

        return httpResponse.value as PDFFromImagesResponse;
    }

    /**
     * PDF from Email
     * @remarks Convert email files (MSG or EML) code into PDF. Extracts attachments (if any) from input email and embeds into PDF as pdf attachments. Please see api documentation at https://developer.pdf.co/api/pdf-from-email/index.html for comprehensive guidelines.
     */
    public async pdffromEmail(input: PDFFromEmailInput, options: ConnectorOperationOptions = {}): Promise<PDFFromEmailResponse> {
        const requestPath = `/v1/pdf/convert/from/email`;
        const requestUrl = this.resolveUrl(requestPath);
        const httpResponse = await this.sendWithTracingAsync<PDFFromEmailResponse>("Pdfco.pdffromEmail", "PDFFromEmail", "POST", requestUrl, input, options);

        return httpResponse.value as PDFFromEmailResponse;
    }

    /**
     * PDF Add Security
     * @remarks Add password and security limitations to existing PDF. Please see api documentation at https://developer.pdf.co/api/password-and-security/index.html#post-tag-pdf-security-add for comprehensive guidelines.
     */
    public async addPDFSecurity(input: PDFAddSecurityInput, options: ConnectorOperationOptions = {}): Promise<PDFAddSecurityResponse> {
        const requestPath = `/v1/pdf/security/add`;
        const requestUrl = this.resolveUrl(requestPath);
        const httpResponse = await this.sendWithTracingAsync<PDFAddSecurityResponse>("Pdfco.addPDFSecurity", "PDFAddSecurity", "POST", requestUrl, input, options);

        return httpResponse.value as PDFAddSecurityResponse;
    }

    /**
     * PDF Remove Security
     * @remarks Remove password and security from existing PDF files. Please see api documentation at https://developer.pdf.co/api/password-and-security/index.html#post-tag-pdf-security-remove for comprehensive guidelines.
     */
    public async removePDFSecurity(input: PDFSecurityRemoveInput, options: ConnectorOperationOptions = {}): Promise<PDFSecurityRemoveResponse> {
        const requestPath = `/v1/pdf/security/remove`;
        const requestUrl = this.resolveUrl(requestPath);
        const httpResponse = await this.sendWithTracingAsync<PDFSecurityRemoveResponse>("Pdfco.removePDFSecurity", "PDFSecurityRemove", "POST", requestUrl, input, options);

        return httpResponse.value as PDFSecurityRemoveResponse;
    }

    /**
     * PDF From Spreadsheet
     * @remarks Create PDF from XLS, XLSX spreadsheet. Please see api documentation at https://developer.pdf.co/api/convert-excel/index.html#post-tag-xls-convert-to-pdf for comprehensive guidelines.
     */
    public async pdffromXLSXLSX(input: PDFFromXLSXLSXInput, options: ConnectorOperationOptions = {}): Promise<PDFFromXLSXLSXResponse> {
        const requestPath = `/v1/xls/convert/to/pdf`;
        const requestUrl = this.resolveUrl(requestPath);
        const httpResponse = await this.sendWithTracingAsync<PDFFromXLSXLSXResponse>("Pdfco.pdffromXLSXLSX", "PDFFromXLSXLSX", "POST", requestUrl, input, options);

        return httpResponse.value as PDFFromXLSXLSXResponse;
    }

    /**
     * Spreadsheet to CSV
     * @remarks Convert XLS/XLSX into CSV comma separated values file. Please see api documentation at https://developer.pdf.co/api/convert-excel/index.html#post-tag-xls-convert-to-csv for comprehensive guidelines.
     */
    public async xlstoCSV(input: XLStoCSVInput, options: ConnectorOperationOptions = {}): Promise<XLStoCSVResponse> {
        const requestPath = `/v1/xls/convert/to/csv`;
        const requestUrl = this.resolveUrl(requestPath);
        const httpResponse = await this.sendWithTracingAsync<XLStoCSVResponse>("Pdfco.xlstoCSV", "XLStoCSV", "POST", requestUrl, input, options);

        return httpResponse.value as XLStoCSVResponse;
    }

    /**
     * Spreadsheet to JSON
     * @remarks Convert XLS/XLSX into JSON file. Please see api documentation at https://developer.pdf.co/api/convert-excel/index.html#post-tag-xls-convert-to-json for comprehensive guidelines.
     */
    public async xlstoJSON(input: XLStoJSONInput, options: ConnectorOperationOptions = {}): Promise<XLStoJSONResponse> {
        const requestPath = `/v1/xls/convert/to/json`;
        const requestUrl = this.resolveUrl(requestPath);
        const httpResponse = await this.sendWithTracingAsync<XLStoJSONResponse>("Pdfco.xlstoJSON", "XLStoJSON", "POST", requestUrl, input, options);

        return httpResponse.value as XLStoJSONResponse;
    }

    /**
     * Spreadsheet to HTML
     * @remarks Convert XLS/XLSX into HTML file. Please see api documentation at https://developer.pdf.co/api/convert-excel/index.html#post-tag-xls-convert-to-html for comprehensive guidelines.
     */
    public async xlstoHTML(input: XLStoHTMLInput, options: ConnectorOperationOptions = {}): Promise<XLStoHTMLResponse> {
        const requestPath = `/v1/xls/convert/to/html`;
        const requestUrl = this.resolveUrl(requestPath);
        const httpResponse = await this.sendWithTracingAsync<XLStoHTMLResponse>("Pdfco.xlstoHTML", "XLStoHTML", "POST", requestUrl, input, options);

        return httpResponse.value as XLStoHTMLResponse;
    }

    /**
     * Spreadsheet to TXT
     * @remarks Convert XLS/XLSX or CSV into Text file. Please see api documentation at https://developer.pdf.co/api/convert-excel/index.html#post-tag-xls-convert-to-txt for comprehensive guidelines.
     */
    public async xlstoTXT(input: XLStoTXTInput, options: ConnectorOperationOptions = {}): Promise<XLStoTXTResponse> {
        const requestPath = `/v1/xls/convert/to/txt`;
        const requestUrl = this.resolveUrl(requestPath);
        const httpResponse = await this.sendWithTracingAsync<XLStoTXTResponse>("Pdfco.xlstoTXT", "XLStoTXT", "POST", requestUrl, input, options);

        return httpResponse.value as XLStoTXTResponse;
    }

    /**
     * Spreadsheet to XML
     * @remarks Convert XLS/XLSX or CSV into XML file. Please see api documentation at https://developer.pdf.co/api/convert-excel/index.html#post-tag-xls-convert-to-xml for comprehensive guidelines.
     */
    public async xlstoXML(input: XLStoXMLInput, options: ConnectorOperationOptions = {}): Promise<XLStoXMLResponse> {
        const requestPath = `/v1/xls/convert/to/xml`;
        const requestUrl = this.resolveUrl(requestPath);
        const httpResponse = await this.sendWithTracingAsync<XLStoXMLResponse>("Pdfco.xlstoXML", "XLStoXML", "POST", requestUrl, input, options);

        return httpResponse.value as XLStoXMLResponse;
    }

    /**
     * Rotate PDF Pages
     * @remarks Rotate PDF Pages. Please see api documentation at https://developer.pdf.co/api/pdf-rotate-pages/index.html for comprehensive guidelines.
     */
    public async pdfrotatePages(input: PDFRotatePagesInput, options: ConnectorOperationOptions = {}): Promise<PDFRotatePagesResponse> {
        const requestPath = `/v1/pdf/edit/rotate`;
        const requestUrl = this.resolveUrl(requestPath);
        const httpResponse = await this.sendWithTracingAsync<PDFRotatePagesResponse>("Pdfco.pdfrotatePages", "PDFRotatePages", "POST", requestUrl, input, options);

        return httpResponse.value as PDFRotatePagesResponse;
    }

    /**
     * Rotate PDF Pages (Auto)
     * @remarks Uses AI to automatically fix the rotation of pages inside scanned PDF based on text analysis. Please see api documentation at https://developer.pdf.co/api/pdf-rotate-pages/index.html#post-tag-pdf-edit-rotate-auto for comprehensive guidelines.
     */
    public async pdfautoRotatePages(input: PDFAutoRotatePagesInput, options: ConnectorOperationOptions = {}): Promise<PDFAutoRotatePagesResponse> {
        const requestPath = `/v1/pdf/edit/rotate/auto`;
        const requestUrl = this.resolveUrl(requestPath);
        const httpResponse = await this.sendWithTracingAsync<PDFAutoRotatePagesResponse>("Pdfco.pdfautoRotatePages", "PDFAutoRotatePages", "POST", requestUrl, input, options);

        return httpResponse.value as PDFAutoRotatePagesResponse;
    }

    /**
     * PDF Delete Pages
     * @remarks Delete pages in PDF. Please see api documentation at https://developer.pdf.co/api/pdf-delete-pages/index.html for comprehensive guidelines.
     */
    public async deletePDFPages(input: PDFDeletePagesInput, options: ConnectorOperationOptions = {}): Promise<PDFDeletePagesResponse> {
        const requestPath = `/v1/pdf/edit/delete-pages`;
        const requestUrl = this.resolveUrl(requestPath);
        const httpResponse = await this.sendWithTracingAsync<PDFDeletePagesResponse>("Pdfco.deletePDFPages", "PDFDeletePages", "POST", requestUrl, input, options);

        return httpResponse.value as PDFDeletePagesResponse;
    }

    /**
     * PDF Compress and Optimize
     * @remarks Optimize PDF document to reduce its size. Please see api documentation at https://developer.pdf.co/api/pdf-optimize/index.html for comprehensive guidelines.
     */
    public async pdfcompress(input: PDFCompressInput, options: ConnectorOperationOptions = {}): Promise<PDFCompressResponse> {
        const requestPath = `/v1/pdf/optimize`;
        const requestUrl = this.resolveUrl(requestPath);
        const httpResponse = await this.sendWithTracingAsync<PDFCompressResponse>("Pdfco.pdfcompress", "PDFCompress", "POST", requestUrl, input, options);

        return httpResponse.value as PDFCompressResponse;
    }

    /**
     * Document Classifier
     * @remarks Auto classification Of Incoming Documents based on keywords-based rules. For example, you can define rules to find which vendor provided the document to find which template to apply accordingly. Please see api documentation at https://developer.pdf.co/api/document-classifier/index.html for comprehensive guidelines.
     */
    public async pdfclassifier(input: PDFClassifierInput, options: ConnectorOperationOptions = {}): Promise<PDFClassifierResponse> {
        const requestPath = `/v1/pdf/classifier`;
        const requestUrl = this.resolveUrl(requestPath);
        const httpResponse = await this.sendWithTracingAsync<PDFClassifierResponse>("Pdfco.pdfclassifier", "PDFClassifier", "POST", requestUrl, input, options);

        return httpResponse.value as PDFClassifierResponse;
    }

    /**
     * Send Email with Attachments
     * @remarks Send Email with Attachments. Please see api documentation at https://developer.pdf.co/api/email-send-and-decode/index.html for comprehensive guidelines.
     */
    public async sendEmail(input: EmailSendInput, options: ConnectorOperationOptions = {}): Promise<EmailSendResponse> {
        const requestPath = `/v1/email/send`;
        const requestUrl = this.resolveUrl(requestPath);
        const httpResponse = await this.sendWithTracingAsync<EmailSendResponse>("Pdfco.sendEmail", "EmailSend", "POST", requestUrl, input, options);

        return httpResponse.value as EmailSendResponse;
    }

    /**
     * Email Information
     * @remarks Get Email Information such as sender/recipient, subject, body, etc. Please see api documentation at https://developer.pdf.co/api/email-send-and-decode/index.html#post-tag-email-decode for comprehensive guidelines.
     */
    public async emailDecode(input: EmailDecodeInput, options: ConnectorOperationOptions = {}): Promise<EmailDecodeResponse> {
        const requestPath = `/v1/email/decode`;
        const requestUrl = this.resolveUrl(requestPath);
        const httpResponse = await this.sendWithTracingAsync<EmailDecodeResponse>("Pdfco.emailDecode", "EmailDecode", "POST", requestUrl, input, options);

        return httpResponse.value as EmailDecodeResponse;
    }

    /**
     * Email Attachment Extraction
     * @remarks Extract Email attachments with basic email information. Please see api documentation at https://developer.pdf.co/api/email-send-and-decode/index.html#post-tag-email-extract-attachments for comprehensive guidelines.
     */
    public async emailAttachmentExtraction(input: EmailAttachmentExtractionInput, options: ConnectorOperationOptions = {}): Promise<EmailAttachmentExtractionResponse> {
        const requestPath = `/v1/email/extract-attachments`;
        const requestUrl = this.resolveUrl(requestPath);
        const httpResponse = await this.sendWithTracingAsync<EmailAttachmentExtractionResponse>("Pdfco.emailAttachmentExtraction", "EmailAttachmentExtraction", "POST", requestUrl, input, options);

        return httpResponse.value as EmailAttachmentExtractionResponse;
    }

    /**
     * PDF Attachment Extraction
     * @remarks Extract attachments from input PDF document. Please see api documentation at https://developer.pdf.co/api/extract-attachments/index.html for comprehensive guidelines.
     */
    public async pdfattachmentExtraction(input: PDFAttachmentExtractionInput, options: ConnectorOperationOptions = {}): Promise<PDFAttachmentExtractionResponse> {
        const requestPath = `/v1/pdf/attachments/extract`;
        const requestUrl = this.resolveUrl(requestPath);
        const httpResponse = await this.sendWithTracingAsync<PDFAttachmentExtractionResponse>("Pdfco.pdfattachmentExtraction", "PDFAttachmentExtraction", "POST", requestUrl, input, options);

        return httpResponse.value as PDFAttachmentExtractionResponse;
    }

}

// #endregion Client
