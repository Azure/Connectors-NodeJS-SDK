// PlivoExtensions.ts - Auto-generated DirectClient SDK for TypeScript
// Do not edit this file directly.

import { ConnectorClientBase } from "../azureConnectors/clientBase.ts";
import { ConnectorException } from "../azureConnectors/connectorException.ts";
import { ConnectorClientOptions } from "../azureConnectors/options.ts";
import { TokenProvider } from "../azureConnectors/authentication.ts";

// #region Types

/**
 * Response for Make a call
 */
export interface MakeCallResponse {
    /** Unique ID of the API request. */
    api_id?: string;
    /** Response obtained when the call was made. */
    message?: string;
    /** Unique ID of the call. */
    request_uuid?: string;
}

/**
 * Response for List all messages
 */
export interface ListMessagesResponse {
    /** Unique ID of the API request. */
    api_id?: string;
    /** Meta information about the response. */
    meta?: Record<string, unknown>;
    /** Details of all messages. */
    objects?: Array<Record<string, unknown>>;
}

/**
 * Response for Send SMS
 */
export interface SendSMSResponse {
    /** Unique ID of the API request. */
    api_id?: string;
    /** Response of the sent message. */
    message?: string;
    /** Unique ID of the message. */
    message_uuid?: Array<string>;
}

/**
 * Response for Get message
 */
export interface GetMessageResponse {
    /** Unique ID of the API request. */
    api_id?: string;
    /** Delivery error code if the message isn't delivered. */
    error_code?: string;
    /** Number from which the message was sent. */
    from_number?: string;
    /** Direction of the message (inbound/outbound). */
    message_direction?: string;
    /** Status of the message (queued/sent/delivered/failed). */
    message_state?: string;
    /** Timestamp at which the message was sent or received (yyyy-MM-dd HH:mm:ss UTC). */
    message_time?: string;
    /** Type of the message. */
    message_type?: string;
    /** Unique ID of the message. */
    message_uuid?: string;
    /** URI to get details of this single message. */
    resource_uri?: string;
    /** Number to which the message was sent. */
    to_number?: string;
    /** Amount charged for the message. */
    total_amount?: string;
    /** Rate per unit of the message. */
    total_rate?: string;
    /** Number of units the message was split into. */
    units?: number;
}

/**
 * Definition: SMS
 */
export interface SMS {
    /** Plivo number from which the message is to be sent. */
    src: string;
    /** Phone number to which the message is to be sent. */
    dst: string;
    /** Text of the message to be sent. */
    text: string;
}

/**
 * Definition: Call
 */
export interface Call {
    /** Plivo number to be used to make the call. */
    from: string;
    /** The phone number to call. */
    to: string;
    /** URL to be called when the call is made. */
    answer_url: string;
    /** HTTP Method (GET/POST) that should be used to call the Answer URL. */
    answer_method?: string;
}
// #endregion Types

// #region Client

/**
 * Typed client for the plivo connector.
 */
export class PlivoClient extends ConnectorClientBase {

    /**
     * Creates a new PlivoClient.
     * @param connectionRuntimeUrl The connection runtime URL from Azure Portal.
     * @param tokenProvider The token provider for authentication.
     * @param options Optional connector client options.
     */
    constructor(connectionRuntimeUrl: string, tokenProvider: TokenProvider, options?: ConnectorClientOptions) {
        super(connectionRuntimeUrl, tokenProvider, options);
    }

    public get connectorName(): string {
        return "plivo";
    }

    /**
     * Make a call
     * @remarks This operation is used to make a call.
     */
    public async makeCallAsync(input: Call, authId: string, abortSignal?: AbortSignal): Promise<MakeCallResponse> {
        const requestPath = `/v1/Account/${authId}/Call/`;
        const requestUrl = this.resolveUrl(requestPath);
        const httpResponse = await this.httpClient.sendAsync<MakeCallResponse>("POST", requestUrl, undefined, input, abortSignal);

        if (!httpResponse.isSuccessStatusCode) {
            throw new ConnectorException(this.connectorName, `POST ${requestPath}`, httpResponse.statusCode, httpResponse.text);
        }

        return httpResponse.value as MakeCallResponse;
    }

    /**
     * List all messages
     * @remarks This operation returns a list of all messages associated with your Plivo account.
     */
    public async listMessagesAsync(authId: string, abortSignal?: AbortSignal): Promise<ListMessagesResponse> {
        const requestPath = `/v1/Account/${authId}/Message/`;
        const requestUrl = this.resolveUrl(requestPath);
        const httpResponse = await this.httpClient.sendAsync<ListMessagesResponse>("GET", requestUrl, undefined, undefined, abortSignal);

        if (!httpResponse.isSuccessStatusCode) {
            throw new ConnectorException(this.connectorName, `GET ${requestPath}`, httpResponse.statusCode, httpResponse.text);
        }

        return httpResponse.value as ListMessagesResponse;
    }

    /**
     * Send SMS
     * @remarks This operation is used to send a text message.
     */
    public async sendSMSAsync(input: SMS, authId: string, abortSignal?: AbortSignal): Promise<SendSMSResponse> {
        const requestPath = `/v1/Account/${authId}/Message/`;
        const requestUrl = this.resolveUrl(requestPath);
        const httpResponse = await this.httpClient.sendAsync<SendSMSResponse>("POST", requestUrl, undefined, input, abortSignal);

        if (!httpResponse.isSuccessStatusCode) {
            throw new ConnectorException(this.connectorName, `POST ${requestPath}`, httpResponse.statusCode, httpResponse.text);
        }

        return httpResponse.value as SendSMSResponse;
    }

    /**
     * Get message
     * @remarks This operation is used to fetch the details of a message, given the message ID.
     */
    public async getMessageAsync(authId: string, messageUuid: string, abortSignal?: AbortSignal): Promise<GetMessageResponse> {
        const requestPath = `/v1/Account/${authId}/Message/${messageUuid}/`;
        const requestUrl = this.resolveUrl(requestPath);
        const httpResponse = await this.httpClient.sendAsync<GetMessageResponse>("GET", requestUrl, undefined, undefined, abortSignal);

        if (!httpResponse.isSuccessStatusCode) {
            throw new ConnectorException(this.connectorName, `GET ${requestPath}`, httpResponse.statusCode, httpResponse.text);
        }

        return httpResponse.value as GetMessageResponse;
    }

}

// #endregion Client
