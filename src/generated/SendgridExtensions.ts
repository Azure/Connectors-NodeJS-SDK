// SendgridExtensions.ts - Auto-generated DirectClient SDK for TypeScript
// Do not edit this file directly.

import type { TokenCredential } from "@azure/core-auth";
import { ConnectorClientBase } from "../azureConnectors/clientBase.ts";
import type { ConnectorClientOptions, ConnectorOperationOptions } from "../azureConnectors/options.ts";

// #region Types

/**
 * Definition: EmailResponse
 */
export interface EmailResponse {
    /** Status message from send email response */
    message?: string;
}

/**
 * Definition: AddGlobalSuppressRequestAndResponse
 */
export interface AddGlobalSuppressRequestAndResponse {
    /** Recipient email */
    recipient_emails?: Array<string>;
}

/**
 * Definition: GetGlobalSuppressResponse
 */
export interface GetGlobalSuppressResponse {
    /** Recipient email */
    recipient_email?: string;
}

/**
 * Definition: GetGlobalSuppressEmptyResponse
 */
export interface GetGlobalSuppressEmptyResponse {
    [key: string]: unknown;
}

/**
 * Definition: EmailRequest
 */
export interface EmailRequest {
    /** Attachments */
    attachments?: Array<EmailAttachment>;
    /** Origination address of your email. Must be a valid email address from your domain */
    from: string;
    /** Origination name of your email */
    fromname?: string;
    /** Valid email addresses separated by a semicolon or comma */
    to: string;
    /** A display name for each email address separated by a semicolon or comma */
    toname?: string;
    /** Subject of your email */
    subject: string;
    /** Content of your email */
    text: string;
    /** Specify whether the content of the email is HTML or plain text */
    ishtml: boolean;
    /** Valid email addresses separated by a semicolon or comma */
    cc?: string;
    /** A display name for each CC email address separated by a semicolon or comma */
    ccname?: string;
    /** Valid email addresses separated by a semicolon or comma */
    bcc?: string;
    /** A display name for each BCC email address separated by a semicolon or comma */
    bccname?: string;
}

/**
 * Definition: EmailAttachment
 */
export interface EmailAttachment {
    /** Attachment file content */
    content?: string;
    /** Attachment file name */
    filename?: string;
    /** Type of content in the attachment. */
    contenttype?: string;
}

/**
 * Definition: Object
 */
export interface ObjectEntity {
    [key: string]: unknown;
}

/**
 * Definition: RecipientLists
 */
export interface RecipientLists {
    /** List of recipient lists */
    lists?: Array<RecipientList>;
}

/**
 * Definition: RecipientList
 */
export interface RecipientList {
    /** Unique list id */
    id?: number;
    /** Name */
    name?: string;
    /** Recipient count */
    recipient_count?: number;
}

/**
 * Definition: Recipients
 */
export interface Recipients {
    /** List of recipients */
    recipients?: Array<Recipient>;
}

/**
 * Definition: Recipient
 */
export interface Recipient {
    /** Email address */
    email?: string;
    /** Last name */
    last_name?: string;
    /** First name */
    first_name?: string;
    /** Unique recipient id */
    id?: string;
}

/**
 * Definition: ScopeList
 */
export interface ScopeList {
    /** List of scopes */
    scopes?: Array<string>;
}

/**
 * Definition: Bounces
 */
export interface Bounces {
    [key: string]: unknown;
}

/**
 * Definition: EmailIsUnsubscribedResponse
 */
export interface EmailIsUnsubscribedResponse {
    /** Is Email Unsubscribed */
    isUnsubscribed?: boolean;
}

/**
 * Definition: Bounce
 */
export interface Bounce {
    /** Created */
    created?: number;
    /** Email */
    email?: string;
    /** Reason */
    reason?: string;
    /** Status */
    status?: string;
}
// #endregion Types

// #region Client

/**
 * Typed client for the sendgrid connector.
 */
export class SendgridClient extends ConnectorClientBase {

    /**
     * Creates a new SendgridClient.
     * @param connectionRuntimeUrl The connection runtime URL from Azure Portal.
     * @param credential The credential used for authentication.
     * @param options Optional connector client options.
     */
    constructor(connectionRuntimeUrl: string, credential: TokenCredential, options?: ConnectorClientOptions) {
        super(connectionRuntimeUrl, credential, options);
    }

    public get connectorName(): string {
        return "sendgrid";
    }

    /**
     * Add a global suppression
     * @remarks Add a global suppression
     */
    public async addGlobalSuppression(input: AddGlobalSuppressRequestAndResponse, options: ConnectorOperationOptions = {}): Promise<AddGlobalSuppressRequestAndResponse> {
        const requestPath = `/suppressions/global`;
        const requestUrl = this.resolveUrl(requestPath);
        const httpResponse = await this.sendWithTracingAsync<AddGlobalSuppressRequestAndResponse>("Sendgrid.addGlobalSuppression", "AddGlobalSuppression", "POST", requestUrl, input, options);

        return httpResponse.value as AddGlobalSuppressRequestAndResponse;
    }

    /**
     * Get the global suppression
     * @remarks Get the global suppression
     */
    public async getGlobalSuppression(email: string, options: ConnectorOperationOptions = {}): Promise<GetGlobalSuppressResponse> {
        const requestPath = `/suppressions/global/${email}`;
        const requestUrl = this.resolveUrl(requestPath);
        const httpResponse = await this.sendWithTracingAsync<GetGlobalSuppressResponse>("Sendgrid.getGlobalSuppression", "GetGlobalSuppression", "GET", requestUrl, undefined, options);

        return httpResponse.value as GetGlobalSuppressResponse;
    }

    /**
     * Delete the global suppression
     * @remarks Delete the global suppression
     */
    public async deleteGlobalSuppression(email: string, options: ConnectorOperationOptions = {}): Promise<void> {
        const requestPath = `/suppressions/global/${email}`;
        const requestUrl = this.resolveUrl(requestPath);
        await this.sendWithTracingAsync<void>("Sendgrid.deleteGlobalSuppression", "DeleteGlobalSuppression", "DELETE", requestUrl, undefined, options);
    }

    /**
     * Add recipient to list
     * @remarks Add an individual recipient to a recipient list.
     */
    public async addRecipientToList(listId: string, recipientId: string, options: ConnectorOperationOptions = {}): Promise<ObjectEntity> {
        const requestPath = `/v3/contactdb/lists/${listId}/recipients/${recipientId}`;
        const requestUrl = this.resolveUrl(requestPath);
        const httpResponse = await this.sendWithTracingAsync<ObjectEntity>("Sendgrid.addRecipientToList", "AddRecipientToList", "POST", requestUrl, undefined, options);

        return httpResponse.value as ObjectEntity;
    }

    /**
     * Get bounce for an email
     * @remarks Get a specific bounce for a given email address.
     */
    public async getBounce(email: string, options: ConnectorOperationOptions = {}): Promise<Array<Bounce>> {
        const requestPath = `/suppression/bounces/${email}`;
        const requestUrl = this.resolveUrl(requestPath);
        const httpResponse = await this.sendWithTracingAsync<Array<Bounce>>("Sendgrid.getBounce", "GetBounce", "GET", requestUrl, undefined, options);

        return httpResponse.value as Array<Bounce>;
    }

    /**
     * Delete an email from bounce list
     * @remarks Delete an email address from your bounce list.
     */
    public async deleteBounce(email: string, options: ConnectorOperationOptions = {}): Promise<void> {
        const requestPath = `/suppression/bounces/${email}`;
        const requestUrl = this.resolveUrl(requestPath);
        await this.sendWithTracingAsync<void>("Sendgrid.deleteBounce", "DeleteBounce", "DELETE", requestUrl, undefined, options);
    }

    /**
     * Check if email is in unsubscribed email list
     * @remarks Check if email is in unsubscribed email list.
     */
    public async listCheckEmailIsInUnsubscribes(email: string, options: ConnectorOperationOptions = {}): Promise<EmailIsUnsubscribedResponse> {
        const requestPath = `/unsubscribes/${email}`;
        const requestUrl = this.resolveUrl(requestPath);
        const httpResponse = await this.sendWithTracingAsync<EmailIsUnsubscribedResponse>("Sendgrid.listCheckEmailIsInUnsubscribes", "CheckEmailIsInUnsubscribesList", "GET", requestUrl, undefined, options);

        return httpResponse.value as EmailIsUnsubscribedResponse;
    }

    /**
     * Send email
     * @remarks Sends an email (V4). Limited to 1000 recipients.
     */
    public async sendEmail(input: EmailRequest, options: ConnectorOperationOptions = {}): Promise<ObjectEntity> {
        const requestPath = `/v4/mail/send`;
        const requestUrl = this.resolveUrl(requestPath);
        const httpResponse = await this.sendWithTracingAsync<ObjectEntity>("Sendgrid.sendEmail", "SendEmailV4", "POST", requestUrl, input, options);

        return httpResponse.value as ObjectEntity;
    }

}

// #endregion Client
