...
export class Office365Client extends ConnectorClientBase {
     ...
     */
    public async contactPatchItemAsync(input: Contact, folder: string, id: string): Promise<ContactResponse> {
        const requestPath = `/codeless/v1.0/me/contactFolders/${folder}/contacts/${id}`;
        const url = `${this.connectionRuntimeUrl}${requestPath}`;
     ...
     */
    public async contactPostItemAsync(input: Contact, folder: string): Promise<ContactResponse> {
     ...
     */
    public async replyToAsync(input: ReplyEmailInput, messageId: string, mailboxAddress?: string): Promise<void> {
        const queryParams: string[] = [];
        if (mailboxAddress !== undefined) {
            queryParams.push(`mailboxAddress=${encodeURIComponent(String(mailboxAddress))}`);
        }
